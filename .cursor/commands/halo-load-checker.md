...
1. Run a **point-in-time** snapshot yourself (do not only tell me what to run):
   - PowerShell: `& "$env:USERPROFILE\workspace\.cursor\skills\halo-load-checker\scripts\halo-load-checker.ps1"`
   - gpu-bash: `bash "$HOME/workspace/.cursor/skills/halo-load-checker/scripts/halo-load-checker.sh"`
