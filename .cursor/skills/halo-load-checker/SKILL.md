...
**Project location:** `.cursor/skills/halo-load-checker/` (repo-relative; run from **hip-ep** repo root).
...
**Git Bash** (from hip-ep repo root):
...
```powershell
& ".cursor/skills/halo-load-checker/scripts/halo-load-checker.ps1" -Filter "onnxruntime"
```
...
1. Run **one** snapshot script (PowerShell on Windows unless the user is clearly in Git Bash only).
...
5. If no `_therock/bin` at all, suggest hip-ep configure — see `$env:BUILD_DIR\_therock` or `$env:USERPROFILE\workspace\build\hip-ep\_therock`.

## Environment hints

| Variable | Default when unset |
|----------|-------------------|
| `WORKSPACE` | `$env:USERPROFILE\workspace` (PowerShell) / `$HOME/workspace` (Git Bash) |
| `BUILD_DIR` | `$WORKSPACE\build\hip-ep` |
| `THEROCK_DIST` | `$BUILD_DIR\_therock` when present |
| `HIP_BUILD_BIN` | `$BUILD_DIR\bin\RelWithDebInfo` (or `Release` per build config) |
...
| hipInfo/clinfo fail | Check AMD driver; ensure `$env:THEROCK_DIST\bin` on PATH for manual runs |
| GPU engine % missing | Task Manager → Performance → GPU |
| No `_therock/bin` | Configure/build hip-ep so CMake populates `$env:BUILD_DIR\_therock` |
