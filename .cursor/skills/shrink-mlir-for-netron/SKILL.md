...
2. **Run the combined script** (streaming; do not load file into chat):
   - PowerShell:
     `& ".cursor/skills/shrink-mlir-for-netron/scripts/shrink_mlir_for_netron.ps1" <path>`
   - gpu-bash:
     `bash ".cursor/skills/shrink-mlir-for-netron/scripts/shrink_mlir_for_netron.sh" <path>`
