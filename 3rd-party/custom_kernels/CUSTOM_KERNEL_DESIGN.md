...
overview: Add custom HIP kernel infrastructure to UDNA-compiler for GQA and RoPE ops -- compile .hip files into a static library via hip_utils.cmake, expose pure C headers for bitcode compilation, and link the kernel lib into model DLLs alongside MIOpen/hipBLASLt.
todos:
  - id: hip-utils-cmake
    content: Add cmake/hip_utils.cmake adapted from onnx-hipdnn-ep
    status: completed
  - id: kernel-header
    content: Create custom_kernels/include/udna_custom_kernels.h with pure C interface for RoPE and GQA launchers
    status: completed
  - id: rope-kernel
  ...
  - id: runtime-cmake
    ...
    status: completed
  - id: compiler-driver
    content: Update CompilerDriver.cpp to discover and link udna_custom_kernels.lib from install path at DLL link time
    status: completed
  - id: top-cmake
    content: Update top-level CMakeLists.txt to add_subdirectory(custom_kernels) when ROCm is available
    status: completed
  - id: install-artifacts
    content: Add CMake install rules to install udna_custom_kernels.lib to lib/ and udna_custom_kernels.h to include/ under CMAKE_INSTALL_PREFIX
    status: pending
isProject: false
---
...
│   ├── include/
│   │   └── udna_custom_kernels.h     # Pure C header (no HIP types)
│   └── hip/
...
└── lib/Compiler/
    ├── CompilerDriver.cpp            # MODIFIED: Add udna_custom_kernels.lib to link line
    └── CMakeLists.txt                # MODIFIED: Configure UDNA_CUSTOM_KERNELS_LIB_PATH
```
...
### 2. `custom_kernels/include/udna_custom_kernels.h`
...
- `extern "C" int udna_rope_forward(...)` -- host launcher that calculates grid/block dims and calls `hipLaunchKernelGGL`
...
hip_add_library(udna_custom_kernels STATIC
    hip/rope_kernel.hip
    hip/gqa_kernel.hip
    INCLUDE_DIRECTORIES
...
set(UDNA_CUSTOM_KERNELS_LIB_PATH "$<TARGET_FILE:udna_custom_kernels>" PARENT_SCOPE)

# Export include directory for Runtime bitcode compilation
set(UDNA_CUSTOM_KERNELS_INCLUDE_DIR "${CMAKE_CURRENT_SOURCE_DIR}/include" PARENT_SCOPE)

# Install the static library and header to the install prefix
install(TARGETS udna_custom_kernels
    ARCHIVE DESTINATION lib
    LIBRARY DESTINATION lib
)
install(FILES include/udna_custom_kernels.h DESTINATION include)
```

After `cmake --install`, the install prefix will contain:

- `<prefix>/lib/udna_custom_kernels.lib` (or `.a` on Linux)
- `<prefix>/include/udna_custom_kernels.h`
...
- Include `udna_custom_kernels.h` (pure C, Clang-compatible)
- Implement `wrap_rotary_embedding(...)` and `wrap_group_query_attention(...)` (declared in [hipdnn_ep_runtime.h](c:\Users\Administrator\shoucair\onnx_hipdnn_workspace\UDNA-compiler\lib\Runtime\hipdnn_ep_runtime.h))
...
- Call the `udna_rope_forward()` / `udna_gqa_forward()` C functions
...
Add them to `RUNTIME_BC_MODULES` list. Also add `-I${CMAKE_SOURCE_DIR}/custom_kernels/include` to the `compile_to_bitcode` macro's include flags so Clang can find `udna_custom_kernels.h`.
...
// Defined via: target_compile_definitions(... UDNA_CUSTOM_KERNELS_LIB_PATH="...")
std::string custom_kernels_lib = UDNA_CUSTOM_KERNELS_LIB_PATH;
...
The path is baked in at CMake configure time via a `#define`, pointing to `<install_prefix>/lib/udna_custom_kernels.lib`. This follows the same pattern as the install prefix (`../../local`) used by the project.

### 9. Install rules summary

The `custom_kernels/CMakeLists.txt` install rules ensure that after `cmake --install`:

- `<prefix>/lib/udna_custom_kernels.lib` -- static library with compiled HIP kernels (fat binaries)
- `<prefix>/include/udna_custom_kernels.h` -- pure C header for the kernel API
