...
morphizen::DllSafe<std::vector<std::unique_ptr<morphizen::ExecutionProvider>>>
compile_onnx_model(const onnxruntime::GraphViewer& graph_viewer,
...
/**
 * Replace EventRecord with std::tuple<std::string, int ,int, long long, long
 * long>, because EventRecord is defined in profiler_common.h which is used
 * inside onnxruntime. However, profiler_collect function will call vitis ep
 * which can't include profiler_common.h.
 */
...
CreateExecutionProviderFromAnotherEp(
    const std::string& lib, const OrtSessionOptions& session_options,
    std::unordered_map<std::string, std::string>& provider_options);

/**
 * Get compiled model compatibility information from execution providers.
 * Returns a JSON string containing compatibility metadata, or an empty string
 * if unavailable.
 */
...
/**
 * Validate compiled model compatibility information against current runtime
 * environment. The model_compatibility is output parameter for the
