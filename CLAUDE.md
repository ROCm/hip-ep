...
- **Output-allocator mode (`use_output_allocator` provider option, default OFF) — compile flag → module attribute and embedded metadata are a single source of truth.** When set, the model compiles to a **2-arg** `inference_compute(state, inputs)` with graph outputs allocated in-graph via `hip.alloc_output` → the EP's allocator callback (instead of the classic 3-arg out-param ABI). The flag is set once on the OnnxToHip half in `lib/Compiler/CompilerDriver.cpp` (`onnxToHipOpts.useOutputAllocator`), where the slot-4.5 pair `hip-use-output-allocator` (IR rewrite) + `hip-set-output-allocator-attr` (stamps the `hipdnn.output_allocator` module attribute) runs. The HipToLLVM half (`convert-hip-to-llvm` + the single `generate-interface`) reads that **module attribute** off the IR to pick the 2-arg ABI — it has no separate flag, so the mode rides on the module and the two halves can never disagree. The same compile flag is also written into the model `Metadata` (`backend-mlir-compiler/proto/metadata.proto`). The EP reads its dispatch arity from the **embedded metadata** (`metadata_.use_output_allocator()` in the `MlirCustomOp` ctor), NOT from the live provider option — so `main_graph` arity, the `inference_compute` wrapper arity, and the EP's dispatch can never disagree, even on a reused EPContext. Key invariants when touching this path:
  ...
  - **The allocator path needs NO present.* share-buffer override — and must not grow one.** `output_allocate_cb` passes the DLL's in-graph `shape` to `GetOutput` verbatim, with zero `present.*`/share-buffer special-casing; every output (KV cache included) is acquired identically. This is correct because a `present.*` output's dynamic seq dim is materialized in-graph by `hip.alloc_output` from `memref.dim %past_key` (the past input buffer's actual extent — see `createEmptyTensor` in `lib/Conversion/OnnxToHip/OnnxToHipUtils.h` + `GqaConversion.cpp`), which under OGA `past_present_share_buffer` already IS the `max_length` capacity buffer. So ORT's single `KernelContext_GetOutput(idx, dims)` (the ONLY output-acquisition call in the custom-op C API — it returns the pre-bound IO-bound `OrtValue` ONLY when `dims` matches the bound capacity) gets capacity and returns the shared buffer, preserving `past == present` pointer identity for in-place GQA append. The classic `marshal_output_tensors` path DOES need the override only because it pre-computes the present shape from `DimSource`→`attention_mask` (the *tight* current length, e.g. 7), which would make `GetOutput` allocate a fresh tensor. **Do NOT re-add the override to the allocator callback** — it would be a provable no-op (`out == past`) AND would recouple the model-agnostic allocator path to OGA conventions (exactly the layering concern raised in PR #306 review). The invariant is pinned end-to-end by `test/lit/e2e/test_gqa_output_allocator_present_dim.mlir` (each dynamic `present.*` `hip.alloc_output` ← `memref.dim` of its `past_key_values.*` input). The override itself lives **inline** in the classic `marshal_output_tensors` (`MlirCustomOp.cpp`) and bumps only **dynamic** (-1) present dims up to the matching past capacity (static dims — architecture constants — never touched). It is intentionally NOT shared with the allocator path (no extracted helper): the allocator path needs none, so sharing would only recouple it.
