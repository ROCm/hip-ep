...
- **`hip-fix-loop-accumulator-offset` pass — iter-driven offset for Concat-grown `hip.loop` accumulators** (`lib/Dialect/Transforms/FixLoopAccumulatorOffset.cpp`, pipeline step 5b in `Pipelines.cpp`). Outlined `hip.loop` body funcs are emitted by OneShotBufferize + BufferResultsToOutParams as in-place writers: the `v_in` (loop-carried in) and `v_out` (`bufferize.result`) args alias the SAME memref descriptor (`LoopLowering.cpp::createOrGetTrampoline`, the "v_out_i == v_in_i" line). The body's Concat-grow accumulator computes the chunk-append byte offset as `arith.muli %v_in.dim(N), %stride`, but MLIR memref descriptors are immutable by-value SSA so `dim(N)` is FROZEN at `v_init`'s compile-time value every iteration — every iter writes the chunk to the SAME offset and only the last iteration's contribution survives (canonical: a windowed-attention `onnx.Loop` body in a transformer vision encoder, where cos vs CPU collapses to ~`1/max_trip` ≈ 0.48 for `max_trip=2`). The pass rewrites that offset to be iter-driven: it prefers the TRUE cumulative start `cu_seqlens_k[iter]` (loaded/synthesized from the body's captured seqlens-style `hip.gather`) and falls back to `iter * chunk_size` when no such table exists. No-op on any body func without the frozen-dim-in-offset pattern. Cross-ref: the trampoline-aliasing comment in `LoopLowering.cpp` points here, and the file header documents architecturally-cleaner alternatives (separate `v_out` buffer, side channel) if the post-bufferize patch is ever retired. **This is the compiler half of a multi-part vision-accuracy fix** — the full end-to-end cos=1.0 result additionally requires the `memrefCopy` strided-copy fix (`lib/Runtime/real/hip.cpp`, ships with the runtime/projector slice of #259) and the dynamic-`Slice` empty-extent path (`SliceConversion.cpp`, ships with dynamic-shape vision support), so on its own this pass is a correct-but-dormant transform until those land. Regression coverage when the full stack is present: `test/python/test_qwen3_5_9b.py::...::test_vision_grid_small_matches_cpu_cosine`.
