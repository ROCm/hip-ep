...
- **Output-allocator mode (`use_output_allocator` provider option, default OFF) — compile flag and embedded metadata are a single source of truth.** When set, the model compiles to a **2-arg** `inference_compute(state, inputs)` with graph outputs allocated in-graph via `hip.alloc_output` → the EP's allocator callback (instead of the classic 3-arg out-param ABI). The flag drives BOTH pipeline halves in `lib/Compiler/CompilerDriver.cpp` (`onnxToHipOpts.useOutputAllocator` gates `hip-use-output-allocator`; `hipToLlvmOpts.useOutputAllocator` gates `generate-allocator-interface` vs `generate-interface`) AND is written into the model `Metadata` (`backend-mlir-compiler/proto/metadata.proto`). The EP reads its dispatch arity from the **embedded metadata** (`metadata_.use_output_allocator()` in the `MlirCustomOp` ctor), NOT from the live provider option — so `main_graph` arity, the `inference_compute` wrapper arity, and the EP's dispatch can never disagree, even on a reused EPContext. Key invariants when touching this path:
