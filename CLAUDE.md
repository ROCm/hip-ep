...
- **Output-allocator mode (`use_output_allocator` provider option, default OFF) — compile flag → module attribute and embedded metadata are a single source of truth.** When set, the model compiles to a **2-arg** `inference_compute(state, inputs)` with graph outputs allocated in-graph via `hip.alloc_output` → the EP's allocator callback (instead of the classic 3-arg out-param ABI). The flag is set once on the OnnxToHip half in `lib/Compiler/CompilerDriver.cpp` (`onnxToHipOpts.useOutputAllocator`), where the slot-4.5 pair `hip-use-output-allocator` (IR rewrite) + `hip-set-output-allocator-attr` (stamps the `hipdnn.output_allocator` module attribute) runs. The HipToLLVM half (`convert-hip-to-llvm` + the single `generate-interface`) reads that **module attribute** off the IR to pick the 2-arg ABI — it has no separate flag, so the mode rides on the module and the two halves can never disagree. The same compile flag is also written into the model `Metadata` (`backend-mlir-compiler/proto/metadata.proto`). The EP reads its dispatch arity from the **embedded metadata** (`metadata_.use_output_allocator()` in the `MlirCustomOp` ctor), NOT from the live provider option — so `main_graph` arity, the `inference_compute` wrapper arity, and the EP's dispatch can never disagree, even on a reused EPContext. Key invariants when touching this path:
  ...
  - **The present.* share-buffer override lives in the callback.** In allocator mode `marshal_output_tensors` is not called, so the present.*→past `max_length` override is replicated inside `output_allocate_cb` via the pure, GPU-free `apply_present_share_buffer_override` helper (`backend-mlir-compiler/custom-op-mlir/src/output_shape_override.h`) — SHARED with the classic marshal path so the two cannot drift. It bumps only **dynamic** (-1) present dims up to the matching past input's capacity (static dims never touched). Unit-tested across several dynamic shapes in `test/runtime/test_output_allocator.cpp` (`OutputAllocatorUnitTest`).
  - **Per-Compute install/clear + output-completeness guard.** The `OutputAllocatorCtx` lives on the stack; the allocator is installed (`self = &ctx`) before `compute_allocator` and cleared (`set_output_allocator(nullptr)`) immediately after so `self` can't dangle. After compute, every metadata output index must have been served by exactly one `hip.alloc_output` callback, else `LOG(FATAL)` (passthrough / deduped-aliased outputs are unsupported in allocator mode today; no target model has them).
...
**Data-dependent outputs (e.g. NonZero) REQUIRE `use_output_allocator=1`.** `DimSource` can only resolve a dynamic output dim to a *single input dim*. An op like `NonZero` whose output `[R, N]` has a data-dependent `N` (the non-zero count) maps to no input dim_param at all — and even its over-allocated upper bound `numel(X)` is a *product* of input dims, which a single-source `DimSource` cannot encode. `build_metadata_json` therefore emits an unresolved `DimSource` sentinel (`input_idx=-1, resolved=false`) for dynamic output dims **only when `useOutputAllocator` is true**; otherwise it hard-`CHECK`s (terse `Output 'Y' dim 1 has dim_param 'nnz' but no graph input declares this symbolic name`). In allocator mode the in-graph `hip.alloc_output` reports the real runtime shape to the EP via `output_allocate_cb` and `marshal_output_tensors`/DimSource is never consulted — so such ops run end-to-end only with the allocator pipeline (`--use-output-allocator` in `run_verify_numerics.sh`, provider option `use_output_allocator=1`). NonZero's GPU kernel (`3rd-party/custom_kernels/hip/nonzero_kernel.hip`, single-block prefix-sum stream compaction) fills the over-allocated `[R, numel]` buffer; the buffer is zero-filled first so the undefined tail `[N, numel)` is deterministic. `wrap_nonzero` (`lib/Runtime/real/nonzero.cpp`) carries a host-side `input_shape` array (built in `NonZeroLowering.cpp` like `wrap_transpose`'s) so the kernel can unravel each flat index into `R` row-major coords. Verified 2026-06-09 against ORT CPU (scenario_1, all-non-zero input → `numel == N`, Combined L2 = 0).
