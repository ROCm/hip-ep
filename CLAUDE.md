...
- **Never put a potentially-large dimension (e.g. an lm_head's `N = vocab_size`) on `blockIdx.y` or `blockIdx.z` — always on `blockIdx.x`.** HIP documents `maxGridSize[0..2]` as 2^31-1 across all current AMD GPUs, but in practice the effective limit on `grid_y`/`grid_z` is tighter on some driver/architecture combinations (an old 65535 CUDA-era cap that AMD's HIP still inherits in places). `grid_x`'s 2^31-1 is uniform. The canonical failure pattern this masks: an ONNX `MatMulNBits` with `bits=4`, `zp_elem_size=1` (AWQ asym, e.g. `Llama-3.1-8B-awq-g128-int4-asym-fp16-onnx-dml`'s lm_head) at decode-time. `wrap_matmul_nbits` calls `hip_matmul_nbits_unpack_zp_u8` (`3rd-party/custom_kernels/hip/matmul_nbits_kernel.hip`) *before* dispatching to GEMV. That unpack launcher used `grid = (packed_cols/THR, N)` — i.e. `grid_y = N = 128256`. The launch fails with `hipErrorInvalidConfiguration`, the launcher returns `void` without checking the error, the sticky error sits on the thread's last-error slot, and **the subsequent GEMV launch's `hipGetLastError()` picks it up** — making the error message read `[custom_kernels] hip_matmul_nbits GEMV launch failed: invalid configuration argument` even though GEMV itself never failed. Three fixes in one (`hip_matmul_nbits_unpack_zp_u8`, `hip_matmul_nbits_convert_zp_fp16`, `launchDequant`+`dequant_u4_to_fp16`): swap the grid so `grid_x = N`, `grid_y = packed_cols_groups` / `k_groups`. The kernels' `blockIdx.x` / `blockIdx.y` references were swapped to match. **Symptom recognition rule of thumb**: if a "launch X failed: invalid configuration argument" line points at a kernel whose launch geometry is obviously valid (small grid, small block, no shared mem), the real culprit is an *earlier* launch with `grid_y > ~65535` whose error went unchecked. Search the call chain immediately upstream for any `dim3(small_x, large_N)`-style launcher.
- **`HIPCK_RDNA3_WMMA` / `HIPCK_RDNA4_WMMA` select the matmul_nbits WMMA inner-loop variant at compile time; neither disables WMMA entirely.** CMake defines exactly one (`3rd-party/custom_kernels/CMakeLists.txt`): `HIPCK_RDNA3_WMMA=1` when any RDNA 3 arch (`gfx1100`/`gfx1101`/`gfx1102`/`gfx1103`/`gfx1150`/`gfx1151`/`gfx1152`/`gfx1153`) is in `HIP_ARCHITECTURES`; otherwise `HIPCK_RDNA4_WMMA=1` (gfx12 and all other non-RDNA-3 targets). Under `HIPCK_RDNA4_WMMA`, `GemmFp16U4Impl` and `GemmFp16I8Impl` in `matmul_nbits_kernel.hip` use half8 A/B fragments + `__builtin_amdgcn_wmma_f32_16x16x16_f16_w32_gfx12`, and the C/D writeback row formula changes from RDNA 3's interleaved `r = e*2 + sub` to RDNA 4's row-block `r = e + sub*8` (col is `lane` on both). The shared-memory layout (`smA[buf][M][K]`, `smB[buf][N][K]`), block grid, swizzle, autotune table, and bias-add kernel are unchanged — only the per-lane K-slice on fragment load and the per-lane row formula on writeback are arch-specific. Future gens add `HIPCK_RDNA5_WMMA` as another mutually-exclusive branch. `gemm_wmma_kernel.hip` and the GQA fused-prefill WMMA path in `gqa_kernel.hip` are trap-stubs under `HIPCK_RDNA4_WMMA` (no caller in `lib/Runtime/`), so they were not ported. If a future runtime caller wakes up either of those, port them following the same pattern: (1) replace `half16` with `half8` in fragment decls, (2) load 4 uint32 (not 8) from `&sm[buf][...][ks*16 + sub*8]`, (3) call the `_gfx12` builtin, (4) change writeback `r = e*2 + sub` → `r = e + sub*8`. Getting the writeback row formula wrong produces a deterministic row permutation in the output tile that builds cleanly and fails silently in the model — the canonical regression signature.
