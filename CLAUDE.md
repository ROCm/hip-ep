...
- **Cygwin git shadows Git for Windows on PATH.** The dev machine has Cygwin (`C:\cygwin\bin\git.exe`, v2.21.0) earlier on PATH than Git for Windows (`C:\Program Files\Git\bin\git.exe`, v2.53.0). Cygwin git 2.21 does not support `--trailer` (added in git 2.30), causing `git commit` to fail with `error: unknown option 'trailer'` when Cursor injects trailer metadata. **Always use the full path** `"C:\Program Files\Git\bin\git.exe"` for commits, pushes, and other git operations in this repo.
- **`hip_custom_kernels.lib` size is a version signal.** The build at `C:\workspace\local\lib\hip_custom_kernels.lib` grows as new kernels are added to main (e.g. `hip_gqa_flash_decode`, `hip_qmoe_decode_fused`, `hip_qmoe_bucket_tokens`). When deploying to `C:\workspace\package_gfx1151_therock7.11\lib\`, always copy all three files from the local build: `onnxruntime_morphizen_ep.dll`, `hip-compiler.dll`, and `hip_custom_kernels.lib`. Missing the lib causes lld-link failures at model-DLL compile time (undefined symbol errors).
- **GQA decode crashes with skv ≥ ~12200 tokens (hipBLASLt access violation).** Running the 120B model with `prompt_12200.txt` (chunk_size=512) completes all prefill chunks but crashes at the first decode step inside `wrap_group_query_attention` → `hipblasLtMatmul` with `0xC0000005`. The 3800-token prompt works fine. Root cause is likely a workspace-buffer or tile-index overflow in hipBLASLt for very large `skv`. This is an independent issue from the swizzle fix (PR #207). Needs investigation.
- **WMMA swizzle bug in `matmul_nbits_kernel.hip` (fixed in PR #207).** When `n_tiles % sw != 0`, the old swizzle mapping was non-bijective: some tiles were overwritten (collision) and others were never written (missing), leaving stale data from prior launches. The bias-add kernel then performed RMW on stale values, making QMoE outputs non-deterministic. This caused autotune to re-run every prefill layer on the 120B model, inflating TTFT ~8× (51 s → 6.2 s after fix). Decode TPS was unaffected (M=1 → GEMV path). Fix: split grid into main-region (full sw-wide groups) and tail-region (plain row-major for leftover columns). Also replaced `__builtin_nontemporal_store` with regular stores to restore cache coherence. Verified by exhaustive bijection test across 8000 (n_tiles, sw) combinations.
