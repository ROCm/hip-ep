...
- **Models mostly need static shapes; embedding subgraphs are the exception.** The MLIR compiler does not yet support symbolic dims for full transformer models (Llama / gpt-oss / etc.) -- the perf-test harness runs `fix_shapes()` to bind `batch_size` / `sequence_length` to concrete values first. The two Qwen `embedding.onnx` subgraphs (9B and 35B-A3B) are exercised with EVERY input `dim_param` left symbolic (`batch_size`, `sequence_length`, `num_logical_patches`) by `test/numeric/tests/test_nonzero_qwen*_embedding_dyn.py` -- one `InferenceSession` services every `(B, S, K)` combination back-to-back without recompile. When extending dyn-input support to a new graph, look at those tests' patterns (single session reused, per-call CPU reference, `assert_subgraph_on_ep` via `make_session`).
...
- **Slot-buffer-coalesce Phase 3 + Phase 4 invariants** (`lib/Dialect/Transforms/SlotLifetimeCoalesce.cpp`, `lib/Dialect/Transforms/IdentityPropagatorRebind.cpp`; see `docs/design/slot-buffer-coalesce.md` for the per-class taxonomy). Two compile-time passes reduce dynamic-shape footprint and kernel-launch count:
  - **`hip-slot-lifetime-coalesce`**: groups published slot ids by canonical DimSpec bytes (`DimSpec::canonicalize()` constant-folds + sorts commutative children) and first-fit-decreasing bin-packs by lifetime within each group. Output-bound slots (consumed by `hipdnn.output_index` results) inherit `lastUseIdx = +∞` so they never coalesce with intermediates that die mid-graph, and two output-bound slots cannot share storage. **Verifier-enforced** by `VerifyDimSpecsPass`: any RuntimeSlot leaf MUST reference a slot id `< hipdnn.dyn_dim_slots_count` AND output-bound slots may not be claimed by two distinct publisher ops. Disabled with `--hip-slot-lifetime-coalesce=disable-coalesce=true` for A/B footprint measurement.
  - **`hip-identity-propagator-rebind`**: erases ops whose registered identity predicate (`shape_interface::isIdentityOp`) returns true. Built-in predicates cover `hip.transpose` (perm = `[0..rank)`), `hip.cast` (same dtype), `hip.expand` / `hip.tile` / `hip.reduce_*` (shape match, with `noop_with_empty_axes=1` for reduce), and `hip.slice` (shape match AND provably `step==1`, `start==0`, `end>=dim` on every sliced axis — see the "Identity-predicate same-shape shortcut" gotcha below for why shape alone is unsafe for slice). Predicates are **compile-time only** — they may only inspect attributes, result types, and compile-time-constant producer chains (`arith.constant`, externalised splat globals); they MUST NOT inspect runtime values. Runs PRE-bufferize in the OnnxToHip tail (the post-bufferize HIP DPS form has zero results, so the pass relies on the tensor-IR form where each op produces a single SSA value). Disabled with `--hip-identity-propagator-rebind=disable-identity-rebind=true`.
  - **`--hip-print-pool-stats`** (`lib/Dialect/Transforms/PrintPoolStats.cpp`) is the before/after diagnostic for both passes. Prints `static_pool_bytes`, `dyn_dim_slots_count`, `slot_publisher_count`, `identity_propagator_predicate_hits` to stderr — wire it around any pipeline edit that should change footprint.
  - **DimSpec leaves inside `output_dim_specs` MUST be rewritten in lockstep with publisher slot id renumber.** Both passes walk `output_dim_specs` ArrayAttr (shape `ArrayAttr<ArrayAttr<ArrayAttr<DenseI64ArrayAttr>>>`) and rewrite the slot id at field index 5 of every `RuntimeSlot` leaf (kind = 3 at field index 0). Forgetting this leaves the slot leaves pointing at a defunct slot id and the EP-side resolver reads garbage post-`stream_sync`. The same helper pattern lives in both `SlotLifetimeCoalesce.cpp` and `IdentityPropagatorRebind.cpp` — keep them in sync if the DimSpec encoding ever changes (add a 9th field, etc.).
  - **Post-bufferize passes that read per-op slot grids MUST size the grid from `DestinationStyleOpInterface::getDpsInits()` , not `op->getNumResults()`.** `hip-slot-lifetime-coalesce` runs AFTER one-shot bufferize, where every `Hip_DpsOp` (`hip.nonzero`, `hip.transpose`, `hip.matmul`, …) has **zero SSA results** — its outputs are exposed only as `outs` operands via the DPS interface. An earlier `readSlotGrid` that iterated `for r in [0, op->getNumResults())` returned an empty grid for every publisher in the post-bufferize IR, silently making the entire pass a no-op (no slot collected → `infoBySlot.empty()` → early return) while still printing a believable `dyn_dim_slots_count`. The footprint regression in `test/lit/e2e/test_slot_lifetime_coalesce_footprint.mlir` only fires when the pass actually reduces the count; an all-zero collect produces a numerically valid (but unmerged) `dyn_dim_slots_count == next_dyn_slot_id` and looks healthy. Pattern fix (in `SlotLifetimeCoalesce.cpp`): a `numLogicalResults(op)` helper that returns `op->getNumResults()` for tensor-form ops and falls back to `cast<DestinationStyleOpInterface>(op).getDpsInits().size()` for buffer-form ops; `readSlotGrid` then trusts the outer ArrayAttr size of `hipdnn.output_slot_ids` when present so a future result-drop during lowering never truncates the grid. Same trap applies to any pass that wants to inspect a Hip op's logical-result types after bufferize (e.g. a future "slot-aware DCE"). Pre-bufferize passes (`ReservePropagatorSlots`, `IdentityPropagatorRebind`) are safe — they walk tensor-form IR by design.
    - **Two other post-bufferize sites that had the same bug shape** (silently truncated to zero per-result iteration on Hip DPS ops). Both fixed alongside the `SlotLifetimeCoalesce` fix; check any new post-bufferize logic that does per-result iteration against this list before adding more:
      - `lib/Dialect/Transforms/DimSpecsDebug.cpp` `VerifyDimSpecsPass` Phase 3 invariant (i) — the `for (r = 0; r < arr.size() && r < op->getNumResults(); …)` loop bound by `getNumResults()` skipped the `hipdnn.output_slot_ids` vs static-dim leak check on every real publisher. Fix mirrors `SlotLifetimeCoalesce` — drop the `getNumResults()` bound (trust the attribute's outer rank) and resolve the per-result type via a `logicalResultType(op, r)` helper that falls back to `getDpsInits()[r]`.
      - `lib/Dialect/Transforms/ComposeDimSpecs.cpp` `resolveSourceForOutParam` — single-result Hip DPS ops were already safe because `dps_result_index` defaulted to 0 and `output_dim_specs` had exactly one outer entry. The latent bug bites future multi-logical-result DPS publishers (`hip.gqa`-style ops with 3 init operands) that write a non-first out-param directly: `getNumResults() == 0` skipped the lookup so `dps_result_index` stayed at 0, indexing the wrong `output_dim_specs` outer entry. Fix adds a fallback that enumerates `DestinationStyleOpInterface::getDpsInits()` and matches by SSA `Value` identity. Flat operand index `k` (`ins + outs`) is NOT a substitute for the DPS-init index; the bug surface here was specifically "find which logical result this out-param corresponds to" not "find which operand."
  - **`@main_graph_internal` fallback in `PrintPoolStats`:** `GenerateInterface` renames `@main_graph` to `@main_graph_internal`, so the diagnostic pass falls back to the renamed symbol when the original is absent. Any future analysis pass that hooks into the post-`GenerateInterface` IR must do the same fallback (or look for `func.func` with the `hipdnn.input_count` attribute on the parent module).
  - **Identity-predicate "same shape ⇒ identity copy" shortcut is ONLY safe for shape-preserving ops without data-permuting attributes.** `isIdentityCast` / `isIdentityTranspose(perm=[0..r))` / `isIdentityExpand` / `isIdentityTile` / `isIdentityReduce(noop_with_empty_axes=1)` are safe because for those ops, equal input/output shape mathematically pins the only attribute that could change behaviour (perm, repeats, broadcast factor). **`hip.slice` is the odd one out**: a full-reverse slice (`start=dim-1, end=-(dim+1), step=-1`) produces the same shape as the input but permutes the data, and an earlier `isIdentitySlice` that relied on shape match alone elided those ops and silently returned a copy of the input (cosine ≈ 0.67 on the reversed-vector regression, same min/max/mean — the classic permutation-bug fingerprint). The current `isIdentitySlice` (`lib/Dialect/IR/HipShapeInterface.cpp`) requires PROOF at compile time that `step==1`, `start==0`, and `end>=dim` on every sliced axis. The helper `readI64Constant` chases producer chains through three patterns: (1) `arith.constant dense<>`, (2) `bufferization.to_tensor` ← `memref.get_global` with in-tree `initial_value`, (3) `bufferization.to_tensor` ← `memref.get_global` with `hip.external_data` whose module-level `hipdnn.constant_source_kinds[idx] == 1` (Splat). When the operands are runtime values, or the constant kind isn't a resolvable splat, the predicate returns false (conservative — the slice runs at runtime, which is always safe). **Rule for future identity predicates**: if the op has any attribute / operand that can permute, reverse, shuffle, or otherwise non-trivially alter data while leaving the shape intact (negative strides, gather indices, scatter indices, non-identity perm, mode flags like "reverse"), shape-match alone is NOT enough — the predicate MUST evaluate or conservatively bail on those attrs / operands.
...
- **EP CPU fallback is now strict by default; set `HIPDNN_EP_ALLOW_CPU_FALLBACK=1` to opt back in.** `backend-mlir-compiler/level-1-pass/src/pass_main.cpp` throws `morphizen::CompilationError` (caught at the ORT boundary as a failed `InferenceSession` construction) when the MLIR pipeline can't lower the EP-claimed subgraph. The old `LOG(WARNING) << "MLIR compilation failed, skipping"` path is still available behind the env var for users who specifically need ORT's silent CPU fallback. **Why this matters**: silent CPU fallback used to mask numeric correctness on the EP — a "passing" test that secretly ran on CPU is indistinguishable from an actually-working EP kernel. The numeric suite's `assert_subgraph_on_ep(session, ep_name)` helper (`test/numeric/framework/ep_diagnostics.py`) is the second line of defense and runs at session-create time inside the `ort_ep` backend via `make_session`.
- **`broadcastToShape` accepts dynamic targets** (`lib/Conversion/OnnxToHip/OnnxToHipUtils.h`). The helper used to assert that every output dim of the target shape was static — fine for fully-bound graphs, fatal the moment any consumer needed a `tensor<?x...>` broadcast target (e.g. `EqualConversion` under `tensor<?x?xi64>` input_ids). The helper now accepts an optional `shapeSource` value that supplies runtime dims via `tensor.dim`; the static-target codepath is unchanged. When adding a new ONNX-to-HIP conversion that broadcasts to a dynamic result, drop any `resultIsStatic` guard and pass the dyn result as the shape source (see `EqualConversion.cpp` for the pattern).
- **`wrap_slice` accepts `-1` sentinels in `output_shape` for dynamic dims** (`lib/Runtime/real/slice.cpp` + `lib/Conversion/HipToLLVM/SliceLowering.cpp`). For Category-C chains (`NonZero -> ... -> Slice`) the slice's output memref is pool-allocated to the upper bound, NOT the per-call extent. The wrapper used to compare its derived extent against `output_shape[d]` (read from the descriptor) and abort on every call whose runtime extent was below the pool's upper bound (e.g. K=4 followed by K=1 in the Qwen embedding graph would size the pool to 16384 then reuse the same buffer for a 4096-element slice — abort). Fix: the lowering passes `-1` for every dynamic output dim and the wrapper trusts the derived extent and substitutes it into a local `resolved_output_shape` before launching `hip_slice`. Static-output slices still get the strict equality check (a mismatch there really IS a lowering bug). When adding new dyn-output ops to the runtime, follow this convention: `-1` in the IR-supplied shape array means "trust your own per-call computation".
- **One InferenceSession must accept multiple shape combinations** (`test/numeric/framework/ort_ep_backend.py::OrtEpBackend.make_session`). For dyn-input regression coverage, the framework exposes `make_session(model_path)` in addition to the per-call `run(model_path, inputs)`. The single-session promise (the same compiled DLL services every per-call shape via ORT's `sess.run`) is what gates "dynamic shapes work" — calling `run` four times with four shapes creates four sessions and silently masks per-call resize bugs in the runtime. The `_dyn` numeric tests use the `make_session` path explicitly; new dyn-shape tests should do the same.
- **Slot-aware consumer annotation MUST search the DPS writer per view step, not after walking to the pool root** (`lib/Dialect/Transforms/AnnotateInputDimSlots.cpp::findMemRefWriter`). After bufferize + `hip-pool-allocs`, every dynamic-shape intermediate ends up as `%cast = memref.cast(%view = memref.view(%pool, offset))`. Both the producer (e.g. `hip.transpose` writing into its own upper-bound DPS init) and the consumer (e.g. `hip.scatter_nd` reading the indices) reference the SAME `%cast` SSA value. The legacy walker called `walkThroughViews(%cast)` to chase `cast → view → pool` first and then looked at `%pool`'s users — but the only users of `%pool` are sibling `memref.view`s, none of which is a DPS writer, so the walker returned `nullptr` and the consumer lost its `hipdnn.input_dim_slots` / `hipdnn.input_slot_buffers` annotation entirely. The lowering then read the dynamic dim from the descriptor (= upper bound, e.g. 524288 instead of the runtime-published 0), and `hip.scatter_nd` launched a 524288-thread kernel that scattered garbage from the uninitialised upper-bound buffer over the gather output — Qwen embedding's `test_n_zero_no_image_tokens` exposed this as a single-element corruption at `output[0,0,0]`. Fix: walk view steps incrementally, searching for a DPS hip writer with `cursor` as a DPS-init operand BEFORE stepping back through `cast/view/subview/expand/collapse`. Stop at the first writer found.
- **Slot publisher vs translucent propagator distinction in `pickProducerBufferSlot`** (`lib/Dialect/Transforms/AnnotateInputDimSlots.cpp::collectInputDimSlots`). When deciding whether to redirect a consumer's input POINTER to the slot table (`hipdnn_ep_state_read_buffer(slot_id)`), only honour DIRECT slot publisher attributes (`slot_id` for single-publisher ops like `hip.nonzero` / `hip.range`; `slot_ids` for multi-axis publishers like `hip.constant_of_shape`). **Do NOT honour `hipdnn.output_slot_ids`** — that attribute is attached by `ReservePropagatorSlots` to TRANSLUCENT propagators (`hip.transpose`, `hip.slice`) which wrote into their own upper-bound DPS-init buffer and never published a buffer to the slot table (only the dim). Redirecting consumer pointers via a translucent's `output_slot_ids` slot returns the slot's never-published `nullptr` and trips a `null required argument` early-exit in the consumer wrapper. SHAPE rewiring (`hipdnn.input_dim_slots`) is independent — it always uses the per-dim `RuntimeSlot` from the DimSpec system and is the only mechanism translucent propagators contribute to.
- **Cat-C slot publishers MUST publish a non-null buffer even when N=0** (`lib/Runtime/real/nonzero.cpp::wrap_nonzero`; same pattern applies to `hip.range` dynamic and any future Cat-C publisher). When the publisher's output has zero elements, allocate a tiny sentinel (1 byte) from the dyn pool and publish that pointer instead of `nullptr`. Consumer wrappers (`wrap_transpose`, `wrap_scatter_nd`, `wrap_gather_nd`, etc.) null-check their input buffer pointers and bail with `-1` regardless of the matching slot-published dim. With the sentinel, the pointer is non-null and the consumer correctly does zero reads/writes because it gets the dim from `hipdnn_ep_state_read_dim(slot_id) = 0`. The sentinel costs 1 alloc (~256 bytes after pool alignment) per Cat-C op per inference but eliminates a class of "N==0 ⇒ false null-pointer error" failures across the entire dynamic-shape pipeline.
...
**Fixed-shape requirement (full transformer models):** The compiler requires all tensor dimensions to be static for full transformer graphs (Llama, gpt-oss, Phi, etc.). Models with dynamic dimensions (e.g. `batch_size`, `sequence_length`) must be converted to fixed shapes before use. The perf test fixtures handle this automatically via `fix_shapes()` in `test/python/conftest.py`. The Qwen `embedding.onnx` subgraphs are the first multi-op graphs run with fully symbolic input dims -- see the `_dyn` numeric tests under `test/numeric/tests/`.
...
**PERF-block `hipEventElapsedTime` MUST guard on per-event "recorded" flags, not just `g_perf.initialized`** (`lib/Runtime/hipdnn_ep_runtime_tensor.cpp::PerfState`). The phase-timing block in `hipdnn_ep_stream_sync` and the previous-inference flush in `hipdnn_ep_tensor_prepare_input` both call `hipEventElapsedTime(&compute_ms, h2d_end, d2h_start)` etc. `d2h_start` is only recorded inside the body of `hipdnn_ep_tensor_finalize_output`, which **early-returns for Category-C deferred sentinel outputs** (`gpu_ptr == host_ptr == nullptr && size_bytes == 0`) — so for models whose outputs are *all* Category-C (canonically NonZero, also any pure dyn-shape op), `d2h_start` is never recorded. Querying elapsed time on an unrecorded event returns `hipErrorInvalidHandle` (400) — not `hipErrorNotReady` — and the failure path silently sets the thread-local sticky HIP error. The next inference's first kernel launch then reports `static_cast<int>(hipGetLastError()) == 400` as a fake "launch failure"; the wrapper bails out before publishing the Category-C dim slot, and `MlirCustomOp` aborts with `LOG(FATAL) << "Output ... RuntimeSlot ... unpublished"`. The fix is to track `h2d_start_recorded` / `h2d_end_recorded` / `d2h_start_recorded` / `d2h_end_recorded` per inference and only call `hipEventElapsedTime` when both endpoints are recorded; reset all four flags at the start of each new inference window in `prepare_input(index==0)`. Symptom signature: `test_nonzero.py` (or any all-Category-C model suite) passes one-test-at-a-time but the 2nd test in a sequence aborts with `RuntimeSlot ... unpublished` — but **only when `HIPDNN_EP_PERF=1` is set in the env**. If you can't reproduce the failure with PERF off, this is the bug.
...
- **Compiler requires fully static shapes for full transformer models.** No `batch_size` / `sequence_length` symbols for Llama / gpt-oss / Phi etc. `fix_shapes()` in `test/python/conftest.py` handles this for the perf tests; the dynseqlen branch (off this stage-1) is the long-term solution. Qwen embedding subgraphs are the exception (see new "Test Models" wording above) and have first-class dyn-input numeric tests.
