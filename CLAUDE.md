...
- **PrecisionFreeCast (PFC) — fp16 saturation in residual / GEMM chains poisons LayerNorm with NaN; fix is at the ONNX-graph level, not the kernel level** (`lib/Conversion/OnnxToHip/SimplifyOnnx.cpp`, `simplifyPrecisionFreeCast`). Transformer blocks export as `<fp16 op> -> onnx.Cast(fp16->fp32) -> <fp32 op>` chains where the fp16 storage between op and Cast is **not actually consumed** by ORT CPU's MLAS — MLAS keeps the fp32 internal value and the Cast is effectively a no-op. Our compiler historically lowered each op independently, so the fp16 storage really happened, and any value that overflowed fp16 max (~65504) saturated to ±Inf at the producer's output. Downstream `Cast(fp16->fp32)` faithfully preserves Inf, the fp32 `LayerNormalization` then computes `var = E[x²] - E[x]² = Inf - Inf = NaN`, and the entire row's output becomes NaN. One NaN row of K or V into all-to-all attention fans out to 100% NaN at the next attention/reshape, so by 1-3 blocks downstream the entire tensor is NaN even though only one input element was non-finite. Canonical trigger: Qwen 3.5 9B vision encoder on a Flickr30k sample whose residual chain accumulates one fp16 ±Inf at block 21 `linear_85` (the attention projection GEMM whose dot product overflows fp16 — CPU and GPU produce IDENTICAL fp16 saturation here, the divergence is entirely in what happens DOWNSTREAM of the saturation). **Strict first-divergence point** (verified by per-op probe of vision encoder block 21->22): `LayerNormalization_43` (block 21 norm2). Inputs are bit-exact between CPU and GPU (`add_3461[728, 1151] = -Inf` on both); CPU LN_43 outputs 0 NaN finite (`[-304.68, 9.50]`); GPU LN_43 outputs 1152 NaN at row 728. **Root cause** (verified by 3 minimal `Cast(fp16,fp32) -> LN`, `Add(fp16) -> Cast -> LN`, `Gemm(fp16) -> Cast -> LN` control experiments under ORT_DISABLE_ALL): ORT CPU's `Add` AND `Gemm` ops compute internally in fp32 and pass the fp32 intermediate value DIRECTLY to a downstream `Cast(fp16->fp32) -> fp32_consumer` chain, COMPLETELY bypassing the fp16 storage round-trip. The fp16 graph output is materialized as a side-channel via saturating cast at the Cast boundary, but the main data flow stays fp32. Our compiler used to materialize fp16 for real, so the saturation happened and propagated. Fix: `simplifyPrecisionFreeCast` mirrors the CPU behavior at the ONNX-dialect level. It walks `onnx.Cast(fp16->fp32)` anchors whose users are all fp32-pure (LayerNormalization OR any op whose results are all fp32) and whose producer is one of {`onnx.Add`, `onnx.Sum`, `onnx.Gemm`, `onnx.MatMul`} with fp16 result, and rewrites the producer to fp32 result type (with per-input fp16->fp32 casts AND all attributes preserved -- alpha/beta/transA/transB for Gemm). The pass loops until no new anchors appear (cap 64; in practice converges in 2-3 iterations because Add promotion exposes Gemm anchors and vice versa). Peek-through eliminates intermediate `Cast(fp32->fp16)` bridges so a multi-block residual chain promotes in one walk. Verified on Qwen 3.5 9B Flickr30k VLM PPL: pre-fix sample 5 / 7 vision is 100% NaN; post-fix all 50/50 samples finite, **PPL = 91.490 vs CPU 91.496 (delta -0.0074%)**, hardhats `image_features` cos = 1.000001 vs CPU. Strictly compiler-level — no kernel band-aid required. Bisection methodology when a new model surfaces a similar NaN cascade: probe the residual chain bottom-up using `hardhats_bisect/build_multi_probe.py` (adds intermediate tensors as graph outputs without breaking PFC anchors — do NOT probe the `Cast(fp16->fp32)` outputs themselves because that breaks PFC's "all users are fp32-pure" precondition); look for the first op where CPU/GPU finite stats agree but only GPU has NaN at a downstream LN. The "exactly one row -> exactly 3 rows after attention" pattern is the diagnostic fingerprint. **Coverage gap to monitor:** PFC currently anchors only on {Add, Sum, Gemm, MatMul} producers; if a future model saturates fp16 in `Mul`, `Pow`, `Conv`, `MatMulNBits`, `ReduceSum`, `Softmax`, etc. -> Cast -> fp32 chain, that op needs to be added to `isPromotableProducer` in SimplifyOnnx.cpp. The clean repro for "is this another PFC-anchor case" is the same: minimal `OpUnderTest(fp16) -> Cast -> LN` ONNX with overflow-prone inputs; ORT CPU should produce 0 NaN, our EP should reproduce the failure pre-fix.
