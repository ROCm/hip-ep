...
- **Output-allocator mode (`use_output_allocator` provider option, default OFF) — compile flag → module attribute and embedded metadata are a single source of truth.** Enable by passing the option as the string `"1"` in the EP provider-options dict: pure ORT `so.add_provider_for_devices(devices, {"use_output_allocator": "1"})`; OGA `genai_config.json` → `"provider_options": [{"MorphiZenEP": {"use_output_allocator": "1"}}]` (it's a literal `== "1"` string compare in `pass_main.cpp`, default `"0"`). When set, the model compiles to a **2-arg** `inference_compute(state, inputs)` with graph outputs allocated in-graph via `hip.alloc_output` → the EP's allocator callback (instead of the classic 3-arg out-param ABI). The flag is set once on the OnnxToHip half in `lib/Compiler/CompilerDriver.cpp` (`onnxToHipOpts.useOutputAllocator`), where the slot-4.5 pair `hip-use-output-allocator` (IR rewrite) + `hip-set-output-allocator-attr` (stamps the `hipdnn.output_allocator` module attribute) runs. The HipToLLVM half (`convert-hip-to-llvm` + the single `generate-interface`) reads that **module attribute** off the IR to pick the 2-arg ABI — it has no separate flag, so the mode rides on the module and the two halves can never disagree. The same compile flag is also written into the model `Metadata` (`backend-mlir-compiler/proto/metadata.proto`). The EP reads its dispatch arity from the **embedded metadata** (`metadata_.use_output_allocator()` in the `MlirCustomOp` ctor), NOT from the live provider option — so `main_graph` arity, the `inference_compute` wrapper arity, and the EP's dispatch can never disagree, even on a reused EPContext. Key invariants when touching this path:
  - **ORT EPContext is NOT keyed on provider options.** Flipping `use_output_allocator` does not invalidate a previously written `*_ctx.onnx`; the stale artifact still carries the old DLL + old metadata bit and runs in the old mode. To switch modes for an EPContext model you MUST regenerate the `*_ctx.onnx`. The in-process compile path is unaffected — `generateTempPath` is always unique, so each ORT session recompiles a fresh DLL and the flag takes effect immediately (this is why the Python e2e tests can just toggle the provider option).
