...
**hip Compiler**: Standalone MLIR-based compiler infrastructure for AMD GPUs. Provides C API, C++ library, and CLI tools for ONNX→MLIR→DLL compilation.
...
**Key Options**:
- \`BUILD_STANDALONE_TOOLS=ON\` (default) - Build hip-opt and hip-compile
...
**Directories**: \`include/hip-compiler/\`, \`lib/\`, \`tools/hip-opt/\`, \`tools/hip-compile/\`, \`dll/\`, \`test/\`, \`proto/\`
