...
- **Static CRT means separate CRT per DLL.** `onnxruntime_morphizen_ep.dll` is built with `/MT` and has its own MSVC CRT instance — `std::getenv()` from runtime-bitcode code merged into the EP DLL (or from JIT-resolved CRT symbols) cannot see env vars set by the host process (the .exe and ORT have their own CRT instances). Use `GetEnvironmentVariableA()` (Win32 API) instead — it queries the per-process env block that all CRTs share. See `debug_log.h` for the pattern.
...
- **TheRock DLLs on PATH at runtime.** `onnxruntime_morphizen_ep.dll` imports `amdhip64_7.dll` directly, and `BitcodeJIT` explicitly `LoadLibrary`s `MIOpen.dll` / `hipblaslt.dll` / `hipdnn_backend.dll` at JIT-init time so ORC's process-search generator can resolve the symbols the JIT'd per-model bitcode references. All of those need to be findable on PATH — point it at `install/therock/bin/` (or the equivalent ROCm dist) before running `hip-onnx-runner`, `hip-test-bc`, or anything else that loads the EP.
...
- **Stale cached EPContext bitcode after runtime changes.** The per-model `.bc` artifact embeds the merged runtime bitcode from build time (`LLVMBackend::linkRuntimeModule` snapshots `runtime.bc` into every compiled model). The MorphiZen cache key is the ONNX graph hash, NOT the runtime version — so editing `lib/Runtime/*.cpp` or `3rd-party/custom_kernels/*.hip` and rebuilding does NOT invalidate cached compiled artifacts. After rebuilding, either delete cached EPContext sidecars (location depends on the EPContext config — typically `*_ctx.onnx` next to the model, or whatever path you configured) or force a recompile via the EPContext "regenerate" knob. The bitcode switchover replaced the previous `%TEMP%\morphizen_mlir_*.dll` cache mechanism.
...
- **wrap_qmoe scratch + pinned host staging — pageable hipMemcpyAsync silently sync-stages on Windows.** `lib/Runtime/real/qmoe.cpp` previously did per-call `hipMalloc`/`hipFree` of 8 transient device buffers AND used `std::vector` heap-allocated host buffers as the source/dest for `hipMemcpyAsync` D2H/H2D of the routing decision and per-expert (id, weight) staging. Two independent fixes (must apply together to see the win):
  ...
  - **Cached-artifact rebuild:** when editing this code, you also need to invalidate any cached EPContext sidecars (location depends on EPContext config; see the "Stale cached EPContext bitcode" gotcha above). The cache key is the ONNX hash, not the runtime version — without invalidation, JIT'd bitcode keeps using the stale runtime.
...
```
ONNX (.onnx) → [onnx-to-hip-pipeline] → HIP dialect → [bufferize + optimize] → [hip-to-llvm-pipeline] → LLVM IR → linkRuntimeModule → model.bc + constants.bin (embedded in EPContext sidecar, JITted in-process by BitcodeJIT at session creation)
```
...
- **Real** (`lib/Runtime/real/`): GPU execution via MIOpen, hipBLASLt, custom HIP kernels. Compiled to `runtime.bc` and merged into the per-model bitcode by `LLVMBackend::linkRuntimeModule` at compile time.
- **Mock** (`lib/Runtime/mock/`): CPU stubs for GPU-free development/testing. Enabled with `BUILD_MOCK_RUNTIME=ON`.

### Generated bitcode entry points

`GenerateInterface.cpp` emits four functions into the per-model `.bc`
artifact (each one resolved at session creation by `BitcodeJIT` /
`InferenceState`):
...
- `inference_get_metadata_json` — returns a pointer to the embedded `@__metadata_json` global describing input/output shapes and the constants filename
...
- **Custom Op** JITs the per-model bitcode at session creation via `BitcodeJIT::create`, then executes the resolved `inference_compute` on every `Compute()` call with a shared HIP stream
...
```cmd
rem From the repo root — TheRock ROCm DLLs must be on PATH for the EP DLL
rem and for BitcodeJIT's runtime LoadLibrary calls to find MIOpen/hipblaslt
...
**`inference_state_->begin_compute()` runtime contract** (`backend-mlir-compiler/custom-op-mlir/src/MlirCustomOp.cpp`). Per-Compute() hook into the JIT'd bitcode that invalidates per-Compute caches (currently only the GQA seqlens_k cache). Called by the EP at the top of every `Compute()` before any input marshaling. **Backwards compatibility:** older cached EPContext bitcode predates this export — it doesn't crash, the hook is a cached indirect dispatch that no-ops on missing symbol, but `HIPDNN_EP_GQA_CACHE_SEQLENS` MUST be set to `0` for that case or the cache will return stale total_seq across forward passes. Cost on the fast path is ~1 ns (one cached indirect call).
...
- **Stale per-model bitcode cache** is keyed on the ONNX hash, not the runtime version. Any `lib/Runtime/*.cpp` or `3rd-party/custom_kernels/*.hip` change MUST be followed by EPContext cache invalidation (see the "Stale cached EPContext bitcode" gotcha above), or JIT'd bitcode at session-creation time will silently use the snapshot of `runtime.bc` from the previous build.
