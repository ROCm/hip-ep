...
- **Asym MatMulNBits zero_points unpack cache assumes stable pointers across inferences** (`lib/Runtime/real/matmul_nbits.cpp` + `lib/Runtime/real/zp_unpack_cache.h`). For `bits=4, zp_elem_size=1` (asym AWQ), `wrap_matmul_nbits` and `wrap_qmoe` look up the unpacked uint8 buffer (and, when WMMA/col-major-GEMV-M>1 dispatches, the converted fp16 buffer) from an `unordered_map<const void*, ...>` keyed on the input `zero_points` GPU pointer. Cache miss → `hipMalloc` + one launch of `hip_matmul_nbits_unpack_zp_u8` / `hip_matmul_nbits_convert_zp_fp16`; hit → no kernel launch. **Invariant**: the `zero_points` pointer must be stable across inferences — true today because it points into the `model.dll` constants blob (lifetime = session). If a future codepath ever passes a transient zp pointer (e.g. computed on-the-fly per inference), it MUST bypass this cache (or cache by a content hash, not pointer) — otherwise stale unpacked data is returned. The cache lives in the `ZpUnpackState` op-module (see `docs/design/runtime-module-registry.md` and the "Runtime module system" section below) and is freed by `~ZpUnpackState` through the module registry at session cleanup.
...
- **wrap_qmoe scratch + pinned host staging — pageable hipMemcpyAsync silently sync-stages on Windows.** `lib/Runtime/real/qmoe.cpp` previously did per-call `hipMalloc`/`hipFree` of 8 transient device buffers AND used `std::vector` heap-allocated host buffers as the source/dest for `hipMemcpyAsync` D2H/H2D of the routing decision and per-expert (id, weight) staging. Two independent fixes (must apply together to see the win):
  - **Device side:** route the transient device buffers through the `QmoeState` op-module's `scratch` buffer (a `hipdnn_ep::GrowableDeviceBuffer` from `lib/Runtime/growable_buffer.h`, see "Runtime module system" section). Layout is one contiguous buffer with 64-byte aligned sub-buffers; `wrap_qmoe` calls `qm->scratch.grow(total)` then offsets sub-buffers off `qm->scratch.data()` (1.5× realloc on grow — the free step inside `grow()` implicitly syncs via `hipFree`, never shrinks). Freed by `~QmoeState` through the module registry at session cleanup. This eliminates 8 `hipMalloc` + 8 `hipFree` per call (192 of each per token on 24-layer gpt-oss-20b).
  - **Host side (the perf-critical fix):** route the D2H readback (`h_indices`, `h_weights`) and per-expert H2D staging through the `QmoeState` op-module's `host_scratch` buffer (a `hipdnn_ep::GrowablePinnedBuffer` allocated with `hipHostMalloc(hipHostMallocDefault)`). Pageable memory passed to `hipMemcpyAsync` on Windows silently falls back to a synchronous staging copy, blocking the GPU pipeline; pinned memory enables true async DMA that overlaps with prior in-flight kernels.
...
### Runtime module system (op-state registry)

Per-session persistent state for individual operators lives in **op modules** registered through `lib/Runtime/module_registry.h`, NOT as new fields on `RuntimeState`. Use this whenever a new op needs to cache descriptors / GPU buffers / prepacked weights across `wrap_*` calls.

```cpp
// Inside lib/Runtime/real/my_op.cpp
...
int wrap_my_op(RuntimeState *state, ...) {
  auto *s = my_op_module(state);   // O(1) after first call this session
  s->cache.find(...);
}
```
...
**When NOT to use a module:** truly framework-level shared resources (stream / library handles, the constants blob, the buffer pool). Those stay on `RuntimeState` directly. The shared device-VRAM workspace itself **is** a module (`WorkspaceState`, Pattern B) — see the cross-TU note above.

**Where the helpers live:**
- `lib/Runtime/module_registry.{h,cpp}` — registry, `HIPDNN_OP_MODULE`, lifecycle hooks.
- `lib/Runtime/growable_buffer.h` — `hipdnn_ep::GrowableDeviceBuffer` / `GrowablePinnedBuffer` for the canonical "1.5x grow, never shrink" pattern. The free step relies on `hipFree`/`hipHostFree`'s implicit `hipDeviceSynchronize` (HIP runtime spec) to drain in-flight kernels referencing the old pointer — no explicit `hipStreamSynchronize` is required and none is performed. Prefer these over hand-rolling `hipMalloc`/`hipFree` inside a module.
- Live examples: `WorkspaceState` (the shared device-VRAM scratch reached cross-TU via `workspace_module(state)->ws.{grow,data,size}()`), `CausalConvState`, `ZpUnpackState`, `GqaGemmState`, `GqaSeqlensCache` (with `begin_compute`), `QmoeState`.
...
7. **Per-session state (optional)**: if the op needs to cache descriptors, prepacked weights, or any other persistent buffer across `wrap_*` calls, add an op-module via `HIPDNN_OP_MODULE` in the runtime `.cpp` — **do not add a new field on `RuntimeState`**. See the "Runtime module system (op-state registry)" section above for the pattern.
...
**GQA GEMM descriptor cache is intentionally session-local — disk persistence is deferred.** `queryOrCreateGemmState` (in `lib/Runtime/real/gqa.cpp`, lives on the `GqaGemmState` op-module) keys hipBLASLt descriptors + `hipblasLtMatmulAlgoGetHeuristic`-selected algos on `(m, n, k, batch, transA, outputFp32, strideA, strideB, strideC)`. The heuristic call is the only meaningful cost in `queryOrCreateGemmState` and runs **only on cache miss** — every subsequent inference of the same shape skips it entirely. For Llama-8B prefill the one-time cold-start cost is roughly ~150-250 ms total across all (score, value) GEMM shapes; steady-state cost is zero. **Do not assume this is per-call cost when reading the code at `gqa.cpp:454-473`** — those lines are the miss path. Disk persistence (the way matmul_nbits WMMA does it via `_results.json`) was considered and deferred: `hipblasLtMatmulAlgo_t` is not officially portable across hipBLASLt / driver versions, so a validate-or-rebuild fallback chain would be required; benefit is only on first-prefill of a new shape per process. If we revisit this, the right shape is a small shared `lib/Runtime/disk_cache` helper that both WMMA and GQA use, and persistence stays inside the op-module per the `docs/design/runtime-module-registry.md` "On-disk persistence" convention (no framework-level save/load hook).
