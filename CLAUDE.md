...
- **Windows Defender scans every newly-emitted `%TEMP%\morphizen_mlir_*.dll` and burns ~6–9 s per `LoadLibrary` in the numeric test suite** (`backend-mlir-compiler/custom-op-mlir/src/InferenceState.cpp::Plugin::create` calls `LoadLibraryW`; `backend-mlir-compiler/common/temp_path.hpp::generateTempPath` writes the model DLL to `std::filesystem::temp_directory_path()`). The compiled DLL is ~15 MB (embedded runtime bitcode + relocations against `amdhip64_7.dll` / `MIOpen.dll` / `hipblaslt.dll` / EP runtime). Each numeric test in `--no-cache` mode emits a fresh DLL with a unique filename (`morphizen_mlir_<pid>_<ms_timestamp>_<idx>.dll`), so Defender's image-scan cache never hits. **Symptom**: first test in a pytest session takes ~3.5 s (cold), every subsequent test in the same session takes ~12 s (`LoadLibrary` jumps from 0.1 s → 8–9 s). This makes multi-test suites (`test_range.py`: 15 tests, `test_nonzero.py`: 19 tests, etc.) appear "slow on dynamic-shape tests" — they're NOT — single-test wall time is identical to static-shape (`test_sigmoid`) once you account for which-test-is-first; the suite total just scales with `(N - 1) × Defender_overhead`. Two-line fix:
  ```powershell
  # PowerShell, one-time, must be Admin. Persistent across reboots
  # (stored in HKLM\SOFTWARE\Microsoft\Windows Defender\Exclusions\Paths).
  Add-MpPreference -ExclusionPath "$env:TEMP"
  ```
  Verified end-to-end on `test_range.py::test_range_i64_category_b` (5 tests, Strix Halo 2026-05): **46 s → 30 s** (≈ 1.5 × wall-clock). Per-test `LoadLibrary` drops from 8.9 s → 2.9 s; the steady-state per-test budget drops from ~12 s → ~6 s. The remaining ~2.9 s is inherent Windows loader cost for a 15 MB DLL (IAT resolve + base relocations) and is not Defender-related. **Wildcard caveat**: `Add-MpPreference -ExclusionPath "$env:TEMP\morphizen_mlir_*.dll"` does NOT work as expected — Defender's filename-prefix wildcards are quirky (the documented `C:\MyData\*.txt` form only matches at the last folder level and does not honour our `_<pid>_*` prefix form). Use the folder-level form above instead. **Do NOT disable real-time protection globally** (`Set-MpPreference -DisableRealtimeMonitoring $true`) — Tamper Protection blocks this on modern Windows 11 (build 22631+) unless toggled off manually in Windows Security GUI, and even then it gets reverted on the next signature update. The folder exclusion is the only durable, low-risk option. Undo with `Remove-MpPreference -ExclusionPath "$env:TEMP"`.
