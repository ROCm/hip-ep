...
**gpt-oss-20b benchmark baseline (gfx1151, 2026-05-07, `model_benchmark.exe -r N -w 1`, no `HIPDNN_EP_PERF`).** Dynamic-shape DLL on the EP via OGA. L=128 → ~408 ms TTFT p50, ~40.74 tok/s decode, ~1.44 GB peak WS. L=2048 → ~14.4 s TTFT p50, ~24.79 tok/s decode, ~1.64 GB peak WS. Per-Compute GPU work is dominated by `wrap_qmoe` (24 calls/inference, 32 experts × 4 active per token) and `wrap_group_query_attention` (24 calls/inference, HPG=8 — flash_decode is HPG=4 only so gpt-oss takes the LDS-tiled fused_decode path at sq=1). Expect TTFT to scale roughly linearly with L (24 GQA layers × O(L²) attention dominates prefill); decode TPS to droop ~40% at L=2048 vs L=128 due to fused_decode skv-linear scaling (no flash_decode rescue at HPG=8 — see "Llama 8B at L=1024 with GQA flash_decode" architectural note for why). Numbers are working baselines — re-measure with current build before citing.
