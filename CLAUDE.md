...
- **`memrefCopy` runtime MUST honour memref strides — flat hipMemcpyAsync is only correct for fully-contiguous src+dst** (`lib/Runtime/real/hip.cpp::memrefCopy`). MLIR's standard `memref.copy` lowering invokes this runtime helper with descriptor pointers carrying both `sizes[]` and `strides[]` arrays. The previous implementation computed `bytes = product(sizes) * elemSize` and did a single `hipMemcpyAsync(dst, src, bytes)`, ignoring strides entirely. That is correct only when BOTH src and dst are row-major contiguous. The bufferized `tensor.insert_slice` chains that `ConcatConversion.cpp` emits (one slice per Concat input) regularly produce `memref.copy` whose destination is a strided `memref.subview` — for `Concat(axis=-1)` on rank-2 the dst subview has strides `[outer_total, 1]` while the src is contiguous with strides `[1, 1]`. Under the broken flat-memcpy the two contiguous source blobs are written into the dst buffer back-to-back (each clobbering most of the previous one's data) and the resulting output looks like a stride-shifted copy of just the LAST insert's input. Canonical symptom (Qwen 3.5 vision): `Reshape → Unsqueeze → Concat(axis=-1) → Expand → Tile` chain on two `[16,16,2,2]` index tensors produces an output whose flat bytes match `[0, e2[0..1022], e2[1023], garbage...]` instead of the expected interleaved `[e1[0], e2[0], e1[1], e2[1], ...]`. Fix: three-tier strategy (a) `rowStart == 0`: both sides fully contiguous → one `hipMemcpyAsync` (preserves the old fast path), (b) `rowStart == 1`: exactly one outer dim non-contiguous → one `hipMemcpy2DAsync` (the GPU DMA engine handles the strided pitch, no kernel launch), (c) `rowStart >= 2`: walk the outer index space and issue one `hipMemcpyAsync` per contiguous row. The "row" is the longest trailing suffix `[rowStart..rank-1]` where BOTH src and dst strides match the canonical row-major layout `strides[i] = product(sizes[i+1..rank])`. Regression coverage: `test/numeric/tests/test_shape_ops.py::TestConcat` covers axis=0 (contiguous fast path), axis=-1 / axis=1 on rank-2 and rank-3 (hipMemcpy2DAsync tier), and rank-3 axis=-1 (generic per-row tier). The `[1024,1] + [1024,1] @ axis=-1 → [1024,2]` case is included as the literal Qwen vision reproducer.
- **Outlined `hip.loop` body functions get their OWN pool slot, not main_graph's** (`lib/Runtime/hipdnn_ep_runtime_state.cpp::hipdnn_ep_get_pool_base`, `lib/Runtime/hipdnn_ep_runtime_loop.cpp::runLoopImpl`). All functions (main_graph + every outlined body) call `hip.get_pool` → `hipdnn_ep_get_pool_base(state, needed_size)`. Previously this returned the SAME `state->pool_base` for every caller, and both main_graph and body allocs computed their offsets starting at 0 — so the body's first kernel silently overwrote main_graph values stored at the same physical pool addresses (whenever a value needed to live across a `hip.loop` call). Fix: `thread_local int g_pool_depth = 0` incremented by `runLoopImpl` around `body_fn(...)`; depth-0 uses the original `pool_base/pool_size` slot, depth ≥1 uses `state->nested_pool_bases[depth-1]` (lazily allocated, grow-on-demand identical to the depth-0 slot, freed in cleanup). thread_local so two parallel inference sessions on different host threads don't share the counter. Per-state slots so the lifetime matches the model session (no cross-session pool pollution).
- **Qwen 3.5 vision cosine — `hip-fix-loop-accumulator-offset` pass fixes the in-place v_in==v_out Concat-grow bug** (`lib/Dialect/Transforms/FixLoopAccumulatorOffset.cpp`, `lib/Dialect/Transforms/Pipelines.cpp` step 5b). The pass rewrites every `memref.subview` whose offset for dim N is `memref.dim %v_in, %cN` to use `iter * chunk_size` instead (iter loaded from body's arg1; chunk_size = the subview's own size for that dim). This makes successive iterations write to disjoint regions of the in-place buffer instead of overwriting the same offset every iter. Combined with the `memrefCopy` strided-copy fix (gotcha above), `test_vision_grid_small_matches_cpu_cosine` went 0.482 (pre-pass) → 0.948 (pass in place, but flat-memcpy bug remained) → **cos=1.0000 on EVERY row (full 100% accuracy match to CPU baseline, max-abs diff ≤ 0.16 = fp16 LSB noise)** once memrefCopy honoured strides. Per-row ep_norm matches cpu_norm to within ~0.5%. Both fixes are needed: the pass alone leaves the residual 5% gap that the strided-copy fix closes. **Prerequisite**: SliceConversion's `select(extent==0, upper, extent)` guard must be in place (reverted from the "true dim-0" commit 63e671e) — the in-place buffer needs to be sized to hold max_trip chunks, which `data.dim(slice_axis)` provides exactly. Alternatives to deleting this pass are documented in its file header (separate v_out buffer, side channel, opt-out of BufferResultsToOutParams) — adopt one of them before removing the pass or the regression returns. Cross-reference in `LoopLowering.cpp::createOrGetTrampoline` next to the "v_out == v_in" aliasing line points readers at this pass.
- **Qwen 3.5 vision cosine 0.48 history (PRE-FIX, kept for grep'ability) — dynamic-grow `hip.loop` Concat-accumulator was broken** (`lib/Conversion/HipToLLVM/LoopLowering.cpp::createOrGetTrampoline`, `lib/Dialect/Transforms/...` whichever generates the body's Concat lowering). The test `test_vision_grid_small_matches_cpu_cosine` fails with deterministic cos=0.482351 (every output row 1..31 collapsed to ~(0.33, -3.05, 0.33) — looks like only the LAST loop iteration's contribution survives).

  **Why the "bisect: pinning ANY intermediate fixes it" finding was MISLEADING**: pinning any intermediate as a model output causes the EP to LOG(FATAL) on `pass_main.cpp:354` ("Output 'X' dim 0 is dynamic AND has no matching input dim_param..."). The fatal terminates EP compile; ORT silently falls back to CPU; the test compares CPU vs CPU and reports cos=1.0 max_abs=0. **None of the "pinning fixes" were genuine EP successes — all were CPU fallbacks.** Verified by running with `HIPDNN_EP_STRICT=1` (same fatal, same false cos=1.0).

  **The actual EP bug**: the outlined `hip.loop` body has signature `(arg0 ctx, arg1 iter, arg2 cond, arg3 v_in, ..., arg9 v_out {bufferize.result})`. The body's Concat-accumulator lowering after OneShotBufferize is:
  ```
  %base, %off, %sz:3, %st:3 = memref.extract_strided_metadata %arg9
  %prev = memref.reinterpret_cast %base[0] sizes=[%dim8, %dim9, %dim10] ; %dim9 = memref.dim %arg3, %c1
  memref.copy %arg3, %prev                           ; copy old accumulator (in-place, no-op if same buffer)
  %offset = arith.muli %dim9, %st#1                  ; offset = old_acc_size * stride
  %new = memref.reinterpret_cast %base[%offset] sizes=[1, %chunk, 1152]
  memref.copy %chunk_buf, %new                       ; append new chunk
  return
  ```
  The trampoline passes `v_out = v_in` (the SAME descriptor) per `LoopLowering.cpp` line 239 (`// v_out_i == v_in_i (same buffer, single-pass kernel safety).`). MLIR memref descriptors are by-value SSA, so the body cannot mutate the descriptor's `sizes` field — and the trampoline reloads the slot's descriptor unchanged each iter. Result: `arg3.dim(1)` is FROZEN at v_init's value (0 with the current Slice-true-empty fix; upper_bound with the old guard) for every iteration. The chunk gets written to `offset = arg3.dim(1) * stride` which is the SAME offset every iter, OVERWRITING the previous iter's chunk. After max_trip iters, the buffer holds only the LAST chunk; CPU has all chunks; cos = (1/max_trip) ≈ 0.5 for max_trip=2. Matches our 0.482 observation exactly (grid [2,8,8] has max_trip=2 windowed-attention loops).

  **Why the slice fix (true dim-0 v_init) made it slightly worse than the upper-bound guard** (0.482 vs 0.457): with the OLD guard, `arg3.dim(1) = upper_bound`, so the chunk is written at `upper_bound * stride` which is PAST the end of the in-place buffer (true OOB write). The OOB writes corrupt adjacent allocations — symptom is non-deterministic-looking garbage that happens to integrate to a slightly higher cosine. With the slice fix, chunks consistently overwrite at offset 0, and the buffer ends up with just the last chunk — cleaner failure mode but same root cause.

  **What needs to change for the genuine fix** (significant refactor; out of scope for this turn):
   1. Either: change `LoopLowering.cpp::createOrGetTrampoline` line 239 to pass a SEPARATE v_out buffer (not aliased with v_in), so the body's Concat writes a fresh buffer per iter and the trampoline tracks the new buffer's descriptor into the next iter's v_in slot — making the loop a value-based fold.
   2. Or: change the trampoline so it stores the body's new size back to the lcArrayPtr slot after each iter. Requires a side channel from body to trampoline (extra out-param, or a memory convention).
   3. Or: special-case "empty slice + Loop concat accumulator" in OnnxLoopOutline / SliceConversion to lower differently (e.g., pre-allocate a fixed-size carried buffer, use iter-driven offsets in the body, drop in-place writes).
  Until this is fixed, vision encoders with Concat-grown loop accumulators (windowed attention, per-image-token loops) will fail accuracy. LLMs unaffected because they don't use Concat-grown loop accumulators (KV cache is share-buffer in-place, never grown via Concat inside a Loop body).
...
- **`hip.sub` / `hip.div` broadcast (rank <= 4).** MIOpen has no subtract op, so `hip.sub` routes to `wrap_elementwise_sub`; `hip.div` routes to `wrap_div`. Both lowerings now pass full 4D shapes (left-padded ranks 1-3, same as Add/Mul). When lhs or rhs shape differs from output, the runtime materialises the broadcast via `hip_expand` into the per-session workspace, then calls the flat custom kernel on same-shape buffers. Sub and Div are not commutative — operands are never swapped. Rank > 4 is rejected at lowering. The ONNX-level `BroadcastDivToMulReciprocal` rewrite in `ProjectorOpsRewrites.cpp` remains as an alternative compile-time path for Div.
