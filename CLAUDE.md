...
- **Asym MatMulNBits zero_points unpack cache assumes stable pointers across inferences** (`lib/Runtime/real/matmul_nbits.cpp` + `lib/Runtime/real/zp_unpack_cache.h`). For `bits=4, zp_elem_size=1` (asym AWQ), `wrap_matmul_nbits` and `wrap_qmoe` look up the unpacked uint8 buffer (and, when WMMA/col-major-GEMV-M>1 dispatches, the converted fp16 buffer) from a per-`RuntimeState` `unordered_map<const void*, ...>` keyed on the input `zero_points` GPU pointer. Cache miss → `hipMalloc` + one launch of `hip_matmul_nbits_unpack_zp_u8` / `hip_matmul_nbits_convert_zp_fp16`; hit → no kernel launch. **Invariant**: the `zero_points` pointer must be stable across inferences — true today because it points into the `model.dll` constants blob (lifetime = session). If a future codepath ever passes a transient zp pointer (e.g. computed on-the-fly per inference), it MUST bypass this cache (or cache by a content hash, not pointer) — otherwise stale unpacked data is returned. The cache lives on `RuntimeState->zp_unpack_cache` and is freed in `hipdnn_ep_state_cleanup` via `hipdnn_ep_zp_unpack_cache_destroy`. Measured impact (8B asym Llama-3.1, gfx1151, 3 outer reps × `-r 5 -w 1`, no `HIPDNN_EP_PERF`, 2026-05-07): decode TPS DYN asym L=128 36.7 → 41.85 ± 0.41 tok/s, L=2048 33.0 → 37.00 ± 0.12 tok/s — large absolute lift over the pre-cache baseline. **Residual asym-vs-sym decode gap remains ~5-7%** across both DYN and FIX at both L (DYN L=128: asym 41.85 vs sym 43.94, −4.8%; DYN L=2048: 37.00 vs 39.27, −5.8%; FIX L=128: 41.52 vs 44.13, −5.9%; FIX L=2048: 36.62 vs 39.08, −6.3%) — likely the per-call extra arithmetic in the dequant inner loop (one extra `read_zp` + one extra fp32 mul per K-tile per N-tile). Single-run "<1.5% gap" numbers are a measurement artifact: sym L=128 first-of-3-reps is consistently ~3 tok/s slower than reps 2-3 (autotune-cache cold-start or thermal); asym does not show this. TTFT: DYN asym is ~13% FASTER than DYN sym at L=128 (161.5 vs 183.6 ms; reproducible across reps with std ≤0.6 ms — likely cheaper sym dequant init in the prefill path) and ~2% slower at L=2048; FIX asym is ~4% slower than FIX sym at both L. Peak WS unchanged within a model class (DYN ~1.24 GB, FIX ~2.70 GB; FIX is 2× because it carries two sets of constants); the cache itself adds only `N*ngk` bytes per zp pointer ≈ a few MB. Closing the residual ~5-7% steady-state decode gap is the open follow-up — needs a kernel-level change (template-specialization on `has_zp`, fused `zv*sv` precompute, or block-dim retuning), not a runtime/cache change.
- **wrap_qmoe scratch + pinned host staging — pageable hipMemcpyAsync silently sync-stages on Windows.** `lib/Runtime/real/qmoe.cpp` previously did per-call `hipMalloc`/`hipFree` of 8 transient device buffers (expert_indices/weights, gather/fc1/act/fc2 bufs, token ids/weights) AND used `std::vector` heap-allocated host buffers as the source/dest for `hipMemcpyAsync` D2H/H2D of the routing decision and per-expert (id, weight) staging. Two independent fixes (must apply together to see the win):
  - **Device side:** route the 8 transient device buffers through a per-`RuntimeState` `qmoe_scratch` (`runtime_state_internal.h`). Layout is one contiguous buffer with 64-byte aligned sub-buffers; total size grows on demand via `hipdnn_ep_state_ensure_qmoe_scratch` (sync-then-realloc, never shrinks); freed in state cleanup. This eliminates 8 `hipMalloc` + 8 `hipFree` per call (192 of each per token on 24-layer gpt-oss-20b).
  - **Host side (the perf-critical fix):** route the D2H readback (`h_indices`, `h_weights`) and per-expert H2D staging (`h_ids`, `h_wts_e`) through a per-`RuntimeState` pinned host buffer allocated with `hipHostMalloc(hipHostMallocDefault)` (`qmoe_host_scratch`). Pageable memory passed to `hipMemcpyAsync` on Windows silently falls back to a synchronous staging copy, blocking the GPU pipeline; pinned memory enables true async DMA that overlaps with prior in-flight kernels.
  - **Measured impact (gpt-oss-20b, gfx1151, `model_benchmark.exe -l 128 -g 128 -r 5 -w 1` no `HIPDNN_EP_PERF`, 2026-05-07):** baseline 38.73 tok/s / TTFT 264 ms → device-only fix 39.01 tok/s (+0.7%, within noise — the malloc storm cost was much smaller than predicted because HIP caches mallocs internally on Windows) → device + host pinned 54.90 tok/s / TTFT 189 ms (**+41.7% / -28% over baseline**). The host-side fix is what moves the needle; do not ship one without the other (the device-side change is still architecturally correct and satisfies the user's no-host-allocs-mid-run requirement). Vulkan llama.cpp baseline on the same gfx1151 reports ~70 tok/s, so a residual ~21% gap remains — most plausibly the 24 per-layer `hipStreamSynchronize` calls qmoe still does after the topk D2H (one per layer to bucket on host before per-expert dispatch). Phase 2 step 2 (next bullet) closes ~half of that gap by replacing the host bucketing with a GPU kernel.
  - **Memory hygiene:** matches the durable user requirement of "no host memory allocations in the middle of model run; host memory must be reused from run to run and might grow when shape changes". Steady state at fixed shape: zero per-call `hipMalloc`, `hipFree`, `hipHostMalloc`, or `hipHostFree`. Both buffers are reused across runs and only grow when a larger `num_tokens` is seen — exactly when the GPU pool also grows.
...
- **wrap_qmoe Phase 2 step 2: GPU-side bucketing eliminates per-expert host build + per-expert H2D round-trips.** Builds on the Phase 1 scratch + pinned-host fix above. Old flow per MoE layer: D2H of `expert_indices` (num_tokens·k int32s) + `expert_weights` (num_tokens·k fp16) → `hipStreamSynchronize` → host loop bucketing tokens by expert → for each active expert: build `h_ids`/`h_wts_e` on host → 2× `hipMemcpyAsync` H2D → launch gather/matmul/scatter chain. New flow: `hip_qmoe_bucket_tokens` kernel on the device (one block × `num_experts` threads, three passes — atomicAdd count, exclusive prefix sum into `expert_offsets`, scatter `(token_id, weight)` pairs into `sorted_token_ids`/`sorted_weights` ordered by expert) → D2H of just `num_experts` int32 counts (e.g. 32×4 = 128 bytes) → `hipStreamSynchronize` → host computes `h_offsets` from prefix sum → for each active expert: pointer arithmetic into the on-device sorted buffers (`d_sorted_token_ids + offset`, `d_sorted_weights + offset*elem_size`) handed straight to gather/scatter_add — no per-expert H2D. Sync still happens once per layer (still need `h_counts` on the host to drive the per-expert dispatch loop), but the D2H volume drops from `num_tokens·k·(4 + elem_size)` bytes to `num_experts·4` bytes (~30× smaller at L=128 decode), and 2× per-expert H2D launches per layer × 4 active experts = 8 launches × 24 layers = 192 launches/token are eliminated. Scratch layout in `lib/Runtime/real/qmoe.cpp` adds `expert_counts`/`expert_offsets`/`sorted_token_ids`/`sorted_weights` sub-buffers to `qmoe_scratch`; host scratch shrinks to just `num_experts·int32`. Kernel: `3rd-party/custom_kernels/hip/qmoe_kernel.hip::bucket_tokens_kernel` (fp16-only, num_experts ≤ 1024, block_dim = `((num_experts+31)/32)*32`). **Measured impact (gpt-oss-20b, gfx1151, `model_benchmark.exe -l 128 -g 128 -r 5 -w 1`, 2026-05-07):** decode 54.90 → 62.16 tok/s (**+13.2%**). TTFT 189 → 198 ms (+5%, expected — the bucket_tokens launch + D2H of counts adds a small per-layer overhead in prefill where the savings ratio is smaller). Peak WS 1.44 GB unchanged. 8B Llama (sym dyn) regression check: 42.59 tok/s, no change vs pre-fix baseline. Vulkan baseline still ~70 tok/s; residual gap is now ~12% — closing it requires either eliminating the per-layer sync entirely (needs a fused per-expert dispatch kernel that drives the matmul launches from device-side `expert_counts`) or attacking the qmoe matmul dequant inner loop.
- **wrap_qmoe Phase 3: fully fused decode kernel (num_tokens == 1) BEATS Vulkan baseline.** Builds on Phase 2. For decode the multi-pass path (topk → bucket_tokens → D2H sync of `expert_counts` → host loop over active experts → per-expert gather/fc1/swiglu/fc2/scatter_add) is replaced by **one early-return branch** in `wrap_qmoe` that calls a new `hip_qmoe_decode_fused` launcher right after `hip_qmoe_topk_routing`. The fused launcher issues exactly **three back-to-back kernel launches** with **zero `hipStreamSynchronize` and zero D2H per layer**:
  - `qmoe_decode_fc1_kernel<BS=64, TN=8>`: grid `(2*inter/TN, k)`. Each block reads `expert_indices[slot]` to pick its expert, dequants a `[TN, hidden]` tile of `fc1_weights` per-expert (factored AWQ INT4 dequant `(dot - a_sum*zp) * scale` in fp32, vectorized uint4 K-tile loads identical to matmul_nbits_gemv), warp-shuffle reduces, **fuses pair-wise SwiGLU** (`G * sigmoid(α·G) * (L + β)` with `swiglu_limit` clamp) into the write phase, and stores `[k, inter]` activations to `act_out`. No LDS preload of the input vector — direct global reads cache through L2/MALL across the 720 blocks per slot. BS=64 chosen so K-tile count (`hidden/32 = 90`) gives every thread 1-2 iterations (BS=256 left ~65% of threads idle).
  ...
  - **Measured impact (gpt-oss-20b dyn, gfx1151, `model_benchmark.exe -r 5 -w 1`, no `HIPDNN_EP_PERF`, 2026-05-07):** **L=128 decode 62.16 → 75.96 tok/s (+22.2% — BEATS Vulkan baseline 70 tok/s by +8.5%)**, TTFT 198 → 188 ms (unchanged within noise — fused decode does not fire during prefill), peak WS 1.44 GB unchanged. **L=2048 decode 24.79 → 36.90 tok/s (+49% over the pre-fix baseline).** OGA path measures slightly higher (77.7 tok/s) due to less framework overhead. 8B Llama (sym dyn) regression check: 43.95 tok/s, no change vs pre-fix baseline (Llama doesn't use qmoe). Accuracy: `test_ort_per_step_logits` `decode[1] cosine=0.938176` is **byte-identical to the documented baseline** (CLAUDE.md "decode[1] cosine drops to 0.938"), confirming numeric equivalence to the multi-pass path; `test_ort_fixed_prefill_128` previously listed as a known fragility now passes.
  ...
  - **Open follow-ups for further uplift:** per-shape autotune over `(BS, TN)` (matmul_nbits-style); fp16 `atomicAdd` to fuse FC2+reduce; investigate routing-precision sensitivity that drives the deeper `decode[N]` cosine drift in `test_ort_per_step_logits` (router fp32 precision or top-K intersection compare).
- **OGA per-token IoBinding rebind cost scales with `max_length` (KV buffer size), not actual sequence length — biggest remaining gpt-oss-20b L=2048 cost is OGA-side, not the EP kernel.** OGA's `State::Run` (`install/oga-source/src/models/model.cpp:172-188`) caches an `OrtIoBinding`, then **every call** does `ClearBoundInputs() + ClearBoundOutputs() + BindInput(name, ortvalue) × N + BindOutput(name, ortvalue) × N` for *all* names. In `past_present_share_buffer=true` mode the 48 KV `OrtValue*` (24 layers × 2 × past as input + present as output, both pointing to the same OrtValue per layer/kind) are set once in `DefaultKeyValueCache::Add()` (`kv_cache.cpp:296-299`) and **never change pointer** for the entire generation; `DefaultKeyValueCache::Update` early-returns under share-buffer (`kv_cache.cpp:304-305`). So the ~50 stable Bind calls per Run are wasted work, but they cost **real wall time proportional to the bound OrtValue's byte size**, not its element count or O(1). Measured asymmetry on the input side only: at gpt-oss-20b ml=2080 (KV shape `[1,8,2080,64]` fp16 = 2.03 MB × 48 = 97.5 MB total bound), `clear_binding_inputs` = 3.4 ms, `bind_ortvalue_input × 50` = 8.6 ms; `clear_binding_outputs` and `bind_ortvalue_output × 49` together cost <30 µs (the OrtValues are still ref'd through the input side, so output-side ops are no-ops). At ml=128 the same calls cost <0.1 ms each. **The 12 ms input-side bind/clear at ml=2080 exactly matches the OGA Token-gen gap** previously documented (L=128 ml=160 = 13.05 ms/token, L=128 ml=2080 = 24.98 ms/token, L=2048 ml=2080 = 25.94 ms/token — kernel work is the same 13 ms in all three; the delta is *only* the IoBinding cycle scaling with `ml`). Likely root cause inside ORT IoBinding: `BindInput` of a `device_type="gpu"` OrtValue allocated by the EP's `hipHostMalloc` allocator triggers some allocator-side bookkeeping (page-table touch / device-pointer translation) on every call, scaling with the buffer's VA range rather than O(1). **Implications for measurement and citation:** (1) gpt-oss-20b decode tok/s through OGA is **bimodal in `max_length`**: the kernel ceiling at L=2048 is ~80 tok/s (kernel-only ≈ 12.5 ms/token, see `bench_gptoss_both.py` EP-direct) but OGA delivers 36-38 tok/s because OGA's bind cycle adds ~12 ms per token at ml≈2080. (2) **Always cite gpt-oss decode tok/s with the `max_length` (or `-ml`) value** — comparing 1B at ml=160 vs 20B at ml=2080 mixes two regimes. (3) Pure-ORT `IoBinding` benches (e.g. `test/python/conftest.py::run_timed_iobinding`) keep the bind cycle outside the timed window (`bind_all()` happens before `t0 = perf_counter()`), so they report **kernel-only** tok/s — they do NOT predict OGA-path TPS at large `max_length`. (4) When optimizing kernels for gpt-oss-20b, validate against the EP-direct bench, not OGA, otherwise OGA framework overhead masks kernel wins. **Probe**: `bench_oga_overhead_probe.py` (untracked scratch) — splits clear/bind/run into separate `time.perf_counter` segments, sweeps KV ∈ {128, 2080} on EP-allocated GPU OrtValues. Reproduces the scaling cleanly. **Decision: do NOT modify OGA** (per durable user direction 2026-05-08) — this gotcha exists to (a) prevent future sessions from re-investigating the same gap, and (b) keep gpt-oss-20b L=2048 tok/s claims honest. The fix would be a stable-binding skip in `State::Run` (track last-bound `OrtValue*` per name, only rebind on change), expected to recover ~12 ms/token at ml=2080 → L=2048 decode 36.9 → ~50 tok/s; not in scope.
...
| gpt-oss-20b (webgpu int4-rtn-block-32) | MXFP4 MoE (32 experts × 4 active per token) + fp16 attention | 24 (alternating sliding/full attention, sliding_window=128) | 8 (HPG=8) | 64 | ~12.5 GB (7 `model_q4f16.onnx_data*` blobs) | [onnxruntime/gpt-oss-20b-onnx](https://huggingface.co/onnxruntime/gpt-oss-20b-onnx/tree/main/webgpu/webgpu-int4-rtn-block-32) |
...
**gpt-oss-20b structural notes (distinct from the Llama family).** Mixture-of-Experts backbone (32 experts, 4 active per token, MXFP4-quantized expert weights — **not** MatMulNBits/AWQ INT4), `attention_bias=true`, alternating sliding/full attention layers (sliding_window=128 baked into the graph), YaRN rope scaling (`original_max_position_embeddings=4096`, `factor=32`, `theta=150000`, max position 131072), vocab=201088, hidden=2880. **No `position_ids` input** — rope is driven internally by the graph. OGA exposes the model as `model.type="gptoss"`, distinct from `decoder-only`/`decoder-pipeline` — older OGA forks may not recognize it (tests skip cleanly). Tests use `LlamaModelConfig(has_position_ids=False)`; the `Llama` in the dataclass name is historical — it's a generic per-layer KV-dim container. Weights are split across 7 `*.onnx_data*` external blobs — `ensure_model` / `ensure_fixed_model` in `test/python/conftest.py` accept either a string or a list for the `data_file` parameter to download + hardlink the multi-blob layout. Test file: `test/python/test_gptoss20b.py` mirrors `test_llama8b.py` (DML decode + 6 ORT tests + 4 OGA tests; pipeline_sliding_window and asym tests omitted as they are Llama- or pipeline-specific). GPU dispatch was previously failing silently (CPU fallback) until the `lower-affine` pipeline gotcha (above) was fixed — the MoE expert-major 3D→2D flatten in qmoe is what triggered the bad `affine.apply` survivors.

**gpt-oss-20b benchmark baseline (gfx1151, 2026-05-08, `model_benchmark.exe -r N -w 1`, no `HIPDNN_EP_PERF`).** Dynamic-shape DLL on the EP via OGA. **After unlocking GQA flash_decode for gpt-oss-20b (HPG=8 instantiation + sliding window + head_sink/smooth_softmax + packed-QKV split — see "GQA flash_decode" section below): L=128 → 80.47 tok/s decode (+5.9% vs prior 75.96), L=2048 → 37.97 tok/s decode (+11.0% vs prior 34.21).** Peak WS unchanged (~1.44 GB at L=128, ~1.64 GB at L=2048). Trajectory: pre-fix 38.7 tok/s → Phase 1 (per-state scratch) 39.0 → Phase 1+pinned host 54.9 → Phase 2 step 2 (GPU bucketing) 62.16 → Phase 3 (fused qmoe decode) 75.96 → flash_decode unlock **80.47**. Per-Compute GPU work is dominated by `wrap_qmoe` (24 calls/inference, decode fully fused) and `wrap_group_query_attention` (24 calls/inference, HPG=8 d=64 — now takes flash_decode at skv≥256 for the 12 full-attention layers; the 12 sliding-window layers also take flash_decode with `kv_lo` clamping to the last 128 positions). Vulkan llama.cpp baseline at L=2048 is ~70 tok/s — gap closed from -51% (pre-fix) to **-46%** at L=2048. **The dominant remaining L=2048 gap is OGA framework overhead, not the EP kernel** — EP-direct (`bench_gptoss_both.py`, no OGA) measures ~80 tok/s at KV=2048 (kernel-only ceiling), and OGA's per-token cost exceeds that by ~12 ms because `State::Run`'s IoBinding clear+rebind cycle scales with `max_length` (see "OGA per-token IoBinding rebind cost scales with `max_length`" gotcha above). Other open follow-ups (kernel-side, smaller wins): per-layer `hipStreamSynchronize` in qmoe (24/inference) is the next visible launch-bound stall; K_SPLITS=8 over-splitting per Vulkan's adaptive formula; matmul_nbits GEMV launch overhead. Numbers are working baselines — re-measure with current build before citing.

**gpt-oss-20b known accuracy fragility (MoE expert-routing precision sensitivity).** Two of the eleven `test_gptoss20b.py` tests fail at the conftest `cosine > 0.95` threshold and are deferred:
- `test_ort_fixed_prefill_128` — logits cosine ≈ 0.948. `present.N.value` cosine degrades through the layer stack (~0.93-0.95 starting around layer 12). KV cosine stays ≥ 0.997 in the early layers, drift accumulates.
- `test_ort_per_step_logits` — prefill at sq=7 PASSES (cosine 0.9998, top1_match=True), but `decode[1]` cosine drops to 0.938 within a single decode step.
The signature is consistent with MoE top-K expert-routing flips: tiny CPU↔EP numerical differences cross the routing decision boundary in qmoe's top-4 selection, and the divergence amplifies down the layer stack and across decode steps. Llama (no MoE) does not exhibit this pattern. The 0.95 cosine threshold (`compare_outputs` / `compare_logits` in `test/python/conftest.py`) is calibrated to dense models — investigation needed on whether to (a) raise the per-expert routing precision (e.g. fp32 for the router), (b) lower the threshold for MoE models specifically, or (c) compare on the *intersection* of top-K experts. Generation correctness via OGA still works (the four `test_oga_ep_*` generation tests pass) — these are tight per-tensor numerics, not user-visible.

**Sym vs asym 8B model identity (AVOID PHANTOM-BUG CONFUSION).** The sym dyn 8B (`Llama-3.1-8B-Instruct-awq-g128-int4`) is the **Instruct** fine-tune; the asym dyn 8B (`Llama-3.1-8B-awq-g128-int4-asym-fp16-onnx-dml`) is the **base** Llama-3.1-8B. Different post-training. They produce **different greedy generations** for the same prompt — and the base model in particular tends to echo / repeat / wander on chat-template-style prompts (e.g. "Write a paragraph about Paris" → may emit garbled tail like "lectronique"). This is **NOT a kernel bug**; it's normal base-model behavior on instruction-formatted prompts. Verified 2026-05-07: per-step CPU-vs-EP logit comparison on the asym dyn model gives 100% top-1 token match with cosine ≥ 0.999997 / rel_L2 ~1e-3, which is *higher* fidelity than the sym Instruct model's CPU-vs-EP cosine (~0.9994). **Verification protocol** when an output looks "broken": (a) run `test_llama8b.py::TestLlama8BAsym::test_asym_dynamic_per_step_logits` (or sym equivalent) — if it passes, the kernel is correct; (b) compare CPU-only vs CPU-only output of the same model with the same prompt — if CPU also produces "broken" English, the model is the cause; (c) only after both confirm the kernel matches CPU should you suspect a kernel issue. Do NOT chase a kernel bug based on OGA-smoke generation alone.
...
**GQA smart-dispatch threshold + flash_decode exemption** (`lib/Runtime/real/gqa.cpp`). The legacy `gqa_fused_decode` kernel scales linearly in `total_seq` (skv). At very long contexts it loses ~12× to a GEMM-based decomposed path (which gets bandwidth amplification from hipBLASLt's tiled matmul). The dispatcher caps legacy fused_decode at `HIPDNN_EP_GQA_FUSED_DECODE_MAX_T` (default 256) — beyond that it falls back to the decomposed (GEMM + softmax + GEMM) path. **flash_decode is exempt from this cap**: when `flash_decode_geometry_ok` is satisfied and skv ≥ 256, the smart-dispatch check passes regardless of `total_seq`, because flash_decode's split-K design already kills the linear scaling. The actual gating expression is `size_ok_for_fused = (total_seq_pre < 0) || (total_seq_pre <= MAX_T) || flash_decode_eligible`. Without the exemption clause, 8B at L≥256 (and gpt-oss-20b at any L≥256) would silently fall back to the decomposed path and lose the flash_decode TPS win.
