...
- **VLM embedding sub-model — five EP runtime gotchas that together cause silent shape/value drift on the multi-modal embedding graph (Qwen3.5 / Gemma3 image-placeholder substitution pattern).** The text-embedding sub-model of every VLM in our zoo (Qwen3.5, Gemma3, etc.) shares one ONNX shape: `input_ids → Gather(W_embed) → embeds`, then in parallel `input_ids == PLACEHOLDER → Equal(scalar) → mask → Expand(mask, embed_shape) → NonZero → Transpose → Slice(image_features, count_from_NonZero) → ScatterND(embeds, indices, slice)`. Five independent EP-side correctness bugs all manifest as L2(EP, ORT-CPU) > 0 on the same `output_0_inputs_embeds_fp16` tensor — fix any one in isolation and the L2 drops but stays non-zero, so it's easy to chase the wrong one. All five MUST be in place; verified end-to-end with a six-shape sweep (`seq_len ∈ {1,4,8,16,32,64}`, `placeholders ∈ {0..16}`) producing L2(diff)=0 across the board.
 1. **`hip-onnx-runner` must read `.shape` sidecar files for input shapes, not derive shape from input file size.** The embedding graph has TWO inputs (`input_ids` int64, `image_features` fp16) whose sizes are coupled but not independently recoverable from byte size alone (`image_features` flat byte size depends on `num_patches * hidden`; `num_patches` is dynamic). The generator script writes `<input>.bin.shape` alongside each `<input>.bin`; the runner now reads the sidecar first and only falls back to size-based heuristics if absent. Without this, CPU and EP runners pick different `num_patches`/`seq_len` shape combinations from identical byte streams → different `Equal` masks → divergent outputs.
 2. **`onnx.Equal` lowering MUST pass per-operand element counts to `wrap_equal`, and the runtime kernel MUST broadcast scalar (numel==1) operands via zero stride** (`lib/Conversion/HipToLLVM/EqualLowering.cpp`, `lib/Runtime/real/equal.cpp`, `3rd-party/custom_kernels/hip/elementwise_binary_kernel.hip::hip_elementwise_equal`). The placeholder mask is computed as `input_ids == const(248056)` where `const(248056)` is a rank-0 scalar. The ONNX→HIP conversion does NOT pre-Expand scalars; instead the runtime kernel now accepts `a_num_elements`, `b_num_elements`, `out_num_elements` and walks operands with `lhs_stride = (a_num==1) ? 0 : 1` (and symmetrically for rhs). Same-shape (a_num==b_num==out_num), scalar-rhs (b_num==1), and scalar-lhs (a_num==1) are the three supported modes; anything more general (rank-aware NumPy broadcast) is rejected so the kernel never reads OOB and the bug surfaces loudly rather than silently. **Symptom of regression**: mask comes out all-zero → `NonZero` returns count=0 → `Slice` returns empty → `ScatterND` writes nothing → output is pure embeddings (no image substitution) → L2(EP, ORT-CPU) ≈ ‖image_features‖.
 3. **`hip_nonzero` MUST emit indices in row-major (C-order) order, not atomic-unordered.** ScatterND's `updates` operand is `Slice(image_features, count_from_NonZero)` — image_features is fixed row-major, and ScatterND writes `output[indices[i]] = updates[i]`. If NonZero outputs `indices` in arbitrary order (per-thread atomicAdd), update_i no longer corresponds to the i-th row of image_features → embedding rows get permuted across the placeholder positions → cosine ≈ 0.7 cascading into the LLM trunk. The kernel was rewritten to a single-block collaborative ordered scan: one block runs an exclusive prefix sum across all threads on its non-zero mask, then each thread writes its index pair `(flat_pos / inner_dim, flat_pos % inner_dim)` at its prefix-sum offset. Single-block constrains numel to one kernel launch's worth of threads but the embedding mask is `[batch, seq_len, hidden]` and seq_len is typically ≤ a few hundred — well within budget. **Symptom of regression**: L2 > 0 only when `placeholders > 1`; `placeholders ∈ {0,1}` look fine because there's nothing to permute.
 4. **`hip_slice` MUST permit partial fills when derived output extent < IR static dim, not abort.** When `placeholders < num_patches`, the runtime-derived slice end (`count_from_NonZero / hidden = placeholders`) is smaller than the IR's static output dim (`num_patches`, baked in at compile time). The pre-fix wrap_slice aborted on `derived_end != ir_dim` mismatch. **Fix**: allow `derived_end ≤ ir_dim`; the kernel writes only the first `derived_end` rows; downstream `ScatterND`'s count_ptr also clamps writes to the same count, so the unwritten tail is never read. The reverse (`derived_end > ir_dim`) is still a hard abort — that would be a buffer overflow.
 5. **`hip_scatter_nd` MUST honour the `count_ptr` device-side count from NonZero, not the static `num_slices`.** ScatterND's `indices` and `updates` are sized to the IR static max (`num_patches * hidden`), but only the first `count_ptr` slices are valid. Kernel reads `count_ptr` from device memory in each thread block and skips slices with `i >= count`. **Symptom of regression**: kernel writes stale/zero data past the valid count → embedding rows beyond the actual placeholders get clobbered with garbage → L2 ≈ ‖image_features‖ on the tail rows.
 - Validation harness: `cursor_space/_sweep_qwen35_embedding.bat` runs the six-shape sweep against `Qwen3.5-9B-rtn-int4-int8-128gs-fp16-onnx-gpu/embedding.onnx`. All six L2(diff)=0. With `HIPDNN_EP_DEBUG=1 HIPDNN_EP_STRICT=1` the eight wrappers must dispatch to GPU in order: `equal → gather → expand → expand → nonzero → transpose → slice → scatter_nd`; if any falls through to CPU you'll see no `[REAL] wrap_*` for it and `STRICT=1` will *not* abort (the ONNX op is present but emits a non-MorphiZen kernel) — the silent-CPU-fallback trap. (See "CPU fallback is silent" gotcha.)
