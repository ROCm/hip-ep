...
**gpt-oss-20b benchmark baseline (gfx1151, 2026-05-07, `model_benchmark.exe -r N -w 1`, no `HIPDNN_EP_PERF`).** Dynamic-shape DLL on the EP via OGA. After the qmoe pinned-host-scratch fix (see "wrap_qmoe scratch + pinned host staging" gotcha below): **L=128 → ~189 ms TTFT p50, ~54.9 tok/s decode, ~1.44 GB peak WS** (up from ~38.7 tok/s / 264 ms TTFT pre-fix; +41.7% / -28%). Pre-fix L=2048 was ~14.4 s TTFT p50, ~24.79 tok/s decode, ~1.64 GB peak WS — re-measure post-fix. Per-Compute GPU work is dominated by `wrap_qmoe` (24 calls/inference, 32 experts × 4 active per token) and `wrap_group_query_attention` (24 calls/inference, HPG=8 — flash_decode is HPG=4 only so gpt-oss takes the LDS-tiled fused_decode path at sq=1). Expect TTFT to scale roughly linearly with L (24 GQA layers × O(L²) attention dominates prefill); decode TPS to droop at long L due to fused_decode skv-linear scaling (no flash_decode rescue at HPG=8 — see "Llama 8B at L=1024 with GQA flash_decode" architectural note for why). Vulkan llama.cpp baseline on the same hardware is ~70 tok/s decode (different model + lib but same gfx1151); remaining ~21% gap to that target is plausibly the 24 per-layer `hipStreamSynchronize` stalls in qmoe's host-side expert bucketing — Phase 2 (GPU-side bucketing) is the open follow-up. Numbers are working baselines — re-measure with current build before citing.
