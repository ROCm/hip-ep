...
- **Output-allocator mode is the ONLY EP ABI — no provider option — compile flag → module attribute and embedded metadata are a single source of truth.** `pass_main.cpp::load_config` sets `config.useOutputAllocator = true` **unconditionally**; there is no `use_output_allocator` provider option and no classic out-param fallback reachable from a session. Every model compiles to a **2-arg** `inference_compute(state, inputs)` with graph outputs allocated in-graph via `hip.alloc_output` → the EP's allocator callback. The flag is set once on the OnnxToHip half in `lib/Compiler/CompilerDriver.cpp` (`onnxToHipOpts.useOutputAllocator`), where the slot-4.5 pair `hip-use-output-allocator` (IR rewrite) + `hip-set-output-allocator-attr` (stamps the `hipdnn.output_allocator` module attribute) runs. The HipToLLVM half (`convert-hip-to-llvm` + the single `generate-interface`) reads that **module attribute** off the IR to pick the 2-arg ABI — it has no separate flag, so the mode rides on the module and the two halves can never disagree. The same compile flag is also written into the model `Metadata` (`backend-mlir-compiler/proto/metadata.proto`); the EP reads its dispatch arity from the **embedded metadata** (`metadata_.use_output_allocator()` in the `MlirCustomOp` ctor), so `main_graph` arity, the `inference_compute` wrapper arity, and the EP's dispatch can never disagree, even on a reused EPContext. The compiler-internal `use-output-allocator` *pipeline* option still exists in `lib/` (and the classic pipeline + 3-arg ABI codegen remain) so `hip-compiler` CLI / LIT tests can still exercise both paths — but the EP front-end always selects allocator. Key invariants when touching this path:
  - **Classic pipeline is CLI/LIT-only.** The dual-pipeline code in `Pipelines.cpp` / `GenerateInterface.cpp` / `HipToLLVM.cpp` is retained and tested, but `load_config` hardwires the EP to allocator. Do not re-introduce an EP provider option to select it — if a future need arises, flip `config.useOutputAllocator` in `load_config`, not via a string provider option.
  ...
  - **The allocator path needs NO present.* share-buffer override — and must not grow one.** `output_allocate_cb` passes the DLL's in-graph `shape` to `GetOutput` verbatim; every output (KV cache included) is acquired identically. This is correct because a `present.*` output's dynamic seq dim is materialized in-graph by `hip.alloc_output` from `memref.dim %past_key` (the past input buffer's actual extent), which under OGA `past_present_share_buffer` already IS the `max_length` capacity buffer — so `GetOutput` gets capacity and returns the pre-bound shared `OrtValue`, preserving `past == present` pointer identity for in-place GQA append. Pinned by `test/lit/e2e/test_gqa_output_allocator_present_dim.mlir`. **Do NOT re-add the override to the allocator callback** — it would be a provable no-op (`out == past`) AND would recouple the model-agnostic allocator path to OGA conventions (the layering concern raised in PR #306 review). The classic `marshal_output_tensors` path is now **static-output-only** — it carries no `present.*` override and no dynamic-dim resolution (the `DimSource`→`attention_mask` machinery was removed when allocator mode became the default; classic mode rejects dynamic output dims at compile time). Any KV-cache / dynamic-output model uses the allocator ABI, where the in-graph `memref.dim %past_key` sizing already preserves shared-buffer identity with no override.
  - **Per-Compute install/clear + output-completeness guard.** The `OutputAllocatorCtx` lives on the stack; the allocator is installed (`self = &ctx`) before `InferenceState::compute_with_output_allocator` and cleared (`set_output_allocator(nullptr)`) immediately after so `self` can't dangle. After compute, every metadata output index must have been served by exactly one `hip.alloc_output` callback, else `LOG(FATAL)` (passthrough / deduped-aliased outputs are unsupported in allocator mode today; no target model has them).
...
- **Output-allocator pipeline (`use-output-allocator`, OFF by default): `hip-use-output-allocator` MUST run AFTER `buffer-deallocation`, never before** (`lib/Dialect/Transforms/Pipelines.cpp::buildOnnxToHipPipelineTail`, the "slot 4.5" placement). `hip.alloc_output` carries a Write effect but NO Allocate effect; placed BEFORE deallocation, the ownership-based dealloc pass treats the returned buffer as unowned and inserts `bufferization.clone %out` at the `func.return` — a per-inference alloc + full-output copy that silently defeats the allocator's zero-copy goal (no crash, just a slow extra copy). Run AFTER deallocation, the output is still a plain `memref.alloc` (Allocate effect ⇒ owned) and is returned directly with no clone and no dealloc. It still runs BEFORE `hip-pool-allocs` so the EP-owned output never enters the GPU pool (pool-allocs only absorbs `memref.alloc`). Both orderings are pinned in `test/lit/Pipeline/output-allocator-dealloc.mlir` (FileCheck asserts the clone is present in the before-order, absent in the after-order). **Mode selection is a single module attribute, not a threaded flag or a param-count guess.** `hip-use-output-allocator` (**FuncOp**, `lib/Dialect/Transforms/UseOutputAllocator.cpp`) does BOTH the IR rewrite (returned `memref.alloc` → `hip.alloc_output`) AND stamps the `hipdnn.use_output_allocator` **`BoolAttr` (value `true`)** on the parent module — the single source of truth for allocator mode. The stamp is **unconditional** (it fires even when no alloc was rewritten, e.g. a zero-output graph) because the mode is decided by the pass being scheduled (allocator pipeline only), not by the IR contents. A FuncOp pass writing its parent module relaxes MLIR's pass-isolation contract; it is safe here because the write is an idempotent set of the same value (same rationale as `PoolAllocs` stamping `hipdnn.pool_size`). **The attribute is a typed bool, NOT a presence-only `UnitAttr`** — every reader tests the VALUE, not mere existence: `auto a = module->getAttrOfType<BoolAttr>("hipdnn.use_output_allocator"); bool allocatorMode = a && a.getValue();`. So a missing attribute AND `= false` both mean classic mode (a future caller can therefore explicitly stamp `false` without changing semantics). Both `convert-hip-to-llvm`'s `transformMainFunction` (`lib/Conversion/HipToLLVM/HipToLLVM.cpp`) and the single `generate-interface` pass (`lib/Dialect/Transforms/GenerateInterface.cpp`, three read sites: `verifyPrerequisites`, `generateInferenceCompute`, the debug-log) read it that way. `transformMainFunction` then only *verifies* `main_graph`'s lowered param count matches the chosen mode (each memref unpacks to `3 + 2*rank` params; classic = context+inputs+outputs, allocator = context+inputs), emitting a mode-specific `emitError` on mismatch — it no longer infers the mode from the count, so a zero-output graph (where the two expected counts coincide) is disambiguated by the attribute. The `use-output-allocator` pipeline option lives ONLY on the ONNX-to-HIP half (`OnnxToHipPipelineOptions` / `HipdnnPipelineOptions`) to pick slot-3 `buffer-results-to-out-params` vs the slot-4.5 `hip-use-output-allocator`; `HipToLLVMPipelineOptions` has NO such option (it reads the attribute). In `generateInferenceCompute`, classic-only output staging is wrapped in `if (!allocatorMode)` scopes tagged `classic out-param staging - delete when classic is removed`. EP bridge (Phase 5) is not yet wired. See [docs/design/output-allocator-design.md](docs/design/output-allocator-design.md) Phase 4 (incl. the "Deprecation plan").
...
**Dynamic output shapes (output-allocator ABI, the default).** Dynamic output dims (`-1` in metadata) are sized **in-graph at runtime** by the DLL via the EP's output-allocator callback (`output_allocate_cb` in `MlirCustomOp.cpp` → `hip.alloc_output`). The EP never resolves an output dim itself: `output_allocate_cb` passes the DLL's computed shape to `ctx.GetOutput()` verbatim. For a `present.*` KV-cache output the in-graph dynamic dim is `memref.dim %past_key` — the past input buffer's actual extent — so under OGA `past_present_share_buffer` it already equals the `max_length` capacity buffer and `GetOutput` returns the pre-bound shared `OrtValue`, preserving `past_key == present_key` pointer identity with no EP-side override. See the "Output allocator" section above and `docs/design/output-allocator-design.md`.

**Classic ABI is static-output-only and unreachable from a session.** The legacy 3-arg path (still present in `lib/` for `hip-compiler` CLI / LIT tests) pre-allocates outputs from the static metadata before `inference_compute`. The runtime `DimSource` resolution (output dim ← `dim_params_map` ← first input declaring the symbol) and the `present.*` shared-buffer override that classic mode once used to size dynamic outputs were **removed**. `marshal_output_tensors` uses the static shape directly and `CHECK`-fails on a surviving `-1`. The `DimSource` proto message + `Output.dim_sources` field were retired (field number 5 reserved). The EP front-end always compiles in allocator mode (`load_config` hardwires `config.useOutputAllocator = true`; there is no provider option), so any KV-cache / dynamic-output model is handled in-graph by the allocator.
