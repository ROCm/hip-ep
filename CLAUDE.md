...
**gpt-oss-20b benchmark baseline (gfx1151, 2026-05-07, `model_benchmark.exe -r N -w 1`, no `HIPDNN_EP_PERF`).** Dynamic-shape DLL on the EP via OGA. **After the Phase 3 fused-decode kernel (see "wrap_qmoe Phase 3" gotcha above): L=128 → 188 ms TTFT p50, 75.96 tok/s decode (OGA path: 77.7 tok/s), 1.44 GB peak WS — beats Vulkan llama.cpp baseline (~70 tok/s) by +8.5%.** L=2048 → ~24.3s TTFT (prefill-bound — 24 layers × O(L²) attention; fused decode does not fire during prefill), 36.90 tok/s decode, 1.64 GB peak WS. Trajectory: pre-fix 38.7 tok/s → Phase 1 (per-state scratch) 39.0 → Phase 1+pinned host 54.9 → Phase 2 step 2 (GPU bucketing) 62.16 → Phase 3 (fused decode) **75.96**. Per-Compute GPU work is dominated by `wrap_qmoe` (24 calls/inference, 32 experts × 4 active per token; decode now fully fused — three back-to-back launches per layer) and `wrap_group_query_attention` (24 calls/inference, HPG=8 — flash_decode is HPG=4 only so gpt-oss takes the LDS-tiled fused_decode path at sq=1). Open follow-ups: per-shape autotune over fused-decode `(BS, TN)`; fp16 atomicAdd to fuse FC2+reduce; GQA flash_decode HPG=8 instantiation. Numbers are working baselines — re-measure with current build before citing.
