...
1. **C API** (`udna-compiler.dll`) - DLL-safe interface using morphizen FileReader/FileWriter
2. **C++ Library** (`UdnaCompiler`) - Reusable CompilerDriver for MLIR → DLL/Object/IR compilation
3. **CLI Tools** (`udna-opt`, `udna-compile`) - Standalone compilation tools

## Architecture

```
udna-compiler/
├── include/udna-compiler/  # Public headers
...
│
├── tools/
│   ├── udna-opt/        # Pass runner (debugging)
│   ├── udna-compile/    # MLIR → DLL compiler
...
# Build
cmake --build ../../build/udna-compiler --config Debug --parallel

# Test
ctest --test-dir ../../build/udna-compiler --verbose
```

### Build Options

- `BUILD_COMPILER_DLL` - Build udna-compiler.dll (default: ON, future work)
- `BUILD_STANDALONE_TOOLS` - Build udna-opt and udna-compile (default: ON)
...
| `THEROCK_DIST` | Still required at **compile time** (when `udna-compile` generates the DLL) so that `lld-link` can locate the import libraries. Not needed at DLL execution time. |
...
:: Compile the model
udna-compile model.mlir -o model.dll --mode dll -v
...
```bash
# Compile MLIR to DLL
udna-compile input.mlir -o output.dll --from-onnx-mlir -O2 -v

# Debug MLIR passes
udna-opt input.mlir --morphizen-pipeline
udna-opt input.mlir --convert-onnx-to-hip --convert-hip-to-llvm
```
...
CompilerError error;
CompilerErrorCode result = udna_compile(
