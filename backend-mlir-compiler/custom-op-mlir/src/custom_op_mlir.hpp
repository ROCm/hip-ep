...
// ABI between marshalling (onnxruntime_morphizen_ep.dll) and the JITted
// per-model bitcode.
//
...
// behaviour for hip-test, hip-onnx-runner, and any other host-input
// caller.
//
...
//   * `tools/hip-test/hip-test.cpp`          (test driver, local)
struct tensor_t {
...
// ============================================================================
// JIT function pointer typedefs — stable C ABI exported by the per-model
// bitcode and resolved via LlvmIrJit::lookup_raw inside the EP DLL.
//
...
//
// These typedefs are documentation-only; the real call sites in InferenceState
// resolve symbols via jit_->get_method<R, Args...>() with explicit template
...
//   fs         [in]  morphizen::FileSystem* (typed as void* across the C ABI).
//                    The same FileSystem instance the level-1 pass used at
//                    compile time, plumbed through MlirCustomOp.
//
// Return values:
//   0  Success
...
//                     The JITted code writes results into tensor_t.data on
//                     success.
//
// Return values:
//   0  Success
