...
if exist "%WORKSPACE_ROOT%\build\onnx-hipdnn-ep\build.ninja" (
    echo Skipping CMake configuration - build.ninja already exists
    echo To force reconfigure, delete %WORKSPACE_ROOT%\build\onnx-hipdnn-ep\build.ninja
    echo.
) else (
...
cmake --build %WORKSPACE_ROOT%/build/onnx-hipdnn-ep
...
cmake --install %WORKSPACE_ROOT%/build/onnx-hipdnn-ep
