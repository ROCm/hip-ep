...
TimeoutStatus HipContext::sync_stream_with_timeout(int timeout_ms) const {
  ...
  MY_LOG(2) << "[HipContext] Synchronizing stream with " << timeout_ms
            << "ms timeout...";
...
RocmCustomOp::RocmCustomOp(std::shared_ptr<const PassContext> context,
  ...
  hip_context_ =
      SessionContextRegistry::instance().get_or_create(context.get());
  ...
  auto status =
      google::protobuf::util::JsonStringToMessage(json_str, &subgraph_);
  ...
  if (status.ok() && subgraph_.nodes_size() > 0) {
    ...
    for (int i = 0; i < subgraph_.outputs_size(); ++i) {
      ...
      MY_LOG(2) << "[ROCm CustomOp] Output mapping: " << ext_output.name()
                << " -> index " << i << " (from node "
                << ext_output.producer_node_id() << ", output "
                << ext_output.output_index() << ")";
    }

  } else {
    // Try to parse as RocmParamProto (legacy single-node format)
    status = google::protobuf::util::JsonStringToMessage(json_str,
                                                         &single_node_proto_);
    if (!status.ok()) {
      LOG(ERROR) << "[ROCm CustomOp] Failed to parse JSON params: "
                 << status.ToString();
      throw std::runtime_error("Failed to parse ROCm params: " +
                               status.ToString());
    }
  ...
  if (op_type == "conv") {
    ...
    if (!weight_file.empty()) {
      ...
      MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
                << ": Loading conv weight from: " << weight_file << " ("
                << weight_size << " bytes)";
      ...
      if (reader) {
        ...
        MY_LOG(1) << "[ROCm CustomOp] Node " << node_id
                  << ": Loaded weight: " << data.host_weight.size()
                  << " floats";
      } else {
    ...
    if (!bias_file.empty() && conv_params.has_bias()) {
      ...
      MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
                << ": Loading conv bias from: " << bias_file << " ("
                << bias_size << " bytes)";
      ...
      if (reader) {
        ...
        MY_LOG(1) << "[ROCm CustomOp] Node " << node_id
                  << ": Loaded bias: " << data.host_bias.size() << " floats";
      } else {
        LOG(ERROR) << "[ROCm CustomOp] Node " << node_id
    ...
    if (!weight_file.empty()) {
      ...
      MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
                << ": Loading gemm weight from: " << weight_file << " ("
                << weight_size << " bytes)";
    ...
    if (!bias_file.empty() && gemm_params.has_bias()) {
      ...
      MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
                << ": Loading gemm bias from: " << bias_file << " ("
                << bias_size << " bytes)";
    ...
    if (matmul_params.b_is_constant()) {
      ...
      if (!weight_file.empty()) {
        ...
        MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
                  << ": Loading matmul weight from: " << weight_file << " ("
                  << weight_size << " bytes)";
...
void RocmCustomOp::AllocateIntermediateBuffers() {
  ...
  for (int i = 0; i < subgraph_.nodes_size(); ++i) {
    ...
    if (!data.host_weight.empty() && data.d_weight == nullptr) {
      ...
      if (err != hipSuccess) {
        LOG(ERROR)
            << "[ROCm CustomOp] Failed to allocate weight buffer for node "
            << i;
    ...
    if (!data.host_bias.empty() && data.d_bias == nullptr) {
      ...
      if (err != hipSuccess) {
        LOG(ERROR) << "[ROCm CustomOp] Failed to allocate bias buffer for node "
                   << i;
...
float* RocmCustomOp::ResolveTensorRef(const rocm::TensorRefProto& ref) const {
  if (ref.has_external_name()) {
    ...
    LOG(ERROR) << "[ROCm CustomOp] External input not found: "
               << ref.external_name();
    return nullptr;
    ...
    LOG(ERROR) << "[ROCm CustomOp] Internal tensor not found: node "
               << producer_node << ", output " << output_idx;
    return nullptr;
  }
...
void RocmCustomOp::UploadExternalInputs(const OrtApi* api,
  ...
  // This eliminates runtime deduplication overhead - Level-1 pass already
  // computed the unique set of external inputs during graph compilation
  MY_LOG(2) << "[ROCm CustomOp] Phase 1: Uploading "
            << subgraph_.external_inputs_size() << " external inputs";
  ...
  for (int input_idx = 0; input_idx < subgraph_.external_inputs_size();
    ...
    if (it == external_input_buffers_.end() ||
      ...
      if (err != hipSuccess) {
        LOG(ERROR) << "[ROCm CustomOp] Failed to allocate device buffer for "
                   << name;
        throw std::runtime_error(
            "Failed to allocate GPU memory for external input");
      }
    ...
    // Async H2D copy
    hipError_t err = hipMemcpyAsync(external_input_buffers_[name], h_data,
...
void RocmCustomOp::ExecuteNode(const rocm::RocmNodeProto& node) const {
  ...
  for (int i = 0; i < node.inputs_size(); ++i) {
    ...
    if (ENV_PARAM(MORPHIZEN_DEBUG_ROCM) >= 2) {
      ...
      if (input_ref.has_external_name()) {
        ...
        ref_info = "internal:node" +
                   std::to_string(input_ref.internal().producer_node_id()) +
                   "_out" + std::to_string(input_ref.internal().output_index());
      }
      MY_LOG(1) << "[ROCm CustomOp] Node " << node_id << " input[" << i
                << "]: " << ref_info;
    }
  }
  ...
  if (ENV_PARAM(MORPHIZEN_DRY_RUN) >= 1) {
    ...
    if (output != nullptr && output_size > 0) {
      ...
      if (err != hipSuccess) {
        MY_LOG(0) << "[ROCm CustomOp DRY RUN] hipMemsetAsync failed: "
                  << hipGetErrorString(err);
      }
    }
    return;
...
void RocmCustomOp::ExecuteConvNode(int32_t node_id,
  ...
  miopenSet4dTensorDescriptor(input_desc, miopenFloat, params.batch_size(),
                              params.in_channels(), params.in_height(),
                              params.in_width());

  miopenSet4dTensorDescriptor(weight_desc, miopenFloat, params.out_channels(),
                              params.in_channels(), params.filter_height(),
                              params.filter_width());

  miopenSet4dTensorDescriptor(output_desc, miopenFloat, params.batch_size(),
                              params.out_channels(), params.out_height(),
                              params.out_width());

  miopenInitConvolutionDescriptor(conv_desc, miopenConvolution, params.pad_h(),
                                  params.pad_w(), params.stride_h(),
                                  params.stride_w(), params.dilation_h(),
                                  params.dilation_w());

  // Determine workspace size - use cached if available
  size_t workspace_size =
      node_data.conv_algo_cached ? node_data.cached_conv_workspace_size : 0;

  // If not cached, query workspace size
  if (!node_data.conv_algo_cached) {
    miopenConvolutionForwardGetWorkSpaceSize(miopen_handle, weight_desc,
  ...
  if (workspace_size > 0) {
    if (node_data.workspace == nullptr ||
      ...
      if (node_data.workspace)
        hipFree(node_data.workspace);
  ...
  if (node_data.conv_algo_cached) {
    ...
    miopenStatus_t status = miopenFindConvolutionForwardAlgorithm(
        miopen_handle, input_desc, d_input, weight_desc, d_weight, conv_desc,
        output_desc, output, 4, &algo_count, perf_results, workspace_ptr,
    ...
    if (status != miopenStatusSuccess || algo_count == 0) {
      ...
      LOG(ERROR)
          << "[ROCm CustomOp] Failed to find convolution algorithm for node "
          << node_id;
    ...
    MY_LOG(1) << "[ROCm CustomOp] Node " << node_id << ": Cached algorithm, "
              << "found " << algo_count
              << " algorithms, best time: " << perf_results[0].time
              << " ms, workspace: " << perf_results[0].memory << " bytes";
  }
  ...
  miopenStatus_t status = miopenConvolutionForward(
      ...
      conv_desc, algo, &beta, output_desc, output, workspace_ptr,
      workspace_size);

  // Cleanup descriptors (safe to destroy immediately - values copied to command
  // buffer)
  miopenDestroyConvolutionDescriptor(conv_desc);
  miopenDestroyTensorDescriptor(output_desc);
  miopenDestroyTensorDescriptor(weight_desc);
  ...
  if (status != miopenStatusSuccess) {
    LOG(ERROR) << "[ROCm CustomOp] miopenConvolutionForward failed for node "
               << node_id;
  ...
  if (params.has_bias() && d_bias != nullptr) {
    ...
    MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
              << ": Adding bias (batch=" << params.batch_size()
  ...
  MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
            << ": Convolution executed successfully";
}
...
void RocmCustomOp::ExecuteGemmNode(int32_t node_id,
  ...
  //   D' = B' * A' with swapped dimensions (where X' means X in column-major
  //   view)
  //
  ...
  if (!node_data.gemm_algo_cached) {
    ...
    MY_LOG(1) << "[ROCm CustomOp] Node " << node_id
              << ": Searching for best GEMM algorithm...";
    ...
    hipblasStatus_t status = hipblasLtMatmulAlgoGetHeuristic(
        blaslt_handle, matmul_desc, layout_B, layout_A, layout_D, layout_D,
    ...
    if (status == HIPBLAS_STATUS_SUCCESS && returned > 0) {
      ...
      MY_LOG(1) << "[ROCm CustomOp] Node " << node_id
                << ": Cached GEMM algorithm, "
                << "found " << returned
                << " algorithms, workspace: " << results[0].workspaceSize
                << " bytes";
    } else {
      MY_LOG(1) << "[ROCm CustomOp] Node " << node_id
  ...
  // Handle bias (C matrix): For ONNX Gemm, C is typically 1D bias [N] that
  // broadcasts We need to handle this differently - beta * C is added after the
  // matmul For now, initialize output with bias if present, then set beta = 1.0
  // for the add
  ...
  if (node_data.gemm_algo_cached) {
    ...
    status = hipblasLtMatmul(blaslt_handle, matmul_desc, &alpha, d_b, layout_B,
                             d_a, layout_A, &beta, output, layout_D, output,
                             layout_D, &node_data.cached_gemm_algo,
                             workspace_ptr, workspace_size, stream);
  } else {
    status =
        hipblasLtMatmul(blaslt_handle, matmul_desc, &alpha, d_b, layout_B, d_a,
                        layout_A, &beta, output, layout_D, output, layout_D,
  ...
  if (d_c != nullptr && params.has_bias()) {
    ...
    // Note: add_bias_nchw with spatial_size=1 effectively adds bias[n] to each
    // row If original_beta != 1.0, we'd need to scale the bias first (not
    // common)
  }

  MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
            << ": GEMM executed successfully";
}
...
void RocmCustomOp::ExecuteMatmulNode(int32_t node_id,
  ...
  if (params.b_is_constant()) {
    ...
    if (node_data.d_weight == nullptr) {
      LOG(ERROR) << "[ROCm CustomOp] MatMul weights not found for node "
                 << node_id;
    ...
    if (inputs.size() < 2) {
      LOG(ERROR) << "[ROCm CustomOp] MatMul requires 2 runtime inputs when B "
                    "is not constant";
  ...
  //
  // So we swap A and B, and the result D'[N,M] in column-major is D[M,N] in
  // row-major
  ...
  if (batch_count > 1) {
    ...
    hipblasLtMatrixLayoutSetAttribute(layout_B,
                                      HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT,
                                      &batch_count, sizeof(batch_count));
    hipblasLtMatrixLayoutSetAttribute(
        layout_B, HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &stride_b,
        sizeof(stride_b));

    hipblasLtMatrixLayoutSetAttribute(layout_A,
                                      HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT,
                                      &batch_count, sizeof(batch_count));
    hipblasLtMatrixLayoutSetAttribute(
        layout_A, HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &stride_a,
        sizeof(stride_a));

    hipblasLtMatrixLayoutSetAttribute(layout_D,
                                      HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT,
                                      &batch_count, sizeof(batch_count));
    hipblasLtMatrixLayoutSetAttribute(
        layout_D, HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &stride_d,
        sizeof(stride_d));
  }
  ...
  if (!node_data.gemm_algo_cached) {
    // First call: Find best algorithm
    MY_LOG(1) << "[ROCm CustomOp] Node " << node_id
              << ": Searching for best MatMul algorithm...";
  ...
  if (ENV_PARAM(MORPHIZEN_DEBUG_ROCM) >= 2) {
    ...
    hipMemcpy(debug_out.data(), output, 5 * sizeof(float),
              hipMemcpyDeviceToHost);

    MY_LOG(1) << "[ROCm CustomOp] Node " << node_id << " DEBUG:"
              << " A[0:5]=[" << debug_a[0] << "," << debug_a[1] << ","
              << debug_a[2] << "," << debug_a[3] << "," << debug_a[4] << "]"
              << " B[0:5]=[" << debug_b[0] << "," << debug_b[1] << ","
              << debug_b[2] << "," << debug_b[3] << "," << debug_b[4] << "]"
              << " Out[0:5]=[" << debug_out[0] << "," << debug_out[1] << ","
              << debug_out[2] << "," << debug_out[3] << "," << debug_out[4]
              << "]";
  }

  MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
            << ": MatMul executed successfully";
}
...
void RocmCustomOp::ExecuteMulNode(int32_t node_id,
  ...
  if (params.b_is_scalar() && params.b_is_constant()) {
    ...
    rocm_kernels::mul_scalar(d_a, params.scalar_value(), output,
    ...
    rocm_kernels::mul_elementwise(d_a, d_b, output, params.size_a(),
                                  params.size_b(), stream);
  }

  MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
            << ": Mul executed successfully";
}
...
void RocmCustomOp::ExecuteSoftmaxNode(int32_t node_id,
  ...
  rocm_kernels::softmax_3d(d_input, output, axis_size, inner_size, outer_size,
                           stream);

  MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
            << ": Softmax executed successfully";
}
...
void RocmCustomOp::ExecuteTransposeNode(int32_t node_id,
  ...
  if (params.is_0213() && params.ndim() == 4) {
    ...
    std::vector<int64_t> in_shape(params.shape_in().begin(),
                                  params.shape_in().end());
    std::vector<int64_t> out_shape(params.shape_out().begin(),
                                   params.shape_out().end());
    std::vector<int32_t> perm(params.perm().begin(), params.perm().end());

    rocm_kernels::transpose(d_input, output, in_shape.data(), out_shape.data(),
                            perm.data(), params.ndim(), params.total_size(),
                            stream);
  }

  MY_LOG(2) << "[ROCm CustomOp] Node " << node_id
            << ": Transpose executed successfully";
}
...
void RocmCustomOp::ExecuteTileNode(int32_t node_id,
  ...
  std::vector<int64_t> in_shape(params.shape_in().begin(),
                                params.shape_in().end());
  std::vector<int64_t> repeats(params.repeats().begin(),
                               params.repeats().end());

  rocm_kernels::tile(d_input, output, in_shape.data(), repeats.data(),
                     params.ndim(), params.in_size(), params.out_size(),
                     stream);
...
void RocmCustomOp::DownloadExternalOutputs(const OrtApi* api,
  ...
  MY_LOG(2) << "[ROCm CustomOp] Phase 3: Downloading "
            << subgraph_.outputs_size() << " external outputs";
  ...
  for (int i = 0; i < subgraph_.outputs_size(); ++i) {
    ...
    // Async D2H copy
    hipError_t err =
        hipMemcpyAsync(h_data, d_data, bytes, hipMemcpyDeviceToHost, stream);
    if (err != hipSuccess) {
      LOG(ERROR) << "[ROCm CustomOp] Failed to download output "
                 << ext_output.name();
      throw std::runtime_error("Failed to copy output from GPU");
    }

    MY_LOG(2) << "[ROCm CustomOp] Scheduled D2H for output: "
              << ext_output.name() << " (" << bytes << " bytes)";
  }
