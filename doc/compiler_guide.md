...
├── UDNA-compiler\                 # MLIR-based ONNX→DLL compiler
...
| Setting | Why |
|---------|-----|
| `LLVM_ENABLE_PROJECTS="mlir;lld;clang"` | MLIR dialects, LLD linker, Clang compiler — all required by UDNA-compiler |
...
|------|----------|----------|
| `morphizen_config_mlir.json` | MLIR (init → mlir-pass) | UDNA-compiler path |
...
## Step 3: UDNA-compiler

- **Source**: `%WORKSPACE%\UDNA-compiler`
- **Build dir**: `C:\b\udna`
...
```bat
cmake --build C:\b\udna --config Release --parallel
```

### Install

```bat
cmake --install C:\b\udna --config Release
```
...
| File | Location |
|------|----------|
| `udna-compiler.dll` | `%LOCAL%\bin\` — compiler plugin loaded by MorphiZen EP |
| `udna-compile.exe` | `%LOCAL%\bin\` — standalone ONNX/MLIR → DLL compiler |
| `udna-opt.exe` | `%LOCAL%\bin\` — standalone MLIR pass runner |
| `test-model-dll.exe` | `%LOCAL%\bin\` — run compiled DLLs with random inputs |

### LIT Tests

```bat
ctest --test-dir C:\b\udna --config Release --verbose
```
...
### B. UDNA-compiler Standalone Compile + Run

Compile an ONNX model or MLIR file to DLL:

```bat
%LOCAL%\bin\udna-compile.exe ^
...
2. **`udna-opt.exe` file lock** — If `udna-opt.exe` is running in another terminal, `ninja` cannot overwrite it during rebuild. Kill the process first: `taskkill /F /IM udna-opt.exe`.
