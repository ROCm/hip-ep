...
- **Fork Branch**: fork-master (commit `fbf9984b`)
...
**Problem**: udna-opt tool had CLI option conflict
- Error: `CommandLine Error: Option 'o' registered more than once!`
- Root cause: Both udna-opt and onnx-mlir libraries register `-o` option via OMCompilerOptions
- Bad dependency chain: `udna-opt → HipDialect → OMONNXOps → OMMlirDialects → OMCompilerOptions`
...
- **udna-opt**: Don't define macro → CLI registration disabled, no `-o` conflict!
...
**Benefits**:
1. ✅ No CLI option conflicts - udna-opt can now build and run
...
The fork contains **7 targeted changes**:
...
6. **CMake fix**: Respect ONNX_MLIR_BUILD_TESTS flag (CMakeLists.txt line 244) - ✅ COMMITTED (70299fed)
7. **LLVM 22.1 fix**: Use `auto&` for `getRegisteredOptions()` return type (CompilerOptions.cpp) - ✅ COMMITTED (fbf9984b)
...
# Build udna-opt
ninja -C ../../build/onnx-hipdnn-ep udna-opt

# Verify it runs without option conflicts
../../build/onnx-hipdnn-ep/bin/udna-opt.exe --help
```

Expected: udna-opt runs successfully, no "Option 'o' registered more than once" error.
...
1. ✅ udna-opt builds successfully on Windows (MSVC + Ninja)
2. ✅ udna-opt runs without CLI option conflicts
...
- ✅ Conditional CLI registration to fix udna-opt conflict (252a588b)
- ✅ Restored OMCompilerOptions dependency (252a588b)
- ✅ Fixed broken ONNX_MLIR_BUILD_TESTS flag (70299fed)
- ✅ LLVM 22.1 compatibility: `auto&` for `getRegisteredOptions()` (fbf9984b)
- ✅ No behavioral changes - all fixes are compile-time only
- ✅ Safe for production use
- ✅ All changes committed and tested working

**Verdict**: Clean fork with targeted improvements for Windows/MSVC compatibility, udna-opt integration, proper CMake option handling, and LLVM 22.1 API compatibility.
...
- Change 7: `getRegisteredOptions()` changed return type from `StringMap<Option*>&` to `DenseMap<StringRef, Option*>&` between LLVM 22.0 and 22.1. Using `auto&` + `entry.second` works with both since `StringMapEntry` exposes `.second` just like a map pair.
