...
**Status:** Draft
**Related:** [CONSTANT-HANDLING-DESIGN.md](CONSTANT-HANDLING-DESIGN.md), [MORPHIZEN-EP-INTEGRATION.md](MORPHIZEN-EP-INTEGRATION.md), [RUNTIME-ARCHITECTURE.md](RUNTIME-ARCHITECTURE.md)
...
The [`GenerateInterface`](mlir/passes/06-GenerateInterfacePass.md) pass builds
the FlatBuffers blob from MLIR module attributes and emits two globals into the
LLVM module with **internal linkage** (not visible outside `model.dll`):
...
- [RUNTIME-ARCHITECTURE.md](RUNTIME-ARCHITECTURE.md) — opaque `RuntimeState`, runtime internals
- [mlir/INTERFACE-DESIGN.md](mlir/INTERFACE-DESIGN.md) — full C interface specification for `model.dll`
