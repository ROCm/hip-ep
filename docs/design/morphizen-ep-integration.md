...
```c
int  inference_init(void** out_state, void* fs, const void* config);
...
`config` carries the session's provider options, so a runtime subsystem can be
configured per session rather than only through an environment variable. It is a
...
the runtime copies **every** entry onto `RuntimeState`; later lookups therefore
do not depend on the EP table remaining live. `config` may be null, which is
how callers without an ORT session (for example the `hip-test` tool) invoke the
artifact.
