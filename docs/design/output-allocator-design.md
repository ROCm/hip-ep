...
└─ compute_with_output_allocator(context)
   ...
   │     │     ├─ <compute ops> (e.g. wrap_hipblasLtMatmul, wrap_elementwise) -> pool slots
...
4. **KV cache share-buffer needs no special case in the callback.** `output_allocate_cb` passes the DLL's in-graph `shape` to `GetOutput` verbatim — every output is acquired the same way. The shape is already right because `hip.alloc_output`'s dynamic dims come from its producer's operands: a `present.*` output is sized from `memref.dim %past_key` (the past input buffer's actual extent), which under OGA `past_present_share_buffer` already **is** the `max_length` capacity buffer. So `GetOutput` returns the pre-bound shared `OrtValue` and the `past == present` pointer identity is preserved. Pinned by `test/lit/e2e/test_gqa_output_allocator_present_dim.mlir`. (`build_metadata_json` emits each output shape verbatim — static extent or `-1`.)
   ...
   The converters reach that sizing by different routes, because they are told different things. `onnx.Custom(GroupQueryAttention)` is handed real `seqlens_k` and `total_seq_len` inputs, so it can size `present.*` from the past extent directly. Opset-23 `onnx.Attention` has no past-length input at all, so `OnnxAttentionConversion` takes the total KV length from the additive mask's KV extent — the mask tracks the history rather than the cache buffer, so it reads the same under either binding — and sizes the buffer as `max(dim(past_key, 2), total)`. That max is what makes one lowering serve both: a cache grown a step at a time has a past extent below the total, and a shared buffer has a capacity at or above it. A mask whose trailing extent is not a KV length (rank-2, or a broadcast 1) falls back to `past + current`, which is correct only for a growing cache; an `onnx.Attention` model in that shape is not share-buffer capable, and nothing at compile time can tell us it is being run that way.
