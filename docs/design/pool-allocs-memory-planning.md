...
> **Pool-allocs buckets by SSA-identical extents, so a pre-bufferize merge IS required (`--hip-dedup-dps-inits`, not `-cse`).** Pool-allocs only co-locates dynamic buffers whose `?` dims come from the **same SSA values** (the structural-equivalence rule above). Cross-layer activation buffers whose extents are recomputed per layer (SSA-distinct values that are nonetheless equal at runtime — e.g. a VLM vision tower's per-layer attention temps) therefore land in *different* buckets and never reuse, so pool-allocs **alone** does not recover that memory (gemma3 vision: ~30 GB). The pipeline closes this gap *before* bufferize with `--hip-dedup-dps-inits`, which merges two `tensor.empty` DPS inits when (1) their live ranges are disjoint and (2) their sizes are provably equal via global value numbering — collapsing the SSA-distinct-but-equal extents onto one rep so pool-allocs then buckets them together (vision: 30 GB → 1.235 GB, byte-for-byte the same as a `-cse`-on baseline). A stock `-cse` would achieve the same merge but ignores liveness and so clobbers two simultaneously-live DPS results (the Whisper-encoder miscompile; near-zero cosine), which is why the dedup pass — with its liveness guard — replaces it. Pool-allocs still does the *liveness* half of the job after bufferize: simultaneously-live buffers always get distinct slots (`@cse_hazard_live_distinct` in `test/lit/Dialect/hip-pool-allocs.mlir`) while dead-then-reused buffers share one (`@static_three_allocs_overlap`). See the `buildOnnxToHipPipelineTail` comment in `Pipelines.cpp` and `DedupDPSInits.cpp`.
