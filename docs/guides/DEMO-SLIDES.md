...
```bash
udna-opt.exe demo_two_layer_conv.mlir \
  --convert-onnx-to-hip
```
...
udna-compile.exe demo_with_interface.mlir \
...
**Tool**: `udna-compile` (uses LLVMBackend + DLLLinker libraries)
...
```
ONNX MLIR → udna-opt (passes) → udna-compile → DLL
```
...
- [x] **LLVM IR → DLL compilation** via udna-compile
...
3. ~~**LLVM IR → DLL compilation**~~ ✅ **COMPLETED** via udna-compile
...
cmake --build ../../build/$(basename $PWD) \
  --config Debug --target udna-opt udna-compile test-model-dll

# 2. Run Stage 1: ONNX → HIP
udna-opt.exe demo_two_layer_conv.mlir --convert-onnx-to-hip
...
udna-compile.exe demo_two_layer_conv.mlir \
...
│ • udna-opt: ONNX → MLIR (7 stages)                     │
│   (--convert-onnx-to-hip)                             │
│   (--ownership-based-buffer-deallocation)             │
│   (--memory-pooling)                                  │
│   (--convert-hip-to-llvm)                             │
│   (--generate-interface)                              │
│ • udna-compile: MLIR → DLL                       │
...
```bash
udna-opt input.mlir --convert-onnx-to-hip \
  --ownership-based-buffer-deallocation \
  --memory-pooling \
  --convert-hip-to-llvm \
  --generate-interface > transformed.mlir
udna-compile transformed.mlir -o output.dll
test-model-dll output.dll
```

**One-step (Production)**:
```bash
udna-compile input.mlir --from-onnx-mlir -o output.dll
```
...
- [02b-OneShotBufferize.md](../design/mlir/passes/02b-OneShotBufferize.md) - Bufferization pipeline
