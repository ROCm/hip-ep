...
hip-compile.exe demo_with_interface.mlir \
...
**Tool**: `hip-compile` (uses LLVMBackend + DLLLinker libraries)
...
```
ONNX MLIR → hip-opt (passes) → hip-compile → DLL
```
...
- [x] **LLVM IR → DLL compilation** via hip-compile
...
3. ~~**LLVM IR → DLL compilation**~~ ✅ **COMPLETED** via hip-compile
...
cmake --build ../../build/$(basename $PWD) \
  --config Debug --target hip-opt hip-compile test-model-dll
...
hip-compile.exe demo_two_layer_conv.mlir \
...
hip-compile transformed.mlir -o output.dll
test-model-dll output.dll
```

**One-step (Production)**:
```bash
hip-compile input.mlir --from-onnx-mlir -o output.dll
```
