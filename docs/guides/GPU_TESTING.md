...
# Compile demo model
../../build/onnx-hipdnn-ep/Debug/bin/udna-compile.exe \
  mlir-compiler/tools/udna-opt/demos/demo_two_layer_conv.mlir \
  --from-onnx-mlir \
...
├── Debug/bin/udna-compile.exe              # Compiler tool (262MB)
...
1. **Build Verification** (no GPU needed):
   ...
   cmake --build ../../build/onnx-hipdnn-ep --target udna-compile

   # Verify DLL generation succeeds
   udna-compile test.mlir -o test.dll
