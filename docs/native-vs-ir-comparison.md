...
**Adapted from:** [ROCm/UDNA-compiler `docs/design/alternatives/NATIVE-VS-IR-COMPARISON.md`](https://github.com/ROCm/UDNA-compiler/blob/main/docs/design/alternatives/NATIVE-VS-IR-COMPARISON.md).
...
The UDNA-original framing "recompiles every process start" is technically correct but misleading as shorthand: it is not per-inference, it is per-process.
...
The "memory-load vs disk file" sub-decision covered in the UDNA-original is moot here: both sub-paths require the OS loader (or a bypass of it) to map unsigned per-model code, and both are blocked by the same policy.
