...
| `hip-fusion-transform` | module | HIP-to-HIP rewriting at unrestricted arity (one op → many, many → one, or subgraph → subgraph) driven by PDLL patterns compiled into the library. Carries the Q/DQ fusions (`hip.qadd`, `hip.qmul`, `hip.qmatmul`, `hip.qgemm`, `hip.qconv`, `hip.qsigmoid`, `hip.qlpnormalization`) and Q/DQ round-trip elimination around layout / data-movement ops. |
