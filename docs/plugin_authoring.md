...
**Status:** the plugin surface is stable as it stands — there are no pending
changes to it. It may evolve in a future release, but a plugin always co-builds
with the host (static linkage), so there is no runtime version check: any future
change surfaces as a compile error, not a load-time surprise. Pin your plugin to
a host release and rebuild against that host. (`HIP_EP_PLUGIN_API_VERSION` marks
the registry surface at the source level; it is not a runtime gate.)
...
- [2. Quickstart: the in-tree sample](#2-quickstart-the-in-tree-sample-as-your-worked-example)
- [3. The minimal plugin](#3-the-minimal-plugin)
- [4. Contributing an MLIR pass](#4-contributing-an-mlir-pass)
- [5. Contributing runtime LLVM bitcode](#5-contributing-runtime-llvm-bitcode)
- [6. Contributing external libraries](#6-contributing-external-libraries)
- [7. Contributing a custom op (a vendor dialect)](#7-contributing-a-custom-op-a-vendor-dialect)
- [8. Composing a custom pipeline](#8-composing-a-custom-pipeline)
...
The host never calls back into the plugin after registration completes; the
plugin's pass code, bitcode buffers, and library paths live in static storage
...
- [`sample_plugin_runtime.cpp`](../test/plugin/sample_plugin/sample_plugin_runtime.cpp) —
  ...
  embedded as a C byte array in the plugin lib.
...
- **Namespace your names in a multi-plugin build.** Several ids can be selected
  at once (`-DHIPDNN_EP_COMPILER_PLUGINS=a;b`); their entries run in list order,
  and two plugins must not register the same pass name or collide on a bitcode
  symbol. Prefix both (symbol prefixing is covered in [section 5](#5-contributing-runtime-llvm-bitcode)).
  See the design doc's "Multiple plugins."
...
Note this shadowing only applies to the opt-in **native** link. On the
**default JIT path** the override is not available at all: plugin bitcode is
added as a sibling module in the model's JITDylib, so a name shared with the
in-tree runtime is a hard duplicate-definition error at symbol resolution, not a
silent shadow. Prefixing is therefore mandatory, not merely advisable.
...
- **Buffer lifetime: don't worry about it.** The host copies the bytes
  ...
  fine. The sample plugin embeds a build-time-generated C array only because
  that is the simplest source of the bytes.
