...
lib/Dialect/IR/HipDialect.td          ← authoritative (migrate-udna-compiler)
...
2. `lib/Dialect/IR/CMakeLists.txt` drives TableGen from `lib/*.td` (relative paths like `HipDialect.td`) and writes generated headers to `build/include/udna-compiler/Dialect/Hip/IR/` — both wrong.
...
- `lib/Dialect/IR/CMakeLists.txt:30` — wrong output path `udna-compiler/Dialect/Hip/IR`
...
3. Fix `lib/Dialect/IR/CMakeLists.txt`:
   - Output path: `udna-compiler/Dialect/Hip/IR` → `hip/Dialect/IR`
