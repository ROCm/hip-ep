...
`docs/design/MORPHIZEN-EP-INTEGRATION.md`: `udna-compiler.dll` must write
...
2. **`--constants-dir` → `--output-dir`**: rename the flag in
   `udna-compile` and `udna-opt` (including `PipelineOptions::constantsDir`
...
- `tools/udna-compile/udna-compile.cpp`
- `tools/udna-opt/udna-opt.cpp`
