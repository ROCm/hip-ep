...
## Step 2: `tools/udna-compile/udna-compile.cpp`
...
// After:
static udna::DiskFileSystem makeFileSystem(const std::string& outputDir)
```

---

## Step 3: `tools/udna-opt/udna-opt.cpp`
...
config.udna_build_dir = r"@CMAKE_BINARY_DIR@"
...
// After:
// RUN: udna-opt %s --morphizen-pipeline{output-dir=%T,constants-file=%basename_t.bin} 2>&1 | FileCheck %s
```
...
// After:
// RUN: udna-opt %s --morphizen-pipeline{output-dir=%T,constants-file=%basename_t.bin} | FileCheck %s
```
...
- [ ] `udna-compile --help` shows `--output-dir` (not `--constants-dir`)
- [ ] `udna-opt --help` lists `output-dir` in morphizen-pipeline options
