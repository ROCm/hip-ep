...
This guide builds onnx-hip-ep on Windows. The C++ dependencies
...
```bash
cd <workspace>
git clone https://github.com/ROCm/onnx-hip-ep.git
cd onnx-hip-ep
git submodule update --init --recursive
```

All subsequent commands run from the `onnx-hip-ep/` project root unless
explicitly noted.
...
Run all commands below from the `onnx-hip-ep/` project root.
...
```bash
cd ..  # Go to workspace directory (sibling of onnx-hip-ep)
...
```bash
cd ../onnx-hip-ep  # Back to the project root
pip install \
  ../build/onnxruntime/Release/dist/onnxruntime_directml-*.whl \
  ../build/onnx-hip-ep/python/dist/onnxruntime_ep_hip-*.whl \
...
```bash
# first cd to your onnx-hip-ep directory
