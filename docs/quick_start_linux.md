...
All commands below assume you are in `<workspace>/onnx-hip-ep/` (the
...
# https://github.com/ROCm/onnx-hip-ep/actions/workflows/linux-build.yml
RUN_ID=<paste-id>
mkdir -p ../prebuilt/$RUN_ID
( cd ../prebuilt/$RUN_ID && \
    gh run download $RUN_ID --repo ROCm/onnx-hip-ep \
...
# $WORKSPACE/build/onnx-hip-ep/_therock during the build. It is
