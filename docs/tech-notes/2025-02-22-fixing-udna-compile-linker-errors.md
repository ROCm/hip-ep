...
# Fixing hip-compile Linker Errors
...
After refactoring Backend → Target/LLVM, hip-compile failed to link with 46 unresolved LLVM symbols. Root cause: CMakeLists.txt dropped library dependencies required by lldCOFF.
...
**Build verification:**
- hip-compile.exe: builds successfully, 45 MB (Debug)
- hip-opt.exe: builds successfully, 42 MB (Debug)
