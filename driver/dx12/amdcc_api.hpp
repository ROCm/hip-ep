 ...
 * Source:
 * https://github.amd.com/AMD-Radeon-Driver/dxcp/blob/amd/stg/dxcp/public/AmdExtD3DDeviceApi.h
 *
...
struct AmdExtD3DPipelineCrossCompileInfo : AmdExtD3DCreateInfo {
  /// Pointer to the kernel blob (HSACO ELF bytes for ElfHsa, source for
  /// others).
  const void *pBlob;
...
public:
  virtual void STDMETHODCALLTYPE PushMarker(ID3D12GraphicsCommandList *,
                                            const char *) = 0;
  virtual void STDMETHODCALLTYPE PopMarker(ID3D12GraphicsCommandList *) = 0;
  virtual void STDMETHODCALLTYPE SetMarker(ID3D12GraphicsCommandList *,
                                           const char *) = 0;
};
...
public:
  virtual HRESULT STDMETHODCALLTYPE CheckExtFeatureSupport(
      uint32_t featureType, void *pFeatureData, size_t featureDataSize)
      const = 0;
  virtual HRESULT STDMETHODCALLTYPE CreateComputePipelineState(
      const AmdExtD3DCreateInfo *pAmdExtCreateInfo,
      const D3D12_COMPUTE_PIPELINE_STATE_DESC *pDesc, REFIID refiid,
      void **ppPipelineState) = 0;
};
...
public:
  virtual void STDMETHODCALLTYPE SetPrimitiveTopology(
      ID3D12GraphicsCommandList * pCmdList, uint32_t topology) = 0;
};
...
public:
  virtual HRESULT STDMETHODCALLTYPE CreateComputePipelineFromElf(
      AmdExtD3DPipelineElfInfo * pAmdExtCreateInfo, REFIID refiid,
      void **ppPipelineState) = 0;

  // Bind kernel arguments on a command list before Dispatch().
  virtual void STDMETHODCALLTYPE SetKernelArguments(
      ID3D12GraphicsCommandList * pCmdList, uint32_t firstArg,
      uint32_t argCount, const void *const *ppValues) = 0;
};
...
public:
  virtual void STDMETHODCALLTYPE GetGpuRtInterfaceVersion(uint32_t *
                                                          pVersion) = 0;
...
public:
  ...
  virtual HRESULT STDMETHODCALLTYPE CreateComputePipelineCrossCompile(
      const AmdExtD3DPipelineCrossCompileInfo *pInfo, REFIID riid,
      void **ppPipelineState) = 0;
};
...
// v10 — adds SetPalElfComputeUserData for PAL ELF dispatch (CmdSetUserData
// bypass) Equivalent to metacommand CmdSetUserData(Compute, 0, 2, {cbVaLo,
// cbVaHi}).
...
public:
  virtual void STDMETHODCALLTYPE SetPalElfComputeUserData(
      ID3D12GraphicsCommandList * pCmdList, uint32_t firstSlot,
      uint32_t slotCount, const uint32_t *pData) = 0;

  // PAL ELF full dispatch: embeds CbData in command buffer
  // (CmdAllocateEmbeddedData), sets user data 0:1 = CbData VA, dispatches.
  // Matches metacommand dispatch exactly.
  virtual void STDMETHODCALLTYPE DispatchPalElf(
      ID3D12GraphicsCommandList * pCmdList, const uint32_t *pCbDataDwords,
      uint32_t cbDataDwords, uint32_t dispatchX, uint32_t dispatchY,
      uint32_t dispatchZ) = 0;
};

// ---------------------------------------------------------------------------
// IAmdExtD3DFactory — top-level factory, obtained from
...
public:
  virtual HRESULT STDMETHODCALLTYPE CreateInterface(
      IUnknown * pOuter, REFIID riid, void **ppvObject) = 0;
};
...
// ---------------------------------------------------------------------------
using PFN_AmdExtD3DCreateInterface = HRESULT(__cdecl *)(IUnknown *pOuter,
                                                        REFIID riid,
                                                        void **ppvObject);
