...
inline bool read_sections(const std::vector<std::uint8_t> &elf,
  ...
  const std::uint8_t *shstr_hdr =
      elf.data() + shoff + static_cast<std::uint64_t>(shentsize) * shstrndx;
  std::uint64_t shstr_off = 0, shstr_size = 0;
  std::memcpy(&shstr_off, shstr_hdr + 24, 8);
  std::memcpy(&shstr_size, shstr_hdr + 32, 8);
  if (shstr_off + shstr_size > elf.size())
    return false;
  ...
  for (std::uint16_t i = 0; i < shnum; ++i) {
    ...
    if (nameoff < shstr_size) {
      const char *p =
          reinterpret_cast<const char *>(elf.data() + shstr_off + nameoff);
      s.name.assign(
          p, ::strnlen(p, static_cast<std::size_t>(shstr_size - nameoff)));
    }
...
inline bool find_symbol(const std::vector<std::uint8_t> &elf,
                        ...
                        const std::string &want, SymRef &out,
                        std::string *cuid_name = nullptr) {
  for (const auto &st : secs) {
    if (st.type != 2 /*SHT_SYMTAB*/)
      continue;
    if (st.link >= secs.size())
      continue;
    const Section &strtab = secs[st.link];
    if (strtab.off + strtab.size > elf.size())
      continue;
    ...
    for (std::uint64_t i = 0; i < n; ++i) {
      ...
      const char *np =
          reinterpret_cast<const char *>(elf.data() + strtab.off + st_name);
      const std::string nm(
          np, ::strnlen(np, static_cast<std::size_t>(strtab.size - st_name)));
      ...
      if (nm != want)
        continue;
      if (st_shndx == 0 || st_shndx >= secs.size())
        continue;

      out.file_off =
          secs[st_shndx].off + st_value; // ET_REL: st_value is section-relative
      out.size = st_size;
      out.found = true;
      if (cuid_name == nullptr)
        return true;
    }
  }
  return out.found;
...
inline std::vector<std::uint8_t> filter_note(const std::uint8_t *desc,
  ...
  for (std::size_t i = 0; i < root_n; ++i) {
    ...
    for (std::size_t k = 0; k < nk; ++k) {
      ...
      md::Cursor probe = c;
      std::size_t nf = 0;
      if (!md::read_map_hdr(probe, nf))
        return {};
      std::string this_name;
      for (std::size_t f = 0; f < nf; ++f) {
        std::string fk;
        if (!md::read_str(probe, fk))
          return {};
        if (fk == ".name") {
          if (!md::read_str(probe, this_name))
            return {};
        } else if (!md::skip_value(probe))
          return {};
      }
      if (!md::skip_value(c))
        return {};
      if (this_name == kernel_name) {
        hit_off = eoff;
        hit_size = c.p - eoff;
      }
    }
...
build_single_kernel(const std::vector<std::uint8_t> &src,
                    const std::string &kernel_name) {
  if (src.size() < 64)
    return {};
  ...
  if (!detail::read_sections(src, secs))
    return {};

  detail::SymRef kern{}, kd{};
  std::string cuid = "__hip_cuid_000000000000";
  if (!detail::find_symbol(src, secs, kernel_name, kern, &cuid))
    return {};
  if (!detail::find_symbol(src, secs, kernel_name + ".kd", kd))
    return {};
  if (kern.size == 0)
    return {};

  constexpr std::uint64_t k_kd_size = 64u;
  if (kern.file_off + kern.size > src.size())
    return {};
  if (kd.file_off + k_kd_size > src.size())
    return {};

  const std::uint8_t *note_desc = nullptr;
  std::size_t note_desc_size = 0;
  if (!hip_ep::dx12::hsa_md::detail::find_amdgpu_note(src, note_desc,
                                                      note_desc_size))
    return {};
  std::vector<std::uint8_t> new_note =
      detail::filter_note(note_desc, note_desc_size, kernel_name);
  if (new_note.empty())
    return {};
  ...
  strtab.insert(strtab.end(), kernel_name.begin(), kernel_name.end());
  strtab.push_back(0);
  const std::uint32_t kd_str = static_cast<std::uint32_t>(strtab.size());
  strtab.insert(strtab.end(), kd_name.begin(), kd_name.end());
  strtab.push_back(0);
  const std::uint32_t cuid_str = static_cast<std::uint32_t>(strtab.size());
  strtab.insert(strtab.end(), cuid.begin(), cuid.end());
  strtab.push_back(0);
  ...
  auto add_shstr = [&](const char *s) {
    ...
    while (*s)
      shstr.push_back(static_cast<std::uint8_t>(*s++));
    shstr.push_back(0);
    return idx;
  };
  ...
  detail::push32(note_sec,
                 static_cast<std::uint32_t>(new_note.size())); // descsz
  ...
  while (note_sec.size() % 4)
    note_sec.push_back(0);
  ...
  auto w16 = [&](std::uint64_t o, std::uint16_t v) {
    std::memcpy(out.data() + o, &v, 2);
  };
  auto w32 = [&](std::uint64_t o, std::uint32_t v) {
    std::memcpy(out.data() + o, &v, 4);
  };
  auto w64 = [&](std::uint64_t o, std::uint64_t v) {
    std::memcpy(out.data() + o, &v, 8);
  };
  ...
  std::memcpy(out.data() + text_off, src.data() + kern.file_off,
              static_cast<std::size_t>(kern.size));
  std::memcpy(out.data() + rodata_off, src.data() + kd.file_off,
              static_cast<std::size_t>(k_kd_size));
  ...
  auto shdr = [&](std::uint16_t i, std::uint32_t name, std::uint32_t type,
                  std::uint64_t flags, std::uint64_t off, std::uint64_t size,
                  std::uint32_t link, std::uint32_t info, std::uint64_t align,
                  std::uint64_t entsz) {
