...
inline bool read_uint(Cursor &c, std::uint64_t &out) {
  ...
  if (b == 0xcd && c.p + 2 < c.n) {
    out = be_read(c.d + c.p + 1, 2);
    c.p += 3;
    return true;
  }
  if (b == 0xce && c.p + 4 < c.n) {
    out = be_read(c.d + c.p + 1, 4);
    c.p += 5;
    return true;
  }
  if (b == 0xcf && c.p + 8 < c.n) {
    out = be_read(c.d + c.p + 1, 8);
    c.p += 9;
    return true;
  }
  return false;
...
inline bool read_str(Cursor &c, std::string &out) {
  ...
  if ((b & 0xe0) == 0xa0) {
    len = b & 0x1fu;
    hdr = 1;
  } else if (b == 0xd9 && c.p + 1 < c.n) {
    len = c.d[c.p + 1];
    hdr = 2;
  } else if (b == 0xda && c.p + 2 < c.n) {
    len = static_cast<std::size_t>(be_read(c.d + c.p + 1, 2));
    hdr = 3;
  } else if (b == 0xdb && c.p + 4 < c.n) {
    len = static_cast<std::size_t>(be_read(c.d + c.p + 1, 4));
    hdr = 5;
  } else
    return false;
  if (c.p + hdr + len > c.n)
...
inline bool read_map_hdr(Cursor &c, std::size_t &count) {
  ...
  if (b == 0xde && c.p + 2 < c.n) {
    count = static_cast<std::size_t>(be_read(c.d + c.p + 1, 2));
    c.p += 3;
    return true;
  }
  if (b == 0xdf && c.p + 4 < c.n) {
    count = static_cast<std::size_t>(be_read(c.d + c.p + 1, 4));
    c.p += 5;
    return true;
  }
  return false;
...
inline bool skip_value(Cursor &c) {
  ...
  if ((b & 0xf0) == 0x90) {
    std::size_t n;
    return read_arr_hdr(c, n) && skip_n(c, n);
  }
  if ((b & 0xf0) == 0x80) {
    std::size_t n;
    return read_map_hdr(c, n) && skip_n(c, 2 * n);
  }
...
inline bool find_amdgpu_note(const std::vector<std::uint8_t> &elf,
  ...
  for (std::uint16_t i = 0; i < shnum; ++i) {
    ...
    if (sh_type != 7 /*SHT_NOTE*/)
      continue; // not a note
    if (sh_off + sh_size > elf.size() || sh_size < 12)
      continue;
    ...
    if (12 + name_aligned + descsz > sh_size)
      continue;
    if (std::strncmp(reinterpret_cast<const char *>(elf.data() + sh_off + 12),
                     "AMDGPU", 6) != 0)
      continue;
...
inline std::vector<KernelAbi> parse(const std::vector<std::uint8_t> &elf) {
  ...
  if (!detail::find_amdgpu_note(elf, desc, desc_size))
    return out;

  detail::Cursor c{desc, desc_size, 0};
  std::size_t top_n = 0;
  if (!detail::read_map_hdr(c, top_n))
    return out;

  for (std::size_t i = 0; i < top_n; ++i) {
    std::string key;
    if (!detail::read_str(c, key))
      return out;
    if (key != "amdhsa.kernels") {
      if (!detail::skip_value(c))
        return out;
      continue;
    }

    std::size_t nk = 0;
    if (!detail::read_arr_hdr(c, nk))
      return out;

    for (std::size_t k = 0; k < nk; ++k) {
      std::size_t nf = 0;
      if (!detail::read_map_hdr(c, nf))
        return out;

      KernelAbi abi;
      for (std::size_t f = 0; f < nf; ++f) {
        std::string fk;
        if (!detail::read_str(c, fk))
          return out;

        if (fk == ".name" || fk == ".symbol") {
          std::string v;
          if (!detail::read_str(c, v))
            return out;
          if (fk == ".name")
            abi.name = v;
        } else if (fk == ".kernarg_segment_size") {
          std::uint64_t v = 0;
          if (!detail::read_uint(c, v))
            return out;
          ...
          std::size_t na = 0;
          if (!detail::read_arr_hdr(c, na))
            return out;
          for (std::size_t a = 0; a < na; ++a) {
            std::size_t naf = 0;
            if (!detail::read_map_hdr(c, naf))
              return out;
            KernelArg arg;
            for (std::size_t af = 0; af < naf; ++af) {
              std::string ak;
              if (!detail::read_str(c, ak))
                return out;
              if (ak == ".offset" || ak == ".size") {
                ...
                if (!detail::read_uint(c, v))
                  return out;
                if (ak == ".offset")
                  arg.offset = static_cast<std::size_t>(v);
                else
                  arg.size = static_cast<std::size_t>(v);
              } else if (ak == ".value_kind") {
                if (!detail::read_str(c, arg.value_kind))
                  return out;
              } else if (ak == ".name") {
                if (!detail::read_str(c, arg.name))
                  return out;
                ...
                if (!detail::read_str(c, v))
                  return out;
                if (v == "read_only")
                  arg.access = ArgAccess::ReadOnly;
                else if (v == "write_only")
                  arg.access = ArgAccess::WriteOnly;
                else if (v == "read_write")
                  arg.access = ArgAccess::ReadWrite;
              } else if (!detail::skip_value(c))
                return out;
            }
            if (arg.value_kind == "global_buffer")
              arg.kind = ArgKind::GlobalBuffer;
            else if (arg.value_kind == "by_value")
              arg.kind = ArgKind::ByValue;
            else if (arg.value_kind.rfind("hidden_", 0) == 0)
              arg.kind = ArgKind::Hidden;
