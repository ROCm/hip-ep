...
struct Options {
  ...
  uint64_t num_elements =
      16; // matches sample_hip_add_compiler.hip.mlir (1x4x4xf32)
...
void print_usage(const char *argv0) {
  std::cout
      ...
      << " --kernel <path.elf> [options]\n"
         ...
         "  --entry <name>      Kernel symbol (default: the ELF single "
         "kernel)\n"
         "  --format <fmt>      hsa_rel | hsa_dyn | pal_rel | pal_dyn "
         "(default: hsa_rel)\n"
         ...
         "  --verify <mode>     none | add | sqrt CPU reference (default: "
         "none)\n"
         "  --scalars a,b       by_value arg values in ABI order (default: "
         "--elements)\n"
    ...
    if (arg == "--kernel")
      ...
      for (std::size_t p = 0; p <= s.size();) {
        ...
        const std::string tok =
            s.substr(p, comma == std::string::npos ? comma : comma - p);
        if (!tok.empty())
          opts.scalars.push_back(std::stoll(tok));
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    auto abis = hip_ep::dx12::hsa_md::parse(elf);
    if (abis.empty())
      throw std::runtime_error("no AMDHSA kernel metadata in " +
                               opts.kernel_path);
    ...
    // PAL resolves one kernel via its .kd symbol; a multi-kernel object removes
    // the device.
    if (abis.size() > 1) {
      auto single =
          hip_ep::dx12::elf_split::build_single_kernel(elf, abi->name);
      if (single.empty())
        throw std::runtime_error("failed to extract single kernel: " +
                                 abi->name);
    ...
    for (const auto &a : abi->args) {
      ...
      if (a.kind == AK::GlobalBuffer) {
        ...
        // by_value args default to the element count; --scalars overrides in
        // order.
    ...
    std::cout << "Loading kernel ELF: " << opts.kernel_path
              << " (entry=" << kd.entry_point << ", format=" << opts.elf_format
    ...
    for (uint64_t i = 0; i < opts.num_elements; ++i) {
      ...
      max_abs_diff = (std::max)(
          max_abs_diff, static_cast<double>(std::fabs(output[i] - expected)));
    }
    std::cout << "max |output - "
