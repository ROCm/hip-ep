...
// Set HIPDNN_EP_DEBUG=1 to enable all [ONNX→HIP]/[GenerateInterface]/[HipToLLVM] output.
#include <cstdlib>
...
#define COMPILER_DEBUG_LOG(expr) \
    do { if (hipdnn_ep_debug_enabled()) llvm::errs() << expr; } while (0)

// Usage: DRIVER_DEBUG_LOG("[TAG] text " << value << "\n");
#define DRIVER_DEBUG_LOG(expr) \
    do { if (hipdnn_ep_debug_enabled()) std::cerr << expr; } while (0)
