...
/**
 * @brief Concrete FileReader backed by contiguous memory.
 *
 * Wraps a raw memory region as a FileReader. Supports mmap() for zero-copy
 * access — callers can skip the fread() path entirely when the source data
 * is already resident in host memory.
 *
 * Two ownership modes:
 *   - **Non-owning**: caller ensures the underlying data outlives the reader.
 *     Typical use: pointing into ORT tensor memory that is valid for the
 *     session lifetime.
 *   - **Owning**: the reader holds a shared_ptr to a std::vector<uint8_t>
 *     that keeps the data alive.
 *
 * Thread safety: not thread-safe (mutable read position).
 */
class MemoryFileReader : public FileReader {
public:
  MemoryFileReader(const void* data, size_t size)
      : data_(static_cast<const char*>(data)), size_(size) {}

  MemoryFileReader(std::shared_ptr<std::vector<uint8_t>> owned)
      : owned_(std::move(owned)),
        data_(reinterpret_cast<const char*>(owned_->data())),
        size_(owned_->size()) {}

  size_t size() const override { return size_; }
  void rewind() const override { pos_ = 0; }

  std::size_t fread(void* buffer, std::size_t count) const override {
...
/**
 * @brief FileSystem wrapper that overlays memory-backed files onto a base
 *        FileSystem.
 *
 * create_reader() checks the overlay map first; on a hit it returns a
 * MemoryFileReader without touching the base FileSystem. On a miss — and
 * for all create_writer() calls — it delegates to the base.
 *
 * Primary use case: during first-run compilation the compiler writes
 * constants to the tar cache (via the base FileSystem). The level-1 pass
 * then registers the same constants.bin buffer — obtained through the tar's
 * mmap() — as an overlay. When the generated DLL's inference_init() later
 * calls create_reader("constants.bin"), it gets a zero-copy MemoryFileReader
 * that serves data directly from host memory, bypassing any further tar I/O.
 *
 * For the EP context (second-run) path the overlay map is empty and every
 * call falls through to the base FileSystem unchanged.
 *
 * @par Ownership
 * - The base FileSystem pointer is **non-owning**; the caller must ensure
 *   the base outlives this OverlayFileSystem.
 * - Overlaid memory regions can be non-owning (caller-managed lifetime) or
 *   owning (shared_ptr<vector<uint8_t>>).
 *
 * @par Thread Safety
 * Not thread-safe; intended for single-session use.
 */
class OverlayFileSystem : public FileSystem {
public:
  explicit OverlayFileSystem(FileSystem* base) : base_(base) {}

  void add_overlay(const std::string& name, const void* data, size_t size) {
    overlays_[name] = {data, size, nullptr};
  }

  void add_overlay(const std::string& name,
                   std::shared_ptr<std::vector<uint8_t>> data) {
    overlays_[name] = {data->data(), data->size(), std::move(data)};
  }

  bool has_overlay(const std::string& name) const {
    return overlays_.count(name) > 0;
  }

  FileReader* create_reader(const char* path) override {
    auto it = overlays_.find(path);
    if (it != overlays_.end()) {
      auto& e = it->second;
      FileReader* r = e.owned ? new MemoryFileReader(e.owned)
                              : new MemoryFileReader(e.data, e.size);
      owned_readers_.insert(r);
      return r;
    }
    return base_ ? base_->create_reader(path) : nullptr;
  }

  FileWriter* create_writer(const char* path) override {
    return base_ ? base_->create_writer(path) : nullptr;
  }

  void destroy_reader(FileReader* reader) override {
    auto it = owned_readers_.find(reader);
    if (it != owned_readers_.end()) {
      owned_readers_.erase(it);
      delete reader;
    } else if (base_) {
      base_->destroy_reader(reader);
    }
  }

  void destroy_writer(FileWriter* writer) override {
    if (base_)
      base_->destroy_writer(writer);
    else
      delete writer;
  }
...
private:
  ...
  FileSystem* base_;
  std::unordered_map<std::string, Entry> overlays_;
  std::unordered_set<FileReader*> owned_readers_;
};
