...
void add_bias_nchw(float* data, const float* bias,
    ...
    launch_add_bias_nchw_f32(data, bias, channels, spatial_size, total_size, stream);
}
...
void softmax_3d(const float* input, float* output,
    ...
    if (inner_size == 1) {
        ...
        launch_softmax_3d_f32(input, output, axis_size, inner_size, outer_size, stream);
    }
}
...
void transpose(const float* input, float* output,
    ...
    if (ndim == 4) {
        ...
        launch_transpose_4d_f32(input, output,
                                in_shape[0], in_shape[1], in_shape[2], in_shape[3],
                                static_cast<int64_t>(perm[0]), 
                                static_cast<int64_t>(perm[1]), 
                                static_cast<int64_t>(perm[2]), 
                                static_cast<int64_t>(perm[3]),
                                stream);
    } else {
        // Fallback for non-4D - just copy (transpose is identity in 1D/2D with perm=[0,1,...])
...
void tile(const float* input, float* output,
    ...
    if (ndim == 4) {
        launch_tile_4d_f32(input, output,
                           in_shape[0], in_shape[1], in_shape[2], in_shape[3],
                           repeats[0], repeats[1], repeats[2], repeats[3],
                           out_size, stream);
    } else if (ndim == 5) {
        launch_tile_5d_f32(input, output,
                           in_shape[0], in_shape[1], in_shape[2], in_shape[3], in_shape[4],
                           repeats[0], repeats[1], repeats[2], repeats[3], repeats[4],
                           out_size, stream);
    } else {
        // Use general N-dimensional tile kernel
        launch_tile_nd_f32(input, output, in_shape, ndim, repeats, 
                           in_size, out_size, stream);
    }
}
