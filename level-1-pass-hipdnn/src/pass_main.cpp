...
void AddShapeToProto(hipdnn::TensorShape *shape_proto,
...
bool GenerateConvMetadata(const Node &conv_node,
  ...
  try {
    ...
    if (has_bias) {
      out_proto.add_input_data_types(
          node_arg_get_element_type(*inputs[2].node_arg));
    }
    ...
    for (auto p : pads_vec)
      conv_attrs->add_pads(p);
    for (auto s : strides_vec)
      conv_attrs->add_strides(s);
    for (auto d : dilations_vec)
      conv_attrs->add_dilations(d);
...
struct Level1HipDnn {
  ...
  void process(IPass &self, Graph &ort_graph) {
    // Iterate through all nodes in reverse order looking for supported
    // operations
    ...
    for (auto it = node_indices.rbegin(); it != node_indices.rend(); ++it) {
      ...
      auto input_data =
          NodeArgConstRef::from_node_arg(ort_graph, *conv_inputs[0].node_arg);
      auto weight_data =
          NodeArgConstRef::from_node_arg(ort_graph, *conv_inputs[1].node_arg);
      auto output_data =
          NodeArgConstRef::from_node_arg(ort_graph, *conv_output_node_args[0]);

      // Generate metadata directly into proto
      hipdnn::HipdnnParamProto hipdnn_param;
      std::vector<std::string> constant_names;
      bool success =
          GenerateConvMetadata(*node, input_data, weight_data, output_data,
                               constant_names, hipdnn_param);
      ...
      inputs_list.push_back(
          weight_data.name()); // Weight (may be constant or dynamic)

      // Add bias if present
      bool has_bias = conv_inputs.size() >= 3;
      if (has_bias) {
        auto bias_data =
            NodeArgConstRef::from_node_arg(ort_graph, *conv_inputs[2].node_arg);
        inputs_list.push_back(
            bias_data.name()); // Bias (may be constant or dynamic)
      }
      ...
      MY_LOG(1) << "  meta_def constants: "
                << meta_def->constant_initializers_size();

      // Serialize proto to JSON (proto already populated by
      // GenerateConvMetadata)
