...
struct Level1Rocm {
  ...
  collect_merged_params(const std::vector<const Node*>& group) {
    ...
    for (auto* node : group) {
      ...
      *merged.add_rocm_params() = param;
      MY_LOG(2) << "[HIP EP Level-1] Added node to merged group: " << output_name;
    }
  ...
  void process(IPass& self, Graph& graph) {
    ...
    MY_LOG(1) << "[HIP EP Level-1] Cloned model for sub-pass processing";
    ...
    if (rocm_nodes.empty()) {
      MY_LOG(1) << "[HIP EP Level-1] No ROCm nodes found, nothing to merge";
      return;
    }
    ...
    for (const auto& group : groups) {
      MY_LOG(1) << "[HIP EP Level-1] Processing group " << group_idx
      ...
      MY_LOG(2) << "[HIP EP Level-1] Group " << group_idx << " has "
      ...
      if (meta_def) {
        ...
        MY_LOG(1) << "[HIP EP Level-1] Failed to fuse group " << group_idx
                  << ": " << error.comments;
      }
