...
struct Level1Rocm {
  ...
  bool is_rocm_fused_node(Graph &graph, const Node &node) {
    ...
    MY_LOG(1)
        << "[ROCm EP Level-1] is_rocm_fused_node: found ROCm node with output="
        << output_name;
    return true;
  }
        ...
        if (reader) {
          ...
          if (parse_status.ok()) {
            ...
            *node_proto->mutable_params() = rocm_param;
            MY_LOG(2) << "[ROCm EP Level-1] Node " << i
                      << ": loaded params from " << param_filename;
          } else {
            LOG(WARNING) << "[ROCm EP Level-1] Failed to parse param file: "
                         << param_filename;
          }
        }
      }
  ...
  void process(IPass &self, Graph &graph) {
    ...
    for (const auto &sub_pass_name : config.sub_pass_names()) {
      ...
      if (sub_pass) {
        ...
        LOG(WARNING) << "[ROCm EP Level-1] Failed to create sub-pass: "
                     << sub_pass_name;
      }
    }
      ...
      if (meta_def) {
        ...
        if (serialize_status.ok()) {
          ...
          if (writer) {
            ...
            MY_LOG(1) << "[ROCm EP Level-1] Saved subgraph: "
                      << subgraph_filename;
          }
    ...
    MY_LOG(1) << "[ROCm EP Level-1] Pass completed - merged " << groups.size()
              << " groups";
  }
