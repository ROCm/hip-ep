 ...
 * {
 *   "sub_pass_names": ["morphizen-pass_level2_rocm_conv",
 * "morphizen-pass_level2_rocm_gemm"]
 * }
 *
...
struct Level1Rocm {
  ...
  bool is_rocm_fused_node(Graph& graph, const Node& node) {
    ...
    MY_LOG(2) << "[HIP EP Level-1] Checking node: output=" << output_name
    ...
    // Note: We check for "com.xilinx" domain OR "not_used_op" op_type for
    // flexibility
    ...
    MY_LOG(1)
        << "[HIP EP Level-1] is_rocm_fused_node: found ROCm node with output="
        << output_name;
    return true;
  }
  ...
  // When we process a node, all its input producers have already been
  // processed. We can directly merge a node into its producer's group using
  // Union-Find.
  //
  ...
  std::vector<std::vector<const Node*>> find_mergeable_groups(
    ...
    if (!ENV_PARAM(MORPHIZEN_ROCM_EN_LVL1_MERGE)) {
      MY_LOG(1) << "[HIP EP Level-1] Single-node mode: creating one subgraph "
                   "per node";
    ...
    for (int32_t i = 0; i < static_cast<int32_t>(group.size()); ++i) {
      ...
      if (!param_filename.empty()) {
        ...
        if (reader) {
          ...
          auto parse_status = google::protobuf::util::JsonStringToMessage(
              param_json, &rocm_param);
          if (parse_status.ok()) {
            rocm_param.set_ep_context_file_name(param_filename);
            *node_proto->mutable_params() = rocm_param;
            MY_LOG(2) << "[HIP EP Level-1] Node " << i
                      << ": loaded params from " << param_filename;
          } else {
            LOG(WARNING) << "[HIP EP Level-1] Failed to parse param file: "
                         << param_filename;
          }
        }
      }
      ...
      for (auto* input : inputs) {
        ...
        if (it != output_producer_map.end()) {
          ...
          MY_LOG(2) << "[HIP EP Level-1] Node " << i << ": input "
                    << input_name << " -> internal(node=" << it->second.first
  ...
  void process(IPass& self, Graph& graph) {
    ...
    // Level-2 passes use level_2_fuse() which creates fused nodes but doesn't
    // update context.json
    MY_LOG(2) << "[HIP EP Level-1] Number of sub-passes to run: "
    ...
    for (const auto& sub_pass_name : config.sub_pass_names()) {
      ...
      if (sub_pass) {
        ...
        LOG(WARNING) << "[HIP EP Level-1] Failed to create sub-pass: "
                     << sub_pass_name;
      }
    }
    ...
    MY_LOG(1) << "[HIP EP Level-1] Found " << rocm_nodes.size()
              << " ROCm fused nodes";
    ...
    MY_LOG(1) << "[HIP EP Level-1] Found " << groups.size()
              << " mergeable groups";
    ...
    for (auto& group : groups) {
      ...
      auto external_outputs =
          collect_external_outputs(group, internal_inputs, graph);
      ...
      // For now, we don't merge constant initializers - they stay with their
      // original nodes
      ...
      if (meta_def) {
        ...
        MY_LOG(1) << "[HIP EP Level-1] Created merged fused node: "
                  << merged_name << " with " << group.size() << " operations";
      } else {
    ...
    MY_LOG(1) << "[HIP EP Level-1] Pass completed - merged " << groups.size()
              << " groups";
  }
