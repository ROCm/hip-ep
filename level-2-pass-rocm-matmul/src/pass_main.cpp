...
struct Level2RocmMatmul {
  ...
  std::unique_ptr<Rule> create_rule(IPass* self) {
    auto pattern = PatternBuilder().create_by_json(
        std::string((const char*)matmul_json));
    ...
    return Rule::create_rule(
        ...
        [=](Graph* graph, binder_t& binder) -> bool {
          ...
          if (a_shape_opt) a_shape = *a_shape_opt;
          if (b_shape_opt) b_shape = *b_shape_opt;

          // MatMul requires at least 2D tensors
          if (a_shape.size() < 2 || b_shape.size() < 2) {
            ROCM_LOG(1) << LOG_PREFIX << " Skipping: requires at least 2D tensors";
            return false;
          }
          ...
          if (k != k_b) {
            ROCM_LOG(1) << LOG_PREFIX << " Skipping: K dimension mismatch (A.K=" 
                        << k << ", B.K=" << k_b << ")";
            return false;
          }
          ...
          // Store full shapes
          for (auto dim : a_shape) matmul_params->add_shape_a(dim);
          for (auto dim : b_shape) matmul_params->add_shape_b(dim);
          ...
          y_shape.push_back(m);
          y_shape.push_back(n);
          for (auto dim : y_shape) matmul_params->add_shape_y(dim);
          ...
          if (b_is_constant) {
            ...
            std::string weight_filename = rocm_pass::generate_weight_filename("rocm_matmul", weight_name);
            ...
            if (rocm_pass::save_weight_to_cache(pass_context, weight_data, weight_filename, LOG_PREFIX)) {
              ...
              matmul_params->set_weight_file_size(static_cast<int64_t>(weight_data.size() * sizeof(float)));
            }
            ROCM_LOG(1) << LOG_PREFIX << " B is constant weight: " << weight_name;
          } else {
          ...
          auto [meta_def, error] = self->try_fuse(
              *graph, "rocm_matmul", input_names, output_names, constant_initializers, "ROCm_EP");
