...
struct Level2RocmMul {
  ...
  std::unique_ptr<Rule> create_rule(IPass* self) {
    auto pattern = PatternBuilder().create_by_json(
        std::string((const char*)mul_json));
    ...
    return Rule::create_rule(
        ...
        [=](Graph* graph, binder_t& binder) -> bool {
          ...
          auto b_ref = NodeArgConstRef::from_node_arg(*graph, *input_B.node_arg);
          if (b_ref.is_constant()) {
            mul_params->set_b_is_constant(true);
            
            // If B is a scalar constant, extract the value
            if (b_size == 1) {
              auto b_data = node_arg_get_const_data_as_floats(*graph, *input_B.node_arg);
              if (!b_data.empty()) {
          ...
          auto [meta_def, error] = self->try_fuse(
              *graph, "rocm_mul", input_names, output_names, constant_initializers, "ROCm_EP");
