...
struct Level2RocmTile {
  ...
  std::unique_ptr<Rule> create_rule(IPass* self) {
    auto pattern = PatternBuilder().create_by_json(
        std::string((const char*)tile_json));
    ...
    return Rule::create_rule(
        ...
        [=](Graph* graph, binder_t& binder) -> bool {
          ...
          constant_initializers.push_back(node_arg_get_name(*repeats_arg.node_arg));

          auto [meta_def, error] = self->try_fuse(
              *graph, "rocm_tile", input_names, output_names, constant_initializers, "ROCm_EP");
