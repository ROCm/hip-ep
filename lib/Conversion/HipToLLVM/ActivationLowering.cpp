...
// hip.silu(ctx, input, output)
//   -> wrap_hip_silu(state, input, output, num_elements, data_type)
// SiLU(x) = x * sigmoid(x). MIOpen doesn't support SiLU directly,
// so we use a custom HIP kernel via wrap_hip_silu runtime function.
struct SiluOpLowering : public ConvertOpToLLVMPattern<SiluOp> {
  ...
  matchAndRewrite(SiluOp op, OpAdaptor adaptor,
    ...
    if (dataType < 0)
      return rewriter.notifyMatchFailure(
          op, "unsupported element type for hip.silu");
    Value dataTypeVal = createI64Const(dataType);

    // int wrap_hip_silu(RuntimeState* state, void* input, void* output,
