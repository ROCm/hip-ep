...
struct CausalConvWithStateOpLowering
  ...
  matchAndRewrite(CausalConvWithStateOp op, OpAdaptor adaptor,
    ...
    Value channels = getMemRefDimSize(
        inputType, channelsLast ? 2 : 1, adaptor.getInput(), rewriter, loc);

    // seq_len = product of spatial dimensions (last ndim dims)
    // For ndim=1: just the last dim, or dim 1 when channels moved to the end
    Value seqLen = getMemRefDimSize(
        inputType, channelsLast ? 1 : 2, adaptor.getInput(), rewriter, loc);
