...
// hip.conv1d(%ctx, %input, %weights, %bias, %output)
//   -> wrap_conv1d(state, input, weights, bias, output,
//                  N, Cin, Lin, Cout, K, stride, pad, element_size_bytes)
//
// NCL layout is reinterpreted by the runtime as NC[H=1]L and dispatched to
// MIOpen's 4D forward-convolution path. Symmetric padding only — the lowering
// asserts pads[0] == pads[1].
struct Conv1dOpLowering : public ConvertOpToLLVMPattern<Conv1dOp> {
  using ConvertOpToLLVMPattern::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(Conv1dOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    ModuleOp module = op->getParentOfType<ModuleOp>();
    Type ptrType = getPtrType();
    ...
    if (inputType.getRank() != 3)
      return op.emitError("Input must be rank-3 tensor [N, Cin, Lin]");
    if (weightsType.getRank() != 3)
      return op.emitError("Weights must be rank-3 tensor [Cout, Cin, K]");
    if (outputType.getRank() != 3)
      return op.emitError("Output must be rank-3 tensor [N, Cout, Lout]");

    // Shapes: input [N, Cin, Lin], weights [Cout, Cin, K].
    ...
    // Attributes. wrap_conv1d takes scalar stride / pad (symmetric); enforce
    // pads[0] == pads[1] in the lowering — asymmetric padding would silently
    // miscompile.
    auto kernelShape = op.getKernelShape();
    auto strides = op.getStrides();
    auto pads = op.getPads();
    ...
    int64_t kVal = getI64(kernelShape[0]);
    int64_t strideVal = getI64(strides[0]);
    int64_t padBegin = getI64(pads[0]);
    int64_t padEnd = getI64(pads[1]);
    if (padBegin != padEnd)
      return op.emitError("hip.conv1d requires symmetric padding (pads[0] == "
                          "pads[1]); got [")
             << padBegin << ", " << padEnd << "]";

    Value K = createI64Const(kVal);
    Value stride = createI64Const(strideVal);
    Value pad = createI64Const(padBegin);

    // element_size_bytes — currently fp16 only at the runtime; pass 2.
    auto elemTy = inputType.getElementType();
    int64_t elemSize = static_cast<int64_t>(elemTy.getIntOrFloatBitWidth()) / 8;
    Value elemSizeVal = createI64Const(elemSize);

    // wrap_conv1d signature:
    //   int wrap_conv1d(RuntimeState*, const void* input, const void* weights,
    //                   const void* bias, void* output,
    //                   int64_t N, int64_t Cin, int64_t Lin,
    ...
    FailureOr<LLVM::LLVMFuncOp> funcOp = LLVM::lookupOrCreateFn(
        rewriter, module, kWrapConv1d, paramTypes, i32Type);
    if (failed(funcOp))
      return failure();

    SmallVector<Value, 13> args = {
        statePtr, inputPtr, weightsPtr, biasPtr, outputPtr, N,          Cin,
        Lin,      Cout,     K,          stride,  pad,       elemSizeVal};
...
void populateConvLoweringPatterns(const LLVMTypeConverter &converter,
  ...
  patterns.add<ConvOpLowering, Conv1dOpLowering>(converter);
}
