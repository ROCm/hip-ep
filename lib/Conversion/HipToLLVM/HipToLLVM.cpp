...
void ConvertHipToLLVMPass::runOnOperation() {
  ...
  // AS 0 so the dominant GPU data path and the flat bare-pointer runtime ABI are
  // byte-unchanged; only host/pinned/managed boundary buffers move off the
  // default space, which keeps D2H/H2D boundaries self-describing and lets alias
  // reasoning treat device vs host pointers as non-aliasing.
  //
  //   memref<8xi64, #hip.mem<host>>   -> !llvm.ptr<1>
  //   memref<8xi64, #hip.mem<device>> -> !llvm.ptr<0>  (same as no space)
  //
  // Per-op lowerings cast the runtime's generic AS-0 pointer to/from these
  // spaces at the call boundary (see extractMemRefDataPtr), so runtime decls
  // stay AS-0; the host target flattens the spaces for codegen, so CPU
  // loads/stores stay valid. Without this hook the type converter rejects any
  // non-integer memory space and conversion fails.
