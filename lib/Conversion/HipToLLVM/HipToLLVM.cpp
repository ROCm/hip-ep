...
struct MulOpLowering : public ConvertOpToLLVMPattern<MulOp> {
  ...
  matchAndRewrite(MulOp op, OpAdaptor adaptor,
    ...
    static constexpr const char* kWrapElementwiseMul = "wrap_elementwise_mul";
    FailureOr<LLVM::LLVMFuncOp> funcOp = LLVM::lookupOrCreateFn(
        rewriter, module, kWrapElementwiseMul, paramTypes, i32Type);
    if (failed(funcOp))
      return failure();
...
struct SigmoidOpLowering : public ConvertOpToLLVMPattern<SigmoidOp> {
  ...
  matchAndRewrite(SigmoidOp op, OpAdaptor adaptor,
    ...
    static constexpr const char* kWrapSigmoid = "wrap_sigmoid";
    FailureOr<LLVM::LLVMFuncOp> funcOp = LLVM::lookupOrCreateFn(
        rewriter, module, kWrapSigmoid, paramTypes, i32Type);
    if (failed(funcOp))
      return failure();

    SmallVector<Value, 5> args = {statePtr, inputPtr, outputPtr,
                                  numElementsVal, elemSizeVal};
...
struct SimplifiedLayerNormOpLowering
  ...
  matchAndRewrite(SimplifiedLayerNormOp op, OpAdaptor adaptor,
    ...
    static constexpr const char* kWrapSimplifiedLayerNorm =
        "wrap_simplified_layer_norm";
    FailureOr<LLVM::LLVMFuncOp> funcOp = LLVM::lookupOrCreateFn(
        rewriter, module, kWrapSimplifiedLayerNorm, paramTypes, i32Type);
    if (failed(funcOp))
      return failure();
...
struct MatMulOpLowering : public ConvertOpToLLVMPattern<MatMulOp> {
  ...
  matchAndRewrite(MatMulOp op, OpAdaptor adaptor,
    ...
    static constexpr const char* kWrapMatMul = "wrap_matmul";
    FailureOr<LLVM::LLVMFuncOp> funcOp = LLVM::lookupOrCreateFn(
        rewriter, module, kWrapMatMul, paramTypes, i32Type);
    if (failed(funcOp))
      return failure();
