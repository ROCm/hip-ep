...
//
// `onnx.Attention` carries no `local_window_size` attribute. A model whose
// attention is windowed therefore has nowhere to say so, and bakes the window
// into the additive float mask instead: the mask is 0 for the key positions a
// query may attend to and a large negative value everywhere else. The op
...
//
// This pre-lowering rewrite recovers the window from the mask's producing
// subgraph and stamps it as `hipdnn.local_window_size`, which
// OnnxAttentionConversion reads onto `hip.gqa`. That turns a property the model
// only expressed in data into one the runtime can act on.
//
// What is matched
// ---------------
