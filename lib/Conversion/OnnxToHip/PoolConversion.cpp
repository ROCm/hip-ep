...
struct PoolToHip : public mlir::RewritePattern {
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    // This helper also backs hip.pool reification and static verification.
    mlir::FailureOr<llvm::SmallVector<mlir::OpFoldResult>> resultShape =
        mlir::hip::reifyPoolResultShape(rewriter, loc, input, kernelShape,
                                        strides, pads, dilations, ceilMode,
                                        [&]() { return op->emitError(); });
