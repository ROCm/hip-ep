...
struct AveragePoolToReshapeMean : public mlir::RewritePattern {
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    // returns the same {B, C, outH, kH, outW, kW} tensor. Falls back
    // to all-dynamic of the requested rank if the resolver chokes; the
    // next `InferOnnxShapes` round would then refine it.
    auto reshapedType = inferReshapeResultType(inputType, reshapeShape,
                                               /*outputRank=*/6,
                                               /*allowzero=*/0);
    if (!reshapedType || reshapedType.getElementType() != elemType) {
