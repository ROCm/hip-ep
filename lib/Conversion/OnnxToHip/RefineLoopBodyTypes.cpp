...
static Type reinferOpResultType(Operation *op) {
  ...
  if (llvm::is_contained(kBinaryBroadcast, name) && op->getNumOperands() == 2) {
...
static SmallVector<Type> refineBodySignature(func::FuncOp bodyFn) {
  ...
  SmallVector<Type> newResults = llvm::to_vector(funcType.getResults());
  ...
  if (changed)
    bodyFn.setType(FunctionType::get(bodyFn.getContext(), funcType.getInputs(),
                                     newResults));
  return newResults;
}
...
static void propagateToCallers(ModuleOp module, func::FuncOp bodyFn,
  ...
  for (LoopOp oldLoop : callers) {
    ...
    for (auto [src, target] : llvm::zip(oldLoop.getVInit(), newResultTypes)) {
