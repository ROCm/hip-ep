...
// buffer.  The downstream `RelaxMultiDynExpandShape` workaround does
// not fix the sizes; it only rewrites expand_shape into reinterpret_cast
// to bypass an upstream MLIR assertion.  Net effect: a 1×1 input
  ...
  matchAndRewrite(mlir::Operation *reshapeOp,
    ...
    for (int64_t i = 0; i < rangeLen; ++i) {
      ...
      if (srcType.isDynamicDim(absDim)) {
        ...
        elem =
            mlir::arith::IndexCastOp::create(rewriter, loc, i64Type, dimVal);
      } else {
    ...
    LLVM_DEBUG(llvm::dbgs() << "[" DEBUG_TYPE "] rewrote "
                            << reshapeOp->getName() << " shape operand from "
                            << shapeOp->getName() << " to from_elements over "
                            << rangeLen << " dim(s) of src\n");
