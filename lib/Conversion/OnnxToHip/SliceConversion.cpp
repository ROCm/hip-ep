...
static mlir::Value followCaptureToParent(mlir::Value value) {
  ...
  unsigned numLoopCarried = static_cast<unsigned>(loopOp.getNumLoopCarried());
  ...
  for (int iter = 0; iter < 16; ++iter) {
    ...
    if (auto getGlobal = mlir::dyn_cast<mlir::memref::GetGlobalOp>(defOp)) {
      ...
      if (!module)
        return nullptr;
      auto global =
          module.lookupSymbol<mlir::memref::GlobalOp>(getGlobal.getNameAttr());
      if (!global)
        return nullptr;
...
static bool
resolveSliceExtentsRuntimeIndices(mlir::Operation *op,
                                  mlir::RankedTensorType dataType,
                                  llvm::SmallVectorImpl<SliceAxisInfo> &info) {
  if (op->getNumOperands() < 3)
    return false;
...
struct SliceToHip : public mlir::RewritePattern {
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    if (!haveExtents)
      ...
      haveExtents = resolveSliceExtentsRuntimeIndices(op, dataType, sliceInfo);
    ...
    auto extractI64ToIndex = [&](mlir::Value tensor1D, int64_t k) {
      mlir::Value kIdx = mlir::arith::ConstantIndexOp::create(rewriter, loc, k);
      mlir::Value elem =
    ...
    auto emitRuntimeOperandExtent = [&](mlir::Value dim, int64_t k,
      ...
      mlir::Value sNorm =
          mlir::arith::SelectOp::create(rewriter, loc, sLt0, sPlusDim, start);
      ...
      mlir::Value eNorm =
          mlir::arith::SelectOp::create(rewriter, loc, eLt0, ePlusDim, end);
      // Clamp to [0, dim].
    ...
    for (int64_t i = 0; i < resultType.getRank(); ++i) {
      ...
      if (sliceInfo[i].runtimeFromDim) {
        mlir::Value dim = mlir::tensor::DimOp::create(rewriter, loc, data, i);
        mlir::Value extent =
        ...
        mlir::Value dim =
            dataType.isDynamicDim(i)
                ...
                : mlir::arith::ConstantIndexOp::create(rewriter, loc,
                                                       dataType.getDimSize(i))
                      .getResult();
        mlir::Value extent = emitRuntimeOperandExtent(
            dim, sliceInfo[i].paramIndex, sliceInfo[i].step);
        dynSizes.push_back(guardWithUpper(extent, upper));
        continue;
      }
      dynSizes.push_back(upper);
