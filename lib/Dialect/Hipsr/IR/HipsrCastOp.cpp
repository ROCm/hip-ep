...
PlaceholderType getCastPlaceholderType(CastOp) {
  return PlaceholderType::Normal;
}

void populateCastShapeRegion(OpBuilder &builder, Block &block, CastOp op,
                             PlaceholderType placeholderType) {
  OpBuilder::InsertionGuard guard(builder);
  builder.setInsertionPointToStart(&block);
  CastShapeArgs args(placeholderType, block);
  builder.create<ShapeYieldOp>(op.getLoc(), args.getInput());
}
...
void mlir::hipsr::populateCastShapeRegionPatterns(
    ShapeRegionPopulationPatternSet &patterns) {
  patterns.add<CastOp, getCastPlaceholderType, populateCastShapeRegion>();
}
