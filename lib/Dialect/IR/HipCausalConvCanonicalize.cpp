...
// convolution with `channels_last` set, which reads and writes [B,L,C] directly.
//
...
struct FoldTransposePairIntoChannelsLast
  ...
  LogicalResult matchAndRewrite(CausalConvWithStateOp conv,
    ...
    if (!leading || !isLastTwoAxisSwap(leading))
      return rewriter.notifyMatchFailure(conv, "input is not a [0,2,1] transpose");
    ...
    if (!leading->getResult(0).hasOneUse())
      return rewriter.notifyMatchFailure(leading, "transpose result has other readers");
    ...
    if (!convOutput.hasOneUse())
      return rewriter.notifyMatchFailure(conv, "output has more than one reader");
    auto trailing = dyn_cast<TransposeOp>(*convOutput.getUsers().begin());
    if (!trailing || !isLastTwoAxisSwap(trailing))
      return rewriter.notifyMatchFailure(conv, "output is not consumed by a [0,2,1] transpose");
    if (trailing.getInput() != convOutput)
      return rewriter.notifyMatchFailure(trailing, "convolution output is the transpose destination");

    Value nlcInput = leading.getInput();
    auto nlcInputType = dyn_cast<RankedTensorType>(nlcInput.getType());
    auto nlcOutputType = dyn_cast<RankedTensorType>(trailing->getResult(0).getType());
    ...
    if (nlcInputType.getShape() != nlcOutputType.getShape() ||
      ...
      return rewriter.notifyMatchFailure(conv, "transposes do not compose to the identity");
