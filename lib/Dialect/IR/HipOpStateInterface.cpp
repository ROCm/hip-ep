...
Value emitOpStateConstruct(OpBuilder &builder, Location loc, Value statePtr,
                           StringRef ctorSymbol, ArrayRef<int64_t> i64Args) {
  MLIRContext *ctx = builder.getContext();
  Type ptrType = LLVM::LLVMPointerType::get(ctx, 0);
  Type i64Type = builder.getI64Type();

  // Construct signature: (RuntimeState*, i64 x N) -> OpState*.
  ...
  FailureOr<LLVM::LLVMFuncOp> ctorFn =
      LLVM::lookupOrCreateFn(builder, module, ctorSymbol, paramTypes, ptrType);
  if (failed(ctorFn))
    return Value();
