...
mlir::hip::reifyMatmulResultShape(OpBuilder &b, Location loc, Value A, Value B,
                                  function_ref<InFlightDiagnostic()> emitError,
                                  int64_t transA, int64_t transB) {
  ...
  if (failed(result))
    return failure();
  result->push_back(aSizes[transA ? aSizes.size() - 1 : aSizes.size() - 2]);
  result->push_back(bSizes[transB ? bSizes.size() - 2 : bSizes.size() - 1]);
  return result;
}
