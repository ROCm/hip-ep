...
// lowered module. inference_compute uses the output-allocator ABI:
// (state, inputs) -> i32, with graph outputs allocated in-graph via
...
private:
  ...
  LogicalResult verifyPrerequisites(ModuleOp module) {
    ...
    // runtime-ABI wrapper: (ctx, inputs) -> i32 (no outputs span -- outputs are
    // allocated in-graph). All params are opaque pointers; the result is the
    // i32 status.
  ...
  ///     // 4. Call @main_graph(%state, %input_memrefs) -- graph outputs are
  ///     //    allocated in-graph via hip.alloc_output (the
  ///     //    hipdnn_ep_alloc_output callback) and written directly
  ...
  void generateInferenceCompute(ModuleOp module, ArrayAttr inputShapes) {
    ...
    if (!mainFunc) {
      ...
      mainSuccessBlock = funcOp.addBlock();
      // @main_graph(state, in_memrefs) -- outputs are allocated in-graph via
      // hip.alloc_output and written directly, so there is no outputs arg.
