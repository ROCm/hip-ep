...
// host-fed scalar staging buffers (small static shape, integer/index element
// type, only memref.store/load/dim/dealloc users — no hip dialect ops as
// users) away from the GPU pool.  Each candidate is replaced by a memref.view
// over a single per-function `hip.get_host_scratch(%ctx, %total)` buffer
// (host-mapped, GPU-readable, runtime-owned, grow-on-demand).
//
// Why: on gfx1151 the GPU pool is real device memory; the bufferized
// `tensor.from_elements` lowering emits `memref.alloc + memref.store` which,
// once absorbed by `hip-pool-allocs`, becomes a host store into device memory
// and SEGVs.  gfx1150 silently worked because hipMalloc returned UMA-mapped
// host memory there.  This pass runs BEFORE `hip-pool-allocs` so candidates
// never enter the GPU pool.
//
//===----------------------------------------------------------------------===//
