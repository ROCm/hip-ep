...
//   3. The per-model JIT'd bitcode (hipdnn_graph_execute, called from
//      generated `inference_compute` lowerings)
//
// All functions use C linkage and opaque pointers for cross-module safety
// (DLL boundaries between the EP / hip-compiler, plus the JIT-link
// boundary between the EP process and per-model bitcode).
//
//===----------------------------------------------------------------------===//
...
/// the JIT'd per-model bitcode reads it during inference via
...
/// Execute a compiled hipDNN graph. Called from generated lowerings in
/// the per-model JIT'd bitcode.
