...
struct ModeChoice {
  ...
  // Which lever decided the mode, for the log. Nothing else needs to know.
...
// Every lever, weighed once. Both are matched case-insensitively and with
// whitespace stripped, and an unrecognised value says so instead of quietly
// leaving the session on the default -- the point of the switch is to compare
// the two paths, so silently running the other one is the failure worth a
// message.
//
