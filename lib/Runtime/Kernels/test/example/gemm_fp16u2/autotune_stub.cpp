 ...
 * bits=4 offline-LUT lookup). The real implementation
 * (hip/autotune/matmul_nbits/matmul_nbits_autotune.cpp) reads a FlatBuffers
 * table whose generated header and flatbuffers runtime are produced by the
 * CMake build. This example builds with only the HIP SDK (no CMake, no
 * flatbuffers), so it links this stub instead.
 *
