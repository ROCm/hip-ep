...
Reads M_array and KN_pairs from the JSON, then calls gen_matmul_nbits_data.py
once per (M, K, N) combination, storing each shape's data in its own
sub-directory under --out-dir.

Usage:
    python gen_model_data.py test_model/gpt_oss_20b.json --group-size 128
    python gen_model_data.py test_model/gpt_oss_20b.json --group-size 128 --no-zeros
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate MatMulNBits test data for all model shapes')
    ...
    gen_script = os.path.join(os.path.dirname(__file__), 'gen_matmul_nbits_data.py')
    idx = 0
    ...
    print(f'\nDone: {total} shapes generated in {args.out_dir}/')
