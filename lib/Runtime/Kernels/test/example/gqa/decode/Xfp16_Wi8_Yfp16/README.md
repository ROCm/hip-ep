...
at **every** `COVERAGE` tier. Crossed with that is a small typical
...
run. At startup the exe prints `coverage=N -> running <n> typical length(s)
x 11 categorical case(s) = <n*11> gqa_decode_i8 cases`. No human edits a
