...
Standalone `hipcc`-only test for `hip_matmul_nbits()` (bits=8, one byte per
weight, no bit-packing). No CMake, no EP build.
...
make test_custom OFFLOAD=... HIP_SDK=... SIZE=128x2880x5120 GS=128 [NO_ZEROS=1] [--fp32]
make clean
```
