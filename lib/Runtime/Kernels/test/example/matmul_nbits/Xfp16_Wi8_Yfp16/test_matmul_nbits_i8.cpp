...
// ============================================================
// --model <json> sweep (mirrors fp16u2/fp16u4's runModelSweep): M_array x
// KN_pairs from the shared model shape JSON (example/models/*.json). bits=8
// is unpacked (one byte/weight) so, unlike u2/u3, there is no K%32 skip.
// ============================================================
                         ...
                         bool use_zeros, const std::string& data_root)
{
    ...
    for(size_t ki = 0; ki < model.K_array.size(); ki++)
    {
        ...
        for(size_t mi = 0; mi < model.M_array.size(); mi++)
        {
            ...
            if(!testShape(M, N, K, group_size, shape_dir, use_zeros))
                failed++;
        }
    }

    std::cout << "\nDone: " << (total_shapes - failed) << "/" << total_shapes
              << " shapes passed" << std::endl;
    std::cout << (failed == 0 ? "ALL PASS" : "SOME FAILED")
              << " (" << failed << " failing case(s))" << std::endl;
    return failed == 0 ? 0 : 1;
}
