 ...
 *
 * custom_kernels MatMulNBits bits=8 (uint8, NOT bit-packed) Verification.
 *
 ...
 *         (optional; default zero point is 128 when absent)
...
// B is one byte per weight (uint8, 0..255, NOT packed); scales are
// uniform(0.01,0.05); zeros (when present) are integers in [120,136] cast to
// fp16; dequant is (B - zp) * scale, default zp = 128 when zeros are absent.
// ============================================================
// bits=8 zero-points are passed as a raw uint8 buffer (zp_elem_size=1, one
// byte per group; NOT fp16-cast like the u4/u2/u3 leaves' zero_points).
struct CaseData {
...
static void genCase(int M, int K, int N, int gs, bool zero, bool fp32,
  ...
  std::uniform_int_distribution<int> zdist(120, 136);

  std::vector<float> A(static_cast<size_t>(M) * K);
  for (auto& v : A) v = adist(rng);

  out.B.resize(static_cast<size_t>(N) * K);
  for (auto& v : out.B) v = static_cast<uint8_t>(bdist(rng));
...
static bool runOne(const Case& c, int idx) {
  ...
  HIP_CHECK(hipMalloc(&dB, d.B.size()));
  ...
  HIP_CHECK(hipMemcpy(dB, d.B.data(), d.B.size(), hipMemcpyHostToDevice));
  ...
  auto launch = [&]() {
    return hip_matmul_nbits(stream, dA, dB, dS, c.zero ? dZ : nullptr,
                            ...
                            /*bits=*/8, c.gs, static_cast<int>(elem),
...
static int run(int coverage_tier) {
  ...
  std::printf("coverage=%d -> running %zu matmul_nbits_i8 cases (3 group_size "
...
int main(int argc, char* argv[]) {
  std::printf("custom_kernels MatMulNBits bits=8 (uint8, not packed) Verification\n");
  std::printf("====================================================================\n");
