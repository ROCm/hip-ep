...
void *hipdnn_ep_get_pool_base(RuntimeState *state, size_t needed_size) {
  ...
  if (g_pool_depth == 0) {
    ...
    if (idx >= state->nested_pool_count) {
      ...
      void **newBases = static_cast<void **>(
          std::realloc(state->nested_pool_bases, newCount * sizeof(void *)));
      size_t *newSizes = static_cast<size_t *>(
          std::realloc(state->nested_pool_sizes, newCount * sizeof(size_t)));
