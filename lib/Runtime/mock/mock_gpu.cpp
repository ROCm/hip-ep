...
int wrap_dequantize_linear(
  ...
  (void)output_shape;
  MOCK_PRINT(
      "[MOCK] wrap_dequantize_linear(x_rank=%lld, scale_rank=%lld, "
      "output_rank=%lld, axis=%lld, block_size=%lld, x_dtype=%s, "
      "scale_dtype=%s, out_dtype=%s, zero_point=%s)\n",
      (long long)x_rank, (long long)scale_rank, (long long)output_rank,
      (long long)axis, (long long)block_size,
      hipdnn_ep_datatype_name(x_dtype), hipdnn_ep_datatype_name(scale_dtype),
      hipdnn_ep_datatype_name(output_dtype),
      x_zero_point ? "yes" : "null");
  return 0;
}
...
int wrap_quantize_linear(
  ...
  (void)saturate_attr;
  MOCK_PRINT(
      "[MOCK] wrap_quantize_linear(x_rank=%lld, scale_rank=%lld, "
      "output_rank=%lld, axis=%lld, block_size=%lld, x_dtype=%s, "
      "scale_dtype=%s, out_dtype=%s, zero_point=%s)\n",
      (long long)x_rank, (long long)scale_rank, (long long)output_rank,
      (long long)axis, (long long)block_size,
