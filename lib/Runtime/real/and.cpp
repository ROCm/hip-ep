...
// And: y = a & b  (bool / 1-byte tensors) with ONNX multidirectional
// broadcast (rank <= 4).
//
// Source:
...
// then the flat hip_elementwise_and runs on same-shape buffers. This
// mirrors wrap_div; without it, a broadcast operand (e.g. an attention
// mask of shape [1,1,S] combined with [1,S,S]) is indexed past its end
// by the flat kernel and produces wrong results.
#include "../debug_log.h"
...
int wrap_and(RuntimeState *state, void *a, void *b, void *output, int64_t a_n,
  ...
  // ONNX `And` is bool-only; the wire format is always 1 byte per element,
  ...
  if (!lhs_eq_out || !rhs_eq_out) {
    ...
    if (hipdnn_ep_state_ensure_workspace(state, needed) != 0) {
      fprintf(stderr, "[REAL] wrap_and: workspace ensure failed (%zu bytes)\n",
              needed);
      return -1;
    }
    uint8_t *ws_byte =
    ...
    if (!lhs_eq_out) {
      ...
      int rc = hip_expand(stream, a, ws_byte, in_a, 4, out_shape, 4,
                          HIP_DTYPE_UINT8);
      if (rc != 0) {
        fprintf(stderr, "[REAL] wrap_and: hip_expand(a) failed (%d)\n", rc);
        return -1;
      }
      a_use = ws_byte;
      ws_byte += per_side;
    ...
    if (!rhs_eq_out) {
      ...
      int rc = hip_expand(stream, b, ws_byte, in_b, 4, out_shape, 4,
                          HIP_DTYPE_UINT8);
      if (rc != 0) {
        fprintf(stderr, "[REAL] wrap_and: hip_expand(b) failed (%d)\n", rc);
        return -1;
      }
      b_use = ws_byte;
    }
  ...
  RUNTIME_DEBUG_LOG("[REAL] wrap_and: out=[%lld,%lld,%lld,%lld] lhs%s rhs%s "
                    "-> hip_elementwise_and\n",
                    (long long)out_n, (long long)out_c, (long long)out_h,
                    (long long)out_w, lhs_eq_out ? "(ok)" : "(expanded)",
                    rhs_eq_out ? "(ok)" : "(expanded)");
  return hip_elementwise_and(stream, a_use, b_use, output, out_vol);
}
