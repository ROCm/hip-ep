...
void load_module_once() {
  std::call_once(g_load_once, []() {
    ...
    if (err != hipSuccess) {
      fprintf(stderr, "ck_dsl_fmha_decode: hipModuleLoadData failed: %s\n",
...
extern "C" int ck_dsl_gqa_fmha_decode(void *stream, const void *Q,
  ...
  hipError_t err =
      hipModuleLaunchKernel(g_function,
                            /*gx*/ 1, /*gy*/ H, /*gz*/ 1,
                            ...
                            /*extra*/ extra);
  if (err != hipSuccess) {
    fprintf(stderr, "ck_dsl_fmha_decode: hipModuleLaunchKernel failed: %s\n",
            hipGetErrorString(err));
