...
static int gqa_forward_hipblaslt(
  ...
  //
  //    The pairing is reachable: bidirectional_no_past keys off the absence of
  //    a past KV operand, so a windowed no_causal layer that reads another
  //    layer's present cache lands here with both set. Such a layer simply
  //    keeps scoring the full range and relies on the mask, as it did before.
...

  {
    ...
    if (present_key && present_value) {
      ...
      HIP_CHECK(update_kv_cache(
          ...
          /*k_scale=*/nullptr, /*v_scale=*/nullptr, kv_bnsh));
    }
