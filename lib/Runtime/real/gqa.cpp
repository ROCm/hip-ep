...
static int gqa_forward_hipblaslt(
  ...
  size_t S_bytes = static_cast<size_t>(B) * H * sq * total_seq * elem_sz;
...

  {
    ...
    int scoreBatchStride = static_cast<int>(sq * total_seq);
    if (sq > 1) {
      HIP_CHECK(hip_gqa_causal_mask(
          stream, d_S, static_cast<int>(B * H), static_cast<int>(total_seq),
    ...
    HIP_CHECK(hip_gqa_softmax_inplace(
        stream, d_S, static_cast<int>(B * H * sq), static_cast<int>(total_seq),
