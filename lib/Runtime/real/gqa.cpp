...
static int gqa_forward_fused(
  ...
  if (sq == 1) {
...

    {
      ...
      if (hipdnn_ep::gqa_autotune_mode(state->gqa_autotune_policy) ==
        ...
        // B==1 already paid (and caches) the seqlens_k read above, so use the
        // actual decode length for exact-LUT matching without adding a sync.
        // For B>1, retain the host-known shape length and naturally use bucket
  ...
  if (hipdnn_ep::gqa_autotune_mode(state->gqa_autotune_policy) ==
      hipdnn_ep::GqaAutotuneMode::Online) {
    fp_rc = hip_gqa_flash_prefill_v3(
