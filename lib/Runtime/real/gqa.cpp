...
// Opt-in: route the *non-flash* fused decode branch through a single-pass
// WMMA kernel generated by ck_dsl (see docs/design/ck-dsl-fmha-decode.md and
// real/ck_dsl_fmha_decode.cpp). At Mistral 7B GQA shape on gfx1151 this beats
// hip_gqa_fused_decode by 2.3-2.8x for skv in {64, 128, 192} -- exactly the
// regime fused_decode is selected for.
//
// TEMPORARY (revert before merge): default-on so this PR's CI gpu-test runs
// the ck_dsl path and we get a CI-validated perf delta vs the main baseline.
// Set HIPDNN_EP_CK_DSL_FMHA_DECODE=0 to disable for an A/B at runtime.
//
// The shim auto-rejects (returns -2 for fallback) when the runtime spec
// doesn't match the embedded HSACO (currently gfx1151, B=1, H=32, G=8, d=128
// -- the Mistral 7B / Llama-3.x AWQ shape). Other shapes fall through to
// hip_gqa_fused_decode unchanged.
static bool gqa_ck_dsl_fmha_decode_enabled() {
  static const bool enabled = [] {
    const char *v = std::getenv("HIPDNN_EP_CK_DSL_FMHA_DECODE");
    return !v || std::strcmp(v, "0") != 0;
  }();
  return enabled;
}
...
static int gqa_forward_hipblaslt(
  ...
  if (fused_predicate) {
    ...
    if (use_flash_decode) {
      ...
      // ck_dsl single-pass WMMA decode shim (opt-in). On gfx1151 Mistral 7B
      // shape this beats hip_gqa_fused_decode 2.3-2.8x at skv in {64,128,192}.
      // The shim returns -2 ("fallback") when the runtime shape doesn't match
      // its embedded HSACO; on -2 we drop through to hip_gqa_fused_decode so
      // unsupported shapes / archs keep their current behaviour.
      int rc_decode = 0;
      bool used_ck_dsl = false;
      if (gqa_ck_dsl_fmha_decode_enabled()) {
        rc_decode = ck_dsl_gqa_fmha_decode(
            stream, qSrc, present_key, present_value, output,
            static_cast<int>(B), static_cast<int>(H), static_cast<int>(G),
            static_cast<int>(d), static_cast<int>(skv),
            static_cast<int>(present_seq), scale, seqlens_k_ptr);
        if (rc_decode == 0) {
          used_ck_dsl = true;
        } else if (rc_decode != -2) {
          return -1; // real HIP error, not a fallback signal
        }
      ...
      RUNTIME_DEBUG_LOG(
          ...
          "d=%lld zero_d2h=%d path=%s\n",
          (long long)B, (long long)sq, (long long)skv, (long long)H,
          (long long)G, (long long)d,
          static_cast<int>(seqlens_k_ptr != nullptr && !need_host_past_len),
          used_ck_dsl ? "ck_dsl_wmma_fmha" : "hip_gqa_fused_decode");
    }
    return 0;
  }
