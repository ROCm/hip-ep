...
// running against an older JIT'd bitcode without the begin_compute
...
// pass to invalidate the cache. JIT'd bitcode that doesn't export that
// symbol (cached EPContext artifact compiled before the hook was added)
// is detected at session creation and produces a LOG(WARNING) directing
// the user to set HIPDNN_EP_GQA_CACHE_SEQLENS=0 (otherwise the cache
// would survive across forward passes and return stale total_seq
// values).
