...
static int gqa_forward_hipblaslt(
  ...
  // produces garbage (observed: O == -4.28e37 sentinel leak ΓåÆ token 0 spam in
  ...
  // (Llama / gpt-oss) is fp16 so `fused_fp16` stays true for them ΓÇö byte
  // identical.
  ...
  if (fused_predicate) {
    ...
    // by downstream callers ΓÇö but here we never overwrite it again because
...
int wrap_group_query_attention(
  ...
  // ORT uses scale == 0.0 as sentinel for "auto-compute 1/ΓêÜhead_size"
