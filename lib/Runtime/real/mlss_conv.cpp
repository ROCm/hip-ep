...
int try_mlss_conv_forward(RuntimeState *, const void *, int64_t, int64_t,
                          int64_t, int64_t, const void *, int64_t,
                          const void *, void *, int64_t, int64_t, int64_t,
...
struct MlssApi {
  void *lib = nullptr;
  MLSSstatus (*createContext)(MLSScontext *, const MLSSstring, const MLSSstring) =
      nullptr;
...
#define LOAD(name, sym)                                                        \
  g_mlss_api.name = reinterpret_cast<decltype(g_mlss_api.name)>(GetProcAddress( \
      static_cast<HMODULE>(g_mlss_api.lib), sym))
#else
  g_mlss_api.lib = tryLoadAmdmlssLibrary();
  if (!g_mlss_api.lib)
    return;
#define LOAD(name, sym)                                                        \
  g_mlss_api.name = reinterpret_cast<decltype(g_mlss_api.name)>(dlsym(           \
      g_mlss_api.lib, sym))
#endif
...
bool mlssApiReady() {
  MlssApi &api = mlssApi();
  return api.lib && api.createContext && api.setParameterByEnum && api.getCaps &&
         api.getBinariesEx && api.vectorRetrieveData;
}
...
bool fitsU32(int64_t v) { return v >= 0 && v <= static_cast<int64_t>(UINT32_MAX); }
...
// Winograd Base non-reloc archives ship as gfx1100 ELFs. Query MLSS with gfx1100
// even on gfx1151 hardware, then patch ELF metadata before hipModuleLoadData.
...
struct ConvConfigKey {
  ...
  static ConvConfigKey make(int64_t n, int64_t c, int64_t h, int64_t w, int64_t k,
                            int64_t kh, int64_t kw, int64_t oh, int64_t ow,
                            int64_t sh, int64_t sw, int64_t pt, int64_t pl,
                            int64_t pb, int64_t pr, int64_t dh, int64_t dw,
                            int64_t group, int64_t has_bias, int64_t dtype,
                            const char *runtime_gfx) {
    ConvConfigKey key{};
    int64_t vals[] = {n,  c,  h,  w,  k,  kh, kw, oh, ow, sh, sw,
                      pt, pl, pb, pr, dh, dw, group, has_bias, dtype};
...
const MLSSbinary *pickNonRelocBinary(const MLSSbinary *bins, MLSSsize count) {
  ...
  for (MLSSsize i = 0; i < count; ++i) {
    ...
    if (b->m_isRelocatable)
      continue;
    if (b->m_pKernelName &&
        std::strcmp(b->m_pKernelName, kMainKernelName) == 0)
      return b;
  }
  return nullptr;
}
...
bool buildOrderedArgs(const MLSSbinary &bin, MlssApi &api, const NamedArg *named,
                      size_t named_count, std::vector<ArgValue> &storage) {
  MLSSvoid *raw = nullptr;
  MLSSsize count = 0;
  MLSSenum type = 0;
  if (api.vectorRetrieveData(bin.m_argList, &raw, &count, &type) != MLSS_SUCCESS ||
      !raw || count == 0)
    return false;
...
bool ensureModule(const MLSSbinary &bin, const char *runtime_gfx,
  ...
  if (entry.image.size() >= 4) {
    RUNTIME_DEBUG_LOG(
        "[REAL] try_mlss_conv_forward: module image size=%zu magic=%02x%02x%02x%02x "
        "eflags@0x30=0x%02x kernel=%s runtime=%s\n",
        entry.image.size(), entry.image[0], entry.image[1], entry.image[2],
        entry.image[3],
        entry.image.size() > kElf64EflagsOffset ? entry.image[kElf64EflagsOffset]
                                                : 0,
        bin.m_pKernelName ? bin.m_pKernelName : "?", runtime_gfx ? runtime_gfx : "?");
  }
  std::strncpy(entry.kernel_name, kMainKernelName, sizeof(entry.kernel_name) - 1);
...
bool configureMlssContext(MlssApi &api, MLSScontext *ctx, uint32_t n, uint32_t c,
                          uint32_t h, uint32_t w, uint32_t k, uint32_t r,
                          uint32_t s, uint32_t out_h, uint32_t out_w,
  ...
  // bins were built with crossCorrelation=false (convolution, not cross-correlation).
...
int try_mlss_conv_forward(
  ...
  if (!bin) {
    for (MLSSsize i = 0; i < bin_count; ++i) {
      RUNTIME_DEBUG_LOG(
          "[REAL] try_mlss_conv_forward: bin[%zu] kernel=%s reloc=%d size=%zu\n",
  ...
  if (hipModuleGetFunction(&func, module_entry.module, module_entry.kernel_name) !=
      hipSuccess) {
  ...
  (void)hipFree(acc_dev);
  RUNTIME_DEBUG_LOG(
      "[REAL] try_mlss_conv_forward: completed via MLSS (query=%s runtime=%s)\n",
