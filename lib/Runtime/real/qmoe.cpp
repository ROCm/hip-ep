...
int wrap_qmoe(RuntimeState *state, const void *input, const void *router_probs,
  ...
  // ONLY when grouped could actually run (the pre-bucket predicate below). Below
  // the token threshold (short prompts, VLM image prefills) and for non-WMMA
  // shapes the grouped-only scratch is skipped entirely, so the pool high-water
  // matches the host-loop/main footprint and -- since the pool never shrinks --
  // is not carried into the decode phase. This is realloc-safe: the predicate is
  // computed before bucket_tokens writes into the pool, so the size is final
  // before the single ensure() below (growing after bucketing would realloc and
  // drop the bucketed data). If the shape/scale test passes but fill_ok later
  // fails, pf_* stay reserved-but-unused (rare: tokens >= threshold yet experts
  // don't fill the WMMA tile). Decode buffers stay gated on is_decode.
