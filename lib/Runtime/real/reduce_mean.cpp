...
int wrap_reduce_mean(RuntimeState *state, void *data, void *axes, void *output,
  ...
  // noop_with_empty_axes=1 with empty axes: ONNX defines this as identity
  // (copy input to output, no reduction / no division).
  if (axes_num_elements == 0 && noop_with_empty_axes == 1) {
    ...
    RUNTIME_DEBUG_LOG(
        "[REAL] wrap_reduce_mean: noop_with_empty_axes=1 with empty axes, "
...

  {
    const int fb_rc = hipdnn_cpu_fb_try_reduce_flat(
        state, stream, "ReduceMean", data, axes, output, data_num_elements,
  ...
  int hip_dtype = hipdnn_to_hip_dtype_mean(data_type);
  if (hip_dtype < 0) {
    fprintf(stderr,
  ...
  RUNTIME_DEBUG_LOG(
      "[REAL] wrap_reduce_mean: data_num=%lld, output_num=%lld, "
      "axes_num=%lld, data_type=%s(%lld), keepdims=%lld, "
      "noop_with_empty_axes=%lld, hip_dtype=%d -> calling hip_reduce_mean\n",
