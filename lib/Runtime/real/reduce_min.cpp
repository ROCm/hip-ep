...
static int reduce_min_hipdnn_to_hip_dtype(int64_t hipdnn_type) {
  switch (hipdnn_type) {
...
int wrap_reduce_min(RuntimeState *state, void *data, void *axes, void *output,
  ...
  if (!state || !data || !output || data_num_elements < 0 ||
    ...
    RUNTIME_DEBUG_LOG("[REAL] wrap_reduce_min: invalid argument\n");
    return -1;
  }
  ...
  if (axes_num_elements == 0 && noop_with_empty_axes == 1) {
    ...
    if (element_size_bytes < 0) {
      fprintf(stderr,
              "[REAL] wrap_reduce_min: unsupported data_type=%lld for noop\n",
              (long long)data_type);
      return -1;
    }
    int64_t total_bytes = data_num_elements * element_size_bytes;
    RUNTIME_DEBUG_LOG(
        "[REAL] wrap_reduce_min: noop_with_empty_axes=1, copying %lld bytes\n",
    ...
    if (err != hipSuccess) {
      fprintf(stderr,
              "[REAL] wrap_reduce_min: noop hipMemcpyAsync failed: %s\n",
  ...
  int hip_dtype = reduce_min_hipdnn_to_hip_dtype(data_type);
  if (hip_dtype < 0) {
    fprintf(stderr,
            "[REAL] wrap_reduce_min: unsupported data_type=%s(%lld) "
  ...
  void *stream = hipdnn_ep_state_get_stream(state);
  RUNTIME_DEBUG_LOG("[REAL] wrap_reduce_min: data_num=%lld, output_num=%lld, "
                    "data_type=%s, hip_dtype=%d -> hip_reduce_min\n",
                    (long long)data_num_elements,
                    (long long)output_num_elements,
                    hipdnn_ep_datatype_name(data_type), hip_dtype);

  return hip_reduce_min(stream, data, output, data_num_elements,
