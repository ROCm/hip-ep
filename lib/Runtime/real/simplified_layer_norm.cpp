...
// (f32, hidden_dim=4096). Returns 0 on launch, -2 on shape mismatch or
// load failure (caller falls back to MIOpen).
extern "C" int ck_dsl_simplified_layer_norm(void *stream, const void *input,
                                            const void *gamma, void *output,
                                            int64_t num_rows,
                                            int64_t hidden_dim, float epsilon,
