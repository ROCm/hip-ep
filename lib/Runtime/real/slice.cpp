...
// this). wrap_slice does not D2H the index tensors: hip_slice applies the
// ONNX rules on device and fills any over-allocated tail with zeros.
// Invalid runtime indices set the session device-error flag rather than
// stalling the inference stream on the host.
//
...
int wrap_slice(RuntimeState *state, void *data, void *starts, void *ends,
  ...
  if (axes && axes_num_elements > 0) {
    if (axes_num_elements != starts_num_elements) {
      fprintf(stderr,
              "[REAL] wrap_slice: axes_num_elements(%lld) != "
              "starts_num_elements(%lld)\n",
              (long long)axes_num_elements, (long long)starts_num_elements);
      return -1;
    }
    axes_dev = static_cast<const int64_t *>(axes);
  }

  const int64_t *steps_dev = nullptr;
  if (steps && steps_num_elements > 0) {
    if (steps_num_elements != starts_num_elements) {
      fprintf(stderr,
              "[REAL] wrap_slice: steps_num_elements(%lld) != "
              "starts_num_elements(%lld)\n",
              (long long)steps_num_elements, (long long)starts_num_elements);
      return -1;
    }
    steps_dev = static_cast<const int64_t *>(steps);
  }

  // Shape-only warning: a zero-capacity output on a non-empty input axis is
  // legal ONNX but, in every graph seen so far, the `Slice(x, k, k, axis)`
  // accumulator seed having been sized at its exact (zero) extent instead
  // of the data dim. Nothing here can fail: the damage lands later, when
  // the loop appends into the empty buffer and the strided copy gets a
  // zero pitch. See lib/Conversion/OnnxToHip/SliceConversion.cpp.
  for (int d = 0; d < data_rank; ++d) {
