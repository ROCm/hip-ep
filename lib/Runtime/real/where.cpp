...
int wrap_where(RuntimeState *state, void *condition, void *x, void *y,
  ...
  if (!state || !condition || !x || !y || !output) {
    hipdnn_ep_log_emit("wrap_where: null tensor argument\n");
    return -1;
  }
  ...
  if ((cond_rank > 0 && !cond_shape) || (x_rank > 0 && !x_shape) ||
    ...
    hipdnn_ep_log_emit("wrap_where: null shape argument with non-zero rank\n");
    return -1;
  }

  int hip_dtype = hipdnn_to_hip_dtype_where(data_type);
  if (hip_dtype < 0) {
    hipdnn_ep_log_emit("[REAL] wrap_where: unsupported data_type %lld (%s)\n",
                       (long long)data_type,
  ...
  if (hipdnn_ep_debug_enabled()) {
    auto dump_shape = [](const char *name, const int64_t *shape, int64_t rank) {
      hipdnn_ep_log_emit("[REAL] wrap_where: %s rank=%lld shape=[", name,
                         (long long)rank);
      for (int64_t i = 0; i < rank; ++i)
        hipdnn_ep_log_emit("%s%lld", i ? "," : "", (long long)shape[i]);
      hipdnn_ep_log_emit("]\n");
    };
    hipdnn_ep_log_emit("[REAL] wrap_where: dtype=%s(%lld)\n",
                       hipdnn_ep_datatype_name(data_type),
                       (long long)data_type);
