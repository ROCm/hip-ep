...
# Fixing morphizen-compile Linker Errors
...
After refactoring Backend → Target/LLVM, morphizen-compile failed to link with 46 unresolved LLVM symbols. Root cause: CMakeLists.txt dropped library dependencies required by lldCOFF.
...
**Before (worked):**
```cmake
target_link_libraries(MorphizenBackend PUBLIC
...
target_link_libraries(MorphizenTargetLLVM PUBLIC
...
**Build verification:**
- morphizen-compile.exe: builds successfully, 45 MB (Debug)
- morphizen-opt.exe: builds successfully, 42 MB (Debug)
