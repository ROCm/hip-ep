...
#include "../Backend/DLLLinker.h"
#include "../Backend/LLVMBackend.h"
#include "../HipDialect/HipDialect.h"
#include "../HipDialect/HipPasses.h"
...
bool CompilerPipeline::compile(
  ...
  // Register HIP passes (idempotent)
  ...
  // nulls)
  std::cerr << "[CompilerPipeline::compile] Input size: " << input_mlir.size()
            << " bytes\n";
  if (input_mlir.size() >= 4) {
    std::cerr << "[CompilerPipeline::compile] First 4 bytes: 0x" << std::hex
  ...
  std::cerr << "[CompilerPipeline::compile] MemBuffer size: "
...
bool CompilerPipeline::compileFromModule(
...
bool CompilerPipeline::validate(llvm::StringRef input_mlir,
                                std::string& error_message) {
  // Initialize MLIR context
  mlir::MLIRContext context;
  context.loadDialect<mlir::BuiltinDialect>();
...
bool CompilerPipeline::compileImpl(
    mlir::ModuleOp module, const std::string& output_path,
    const morphizen::mlir_compiler::CompilationOptions& options,
    std::string& error_message) {
...
bool CompilerPipeline::runMLIRPasses(
...
CompilerPipeline::translateToLLVMIR(mlir::ModuleOp module,
                                    llvm::LLVMContext& llvmContext,
                                    std::string& error_message) {
...
bool CompilerPipeline::linkRuntime(llvm::Module* llvmModule,
                                   std::string& error_message) {
  hipdnn::LLVMBackend backend;
...
void CompilerPipeline::optimizeLLVMIR(llvm::Module* llvmModule, int optLevel) {
  hipdnn::LLVMBackend backend;
  backend.optimizeLLVMIR(llvmModule, optLevel);
}

bool CompilerPipeline::emitLLVMIR(llvm::Module* llvmModule,
...
bool CompilerPipeline::compileToObject(llvm::Module* llvmModule,
                                       const std::string& outputPath,
                                       std::string& error_message) {
  hipdnn::LLVMBackend backend;
...
bool CompilerPipeline::linkToDLL(const std::string& objPath,
...
void CompilerPipeline::cleanupIntermediates(const std::string& basePath) {
