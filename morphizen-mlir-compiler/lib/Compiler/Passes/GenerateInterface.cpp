...
std::string buildMetadataJson(ModuleOp module) {
  ...
  if (!inputRanks || !outputRanks) {
    return "{}"; // Return empty JSON if attributes missing
  }
  ...
  for (size_t i = 0; i < inputRanksArray.size(); i++) {
    Object input;
    Array shape;

    // Read actual shape values if available (structured array, not flattened)
    if (inputShapes && i < inputShapes.size()) {
      if (auto shapeAttr =
              dyn_cast<DenseI64ArrayAttr>(inputShapes.getValue()[i])) {
      ...
      for (int64_t j = 0; j < inputRanksArray[i]; j++) {
        shape.push_back(-1); // -1 indicates dynamic dimension
      }
    }
  ...
  for (size_t i = 0; i < outputRanksArray.size(); i++) {
    Object output;
    Array shape;

    // Read actual shape values if available (structured array, not flattened)
    if (outputShapes && i < outputShapes.size()) {
      if (auto shapeAttr =
              dyn_cast<DenseI64ArrayAttr>(outputShapes.getValue()[i])) {
        for (int64_t dim : shapeAttr.asArrayRef()) {
          shape.push_back(dim);
        }
      }
      ...
      // Fallback: all dynamic dimensions
      for (int64_t j = 0; j < outputRanksArray[i]; j++) {
        shape.push_back(-1); // -1 indicates dynamic dimension
      }
    }
