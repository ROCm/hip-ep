...
# MorphiZen MLIR Compiler Tests

Quick reference for running all test suites in the morphizen-mlir-compiler.
...
```bash
ctest --test-dir ../build/$(basename $PWD)/morphizen-mlir-compiler --verbose
```
...
ctest --test-dir ../build/$(basename $PWD)/morphizen-mlir-compiler -R LitTests --verbose

# E2E tests only (ORT integration)
ctest --test-dir ../build/$(basename $PWD)/morphizen-mlir-compiler -R OrtIntegration --verbose
```
...
llvm-lit -v morphizen-mlir-compiler/test/lit/Conversion/onnx-to-hip/test_gemm_basic.mlir
```
...
- **morphizen-opt**: Built by this project
...
**Key Options**:
- `morphizen_ENABLE_MLIR_COMPILER=ON` - **REQUIRED**: Enable morphizen-mlir-compiler build
...
**Tests fail to find morphizen-opt**:
```bash
# Ensure morphizen-opt is built
cmake --build ../build/$(basename $PWD) --target morphizen-opt --config Debug
```
