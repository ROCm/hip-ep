...
protected:
  void SetUp() override {
    ...
    OrtStatus* status = Ort::GetApi().RegisterExecutionProviderLibrary(
        *env_, "MorphiZenExecutionProvider", lib_path_w.c_str());
#else
    OrtStatus* status = Ort::GetApi().RegisterExecutionProviderLibrary(
        *env_, "MorphiZenExecutionProvider", lib_path_str);
#endif
...
TEST_F(OrtIntegrationTest, CreateSessionWithMorphiZenProvider) {
  ...
  std::cout << "[Test]   Level-1 pass: vaip-pass_level1_mlir (MLIR integration)"
            << std::endl;
  std::cout << "[Test] About to create Ort::Session with model: "
            << CONV_TEST_MODEL_PATH << std::endl;
  std::cout.flush();

  try {
#ifdef _WIN32
    auto model_path_w = ToWideString(CONV_TEST_MODEL_PATH);
    std::cout << "[Test] Calling Ort::Session constructor..." << std::endl;
    std::cout.flush();
    Ort::Session session(*env_, model_path_w.c_str(), session_options);
    std::cout << "[Test] Ort::Session constructor returned" << std::endl;
    std::cout.flush();
#else
