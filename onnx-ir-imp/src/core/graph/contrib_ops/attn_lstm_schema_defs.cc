...
OpSchema& RegisterAttnLSTMContribOpSchema(OpSchema&& op_schema) {
  return op_schema.SetDomain(kMSDomain)
      .Attr("activations",
            ...
            "functions must "
            "be one of the activation functions specified above. Optional: See "
            "the equations "
      ...
      .Attr("activation_alpha",
            ...
            "values are consumed "
            "in the order of activation functions, for example (f, g, h) in "
            "LSTM. Default values "
            "are the same as of corresponding ONNX operators.For example with "
            "LeakyRelu, the "
            "default alpha is 0.01.",
            AttributeProto::FLOATS, OPTIONAL_VALUE)
      .Attr("activation_beta",
            "Optional scaling values used by some activation functions. The "
            "values are consumed in "
            "the order of activation functions, for example (f, g, h) in LSTM. "
            "Default values are "
            "the same as of corresponding ONNX operators.",
            AttributeProto::FLOATS, OPTIONAL_VALUE)
      .Attr("clip",
            "Cell clip threshold. Clipping bounds the elements of a tensor in "
            "the range of "
            "[-threshold, +threshold] and is applied to the input of "
            "activations. No clip if not "
            "specified.",
      ...
      .Input(3, "B",
             ...
             "Rb[iofc]]`, "
             "and `[WBb[iofc], RBb[iofc]]` (if bidirectional) along dimension "
             "0. This "
             "tensor has shape `[num_directions, 8*hidden_size]`. Optional: If "
             "not "
      ...
      .Input(8, "QW",
             ...
             "Should be of "
             "shape `[num_directions, am_query_depth(hidden_size of lstm), "
             "am_attn_size]` ",
             "T", OpSchema::Optional)
      .Input(9, "MW",
             "The weight tensor of the memory layer in the attention "
      ...
      .Input(10, "V",
             ...
             "T", OpSchema::Optional)
      .Input(11, "M",
             "The sequence of the memory (input) for attention mechanism. "
             "Should be of "
             "`[batch_size, max_memory_step, memory_depth]` ",
             "T", OpSchema::Optional)
      .Input(12, "memory_seq_lens",
             "The sequence length of the input memory for the attention "
             "mechanism. Should be of "
             "`[batch_size]` ",
             "T1", OpSchema::Optional)
      .Input(13, "AW",
             "The weights of attention layer in the attention wrapper. If "
             "exists, should be of "
             "shape `[num_directions, memory_depth+hidden_size, aw_attn_size]. "
             "Please note that "
             "attention mechanism context depth is also memory_depth in the "
             "attention mechanism.` ",
             "T", OpSchema::Optional)
      .Output(0, "Y",
