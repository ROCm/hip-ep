...
void MatMulShapeInference(ONNX_NAMESPACE::InferenceContext& ctx, int input1Idx,
                          int input2Idx);
} // namespace defs::math::utils
} // namespace ONNX_NAMESPACE
...
void DecoderAttentionTypeAndShapeInference(
  ...
  if (ctx.getNumOutputs() > 1) {
    if (hasInputShape(ctx, 6) && hasInputShape(ctx, 7)) {
      ...
      if (cache_dims[0].has_dim_value() && cache_dims[1].has_dim_value() &&
          cache_dims[2].has_dim_value() && cache_dims[3].has_dim_value()) {
...
void RestorePaddingTypeAndShapeInference(
    ONNX_NAMESPACE::InferenceContext& ctx) {
...
void MultiHeadAttentionTypeAndShapeInference(
    ONNX_NAMESPACE::InferenceContext& ctx, int past_key_index,
  ...
  //   Input 0 (query) has shape (batch_size, sequence_length, hidden_size)
  //   Input 1 (key) has shape (batch_size, num_head, kv_sequence_length,
  //   head_size) Input 2 (value) has shape (batch_size, num_head,
  ...
  if (ctx.getNumOutputs() > 1) { // has present output
    if (hasInputShape(ctx, past_key_index)) {
      ...
      auto past_present_share_buffer =
          getAttribute(ctx, "past_present_share_buffer", 0);
      bool mha_buffer_sharing =
          hasInputShape(ctx, 6) &&
          hasInputShape(
              ctx,
              8); // equal to MHA op's definition for past_present_share_buffer
      if (past_present_share_buffer || mha_buffer_sharing) {
        ...
        propagateElemTypeFromInputToOutput(
            ctx, static_cast<size_t>(past_key_index) + 1, 2);
      } else {
        if (sequence_length > 0 && past_dims[2].has_dim_value()) {
          int64_t total_sequence_length =
              sequence_length + past_dims[2].dim_value();
...
void BaseGroupQueryAttentionTypeAndShapeInference(
    ONNX_NAMESPACE::InferenceContext& ctx, int past_key_index = -1,
  ...
  if (hasInputShape(ctx, 0)) {
    ...
    if (hasInputShape(ctx, 2)) {
      //   Input 0 (query) has shape (batch_size, sequence_length, num_heads *
      //   head_size) Input 1 (key) has shape (batch_size, kv_sequence_length,
      //   kv_num_heads * head_size) Input 2 (value) has shape (batch_size,
      //   kv_sequence_length, kv_num_heads * head_size) Output 0 has shape
      ...
      // Packed QKV:
      //   Input 0 (query) has shape (batch_size, sequence_length, (num_heads +
      //   2 * kv_num_heads) * head_size) Input 1 (key) is not present Input 2
  ...
  if (ctx.getNumOutputs() > 1) { // has present output
    ...
    if (past_key_index >= 0 && hasInputShape(ctx, past_key_index)) {
      ...
      // past key has shape (batch_size, kv_num_heads,
      ...
      if (use_max_past_present_buffer == 1) {
        ...
        ONNX_NAMESPACE::propagateShapeFromInputToOutput(
            ctx, static_cast<size_t>(past_key_index) + 1, 2);
      } else if (use_max_past_present_buffer == 0) {
        if (kv_sequence_length > 0 && past_dims[2].has_dim_value()) {
          int64_t total_sequence_length =
              kv_sequence_length + past_dims[2].dim_value();
          ...
          // shape of present key/value is (batch_size, kv_num_heads,
        ...
        if (total_sequence_length_data != nullptr &&
          ...
          int64_t present_sequence_length =
              total_sequence_length_value > past_dims[2].dim_value()
          ...
          // shape of present key/value is (batch_size, kv_num_heads,
          // present_sequence_length, head_size)
...
void GroupQueryAttentionTypeAndShapeInference(
    ONNX_NAMESPACE::InferenceContext& ctx, int past_key_index) {
  // TODO(aciddelgado): propagate output shapes depending if kv-share buffer is
  // on or not
  constexpr int use_max_past_present_buffer = -1;
  BaseGroupQueryAttentionTypeAndShapeInference(ctx, past_key_index,
                                               use_max_past_present_buffer);
}

void SparseAttentionTypeAndShapeInference(ONNX_NAMESPACE::InferenceContext& ctx,
                                          int past_key_index) {
  constexpr int use_max_past_present_buffer = 1;
  BaseGroupQueryAttentionTypeAndShapeInference(ctx, past_key_index,
                                               use_max_past_present_buffer);
...
// Currently, the `convert_generation.py` script renames the `Attention` nodes
// to `DecoderMaskedSelfAttention` if the user requests it. Hence, the schemas
// of `DecoderMaskedSelfAttention` and `Attention` schemas are tightly coupled.
// A change in Attention also needs corresponding schema updates in
// `DecoderMaskedSelfAttention` and its kernel.
// TODO(hasesh): Decouple the schema of `DecoderMaskedSelfAttention` from the
// schema of the `Attention` operator by making appropriate tool changes.
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("rotary_embedding_dim",
              ...
              "Default value is head_size",
              AttributeProto::INT, OPTIONAL_VALUE)
        .Attr("mask_filter_value",
              "The value to be filled in the attention mask. Default value is "
              "-10000.0f",
        ...
        .Input(3, "mask_index",
               ...
               "max_sequence_length), "
               "(batch_size, total_sequence_length) or (batch_size, "
               "sequence_length, total_sequence_length), "
               "or index with shape (batch_size) or (2 * batch_size) or (3 * "
               "batch_size + 2)",
               "M", OpSchema::Optional)
        .Input(4, "past",
               ...
               "its shape is (2, batch_size, num_heads, max_sequence_length, "
               "head_size)",
               "T", OpSchema::Optional)
        .Input(5, "attention_bias",
               "additional add to QxK' with shape (batch_size or 1, num_heads "
               "or 1, sequence_length, total_sequence_length)",
               "T", OpSchema::Optional)
        ...
        .Output(1, "present",
                ...
                "its shape is (2, batch_size, num_heads, max_sequence_length, "
                "head_size), "
                "while effective_seq_length = (past_sequence_length + "
                "kv_sequence_length).",
                "T", OpSchema::Optional)
...
// Shape inference for PackedAttention. Here are the shapes of inputs and
// output: Input 'input':                      (token_count, input_hidden_size)
// Input 'weights':                    (input_hidden_size, hidden_size +
// hidden_size + v_hidden_size) Input 'bias':                       (hidden_size
// + hidden_size + v_hidden_size) Input 'token_offset': (batch_size,
// sequence_length) Input 'cumulative_sequence_length': (batch_size + 1) Input
// 'attention_bias':     (batch_size or 1, num_heads or 1, sequence_length,
// sequence_length) Output 'output':                    (token_count,
// v_hidden_size)
void PackedAttentionTypeAndShapeInference(
    ONNX_NAMESPACE::InferenceContext& ctx) {
  // Type inference
  ONNX_NAMESPACE::propagateElemTypeFromInputToOutput(ctx, 0, 0);
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("scale",
              ...
              AttributeProto::FLOAT, OPTIONAL_VALUE)
        .Input(0, "input",
               "Input tensor with shape (token_count, input_hidden_size)", "T")
        .Input(1, "weights",
               "Merged Q/K/V weights with shape (input_hidden_size, "
               "hidden_size + hidden_size + v_hidden_size)",
               "T")
        .Input(2, "bias",
               ...
               "v_hidden_size) for input projection",
               "T")
        .Input(3, "token_offset",
               "In packing mode, it specifies the offset of each "
               "token(batch_size, sequence_length).",
               "M")
        .Input(4, "cumulative_sequence_length",
               "A tensor with shape (batch_size + 1). It specifies the "
               "cumulative sequence length.",
               "M")
        .Input(5, "attention_bias",
               "A tensor with shape (batch_size or 1, num_heads or 1, "
               "sequence_length, sequence_length)."
               "It specifies the additional bias to QxK'",
               "T", OpSchema::Optional)
        .Output(0, "output",
                "2D output tensor with shape (token_count, v_hidden_size)", "T")
        .TypeConstraint("T", {"tensor(float)", "tensor(float16)"},
                        "Constrain input and output types to float tensors.")
        .TypeConstraint("M", {"tensor(int32)"},
                        "Constrain mask index to integer types")
...
// Shape inference for PackedMultiHeadAttention. Here are the shapes of inputs
// and output: When Q, K and V are not packed:
...
//   Input 'key':                        None
//   Input 'value':                      None
// Input 'bias':                         (hidden_size + hidden_size +
// v_hidden_size) Input 'token_offset':                 (batch_size,
// sequence_length) Input 'cumulative_sequence_length':   (batch_size + 1) Input
// 'attention_bias':       (batch_size or 1, num_heads or 1, sequence_length,
// sequence_length) or None Output 'output':                      (token_count,
// v_hidden_size)
void PackedMultiHeadAttentionTypeAndShapeInference(
    ONNX_NAMESPACE::InferenceContext& ctx) {
  // Type inference
  ONNX_NAMESPACE::propagateElemTypeFromInputToOutput(ctx, 0, 0);
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("scale",
              ...
              AttributeProto::FLOAT, OPTIONAL_VALUE)
        .Input(0, "query",
               "Query with shape (token_count, hidden_size) or packed qkv with "
               "shape (token_count, num_heads, 3, head_size)",
               "T")
        .Input(1, "key", "Key with shape (token_count, hidden_size)", "T",
               OpSchema::Optional)
        .Input(2, "value", "Value with shape (token_count, v_hidden_size)", "T",
               OpSchema::Optional)
        .Input(3, "bias",
               "Bias tensor with shape (hidden_size + hidden_size + "
               "v_hidden_size) from input projection",
               "T", OpSchema::Optional)
        .Input(4, "token_offset",
               "Offset of each token before packing, with shape (batch_size, "
               "sequence_length).",
               "M")
        .Input(5, "cumulative_sequence_length",
               "A tensor with shape (batch_size + 1). It specifies the "
               "cumulative sequence length.",
               "M")
        .Input(6, "attention_bias",
               "It specifies the additional bias to QxK'. "
               "The shape is (batch_size or 1, num_heads or 1, "
               "sequence_length, sequence_length)",
               "T", OpSchema::Optional)
        .Output(0, "output",
                "output tensor with shape (token_count, v_hidden_size)", "T")
        .TypeConstraint("T", {"tensor(float)", "tensor(float16)"},
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(2, "bias",
               ...
               "T")
        .Input(3, "mask_index",
               "Mask values of shape (batch_size, total_sequence_length)", "M",
               OpSchema::Optional)
        .Input(4, "past",
               "past state for key and value with shape (2, batch_size, "
               "num_heads, past_sequence_length, head_size)"
               ...
               "its shape is (2, batch_size, num_heads, max_sequence_length, "
               "head_size). "
               "The first `batch_size * num_heads * max_sequence_length * "
               "head_size` elements correspond to keys "
               "and the next `batch_size * num_heads * max_sequence_length * "
               "head_size` elements correspond to values. "
               "The keys buffer is re-ordered in such a way that its virtual "
               "sub-tensor of shape "
               "(batch_size, num_heads, max_sequence_length, head_size) which "
               "may be perceived as being of shape "
               "(batch_size, num_heads, max_sequence_length, head_size / x, x) "
               "is reordered to "
               "become (batch_size, num_heads, head_size / x, "
        ...
        .Input(7, "beam_width",
               ...
               "M", OpSchema::Optional)
        .Input(8, "cache_indirection",
               "A buffer of shape [batch_size, beam_width, max_output_length] "
               "where an `[i, j, k]` entry specifies "
               "which beam the `k`-th token came from for the `j`-th beam for "
               "batch `i` in the current iteration",
               "M", OpSchema::Optional)
        .Output(0, "output",
                "3D output tensor with shape (batch_size, sequence_length, "
                "v_hidden_size)",
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("mask_filter_value",
              ...
              "-10000.0f",
              AttributeProto::FLOAT, OPTIONAL_VALUE)
        .Attr("output_qk", "Need output the cross attention MatMul(Q, K)",
              AttributeProto::INT, OPTIONAL_VALUE)
        .Input(0, "query",
               "Query with shape (batch_size, 1, hidden_size) or packed QKV "
               "with shape "
               "(batch_size, 1, 2 * hidden_size + v_hidden_size)",
               "T")
        .Input(1, "key",
               "Key with shape (batch_size, 1, hidden_size) for self attention "
               "or past_key with shape (batch_size, num_heads, "
               "kv_sequence_length, head_size) for cross attention",
               "T", OpSchema::Optional)
        .Input(2, "value",
               "Value with shape (batch_size, 1, v_hidden_size) for self "
               "attention "
               "or past_value with shape (batch_size, num_heads, "
               "kv_sequence_length, head_size) for cross attention",
               "T", OpSchema::Optional)
        .Input(3, "mask_index",
               "Mask values of shape (batch_size, total_sequence_length) or "
               "(batch_size, kv_sequence_length)",
               "M", OpSchema::Optional)
        .Input(4, "attention_bias",
               "additional add to QxK' with shape (batch_size or 1, num_heads "
               "or 1, sequence_length, total_sequence_length)",
               "T", OpSchema::Optional)
        .Input(5, "past_key",
               "past state for key with shape (batch_size, num_heads, "
               "past_sequence_length, head_size) for self attention"
               "When past_present_share_buffer is set, "
               "its shape is (batch_size, num_heads, max_sequence_length, "
               "head_size). "
               // The re-ordering happens only for CUDA EP at the moment. We
               // probably shall support 4D or 5D shape or attribute to
               // distinguish whether it is re-ordered or not.
               "The keys buffer is re-ordered in such a way that its virtual "
               "sub-tensor of shape "
               "(batch_size, num_heads, max_sequence_length, head_size) which "
               "may be perceived as being of shape "
               ...
               "T", OpSchema::Optional)
        .Input(6, "past_value",
               "past state for value with shape (batch_size, num_heads, "
               "past_sequence_length, head_size) for self attention"
               "When past_present_share_buffer is set, "
               "its shape is (batch_size, num_heads, max_sequence_length, "
               "head_size). ",
               "T", OpSchema::Optional)
        .Input(7, "past_sequence_length",
               "When past_present_share_buffer is used, it is required to "
               "specify past_sequence_length (could be 0)."
        ...
        .Output(1, "present_key",
                "present state for key with shape (batch_size, num_heads, "
                "total_sequence_length, head_size). "
                "If past_present_share_buffer is set, "
                "its shape is (batch_size, num_heads, max_sequence_length, "
                "head_size), "
        ...
        .Output(2, "present_value",
                "present state for value with shape (batch_size, num_heads, "
                "total_sequence_length, head_size). "
                "If past_present_share_buffer is set, "
                "its shape is (batch_size, num_heads, max_sequence_length, "
                "head_size), "
                ...
                "T", OpSchema::Optional)
        .Output(3, "qk",
                "normalized Q * K, of shape (batch_size, num_heads, 1, "
                "total_sequence_length). ",
                "QK", OpSchema::Optional)
        .TypeConstraint("T", {"tensor(float)", "tensor(float16)"},
                        "Constrain input and output types to float tensors.")
        .TypeConstraint("QK", {"tensor(float)", "tensor(float16)"},
                        "Constrain QK output to float32 or float16 tensors, "
                        "independent of input type or output type.")
        .TypeConstraint("M", {"tensor(int32)"},
                        "Constrain mask index to integer types")
        .TypeAndShapeInferenceFunction(
            [](ONNX_NAMESPACE::InferenceContext& ctx) {
              bool is_dmmha_packing =
                  !hasInputShape(ctx, 1) && !hasInputShape(ctx, 2);
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "query",
               ...
               "or packed QKV with shape (batch_size, kv_sequence_length, "
               "num_heads, 3, head_size)",
               "T")
        .Input(1, "key",
               "Key with shape (batch_size, kv_sequence_length, hidden_size), "
               "or packed KV with shape (batch_size, kv_sequence_length, "
               "num_heads, 2, head_size), "
               "or past_key with shape (batch_size, num_heads, "
               "kv_sequence_length, head_size)",
               "T", OpSchema::Optional)
        .Input(2, "value",
               "Value with shape (batch_size, kv_sequence_length, "
               "v_hidden_size), or past_value with shape (batch_size, "
               "num_heads, kv_sequence_length, head_size)",
               "T", OpSchema::Optional)
        .Input(3, "bias",
               "Bias tensor with shape (hidden_size + hidden_size + "
               "v_hidden_size) from input projection",
               "T", OpSchema::Optional)
        .Input(4, "key_padding_mask",
               "Key padding mask with shape (batch_size), (3 * batch_size + "
               "2), (batch_size, kv_sequence_length), (batch_size, "
               "total_sequence_length), "
               "or (batch_size, sequence_length, total_sequence_length)",
               "M", OpSchema::Optional)
        .Input(5, "attention_bias",
               "bias added to QxK' with shape (batch_size or 1, num_heads or "
               "1, sequence_length, total_sequence_length)",
               "T", OpSchema::Optional)
        .Input(6, "past_key",
               "past state for key with shape (batch_size, num_heads, "
               "past_sequence_length, head_size) "
               "or (batch_size, num_heads, max_sequence_length, head_size) "
               "when buffer sharing is used",
               "T", OpSchema::Optional)
        .Input(7, "past_value",
               "past state for value with shape (batch_size, num_heads, "
               "past_sequence_length, head_size) "
               ...
               "when buffer sharing is used",
               "T", OpSchema::Optional)
        .Input(8, "past_sequence_length",
               "The past_sequence_length buffer sharing is used with", "M",
               OpSchema::Optional)
        .Input(9, "cache_indirection",
               "A buffer of shape [batch_size, beam_width, "
               "max_sequence_length] where an [i, j, k] entry specifies"
               "which beam the 'k' th token came from for the 'j' th beam for "
               "batch 'i' in the current iteration",
               "M", OpSchema::Optional)
        .Output(0, "output",
        ...
        .Output(2, "present_value",
                ...
                "T", OpSchema::Optional)
        .Output(3, "qk",
                "normalized Q * K, of shape (batch_size, num_heads, "
                "sequence_length, total_sequence_length). ",
                "QK", OpSchema::Optional)
        .TypeConstraint("T", {"tensor(float)", "tensor(float16)"},
                        "Constrain input and output to float tensors.")
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("num_heads", "Number of attention heads for q",
              AttributeProto::INT)
        .Attr("kv_num_heads", "Number of attention heads for k and v",
              AttributeProto::INT)
        .Attr("scale",
              "Custom scale will be used if specified. Default value is "
              "1/sqrt(head_size)",
        ...
        .Attr("local_window_size",
              "left_window_size for local attention (like Mistral). Default "
              "value is -1 meaning unused.",
        ...
        .Attr("rotary_interleaved",
              ...
              AttributeProto::INT, OPTIONAL_VALUE)
        .Attr("smooth_softmax", "Use a smooth factor in softmax.",
              AttributeProto::INT, static_cast<int64_t>(-1))
        .Input(0, "query",
               "Query with shape (batch_size, sequence_length, hidden_size), "
               "or packed QKV with shape"
               "(batch_size, sequence_length, d) where d is (num_heads * "
               "head_size + 2 * kv_num_heads * head_size).",
               "T")
        .Input(
            1, "key",
        ...
        .Input(
            ...
            "T", OpSchema::Optional)
        .Input(3, "past_key",
               "past state key with support for format BNSH. When past_key "
               "uses same tensor as present_key"
               "(k-v cache), it is of length max_sequence_length... otherwise "
               "of length past_sequence_length.",
               "T", OpSchema::Optional)
        .Input(4, "past_value",
               "past state value with support for format BNSH. When past_value "
               "uses same tensor as present_value"
               "(k-v cache), it is of length max_sequence_length... otherwise "
               "of length past_sequence_length.",
               "T", OpSchema::Optional)
        .Input(5, "seqlens_k",
               "1D Tensor of shape (batch_size). Equivalent to "
               "(total_sequence_lengths - 1).",
               "M")
        .Input(
            6, "total_sequence_length",
            "Scalar tensor equivalent to the maximum total sequence length "
            "(past + new) of the batch. Used for "
        ...
        .Input(9, "position_ids",
               "2D tensor with shape (batch_size, sequence_length). When "
               "processing the first prompt the kernel "
        ...
        .Output(0, "output",
                ...
                "T")
        .Output(1, "present_key",
                "present state key with support for format BNSH. When past_key "
                "uses same tensor as present_key"
                "(k-v buffer), it is of length max_sequence_length... "
                "otherwise of length past_sequence_length +"
                "kv_sequence_length.",
                "T")
        .Output(2, "present_value",
                "present state value with support for format BNSH. When "
                "past_value uses same tensor as present_value"
                "(k-v buffer), it is of length max_sequence_length... "
                "otherwise of length past_sequence_length +"
                "kv_sequence_length.",
                "T")
...
// When Q, K and V are packed:
//   Input 'query':                      (token_count, (num_heads + 2 *
//   kv_num_heads) * head_size) Input 'key':                        None Input
//   'value':                      None
// Input 'key_cache':                    (num_blocks, block_size, kv_num_heads,
// head_size) Input 'value_cache':                  (num_blocks, block_size,
// kv_num_heads, head_size) Input 'cumulative_sequence_length':   (batch_size +
// 1) Input 'seqlens':                      (batch_size) Input 'block_table':
// (batch_size, max_blocks_per_sequence) Input 'cos_cache': (max_seq_len,
// head_size / 2) Input 'sin_cache':                    (max_seq_len, head_size
// / 2) Output 'output':                      (token_count, hidden_size) Output
// 'key_cache_out':               (num_blocks, block_size, kv_num_heads,
// head_size) Output 'value_cache_out':             (num_blocks, block_size,
// kv_num_heads, head_size)
void PagedAttentionTypeAndShapeInference(
    ONNX_NAMESPACE::InferenceContext& ctx) {
  // Type inference
  ONNX_NAMESPACE::propagateElemTypeFromInputToOutput(ctx, 0, 0);
      ...
      *output_shape.add_dim() = query_dims[0];
      ...
      if (hidden_size <= 0 || num_heads <= 0 || kv_num_heads < 0) {
        fail_shape_inference(
            "Invalid hidden size or number of heads. Hidden size, num_heads "
            "and kv_num_heads must be positive integers.");
  ...
  if (ctx.getNumOutputs() > 1) { // has kv cache output
    if (ctx.getNumOutputs() != 3) {
      fail_shape_inference("Key cache and value cache output tensors must be "
    ...
    if (key_cache_dims.size() != 4) {
      fail_shape_inference(
          "The block-based KV cache inputs shall be 4 dimensions");
    }
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("rotary_interleaved",
              ...
              AttributeProto::INT, OPTIONAL_VALUE)
        .Input(0, "query",
               "Query with shape (num_tokens, hidden_size), or packed QKV with "
               "shape (num_tokens, d) "
               "where d is (num_heads * head_size + 2 * kv_num_heads * "
               "head_size).",
               "T")
        .Input(1, "key", "Key with shape (num_tokens, kv_hidden_size) ", "T",
               OpSchema::Optional)
        .Input(2, "value", "Value with shape (num_tokens, kv_hidden_size)", "T",
               OpSchema::Optional)
        .Input(3, "key_cache",
               "Block-based key cache with shape (num_blocks, block_size, "
               "kv_num_heads, head_size). This is updated in "
               "place within the op.",
               "T")
        .Input(4, "value_cache",
               "Block-based value cache with shape (num_blocks, block_size, "
               "kv_num_heads, head_size). This is updated "
               "in place within the op. This should be the same shape as "
               "key_cache.",
               "T")
        .Input(5, "cumulative_sequence_length",
               "A tensor with shape (batch_size + 1). It specifies the "
               "cumulative sequence lengths between the packed "
               "entries in Q/K/V.",
               "S")
        .Input(6, "past_seqlens",
               "A tensor with shape (batch_size). It specifies the past "
               "lengths of cached sequence in the KV cache.",
               "S")
        .Input(7, "block_table",
               "2D tensor with shape (batch_size, max_blocks_per_sequence) "
               "that maps each sequence in the batch to its"
               "corresponding blocks in the KV cache.",
               "S")
        .Input(8, "cos_cache",
               "2D tensor with shape (max total seqlen, head_size / 2).", "T",
               OpSchema::Optional)
        .Input(9, "sin_cache",
               "2D tensor with shape (max total seqlen, head_size / 2).", "T",
               OpSchema::Optional)
        .Output(0, "output",
                "3D output tensor with shape (num_tokens, hidden_size)", "T")
        .Output(1, "key_cache_out",
                "Block-based key cache with shape (num_blocks, block_size, "
                "kv_num_heads, head_size). This is always "
                "the same tensor as key_cache.",
                "T", OpSchema::Optional)
        .Output(2, "value_cache_out",
                "Block-based value cache with shape (num_blocks, block_size, "
                "kv_num_heads, head_size). This is always "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("num_heads", "Number of attention heads for query",
              AttributeProto::INT)
        .Attr("kv_num_heads", "Number of attention heads for key and value",
              AttributeProto::INT)
        .Attr("scale",
              "Scaling factor applied prior to softmax. The default value is "
              "1/sqrt(head_size)",
              AttributeProto::FLOAT, OPTIONAL_VALUE)
        .Attr("sparse_block_size",
        ...
        .Attr("rotary_interleaved",
              ...
              AttributeProto::INT, OPTIONAL_VALUE)
        .Input(0, "query",
               "Query with shape (batch_size, sequence_length, num_heads * "
               "head_size), or packed QKV with shape is"
               "(batch_size, sequence_length, d) where d is (num_heads + 2 * "
               "kv_num_heads) * head_size.",
               "T")
        .Input(1, "key",
               "Key with shape (batch_size, sequence_length, kv_num_heads * "
               "head_size)",
               "T", OpSchema::Optional)
        .Input(2, "value",
               "Value with shape (batch_size, sequence_length, kv_num_heads * "
               "head_size)",
               "T", OpSchema::Optional)
        .Input(3, "past_key",
               "Key cache with shape (batch_size, kv_num_heads, "
               "max_cache_sequence_length, head_size)",
               "T")
        .Input(4, "past_value",
               "Value cache with shape (batch_size, kv_num_heads, "
               "max_cache_sequence_length, head_size)",
               "T")
        .Input(5, "block_row_indices",
               "The row indices of CSR format of block mask with shape "
               "(num_layout, max_blocks + 1)."
               "The num_heads is divisible by num_layout, and max_blocks is "
               "max_sequence_length / sparse_block_size.",
               "M")
        .Input(6, "block_col_indices",
               "The col indices of CSR format of block mask with shape "
               "(num_layout, max_nnz_blocks)."
               "The max_nnz_blocks is the maximum number of non-zeros per "
               "layout in block mask.",
               "M")
        .Input(7, "total_sequence_length",
               "Scalar tensor of maximum total sequence length "
               "(past_sequence_length + sequence_length) among keys.",
               "M")
        .Input(8, "key_total_sequence_lengths",
               "1D tensor with shape (batch_size) where each value is total "
               "sequence length of key excluding paddings.",
               "M")
        .Input(9, "cos_cache",
               "Cos cache of rotary with shape (max_rotary_sequence_length, "
               "head_size / 2).",
               "T", OpSchema::Optional)
        .Input(10, "sin_cache",
               "Sin cache of rotary with shape (max_rotary_sequence_length, "
               "head_size / 2).",
               "T", OpSchema::Optional)
        .Output(0, "output",
                "3D output tensor with shape (batch_size, sequence_length, "
                ...
                "T")
        .Output(1, "present_key",
                "Updated key cache with shape (batch_size, kv_num_heads, "
                "max_cache_sequence_length, head_size).",
                "T")
        .Output(2, "present_value",
                "Updated value cache with shape (batch_size, kv_num_heads, "
                "max_cache_sequence_length, head_size).",
                "T")
        .TypeConstraint("T",
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("num_heads", "Number of attention heads", AttributeProto::INT)
        .Attr("window",
              "One sided attention windows length W, or half of total window "
              "length",
              AttributeProto::INT)
        .Input(0, "input",
               ...
               "T")
        .Input(1, "weight",
               "2D input tensor with shape (hidden_size, 3 * hidden_size)", "T")
        .Input(2, "bias", "1D input tensor with shape (3 * hidden_size)", "T")
        .Input(3, "mask",
               "Attention mask with shape (batch_size, sequence_length)", "T")
        .Input(4, "global_weight",
               "2D input tensor with shape (hidden_size, 3 * hidden_size)", "T")
        .Input(5, "global_bias", "1D input tensor with shape (3 * hidden_size)",
               "T")
        .Input(
            6, "global",
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(1, "key",
               ...
               "T")
        .Input(2, "q_weight",
               "2D input tensor with shape (hidden_size, hidden_size)", "T")
        .Input(3, "kv_weight",
               "2D input tensor with shape (hidden_size, 2 * hidden_size)", "T")
        .Input(4, "bias", "1D input tensor with shape (3 * hidden_size)", "T")
        .Input(5, "key_padding_mask",
               "2D input tensor with shape (batch_size, total_sequence_length)",
               "B", OpSchema::Optional)
        .Input(6, "key_cache",
               "input tensor with shape (batch_size, num_heads, "
               "sequence_length or total_sequence_length, head_size)",
               "T", OpSchema::Optional) // self & cross
        .Input(7, "value_cache",
               "input tensor with shape (batch_size, num_heads, "
               "sequence_length or total_sequence_length, head_size)",
               "T", OpSchema::Optional) // self & cross
        .Input(8, "static_kv",
               "If static_kv = true, cross-attention; else self-attention", "B")
        .Input(9, "use_past", "If use_past = true, use cache; else no cache",
               "B")
        .Input(10, "has_layer_state",
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("scale",
              ...
              AttributeProto::FLOAT, OPTIONAL_VALUE)
        .Attr("interleaved",
              "Indicates whether the input has real and imaginary parts "
              "interleaved. "
              "Default value is 0 (False), meaning the first half of the input "
        ...
        .Attr("rotary_embedding_dim",
              ...
              AttributeProto::INT, OPTIONAL_VALUE)
        .Attr("num_heads",
              "Number of attention heads. Default value is 0. Must use with "
        ...
        .Input(0, "input",
               ...
               "hidden_size) or 4D with shape (batch_size, num_heads, "
               "sequence_length, head_size)",
               "T")
        .Input(1, "position_ids",
               "1D tensor with shape (1) or 2D tensor with shape (batch_size, "
               "sequence_length)",
               "M")
        .Input(2, "cos_cache",
        ...
        .Input(3, "sin_cache",
               ...
               "(max_sequence_length, rotary_embedding_dim / 2)",
               "T")
        .Output(0, "output", "tensor with same shape as input.", "T")
        .TypeConstraint("T",
                        {"tensor(float)", "tensor(float16)",
                         "tensor(bfloat16)"},
                        "Constrain input and output types to float tensors.")
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "emb",
               ...
               "U")
        .Input(1, "q",
               "q state - 4D tensor with shape (batch_size, num_heads, "
               "seq_len, dim)",
               "T")
        .Input(2, "q_rot",
               "half rotated q state - 4D tensor with shape (batch_size, "
               "num_heads, seq_len, dim)",
               "T")
        .Input(3, "k",
               "k state - 4D tensor with shape (batch_size, num_heads, "
               "seq_len, dim)",
               "T")
        .Input(4, "k_rot",
               "k state - 4D tensor with shape (batch_size, num_heads, "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("epsilon", "The epsilon value to use to avoid division by zero.",
              AttributeProto::FLOAT, kDefaultEmbedLayerNormEpsilon)
        .Attr("mask_index_type",
              "The mask index tensor type for shape inference (0: None, 1: 1D "
              "mask_index)",
              AttributeProto::INT, OPTIONAL_VALUE)
        .Input(0, "input_ids",
               "2D words IDs with shape (batch_size, sequence_length)", "T1")
        .Input(1, "segment_ids",
               "2D segment IDs with shape (batch_size, sequence_length)", "T1",
               OpSchema::Optional)
        ...
        .Input(8, "position_ids",
               "2D position ids with shape (batch_size, sequence_length) or "
               "(1, sequence_length)",
               "T1", OpSchema::Optional)
        .Output(0, "output",
                "3D output tensor with shape (batch_size, sequence_length, "
                "hidden_size)",
                "T")
        .Output(1, "mask_index", "1D mask_index tensor with shape (batch_size)",
                "T1", OpSchema::Optional)
        .Output(2, "embedding_sum",
        ...
        .SetContextDependentFunctionBodyBuilder(
            [](const FunctionBodyBuildContext& ctx, const OpSchema& schema,
              ...
              if ((tp == nullptr) || (!tp->has_tensor_type()))
                return false;
              auto elem_type =
                  (TensorProto_DataType)(tp->tensor_type().elem_type());
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .SetDoc("Compute binned relative position bias for T5 model. ref: "
                "https://arxiv.org/abs/1803.02155v2")
        .Attr("max_distance", "Max distance", AttributeProto::INT)
        .Attr("is_bidirectional", "Default value is 0.", AttributeProto::INT,
              static_cast<int64_t>(0))
        .Input(0, "bias_table",
               "2D input tensor with shape (num_buckets, num_heads), "
               "COL-major(See UT for example)",
               "T")
        .Input(1, "query_length",
               "The length of query. Self Attention requires query_length = "
               "key_length",
               "U")
        .Input(2, "key_length", "The length of key.", "U")
        .Output(0, "output",
                "4D output tensor with shape (1, num_heads, sequence_length, "
                "sequence_length)",
                "T")
        .TypeConstraint(
            "T", {"tensor(float)", "tensor(float16)"},
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("epsilon", "The epsilon value to use to avoid division by zero.",
              AttributeProto::FLOAT, kDefaultSkipLayerNormEpsilon)
        .Input(0, "input",
               "3D input tensor with shape (batch_size, sequence_length, "
               "hidden_size)",
               "T")
        .Input(1, "skip",
               "3D skip tensor with shape (batch_size, sequence_length, "
               "hidden_size) or (1, sequence_length, hidden_size) or "
               "(sequence_length, hidden_size)",
               "T")
        .Input(2, "gamma", "1D input tensor with shape (hidden_size)", "T")
        .Input(3, "beta", "1D skip tensor with shape (hidden_size", "T",
               OpSchema::Optional)
        .Input(4, "bias", "1D bias tensor with shape (hidden_size", "T",
               OpSchema::Optional)
        .Output(0, "output",
                "3D output tensor with shape (batch_size, sequence_length, "
                "hidden_size)",
        ...
        .Output(
            ...
            "U", OpSchema::Optional)
        .Output(2, "inv_std_var",
                "Saved inverse standard variance used during training to speed "
                "up gradient computation.",
                "U", OpSchema::Optional)
        .Output(3, "input_skip_bias_sum",
                "Sum of the input and skip inputs (and bias if it exists) with "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(1, "skip",
               ...
               "T")
        .Input(2, "gamma", "1D input tensor with shape (hidden_size)", "T")
        .Input(3, "bias", "1D bias tensor with shape (hidden_size", "T",
               OpSchema::Optional)
        .Output(0, "output",
                "3D output tensor with shape (batch_size, sequence_length, "
                "hidden_size)"
        ...
        .Output(3, "input_skip_bias_sum",
                ...
                "with shape (batch_size, sequence_length, hidden_size) or "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "input_ids",
               ...
               "Tid")
        .Input(1, "scores",
               "2D input tensor with shape (batch_size, vocab_size)", "T")
        .Output(0, "scores_out",
                "2D output tensor with shape (batch_size, vocab_size)", "T")
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("min_ngram_size", "The minimum NGram size for suffix matching.",
              AttributeProto::INT, static_cast<int64_t>(1))
        .Attr("max_ngram_size", "The maximum NGram size for suffix matching.",
              AttributeProto::INT, static_cast<int64_t>(3))
        ...
        .Input(3, "pred_tokens", "Predicted token ids from aggressive decoding",
               "T", OpSchema::Optional)
        .Output(0, "tokens", "Decoder input ids after merging predicted tokens",
                "T")
        ...
        .TypeAndShapeInferenceFunction(
            [](ONNX_NAMESPACE::InferenceContext& ctx) {
              ...
              // output tokens lengths is dynamic as it depends on the
              // bifurcation index of predicted tokens and source tokens, and
              // current tokens length. tokens_length = cur_tokens_length +
              // bifurcation_index + 1.
            }));
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction(
            [](ONNX_NAMESPACE::InferenceContext& ctx) {
              ONNX_NAMESPACE::propagateElemTypeFromInputToOutput(ctx, 0, 0);
              ONNX_NAMESPACE::defs::math::utils::MatMulShapeInference(ctx, 0,
                                                                      1);
            }));
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "input",
               ...
               "hidden_size)",
               "T")
        .Input(1, "sequence_token_count",
               "Number of non-padding tokens in each sequence with shape "
               "(batch_size).",
               "M")
        .Output(0, "output",
                "output tensor with shape (total_tokens, hidden_size)", "T")
        .Output(1, "token_offset",
                "Offset of non-padding tokens, and those of padding tokens. "
                "Its shape is (batch_size, sequence_length)",
                "M")
        .Output(2, "cumulated_seq_len",
                "Cumulated sequence lengths. Its shape is (batch_size + 1)",
                "M")
        .Output(3, "max_seq_len",
                "Max sequence length without padding. Its shape is (1)", "M")
        .TypeConstraint("T", {"tensor(float)", "tensor(float16)"},
                        "Constrain input and output types to float tensors.")
        .TypeConstraint(
            "M", {"tensor(int32)"},
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "input",
               "Input tensor with shape (total_tokens, hidden_size)", "T")
        .Input(1, "token_offset",
               "Offset of non-padding tokens and paddings. Its shape is "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "query_layer",
               ...
               "T")
        .Input(1, "query_bias", "1-d tensor with shape (num_heads x head_size)",
               "T")
        .Input(2, "rel_pos",
               "tensor with shape (1, num_head, seq_len, seq_len)", "T")
        .Input(3, "weight",
               "gemm weight for the gated_ur_linear, shape (head_size, D), D "
               "is divisible by 2",
               "T")
        .Input(4, "bias", "bias for the gated_ur_linear, shape (D)", "T")
        .Input(5, "eco_a", "tensor of shape (1, num_heads, 1, 1)", "T")
        .Input(6, "token_offset",
               "offset of each token with shape (batch_size, seq_len)", "M",
               OpSchema::Optional)
        .Output(0, "output",
                "output tensor with shape (batch_size, num_heads, seq_len, "
                "seq_len)",
                "T")
        .TypeConstraint("T", {"tensor(float)", "tensor(float16)"},
                        "Constrain input and output types to float tensors.")
