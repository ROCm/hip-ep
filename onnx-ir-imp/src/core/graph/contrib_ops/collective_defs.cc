...
void RegisterCollectiveOps() {
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(AllGather)
      .SetDomain(kMSDomain)
      .SinceVersion(1)
      .Attr("axis", "the axis to gather on.", AttributeProto::INT,
            static_cast<int64_t>(1))
      .Attr("group_size", "total size in the group that need to be gathered.",
            AttributeProto::INT, static_cast<int64_t>(1))
      .Input(0, "input", "tensors to be sent", "T", OpSchema::Variadic)
      .Output(0, "output", "gathered tensors", "T", OpSchema::Variadic)
      .TypeConstraint("T",
                      {"tensor(float16)", "tensor(float)", "tensor(double)",
                       "tensor(int64)", "tensor(bool)"},
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(AllToAll)
      .SetDomain(kMSDomain)
      .SinceVersion(1)
      .Attr("group_size", "total size in the group that need to participate.",
            AttributeProto::INT, static_cast<int64_t>(1))
      .Input(0, "input", "tensors to be sent", "T", OpSchema::Variadic)
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(ShardedMoE)
      ...
      .Attr("activation_type",
            "Activation function to use. Choose from relu, gelu, silu and "
            "identity. Default is relu",
            AttributeProto::STRING, std::string("relu"))
      .Attr("k", "Number of top experts to select from expert pool",
            AttributeProto::INT, static_cast<int64_t>(1))
      .Attr("normalize_routing_weights", "Whether to normalize routing weights",
            AttributeProto::INT, static_cast<int64_t>(0))
      .Attr("use_sparse_mixer", "Whether to use sparse mixer",
            AttributeProto::INT, static_cast<int64_t>(0))
      .Attr("local_experts_start_index", "The start index of local experts",
            AttributeProto::INT, static_cast<int64_t>(0))
      .Attr("tensor_shards",
            "Tensor parallelism config. The number of shards for each expert "
            "weight and bias",
            AttributeProto::INT, static_cast<int64_t>(1))
      .Input(0, "input",
      ...
      .Input(1, "router_probs",
             "2D input tensor with shape (num_rows, num_experts)", "T")
      .Input(2, "fc1_experts_weights",
             "3D input tensor with shape (local_num_experts, hidden_size, "
             "local_inter_size)",
             "T")
      .Input(3, "fc1_experts_bias",
             "2D optional input tensor with shape (local_num_experts, "
             "local_inter_size)",
             "T", OpSchema::Optional)
      .Input(4, "fc2_experts_weights",
             "3D input tensor with shape (local_num_experts, local_inter_size, "
             "hidden_size)",
             "T")
      ...
      .Input(6, "fc3_experts_weights",
             "3D optional input tensor with shape (local_num_experts, "
             "hidden_size, local_inter_size)",
             "T", OpSchema::Optional)
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(DistributedMatMul)
      ...
      .Attr("input_device_mesh_elements",
            "device_mesh_elements[i] defines the device mesh's value for the "
            "i-th input. "
            "E.g., device_mesh_elements=[\"[0, 1]\", \"[0, 1]\"] means the 1st "
            "and the 2nd "
            " inputs are stored on the 0-th and the 1st devices, respectively.",
            AttributeProto::STRINGS)
      .Attr("input_device_mesh_shapes",
            "device_mesh_shape[i] defines the device mesh's shape for the i-th "
            "input.",
            AttributeProto::STRINGS)
      .Attr("input_shard_specs",
            "The sharding spec of inputs. "
            "E.g., if input_shard_specs[i] is \"RRR\", the i-th input is a "
            "unsharded 3-D tensor.",
            AttributeProto::STRINGS)
      ...
      .Attr("output_shard_specs",
            ...
            AttributeProto::STRINGS)
      .Input(0, "A", "N-dimensional matrix A", "T", OpSchema::Single, true, 1,
             OpSchema::Differentiable)
      .Input(1, "B", "N-dimensional matrix B", "T", OpSchema::Single, true, 1,
             OpSchema::Differentiable)
      .Output(0, "Y", "Matrix multiply results from A * B", "T",
              OpSchema::Single, true, 1, OpSchema::Differentiable)
      .TypeConstraint("T",
                      {
                          "tensor(float16)",
                          "tensor(float)",
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(DistributedSlice)
      ...
      .Input(1, "starts",
             ...
             "Tind", OpSchema::Single, true, 1, OpSchema::NonDifferentiable)
      .Input(2, "ends",
             "1-D tensor of ending indices (exclusive) of corresponding axis "
             "in `axes`",
             "Tind", OpSchema::Single, true, 1, OpSchema::NonDifferentiable)
      .Input(3, "axes",
             "1-D tensor of axes that `starts` and `ends` apply to. Negative "
             "value means counting dimensions "
             "from the back. Accepted range is [-r, r-1] where r = rank(data). "
             "Behavior is undefined if an "
             "axis is repeated.",
             "Tind", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
      .Input(4, "steps",
             "1-D tensor of slice step of corresponding axis in `axes`. "
             "Negative value means slicing backward. 'steps' cannot be 0. "
             "Defaults to 1s.",
             "Tind", OpSchema::Optional, true, 1, OpSchema::NonDifferentiable)
      .Output(0, "output", "Sliced data tensor.", "T", OpSchema::Single, true,
              1, OpSchema::Differentiable)
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(DistributedReshape)
      ...
      .Attr("output_shard_specs",
            ...
            AttributeProto::STRINGS)
      .Attr("allowzero",
            "(Optional) By default, when any value in the 'shape' input is "
            "equal to zero "
            "the corresponding dimension value is copied from the input tensor "
            "dynamically. "
            "allowzero=1 indicates that if any value in the 'shape' input is "
            "set to zero, "
      ...
      .Input(1, "shape", "Specified shape for output.", "tensor(int64)",
             OpSchema::Single, true, 1, OpSchema::NonDifferentiable)
      .Output(0, "reshaped", "Reshaped data.", "T", OpSchema::Single, true, 1,
              OpSchema::Differentiable)
      .TypeConstraint("T", OpSchema::all_tensor_types_ir4(),
                      "Constrain input and output types to all tensor types.");
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(DistributedExpand)
      ...
      .Attr("output_shard_specs",
            "Similar to input_shard_specs but for outputs.",
            AttributeProto::STRINGS)
      .Input(0, "input", "Input tensor", "T", OpSchema::Single, true, 1,
             OpSchema::Differentiable)
      .Input(1, "shape",
             "A 1-D tensor indicates the shape you want to expand to, "
             "following the broadcast rule",
             "tensor(int64)", OpSchema::Single, true, 1,
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(DistributedUnsqueeze)
      ...
      .Input(0, "input", "Input tensor", "T", OpSchema::Single, true, 1,
             OpSchema::Differentiable)
      .Input(1, "axes", "A 1-D tensor indicates the axes to add.",
             "tensor(int64)", OpSchema::Single, true, 1,
             OpSchema::NonDifferentiable)
      .Output(0, "output", "Output tensor", "T", OpSchema::Single, true, 1,
              OpSchema::Differentiable)
