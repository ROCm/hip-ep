...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("groups",
              "The number of groups of channels. It should be a divisor of the "
        ...
        .Attr("activation",
              ...
              AttributeProto::INT)
        .Attr("channels_last",
              "1 if the input and output are in the NHWC layout, 0 if it is in "
              "the NCHW layout. Defaults to 1.",
              AttributeProto::INT, static_cast<int64_t>(1))
        .Input(0, "X",
               "Input data tensor. Dimensions are (N x H x W x C) when "
               "channels_last is 1 or (N x C x H x W) otherwise, where N is "
               "the batch size, C is the number of channels, and H and W are "
               "the height and width of the data",
               "T")
        .Input(1, "gamma",
               "1D gamma tensor for normalization with shape (C), where C is "
               "number of channels",
               "M")
        .Input(2, "beta",
               "1D beta tensor for normalization  with shape (C), where C is "
               "number of channels",
               "M")
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "X",
               "Input data tensor. Dimensions are (N x H x W x C) when "
               "channels_last is 1 "
               " or (N x C x H x W) otherwise, where N is the batch size, C is "
        ...
        .Input(2, "beta",
               ...
               "M")
        .Input(3, "skip",
               "4D or 2D skip tensor. The shape can be (N x H x W x C) or (N x "
               "1 x 1 x C) or (N x C)",
               "T")
        .Input(
            4, "bias",
        ...
        .Output(0, "Y", "The output tensor of the same shape as X", "T")
        .Output(1, "S",
                "The element-wise sum of input x, skip and bias tensors. It "
                "has the same shape as X",
                "T", OpSchema::Optional)
        .TypeConstraint("T", {"tensor(float16)", "tensor(float)"},
                        "Constrain input X, skip, bias and output Y, S types "
                        "to float tensors.")
        .TypeConstraint("M", {"tensor(float16)", "tensor(float)"},
                        "Constrain gamma and beta to float tensors.")
        .TypeAndShapeInferenceFunction(
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "X",
               "Input tensor. Dimensions are (N, S, D), where N is the batch "
               "size, S are image size, and D is hidden dimension",
               "T")
        .Input(1, "bias",
               "Bias tensor. Dimensions are (D), where D is the same hidden "
               "dimension as input tensor",
               "T")
        .Output(0, "Y", "The output tensor with dimensions (N, S, D/2)", "T")
        .TypeConstraint(
            "T", {"tensor(float16)", "tensor(float)"},
            "Constrain input X and output Y types to float tensors.")
        .TypeAndShapeInferenceFunction(
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "X",
               "Input tensor. Dimensions are (N, S, C), where N is the batch "
               "size, S is image size H*W, and C is number of channels",
               "T")
        .Input(1, "bias", "Bias tensor. Dimensions are (C)", "T")
        .Input(2, "skip", "Residual tensor. Dimensions are (N, S, C)", "T")
        .Output(0, "Y", "The output tensor with dimensions (N, S, C)", "T")
        .TypeConstraint("T", {"tensor(float16)", "tensor(float)"},
