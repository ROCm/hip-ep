...
void RegisterNHWCSchema(const RegistrationFunc& f,
  ...
  f(std::move(
      ::ONNX_NAMESPACE::OpSchema(schema)
          .TypeAndShapeInferenceFunction(
              [onnx_inferencing_func](ONNX_NAMESPACE::InferenceContext& ctx) {
                // use the NHWC inferencing context to convert input 0 and
                // output 0 to NCHW so the ONNX shape inferencing can be used.
                // Once that completes, the call to PropagateOutputShape will
                // convert the inferred shape from NCHW to NHWC
...
// Registration function that uses the default InferenceContext to leverage the
// default ONNX type/shape inferencing with the kMSInternalNHWCDomain domain.
// Used by NHWC Resize operator.
void RegisterNCHWSchemaWithNHWCDomain(const RegistrationFunc& f,
                                      ::ONNX_NAMESPACE::OpSchema&& schema) {
  auto onnx_inferencing_func = schema.GetTypeAndShapeInferenceFunction();
  f(std::move(
      ::ONNX_NAMESPACE::OpSchema(schema)
...
void RegisterNHWCSchemaWithActivation(const RegistrationFunc& f,
                                      ::ONNX_NAMESPACE::OpSchema&& schema) {
  auto onnx_inferencing_func = schema.GetTypeAndShapeInferenceFunction();
  f(std::move(
      ::ONNX_NAMESPACE::OpSchema(schema)
          .Attr("activation", "", ONNX_NAMESPACE::AttributeProto::STRING,
                ONNX_NAMESPACE::OPTIONAL_VALUE)
          .Attr("activation_params", "", ONNX_NAMESPACE::AttributeProto::FLOATS,
                ONNX_NAMESPACE::OPTIONAL_VALUE)
          .TypeAndShapeInferenceFunction(
              [onnx_inferencing_func](ONNX_NAMESPACE::InferenceContext& ctx) {
                NhwcInferenceContext nhwc_ctx(ctx);
...
#  define REGISTER_NHWC_SCHEMA_FROM_MSDOMAIN(RegistrationFn, Op, SinceVersion) \
    RegisterNHWCSchema(                                                        \
        RegistrationFn,                                                        \
        contrib::GetOpSchema<contrib::ONNX_OPERATOR_SET_SCHEMA_CLASS_NAME(     \
            Microsoft, SinceVersion, Op)>())

#  define REGISTER_NHWC_SCHEMA(RegistrationFn, Op, SinceVersion)               \
    RegisterNHWCSchema(                                                        \
        RegistrationFn,                                                        \
        ::ONNX_NAMESPACE::GetOpSchema<                                         \
            ::ONNX_NAMESPACE::ONNX_OPERATOR_SET_SCHEMA_CLASS_NAME(             \
                Onnx, SinceVersion, Op)>())

#  define REGISTER_NHWC_SCHEMA_WITH_ACTIVATION(RegistrationFn, Op,             \
                                               SinceVersion)                   \
...
#  define REGISTER_NCHW_SCHEMA_WITH_NHWC_DOMAIN(RegistrationFn, Op,            \
                                                SinceVersion)                  \
...
void OpSet_Internal_NHWC_ONNX::ForEachSchema(
    const std::function<void(ONNX_NAMESPACE::OpSchema&&)>& fn) {
  // if the operator may be fused with an activation, use the WITH_ACTIVATION
  // variant to add optional attributes for the activation parameters. We mainly
  // register operators from opset 11 on . Models can easily have their opset
  // updated using ONNX tools so supporting older opsets is unnecessary. Older
  // opsets are included on a per-operator basis as needed.
  ...
  // The REGISTER_NCHW_SCHEMA_WITH_NHWC_DOMAIN macro uses the default ONNX
  // type/shape inferencing, which assumes data layouts in NCHW. We use the
  // default ONNX type/shape inferencing for NHWC Resize to avoid having to
  // modify the 'scales' or 'sizes' inputs within a custom InferenceContext
  // class.
  //
  // An alternative could have been to leave NHWC Resize in the ONNX domain
  // (instead of the internal NHWC domain). However, the internal NHWC domain is
  // necessary to allow EPs to detect and reject Resize ops with unsupported
