...
void NchwcPoolOpSchemaGenerator(OpSchema& schema) {
  ...
  schema.Input(0, "X", "", "T");
  schema.Output(0, "Y", "", "T");
  schema.TypeConstraint("T", {"tensor(float)"},
...
void RegisterNchwcSchemas() {
  ONNX_CONTRIB_OPERATOR_SCHEMA(ReorderInput)
      ...
      .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
        ...
        *output_shape->add_dim() = input_shape.dim(0);
        ...
        const auto& input_channel_dim =
            input_shape.dim((channels_last == 0) ? 1 : input_rank - 1);
        ...
        if (input_channel_dim.has_dim_value()) {
          ...
          const int64_t nchwc_block_size =
              static_cast<int64_t>(MlasNchwcGetBlockSize());
          int64_t nchwc_channels =
              (channels + nchwc_block_size - 1) & ~(nchwc_block_size - 1);
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(AveragePool)
      .FillUsing(NchwcPoolOpSchemaGenerator)
      .Attr("count_include_pad", "", AttributeProto::INT,
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(Upsample)
      ...
      .Attr("coordinate_transformation_mode", "", AttributeProto::STRING,
