...
/** Adapter class to enable ONNX shape inferencing to be used with the
 * equivalent NHWC operators.
 *
 * Can be used with operators that match the ONNX spec but have NHWC layout for
 * input 0 and output 0. This adapter context will handle conversion between
 * NHWC and NCHW for the relevant shapes. Once ONNX shape inferencing has
 * completed, call PropagateOutputShape to convert the inferred NCHW shape to
 * NHWC, and propagate it to output 0.
 */
...
public:
  NhwcInferenceContext(ONNX_NAMESPACE::InferenceContext& ctx) : ctx_(ctx) {
    // copy any existing type and shape info, and convert to NCHW for usage with
    // the ONNX inferencing
  ...
  size_t getNumInputs() const noexcept override { return ctx_.getNumInputs(); }
  ...
  const ONNX_NAMESPACE::TensorProto* getInputData(size_t index) const override {
    // we can't return the NHWC input data without transposing it, but wouldn't
    // expect to be asked for it during shape inferencing as getInputData is
    // only used to retrieve things that may have small constant initializers
    // (e.g. something like the min and max values of a Clip operator).
  ...
  ONNX_NAMESPACE::GraphInferencer*
  ...
  // Propagate the inferred type/shape info to output 0, converting any inferred
  // shape from NCHW to NHWC.
  void PropagateOutputShape() {
    ...
    if (output_type_.tensor_type().has_shape()) {
      ...
      // N and C dims are required. Some operators like AveragePool allow 1D
      // input
      if (rank < 3) {
...
private:
  void TransposeToNchw(const ONNX_NAMESPACE::TypeProto& nhwc_tp,
                       ONNX_NAMESPACE::TypeProto& nchw_tp) {
    if (nhwc_tp.tensor_type().has_shape()) {
      ...
      if (rank < 3) {
        fail_shape_inference(
            "Tensor must have at least 3 dimensions to convert between "
