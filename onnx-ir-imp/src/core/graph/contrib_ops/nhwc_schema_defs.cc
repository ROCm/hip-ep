...
void convPoolShapeInference(InferenceContext& ctx, bool use_dilation,
                            bool require_kernel_shape, int input1Idx,
                            int input2Idx);
} // namespace ONNX_NAMESPACE
...
void convPoolShapeInferenceNhwc(InferenceContext& ctx, bool use_dilation,
                                bool require_kernel_shape, int input1Idx,
                                int input2Idx) {
  ...
  convPoolShapeInference(nhwc_ctx, use_dilation, require_kernel_shape,
                         input1Idx, input2Idx);
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
          ...
          ::onnxruntime::contrib::convPoolShapeInferenceNhwc(ctx, true, true, 0,
                                                             1);
        }));
...
equal to the spatial dimension of input tensor. Input is of type uint8_t or int8_t.
        ...
        .Input(
            ...
            "in the form of (N x C x D1 x D2 ... Dn), or (N x D1 X D2 ... Dn x "
            "C) "
        ...
        .Input(2, "x_zero_point",
               "Zero point tensor for input 'X'. It must be a scalar.", "T")
        .Input(3, "y_scale",
               "Scale of quantized output 'Y'. It must be a scalar.",
               "tensor(float)")
        .Input(4, "y_zero_point",
               "Zero point tensor for output 'Y'. It must be a scalar.", "T")
        .Output(0, "Y",
        ...
        .TypeAndShapeInferenceFunction(
            [](ONNX_NAMESPACE::InferenceContext& ctx) {
              ...
              for (auto n_hw_dims = input_shape.dim_size() - 2; n_hw_dims > 0;
                   --n_hw_dims) {
...
constexpr const char* contrib_ops_pads_doc =
    ...
    "any value greater "
    "than or equal to 0. The value represent the number of pixels added to the "
    "beginning "
    "and end part of the corresponding axis. `pads` format should be as follow "
    "[x1_begin, x2_begin...x1_end, x2_end,...], where xi_begin the number of "
    "pixels "
    "added at the beginning of axis `i` and xi_end, the number of pixels added "
    "at "
    "the end of axis `i`. This attribute cannot be used simultaneously with "
    "auto_pad attribute. If not present, the padding defaults to 0 along start "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("count_include_pad",
              ...
              "edges. Default is 0, doesn't count include "
              "pad.",
        ...
        .Attr("auto_pad", contrib_ops_auto_pad_doc, AttributeProto::STRING,
              std::string("NOTSET"))
        .Attr("pads", contrib_ops_pads_doc, AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Attr("ceil_mode",
        ...
        .TypeAndShapeInferenceFunction(
            [](ONNX_NAMESPACE::InferenceContext& ctx) {
              ...
              onnxruntime::contrib::ValidateTypeAndShapeForScaleAndZP(
                  ctx, 1, ONNX_NAMESPACE::TensorProto::FLOAT,
                  QuantParamTensorType::Scalar);
              onnxruntime::contrib::ValidateTypeAndShapeForScaleAndZP(
                  ctx, 2, data_type->tensor_type().elem_type(),
                  QuantParamTensorType::Scalar);
              onnxruntime::contrib::ValidateTypeAndShapeForScaleAndZP(
                  ctx, 3, ONNX_NAMESPACE::TensorProto::FLOAT,
                  QuantParamTensorType::Scalar);
              onnxruntime::contrib::ValidateTypeAndShapeForScaleAndZP(
              ...
              if (getAttribute(ctx, "channels_last", 0) == 0) {
                ...
                onnxruntime::contrib::convPoolShapeInferenceNhwc(ctx, false,
                                                                 true, 0, 5);
              }
            }));
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
          ...
          if (getAttribute(ctx, "channels_last", 0) == 0) {
            ...
            onnxruntime::contrib::convPoolShapeInferenceNhwc(ctx, true, false,
                                                             0, 3);
          }
        }));
std::function<void(OpSchema&)> ConvOpSchemaGenerator() {
  return [=](OpSchema& schema) {
    ...
    schema.Input(
        ...
        OpSchema::Optional, true, 1, OpSchema::Differentiable);
    schema.Output(0, "Y",
    ...
    schema.Attr("kernel_shape",
                "The shape of the convolution kernel. If not present, should "
                "be inferred from input W.",
                AttributeProto::INTS, OPTIONAL_VALUE);
...
ONNX_MS_OPERATOR_SET_SCHEMA(NhwcConv, 1,
...
Only has fp16 implementation as of 2023/04/15.
        ...
        .Input(3, "Z",
               "Tensor to be added to the output, must be the same shape and "
