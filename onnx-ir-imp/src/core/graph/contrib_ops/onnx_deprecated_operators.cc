...
// Experimental operators do not have version history. However, Windows 10
// 1809(RS5) takes bunch of experimental operators as production ops. In order
// to maintain backward compatibility when the experimental ops are removed from
// ONNX they need to be added in onnxruntime as contrib ops. ONNX exp
// ops(Affine, Crop, ParametricSoftplus, ImageScaler, ThresholdedRelu,
// DynamicSlice, ScaledTanh, MVN) old version history maintenance See:
// https://github.com/onnx/onnx/pull/1909
...
ONNX_CONTRIB_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("bias", "Bias applied to each channel, same size as C.",
...
ONNX_CONTRIB_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("border",
              "A 1-D values of (leftBorder, topBorder, rightBorder, "
              "bottomBorder).",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("scale", "A 1-D values of (height, width).", AttributeProto::INTS,
              OPTIONAL_VALUE)
        .Input(0, "input", "Input tensor of shape [N,C,H,W]", "T")
        .Output(
            0, "output",
...
ONNX_CONTRIB_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(2, "ends",
               ...
               "Tind")
        .Input(3, "axes",
               "1-D tensor of axes that `starts` and `ends` apply to.", "Tind",
               OpSchema::Optional)
...
ONNX_CONTRIB_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("across_channels",
              "If 1, mean and variance are computed across channels. Default "
              "is 0.",
              AttributeProto::INT, static_cast<int64_t>(0))
        .Attr("normalize_variance",
        ...
        .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext&
          ...
          if (ONNX_NAMESPACE::hasNInputShapes(ctx, 1)) {
            ...
            if (!border_present || border.size() != 4)
              fail_shape_inference(
                  "'Border' attribute must be present and must contain exactly "
                  "4 values - "
            ...
            *output_shape->mutable_dim(static_cast<int>(0)) =
                input_shape.dim(static_cast<int>(0));
            *output_shape->mutable_dim(static_cast<int>(1)) =
                input_shape.dim(static_cast<int>(1));

            // process 'H' and 'W'
            if (!utils::HasDimValue(input_shape.dim(static_cast<int>(2))) ||
                !utils::HasDimValue(input_shape.dim(static_cast<int>(3)))) {
              // either height and width input has symbolic dims, so can't
              // proceed further add two dims as placeholders for output_H and
              // output_W and return
            ...
            int64_t left_border = border[0], top_border = border[1],
                    right_border = border[2], bottom_border = border[3];
...
// End of ONNX exp ops(Affine, Crop, ParametricSoftplus, ImageScaler,
// ThresholdedRelu, DynamicSlice, ScaledTanh, MVN) old version history
// maintenance
} // namespace contrib
} // namespace onnxruntime
