...
// This file contains deprecated ONNX operators that have been removed from ONNX
// spec, but we still need to keep them to maintain backward compatibility.
// Strictly speaking, this file doesn't define an opset. It only contains a
// group of operators.
...
public:
  ...
  ForEachSchema(std::function<void(ONNX_NAMESPACE::OpSchema&&)> fn) {
    ...
    fn(GetOpSchema<ONNX_OPERATOR_SET_SCHEMA_CLASS_NAME(Onnx, 1,
                                                       ParametricSoftplus)>());
    fn(GetOpSchema<ONNX_OPERATOR_SET_SCHEMA_CLASS_NAME(Onnx, 1,
                                                       ImageScaler)>());
    ...
    fn(GetOpSchema<ONNX_OPERATOR_SET_SCHEMA_CLASS_NAME(Onnx, 10,
                                                       GivenTensorFill)>());
