...
void convPoolShapeInference(ONNX_NAMESPACE::InferenceContext& ctx,
...
void ValidateTypeAndShapeForScaleAndZP(ONNX_NAMESPACE::InferenceContext& ctx,
                                       int index,
                                       ::google::protobuf::int32 expectedType,
                                       QuantParamTensorType expectedScalar,
                                       int expectedTensorSize) {
  if (ctx.getNumInputs() > static_cast<size_t>(index)) {
    ...
    if (data_type->value_case() != ONNX_NAMESPACE::TypeProto::kTensorType ||
      ...
      fail_type_inference("Input data type does not match the expected data "
                          "type. Current data type is ",
  ...
  if (hasInputShape(ctx, index)) {
    ...
    if (expectedScalar == QuantParamTensorType::Scalar) {
      ...
      if (shape.dim((int)0).has_dim_value() &&
          shape.dim((int)0).dim_value() != expectedTensorSize) {
        fail_type_inference(
            "Scale and Zero-point must be of rank 1 and the number of elements "
...
QLinearMathDocGenerator(const char* name, const char* additionalDocumentation) {
...
{additionalDocumentation}
    ...
    ONNX_NAMESPACE::ReplaceAll(doc, "{additionalDocumentation}",
                               additionalDocumentation);
    schema.SetDoc(doc);
    schema.Input(0, "A", "First operand.", "T");
    schema.Input(1, "A_scale",
                 "Input A's scale. It's a scalar, which means a "
                 "per-tensor/layer quantization.",
                 "tensor(float)");
    schema.Input(2, "A_zero_point",
                 "Input A zero point. Default value is 0 if it's not "
                 "specified. It's a scalar, which means a "
                 "per-tensor/layer quantization.",
                 "T", OpSchema::Optional);
    schema.Input(3, "B", "Second operand.", "T");
    schema.Input(4, "B_scale",
                 "Input B's scale. It's a scalar, which means a "
                 "per-tensor/layer quantization.",
                 "tensor(float)");
    schema.Input(5, "B_zero_point",
                 "Input B zero point. Default value is 0 if it's not "
                 "specified. It's a scalar, which means a "
                 "per-tensor/layer quantization.",
                 "T", OpSchema::Optional);
    schema.Input(6, "C_scale",
                 "Output scale. It's a scalar, which means a per-tensor/layer "
                 "quantization.",
                 "tensor(float)");
    schema.Input(7, "C_zero_point",
                 "Output zero point. Default value is 0 if it's not specified. "
                 "It's a scalar, which means a "
                 "per-tensor/layer quantization.",
                 "T", OpSchema::Optional);
    schema.Output(0, "C", "Result, has same element type as two inputs", "T");
    schema.TypeConstraint("T", {"tensor(uint8)", "tensor(int8)"},
                          "Constrain input and output types to 8 bit signed "
                          "and unsigned tensors.");
    schema.TypeAndShapeInferenceFunction(
        [](ONNX_NAMESPACE::InferenceContext& ctx) {
          ...
          ValidateTypeAndShapeForScaleAndZP(ctx, 1,
                                            ONNX_NAMESPACE::TensorProto::FLOAT,
                                            QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(ctx, 2,
                                            a_type->tensor_type().elem_type(),
                                            QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(ctx, 4,
                                            ONNX_NAMESPACE::TensorProto::FLOAT,
                                            QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(ctx, 5,
                                            b_type->tensor_type().elem_type(),
                                            QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(ctx, 6,
                                            ONNX_NAMESPACE::TensorProto::FLOAT,
                                            QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(ctx, 7,
                                            a_type->tensor_type().elem_type(),
                                            QuantParamTensorType::Scalar);
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .Attr("axis",
              "The axis along which same quantization parameters are applied. "
              "It's optional."
              "If it's not specified, it means per-tensor quantization and "
              "input 'x_scale' and 'x_zero_point' must be "
              "scalars."
              "If it's specified, it means per 'axis' quantization and input "
              "'x_scale' and 'x_zero_point' must be 1-D "
              "tensors.",
              AttributeProto::INT, false)
        .Input(0, "x", "N-D full precision Input tensor to be quantized.", "T1")
        .Input(1, "y_scale",
               "Scale for doing quantization to get 'y'. It can be a scalar, "
               "which means per-tensor/layer "
        ...
        .TypeConstraint("T2",
                        {"tensor(int8)", "tensor(uint8)", "tensor(int16)",
                         "tensor(uint16)", "tensor(int4)", "tensor(uint4)"},
                        "Constrain 'y_zero_point' and 'y' to 8-bit and 16-bit "
                        "integer tensors.")
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(1, "x_scale",
               "Scale for input 'x'. It can be a scalar, which means a "
               "per-tensor/layer "
               "dequantization, or a 1-D tensor for per-axis dequantization.",
               "T2")
        .Input(2, "x_zero_point",
               "Zero point for input 'x'. Shape must match x_scale. It's "
               "optional. "
        ...
        .TypeConstraint(
            ...
            {"tensor(int8)", "tensor(uint8)", "tensor(int16)", "tensor(uint16)",
             "tensor(int32)", "tensor(int4)", "tensor(uint4)"},
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("block_dim",
              ...
              "Typically, the block dimension corresponds to the reduction "
              "dimension of the matrix multipication that "
              "consumes the output of this operator."
              "For example, for a 2D matrix multiplication A@W, QuantizeBFP(A) "
              "would use block_dim 1 and "
        ...
        .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext&
                                              ctx) {
          // Shape of raw, quantized tensor is specific to the hardware; for
          // example, different hardware pad the data in different ways. So do
          // not set the shape.
          ONNX_NAMESPACE::setTensorElementType(
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("dtype", "The datatype to dequantize to.", AttributeProto::INT,
              static_cast<int64_t>(ONNX_NAMESPACE::TensorProto_DataType::
                                       TensorProto_DataType_FLOAT)) // default
        .Input(0, "x", "1-D, contiguous, raw, BFP data to be de-quantized.",
               "T1")
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .SetDoc(
            R"DOC(Performs element-wise binary quantized multiplication (with Numpy-style broadcasting support).
...
```
        ...
        .Input(1, "A_zero_point",
               "Input A zero point. Default value is 0 if it's not specified. "
               "It's a scalar, which means a "
               "per-tensor/layer quantization.",
               "T", OpSchema::Optional)
        .Input(2, "B", "Second operand.", "T")
        .Input(3, "B_zero_point",
               "Input B zero point. Default value is 0 if it's not specified. "
               "It's a scalar, which means a "
               "per-tensor/layer quantization.",
               "T", OpSchema::Optional)
        ...
        .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext&
          ...
          ValidateTypeAndShapeForScaleAndZP(ctx, 1,
                                            a_type->tensor_type().elem_type(),
                                            QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(ctx, 3,
                                            b_type->tensor_type().elem_type(),
                                            QuantParamTensorType::Scalar);
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(2, "b_scale",
               "Scale of quantized input 'B'. It could be a scalar or a 1-D "
               "tensor, "
               "which means a per-tensor or per-column quantization. If it's a "
               "1-D tensor, its number "
               "of elements should be equal to the number of columns of input "
               "'B'.",
               "T1")
        .Input(3, "b_zero_point",
               "Zero point tensor for input 'B'. It's optional and default "
               "value is 0.  It could be a scalar or a 1-D "
               "tensor, "
               "which means a per-tensor or per-column quantization. If it's a "
               "1-D tensor, its number "
               "of elements should be equal to the number of columns of input "
        ...
        .TypeConstraint("T1", {"tensor(float)"},
                        "Constrain input A, b_scale and output Y data type as "
                        "float tensor.")
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(1, "B", "N-dimensional matrix B", "T2")
        .Input(2, "a_scale",
               "Scale of quantized input 'A'. It could be a scalar or a 1-D "
               "tensor, "
               "which means a per-tensor or per-column quantization. If it's a "
               "1-D tensor, its number "
               "of elements should be equal to the number of columns of input "
        ...
        .Input(4, "a_zero_point",
               "Zero point tensor for input 'A'. It's optional and default "
               "value is 0.  It could be a scalar or a 1-D "
               "tensor, "
               "which means a per-tensor or per-column quantization. If it's a "
               "1-D tensor, its number "
        ...
        .TypeConstraint("T3", {"tensor(float)", "tensor(float16)"},
                        "Constrain input a_scale, b_scale and output Y data "
                        "type as float tensor.")
        .TypeAndShapeInferenceFunction(
            [](ONNX_NAMESPACE::InferenceContext& ctx) {
              propagateElemTypeFromInputToOutput(ctx, 2, 0);
              ONNX_NAMESPACE::defs::math::utils::MatMulShapeInference(ctx, 0,
...
ONNX_MS_OPERATOR_SET_SCHEMA(QLinearAdd, 1,
                            OpSchema().FillUsing(QLinearMathDocGenerator(
                                "addition",
                                "C = (A_scale * (A - A_zero_point) + B_scale * "
                                "(B - B_zero_point))/C_scale + C_zero_point")));

ONNX_MS_OPERATOR_SET_SCHEMA(
    QLinearMul, 1,
    OpSchema().FillUsing(QLinearMathDocGenerator(
        "multiplication", "C = ((A - A_zero_point) * (B - B_zero_point)) * "
                          "(A_scale * B_scale)/C_scale + C_zero_point")));
...
```
        ...
        .Input(2, "data_zero_point",
               "Input zero point. Default value is 0 if it's not specified. "
               "It's a scalar, which means a "
               "per-tensor/layer quantization.",
               "T", OpSchema::Optional)
        .Input(3, "reduced_scale",
               "Output scale. It's a scalar, which means a per-tensor/layer "
               "quantization.",
               "tensor(float)")
        ...
        .Attr("axes",
              "A list of integers, along which to reduce. The default is to "
              "reduce over all the dimensions of the "
              "input tensor.",
              AttributeProto::INTS)
        .Attr("keepdims",
              "Keep the reduced dimension or not, default 1 mean keep reduced "
        ...
        .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext&
          ...
          if (nullptr == data_type ||
              data_type->value_case() !=
                  ONNX_NAMESPACE::TypeProto::kTensorType) {
            fail_type_inference("inputs are expected to have tensor type.");
          }
          ...
          ValidateTypeAndShapeForScaleAndZP(ctx, 1,
                                            ONNX_NAMESPACE::TensorProto::FLOAT,
                                            QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(
              ctx, 2, data_type->tensor_type().elem_type(),
              QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(ctx, 3,
                                            ONNX_NAMESPACE::TensorProto::FLOAT,
                                            QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(
              ctx, 4, data_type->tensor_type().elem_type(),
              QuantParamTensorType::Scalar);
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(1, "X_scale",
               ...
               "tensor(float)")
        .Input(2, "X_zero_point",
               "Input X's zero point. Default value is 0 if it's not "
               "specified. It's a scalar, which means a "
               "per-tensor/layer quantization.",
               "T", OpSchema::Optional)
        .Input(3, "Y_scale",
               "Output Y's scale. It's a scalar, which means a "
               "per-tensor/layer quantization.",
               "tensor(float)")
        .Input(4, "Y_zero_point",
               "Output Y's zero point. Default value is 0 if it's not "
               "specified. It's a scalar, which means a "
               "per-tensor/layer quantization.",
               "T", OpSchema::Optional)
...
The output tensor has the same shape.
        ...
        .Attr("axis",
              ...
              AttributeProto::INT, static_cast<int64_t>(-1))
        .Attr("opset", "opset version of corresponding SoftMax.",
              AttributeProto::INT)
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("hidden_size", "Number of neurons in the hidden layer",
              AttributeProto::INT, OPTIONAL_VALUE)
        .Attr("activation_alpha",
              "Optional scaling values used by some activation functions. The "
              "values "
              "are consumed in the order of activation functions, for example "
              "(f, g, h) "
        ...
        .Attr("clip",
              ...
              "in the range of [-threshold, +threshold] and is applied to the "
              "input "
        ...
        .Attr("activations",
              ...
              "for input, output, forget, cell, and hidden. The activation "
              "functions must "
              "be one of the activation functions specified above. Optional: "
              "See the equations "
              "for default if not specified.",
              AttributeProto::STRINGS, OPTIONAL_VALUE)
        .Attr("input_forget", "Couple the input and forget gates if 1.",
        ...
        .Input(3, "B",
               ...
               "Rb[iofc]]`, "
               "and `[WBb[iofc], RBb[iofc]]` (if bidirectional) along "
               "dimension 0. This "
               "tensor has shape `[num_directions, 8*hidden_size]`. Optional: "
               "If not "
        ...
        .Input(
            ...
            "to be 0. It has shape `[num_directions, batch_size, "
            "hidden_size]`.",
            "T", OpSchema::Optional)
        .Input(6, "initial_c",
        ...
        .Input(9, "W_zero_point",
               "W's zero point. Its size is [num_directions] for "
               "per-tensor/layer quantization, "
               "or [num_directions, 4*hidden_size] for per-channel "
               "quantization on the axis input_size.",
               "T2")
        ...
        .Input(11, "R_zero_point",
               "R's zero point. Its size is [num_directions] for "
               "per-tensor/layer quantization, "
               "or [num_directions, 4*hidden_size] for per-channel "
               "quantization on the axis input_size.",
               "T2")
        .Output(0, "Y",
                "A tensor that concats all the intermediate output values of "
                "the hidden. "
                "It has shape `[seq_length, num_directions, batch_size, "
                "hidden_size]`. ",
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(2, "inputs",
               "List of tensors/scale/zero_point for concatenation", "TV",
               OpSchema::Variadic, false)
        .Output(0, "Y", "Concatenated tensor", "T8")
        .TypeConstraint("T8", {"tensor(uint8)", "tensor(int8)"},
                        "Constrain input and output types to 8 bit signed and "
                        "unsigned tensors.")
        .TypeConstraint("TF", {"tensor(float)"},
                        "Constrain scale types to any float tensor type.")
        .TypeConstraint("TV",
                        {"tensor(uint8)", "tensor(int8)", "tensor(float)"},
                        "Sequence of (Tensor, Scale, ZeroPoint) tuples. The "
                        "type is sequence of (T8, TF, T8).")
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(0, "condition",
               " When True (nonzero), yield x, otherwise yield y", "B")
        ...
        .Output(0, "Z",
                "Tensor of shape equal to the broadcasted shape of condition, "
                "X, and Y",
                "T")
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(2, "a_zero_point",
               "Zero point tensor for input 'A'. It is a scalar.", "TA")
        .Input(3, "B",
               "Input tensor B. "
        ...
        .Input(7, "y_scale",
               ...
               "quantization. "
               "It is optional. The output is full precision(float32) if it is "
               "not provided. "
               "Or the output is quantized.",
               "T", OpSchema::Optional)
        .Input(8, "y_zero_point",
               "Zero point tensor for output 'Y'. It is a scalar, which means "
        ...
        .Attr("transA", "Whether A should be transposed", AttributeProto::INT,
              static_cast<int64_t>(0))
        .Attr("transB", "Whether B should be transposed", AttributeProto::INT,
              static_cast<int64_t>(0))
        .Attr("alpha",
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(3, "input_scale",
               "scale of quantized input tensor. It's a scalar, which means a "
               "per-tensor/layer quantization.",
               "T3")
        .Input(4, "weight_scale",
               "scale of weight scale. It's a scalar or a 1D tensor, which "
               "means a per-tensor/per-column quantization."
               "Its size should be 3 * hidden_size if it is per-column "
               "quantization",
               "T3")
        .Input(5, "mask_index", "Attention mask index with shape (batch_size)",
               "T4", OpSchema::Optional)
        .Input(6, "input_zero_point",
               "zero point of quantized input tensor.It's a scalar, which "
               "means a per-tensor/layer quantization.",
               "T1", OpSchema::Optional)
        .Input(7, "weight_zero_point",
               "zero point of quantized weight tensor. It's a scalar or a 1D "
               "tensor, which means a "
        ...
        .Output(1, "present",
                ...
                "num_heads, past_sequence_length + "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(3, "position_embedding_quant", "2D with shape (, hidden_size)",
               "T2")
        ...
        .Input(9, "position_embedding_scale", "Scale for position embeddings",
               "T")
        .Input(10, "segment_embedding_scale", "Scale for segment embeddings",
               "T", OpSchema::Optional)
        .Input(11, "gamma_scale", "Scale for 1D gamma tensor", "T")
        .Input(12, "beta_scale", "Scale for 1D beta tensor", "T")
        .Input(13, "word_embedding_zero_point",
               "Zero point for word embeddings", "T2")
        .Input(14, "position_embedding_zero_point",
               "Zero point for position embeddings", "T2")
        .Input(15, "segment_embedding_zero_point",
               "Zero Point for segment embeddings", "T2", OpSchema::Optional)
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .SetDoc(
            R"DOC(Quantize input matrix to specific layout used in cublaslt.)DOC")
        .Attr("order_input",
              "cublasLt order of input matrix. ORDER_COL = 0, ORDER_ROW = 1, "
              "ORDER_COL32 = 2, ORDER_COL4_4R2_8C = 3, "
              "ORDER_COL32_2R_4R4 = 4. "
              "Please refer "
              "https://docs.nvidia.com/cuda/cublas/index.html#cublasLtOrder_t "
              "for their meaning.",
              AttributeProto::INT)
        .Attr("order_output", "cublasLt order of output matrix.",
              AttributeProto::INT)
        .Input(0, "input",
               "TODO: input tensor of (ROWS, COLS). if less than 2d, will "
               "broadcast to (1, X). If 3d, it is treated as "
               "(B, ROWS, COS)",
               "F")
        ...
        .TypeAndShapeInferenceFunction(
            [](ONNX_NAMESPACE::InferenceContext& ctx) {
              propagateElemTypeFromDtypeToOutput(
                  ctx, ONNX_NAMESPACE::TensorProto::INT8, 0);
              if (!hasInputShape(ctx, 0))
                return;
              auto& input_shape = getInputShape(ctx, 0);
              updateOutputShape(ctx, 0, input_shape);
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("order_input",
              "cublasLt order of input matrix. See the schema of "
              "QuantizeWithOrder for order definition.",
              AttributeProto::INT)
        .Attr("order_output", "cublasLt order of output matrix",
              AttributeProto::INT)
        .Attr("to",
              "The output data type, only support TensorProto_DataType_FLOAT "
              "(1) and TensorProto_DataType_FLOAT16 (10)",
              AttributeProto::INT)
        .Input(0, "input",
               "TODO: input tensor of (ROWS, COLS). if less than 2d, will "
               "broadcast to (1, X). If 3d, it is treated as "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("order_A",
              "cublasLt order of matrix A. See the schema of QuantizeWithOrder "
              "for order definition.",
              AttributeProto::INT)
        .Attr("order_B", "cublasLt order of matrix B", AttributeProto::INT)
        .Attr("order_Y", "cublasLt order of matrix Y and optional matrix C",
              AttributeProto::INT)
        ...
        .Input(2, "B",
               ...
               "Q")
        .Input(3, "scale_B", "scale of the input B. Scalar or 1-D float32.",
               "S")
        .Input(4, "scale_Y", "scale of the output Y.", "S")
        .Input(5, "bias", "1d bias, not scaled with scale_Y.", "S",
               OpSchema::Optional)
        .Input(6, "C",
               "3d or 2d matrix C. if 2d expand to 3d first. Shape[0] should "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("order_input",
              ...
              "QuantizeWithOrder for order definition.",
              AttributeProto::INT)
        .Attr("order_weight", "cublasLt order of weight matrix",
              AttributeProto::INT)
        .Attr("order_output", "cublasLt order of global bias",
              AttributeProto::INT)
        .Input(0, "input",
               "3D input tensor with shape (batch_size, sequence_length, "
               "input_hidden_size)",
               "Q")
        .Input(1, "scale_input",
               "scale of the input, scalar value (per tensor) currently.", "S")
        .Input(2, "scale_Q_gemm",
               "scale of the gemm - scalar (per-tensor quantization)", "S")
        .Input(3, "scale_K_gemm",
               "scale of the gemm - scalar (per-tensor quantization)", "S")
        .Input(4, "scale_V_gemm",
               "scale of the gemm - scalar (per-tensor quantization)", "S")
        .Input(5, "Q_weight",
               "2D input tensor with shape (input_hidden_size, hidden_size), "
        ...
        .Input(8, "scale_Q_weight",
               "scale of the weight (scalar for per-tensor quantization or 1-D "
               "of dims [hidden_size] for per-channel "
               "quantization)",
               "S")
        ...
        .Input(14, "scale_QKT_gemm",
               "scale of the gemm - scalar (per-tensor quantization)", "S",
               OpSchema::Optional)
        .Input(15, "scale_QKT_softmax",
               "scale of the softmax result - scalar (per-tensor quantization)",
               "S", OpSchema::Optional)
        .Input(16, "scale_values_gemm",
               "scale of the gemm - scalar (per-tensor quantization). Also "
               "this is the output scale for the operator.",
               "S")
        .Input(17, "mask_index",
               "Attention mask with shape (batch_size, 1, max_sequence_length, "
               "max_sequence_length), (batch_size, "
               "past_sequence_length + sequence_length)"
               "or (batch_size, sequence_length, past_sequence_length + "
               "sequence_length), or index with shape "
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("order_X",
              "cublasLt order of input X. Default is ROW MAJOR. See the schema "
              "of "
              "QuantizeWithOrder for order definition.",
              AttributeProto::INT, static_cast<int64_t>(1))
        .Attr("order_Y",
              "cublasLt order of matrix Y, must be same as order_X. Default is "
              "ROW MAJOR.",
              AttributeProto::INT, static_cast<int64_t>(1))
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("order_X",
              "cublasLt order of input X. Optional. See the schema of "
              "QuantizeWithOrder for order definition.",
              AttributeProto::INT, OPTIONAL_VALUE)
        .Attr("order_Y",
              "cublasLt order of matrix Y, must be same as order_X if "
              "specified together. Optional.",
              AttributeProto::INT, OPTIONAL_VALUE)
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        .SetDoc(
            R"DOC(Quantized version of Longformer Self Attention (using int8 with specific matrix Layout).)DOC")
        .Attr("num_heads", "Number of attention heads", AttributeProto::INT)
        .Attr("window",
        ...
        .Attr("order_weight", "cublasLt order of weight matrix",
              AttributeProto::INT)
        .Attr("order_global_weight", "cublasLt order of weight matrix",
              AttributeProto::INT)
        .Attr("order_output", "cublasLt order of global bias",
              AttributeProto::INT)
        .Input(0, "input",
        ...
        .Input(1, "scale_input", "scale of the input", "S")
        .Input(2, "weight",
               "2D input tensor with shape (hidden_size, 3 * hidden_size)", "Q")
        .Input(3, "scale_weight", "scale of the weight", "S")
        .Input(4, "bias",
               "1D input tensor with shape (3 * hidden_size), fp32 only "
               "currently.",
               "S")
        .Input(5, "scale_bias",
               "reserved. (not used as add bias need float value in cublasLt "
               "for normal order.)",
               "S")
        .Input(6, "scale_qkv_gemm", "scale of the output for fused kqv gemm",
               "S")
        .Input(7, "mask",
               "Attention mask with shape (batch_size, sequence_length)", "F")
        .Input(8, "global_weight",
               "2D input tensor with shape (hidden_size, 3 * hidden_size)", "Q")
        .Input(9, "scale_global_weight", "scale of the global_weight", "S")
        .Input(10, "global_bias",
               "1D input tensor with shape (3 * hidden_size)", "S")
...
static void convTransposeShapeInference(InferenceContext& ctx,
  ...
  class InputIdxReMappingInferContext : public InferenceContext {
   ...
   public:
    InputIdxReMappingInferContext(InferenceContext& ctx, std::array<int, 3>&
input_map) : ctx_pa_(ctx), input_map_(input_map) {} const onnx::TypeProto*
getInputType(size_t index) const override { return
ctx_pa_.getInputType(input_map_[index]);
    }
    ...
    onnx::GraphInferencer* getGraphAttributeInferencer(const std::string&
attribute_name) override { return
ctx_pa_.getGraphAttributeInferencer(attribute_name);
    }
    const onnx::SparseTensorProto* getInputSparseData(size_t index) const
override { return ctx_pa_.getInputSparseData(input_map_[index]);
    }
...
output_shape can also be explicitly specified in which case pads values are auto generated using these equations:
  ...
  Else: pads[start_i] = total_padding[i] - (total_padding[i]/2); pads[end_i] = (total_padding[i]/2).
        ...
        .Input(0, "x",
               ...
               " H and W are the height and width. Note that this is for the "
               "2D image. "
        ...
        .Input(3, "w",
               ...
               "T2")
        .Input(4, "w_scale",
               "Scale tensor for input 'w'. It could be a scalar or a 1-D "
               "tensor, which means a per-tensor/layer or per output channel "
               "quantization. If it's a 1-D tensor, its number of elements "
               "should be equal to the number of output channels (M).",
               "tensor(float)")
        .Input(5, "w_zero_point",
        ...
        .Input(
            ...
            "Bias must be quantized using scale = x_scale * w_scale and "
            "zero_point = 0",
            "T4", OpSchema::Optional)
        .Output(0, "y",
        ...
        .Attr("kernel_shape",
              ...
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("output_shape",
              "The shape of the output can be explicitly set which will cause "
              "pads values to be auto generated. If output_shape is specified "
              "pads values are ignored. See doc for details for equations to "
              "generate pads",
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr(
            "output_padding",
            "Additional elements added to the side with higher coordinate "
            "indices in the output. "
            "Each padding value in \"output_padding\" must be less than the "
            "corresponding stride/dilation dimension. "
            "By default, this attribute is a zero vector. "
            "Note that this attribute doesn't directly affect the computed "
            "output values. "
            ...
            "\"output_padding\" does not contribute additional size to "
            "\"output_shape\" but "
        ...
        .Attr("dilations",
              ...
              AttributeProto::INTS, OPTIONAL_VALUE)
        .Attr("strides",
              "Stride along each spatial axis. If not present, the stride "
        ...
        .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext&
          ...
          // validate scale and zero points
          // scale and zero points could be scalar or 1-D tensor which depends
          // on quanization per-channel or per-tensor
          ValidateTypeAndShapeForScaleAndZP(ctx, 1,
                                            ONNX_NAMESPACE::TensorProto::FLOAT,
                                            QuantParamTensorType::Scalar);
          ValidateTypeAndShapeForScaleAndZP(ctx, 2,
          ...
          // TODO: uncomment this after we really need to infer the output
          // shape. Since we haven't implemented QLinearConvTranspose in CPU EP
          // yet, we just leverage ConvTranspose's shape inference. input,
          // weight, bias std::array<int, 3> input_remap = {0, 3, 8};
