...
template <typename T> static T get_data(const TensorProto*) {
  fail_shape_inference("Unsupported non-raw-data data type!");
}

template <> int32_t get_data<int32_t>(const TensorProto* shapeInitializer) {
  if (shapeInitializer->int32_data_size() > 0)
    return shapeInitializer->int32_data(0);
  fail_shape_inference("Can not get shape initializer data!");
}

template <> int64_t get_data<int64_t>(const TensorProto* shapeInitializer) {
  if (shapeInitializer->int64_data_size() > 0)
    return shapeInitializer->int64_data(0);
  fail_shape_inference("Can not get shape initializer data!");
}

template <> float get_data<float>(const TensorProto* shapeInitializer) {
  if (shapeInitializer->float_data_size() > 0)
    return shapeInitializer->float_data(0);
  fail_shape_inference("Can not get shape initializer data!");
}

template <> double get_data<double>(const TensorProto* shapeInitializer) {
  if (shapeInitializer->double_data_size() > 0)
    return shapeInitializer->double_data(0);
  fail_shape_inference("Can not get shape initializer data!");
}

template <typename T>
static T GetFirstElement(const TensorProto* shapeInitializer) {
  if (shapeInitializer == nullptr)
    return T{1};
...
static int64_t CalcResultDim(const TensorProto* startShapeInitializer,
  ...
  if (dtype == TensorProto::FLOAT) {
    dim = CalcRangeDim<float>(startShapeInitializer, limitShapeInitializer,
                              deltaShapeInitializer);
  } else if (dtype == TensorProto::INT32) {
    dim = CalcRangeDim<int32_t>(startShapeInitializer, limitShapeInitializer,
                                deltaShapeInitializer);
  } else if (dtype == TensorProto::INT64) {
    dim = CalcRangeDim<int64_t>(startShapeInitializer, limitShapeInitializer,
                                deltaShapeInitializer);
  } else if (dtype == TensorProto::INT16) {
    dim = CalcRangeDim<int16_t>(startShapeInitializer, limitShapeInitializer,
                                deltaShapeInitializer);
  } else if (dtype == TensorProto::DOUBLE) {
    dim = CalcRangeDim<double>(startShapeInitializer, limitShapeInitializer,
                               deltaShapeInitializer);
  } else {
...
OpSchema& RegisterRangeOpSchema(OpSchema&& op_schema) {
  return op_schema.SetDomain(kMSDomain)
      .SinceVersion(1)
      .TypeConstraint("T",
                      {"tensor(float)", "tensor(double)", "tensor(int16)",
                       "tensor(int32)", "tensor(int64)"},
                      "Constrain input and output types.")
      .Input(0, "start",
             "Tensor(scalar, or dims=[1]). First entry in the range.", "T")
      .Input(1, "limit",
             "Tensor(scalar, or dims=[1]). Upper limit of sequence, exclusive.",
             "T")
      .Input(2, "delta",
             "Tensor(scalar, or dims=[1]). Number that increments start. "
             "Defaults to 1.",
             "T", OpSchema::Optional)
      .Output(0, "Y", "1-D Tensor of the range.", "T")
      .SetDoc(Range_ver1_doc)
      .TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
        propagateElemTypeFromInputToOutput(ctx, 0, 0);

        TensorShapeProto::Dimension dim;
        bool enoughShapeInfo =
            ctx.getInputData(0) != nullptr && ctx.getInputData(1) != nullptr &&
        ...
        if (enoughShapeInfo) {
          ...
          int64_t n =
              CalcResultDim(startShapeInitializer, limitShapeInitializer,
                            deltaShapeInitializer, dtype);
          dim.set_dim_value(n);
