...
void EmbedLayerNormalizationShapeInference(
  ...
  // Ensure that all embeddings + the gamma/beta tensors have the same
  // hidden_size:
  ...
  if (gamma_dims.size() != 1 || !gamma_dims[0].has_dim_value() ||
      gamma_shape.dim(0).dim_value() != hidden_size) {
    fail_shape_inference("gamma should have 2 dimension, dimension size known, "
                         "and same hidden size as word_embedding.");
  }

  auto& beta_shape = getInputShape(ctx, 6);
  auto& beta_dims = gamma_shape.dim();
  if (beta_dims.size() != 1 || !beta_dims[0].has_dim_value() ||
      beta_shape.dim(0).dim_value() != hidden_size) {
    fail_shape_inference("beta should have 1 dimension, dimension size known, "
                         "and same hidden size as word_embedding.");
  }

  // input shape is (batch_size, sequence_length), output shape is (batch_size,
  ...
  *output_shape.add_dim() = input_ids_dims[1];
  ...
  if (ctx.getNumOutputs() == 3 ||
      (ctx.getNumOutputs() == 2 && mask_index_type == 0)) {
    updateOutputShape(ctx, 2, output_shape);
    propagateElemTypeFromInputToOutput(ctx, 0, 2);
  }
}

void SkipLayerNormalizationShapeInference(
    ::ONNX_NAMESPACE::InferenceContext& ctx) {
  ...
  if (ctx.getNumOutputs() > 1) {
    ...
    output_type->mutable_tensor_type()->set_elem_type(
        static_cast<int32_t>(stash_type));
  }
...
void AttentionTypeAndShapeInference(ONNX_NAMESPACE::InferenceContext& ctx,
                                    int past_input_index) {
  // Input 0, 1, 2 are input, weights and bias.
  // The other inputs may vary in Attention and QAttention. For example,
  // past_input_index is 4 for Attention, and 8 for QAttention.
  //
  // Input 0 has 3D shape (batch_size, sequence_length, input_hidden_size)
  // Input 1 has 2D shape (input_hidden_size, hidden_size + hidden_size +
  // v_hidden_size) Input 2 has 1D shape (hidden_size + hidden_size +
  // v_hidden_size) The past input has shape (2, batch_size, num_heads,
  // past_sequence_length, head_size) Output 0 has 3D shape (batch_size,
  // sequence_length, v_hidden_size) Output 1 (present) has 5D shape (2,
  // batch_size, num_heads, total_sequence_length, head_size) When past and
  // present share buffer, they shape is same: (2, batch_size, num_heads,
        ...
        if (past_present_share_buffer) {
          ...
          if (input_dims[1].has_dim_value() && past_dims[3].has_dim_value()) {
            int64_t total_sequence_length =
                input_dims[1].dim_value() + past_shape.dim(3).dim_value();
