...
protected:
  ...
  vaip_core::Model* simple_conv_relu_model_ = nullptr;
};
...
void VaipOrtApiTest::Test06_GraphAdvancedOperations() {
  try {
    ...
    std::vector<const vaip_core::Node*> leaf_nodes;
    ...
    auto nodes_entering = std::vector<const vaip_core::Node*>{};
    auto nodes_leaving = std::vector<const vaip_core::Node*>{};
    auto verbose_log = 0;
    wrapped_api_->graph_reverse_dfs_from(
        graph, gsl::span<const vaip_core::Node* const>(leaf_nodes),
        [this, &nodes_entering, verbose_log](const vaip_core::Node* node) {
        ...
        [this, &nodes_leaving, verbose_log](const vaip_core::Node* node) {
          auto op_type = wrapped_api_->node_op_type(*node);
          if (op_type == "Constant") {
            LOG_IF(INFO, verbose_log)
                << "Skipping Constant node: "
        ...
        [](const vaip_core::Node*, const vaip_core::Node*) {
          return false; /* stop */
        });
    ...
    wrapped_api_->graph_reverse_dfs_from_preemp(
        ...
        [this, &nodes_entering2,
         verbose_log](const vaip_core::Node* node) -> bool {
          auto op_type = wrapped_api_->node_op_type(*node);
          if (op_type == "Constant") {
            LOG_IF(INFO, verbose_log)
                << "Skipping Constant node: "
        ...
        nullptr,
        [](const vaip_core::Node*, const vaip_core::Node*) -> bool {
          return false; /* stop */
        });
...
void VaipOrtApiTest::Test11_AttributeProtoOperations() {
  ...
  }
  {
    ...
    const vaip_core::Graph* unused_graph = nullptr;
...
void VaipOrtApiTest::Test19_add_sin_op_before_relu_op() {
  ...
  try {
    ...
    auto& graph = wrapped_api_->model_main_graph(*cloned_model);
    {
      ...
      for (auto node_index : topo_node_indices) {
        ...
        LOG(INFO) << " Node at index " << node_index
                  << " is: " << vaip_core::node_as_string(*node);
      }
    }
    ...
    std::vector<const vaip_core::NodeArg*> sin_inputs = {&sin_input_arg};
    std::vector<const vaip_core::NodeArg*> sin_outputs = {&sin_output_arg};
    ...
    std::vector<const vaip_core::NodeArg*> cos_inputs = {&sin_output_arg};
    std::vector<const vaip_core::NodeArg*> cos_outputs = {&cos_output_arg};
    ...
    std::vector<const vaip_core::NodeArg*> new_relu_inputs = {&cos_output_arg};
    std::vector<const vaip_core::NodeArg*> new_relu_outputs = {relu_output_arg};
...
void VaipOrtApiTest::Test20_conv_relu_fuse_conv2d_nchw() {
  ...
  try {
    ...
    auto& conv_attrs = wrapped_api_->node_get_attributes(
        *const_cast<vaip_core::Node*>(conv_node));
    ...
    std::vector<const vaip_core::NodeArg*> new_conv2d_input_args;
    ...
    std::vector<const vaip_core::NodeArg*> new_conv2d_output_args = {
        (*relu_outputs)[0]};
    ...
    wrapped_api_->graph_remove_node(graph,
                                    vaip_core::NodeInput{relu_node, nullptr});
...
void VaipOrtApiTest::Test21_fuse_relu_q() {
  try {
    ...
    std::shared_ptr<vaip_core::Pattern> input = builder.wildcard();
    std::shared_ptr<vaip_core::Pattern> relu =
        builder.node2("Relu", {builder.wildcard()});
    std::shared_ptr<vaip_core::Pattern> pattern_q = builder.node2(
    ...
    std::shared_ptr<vaip_core::PassContext> pass_context =
    ...
    auto pass = vaip_core::IPass::create_pass(pass_context, *pass_proto);
    ...
    while (has_matched) {
      ...
      for (auto index : vaip_core::graph_get_node_in_topoligical_order(graph)) {
        ...
        // LOG(INFO) << vaip_core::node_as_string(*node);
        auto op_type = vaip_core::node_op_type(*q_node);
        ...
        if (bind) {
          ...
          LOG(INFO) << "==== Found matching pattern for node: "
                    << vaip_core::node_as_string(*q_node);
          ...
          vaip_core::NodeBuilder(graph, *pass)
        ...
        << "Expected to fuse 49 ReLU-Quantize pairs, but found: " << fuse_count;
    {
      ...
      for (auto node_index : topo_node_indices) {
        ...
        ASSERT_TRUE(node != nullptr)
            << "Node at index " << node_index << " should not be null";
        auto node_type = vaip_core::node_op_type(*node);
        if (node_type == "QRelu") {
          LOG(INFO) << " ==== QRelu Node at index " << node_index
                    << " is: " << vaip_core::node_as_string(*node);
          auto attr_int = vaip_core::node_get_attr_int(*node, "attr_int");
          CHECK_EQ(attr_int, 1)
              << "Expected attr_int to be present in QRelu node attributes"
              << "node:" << vaip_core::node_as_string(*node);
        }
      }
    }
...
void VaipOrtApiTest::Test23_try_fuse_and_fuse() {
  ...
  try {
    ...
    for (auto index : vaip_core::graph_get_node_in_topoligical_order(graph)) {
      ...
      auto op_type = vaip_core::node_op_type(*node);
      // try_fuse and fuse
      if (op_type != "Conv") {
        LOG(INFO) << "Skipping node: " << vaip_core::node_as_string(*node)
      ...
      auto fuse_node_order =
          vaip_core::graph_get_node_in_topoligical_order(function_body);
      for (auto fuse_index : fuse_node_order) {
        ...
        LOG(INFO) << "Fused node at index " << fuse_index
                  << " is: " << vaip_core::node_as_string(*fuse_node_ptr);
      }
    }
...
void VaipOrtApiTest::Test07_create_simple_conv_relu_model() {
  ...
  try {
    ...
    wrapped_api_->graph_set_inputs(
        graph, gsl::span<const vaip_core::NodeArg* const>({&input_arg}));
    ...
    std::vector<const vaip_core::NodeArg*> conv_inputs = {&input_arg,
                                                          weight_arg, bias_arg};
    std::vector<const vaip_core::NodeArg*> conv_outputs = {&conv_output_arg};
    ...
    std::vector<const vaip_core::NodeArg*> relu_inputs = {&conv_output_arg};
    std::vector<const vaip_core::NodeArg*> relu_outputs = {&final_output_arg};
    ...
    std::vector<const vaip_core::NodeArg*> graph_inputs = {&input_arg};
    std::vector<const vaip_core::NodeArg*> graph_outputs = {&final_output_arg};

    wrapped_api_->graph_set_outputs(
        graph, gsl::span<const vaip_core::NodeArg* const>(graph_outputs));
