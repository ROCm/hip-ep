...
tg.config.tg_stack_size = 0 : ui32
tg.config.tg_heap_size = 0 : ui32
config.generator.operator_args.iteration_domains.B0.size = {b} : ui32
config.generator.operator_args.iteration_domains.B1.size = {b} : ui32
config.generator.operator_args.iteration_domains.M.size = {m} : ui32
config.generator.operator_args.iteration_domains.K.size = {k} : ui32
config.generator.operator_args.iteration_domains.N.size = {n} : ui32
"""
    opargs = opargs_template.format(b=b, m=m, k=k, n=n)
...
###############################################################################
def gen_masked_softmax(overlay, b, m, k):
    if overlay == "2x4x4":
        mlir_template = """module {{
func.func @dummy(%arg0: tensor<1x1x1xbf16>) -> tensor<1x1x1xbf16> {{
    return %arg0 : tensor<1x1x1xbf16>
...
"""
        opargs = opargs_template.format(b=b, m=m, k=k, head_size=128)
...
###############################################################################
def gen_op(output_path, model_name, overlay, op_type, b, m, k, n, h, g, s):
    """
    ...
    if op_type == "silu":
        ...
        graph = gen_matmul(b, m, n, k)
    elif op_type in ["bmm1"]:
        graph, opargs, top_cpp = gen_bmm1(overlay, b, m, n, k)
        model_type = "ukernel.mlir"
    elif op_type in ["bmm2"]:
        graph, opargs, top_cpp = gen_bmm2(overlay, b, m, n, k)
        model_type = "ukernel.mlir"
    elif op_type == "masked_softmax":
        graph, opargs, top_cpp = gen_masked_softmax(overlay, b, m, k)
        model_type = "ukernel.mlir"
    elif op_type in ["rope_bmk_bmk", "rope_mbk_mbk", "rope_mbk_bmk"]:
        graph, opargs, top_cpp = gen_rope(overlay, b, m, k)
        model_type = "ukernel.mlir"
    elif op_type in [
        "rope_variant_bmk_bmk",
        "rope_variant_mbk_mbk",
        ...
        graph, opargs, top_cpp = gen_rope_variant(overlay, b, m, k)
        model_type = "ukernel.mlir"
...
if __name__ == "__main__":
    ...
    # gen_op("llama2-dummy", "2x4x4", "bmm1", 32, 512, 512, 128, 0, 0)
    # gen_op("llama2-dummy", "2x4x4", "bmm2", 32, 512, 512, 128, 0, 0)
