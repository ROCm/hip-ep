 ...
 *
 * This function handles the common post-fusion logic for all ROCm Level-2
 * passes:
...
inline bool finalize_level2_fuse(morphizen::IPass *self,
  ...
  auto status =
      google::protobuf::util::MessageToJsonString(rocm_param, &rocm_param_json);
  if (!status.ok()) {
    LOG(ERROR) << log_prefix
               << " Failed to serialize params: " << status.ToString();
    return false;
  }
  ...
  if (param_writer) {
    ...
    LOG(ERROR) << log_prefix
               << " Failed to write param file: " << param_filename;
  }
  ...
  ROCM_LOG(1) << log_prefix
              << " Created fused node via level_2_fuse: " << output_name
...
inline bool save_weight_to_cache(
  ...
  if (writer) {
    ...
    LOG(ERROR) << log_prefix
               << " Failed to open weight file for write: " << filename;
    return false;
  }
}
