...
class DllMlp(nn.Module):
    ...
    def __init__(self, decode_runner, prefill_runner, gate_w, up_w, down_w, orig_mlp):
        super().__init__()
        ...
        self._decode_shape = (
            tuple(decode_runner.input_metas[-1]["shape"]) if decode_runner else None
        )
        self._prefill_shape = (
            tuple(prefill_runner.input_metas[-1]["shape"]) if prefill_runner else None
        )
    ...
    def forward(self, x):
        ...
        out = runner(self._gate_w, self._up_w, self._down_w, x_f16)[0]
        return out.to(dtype).to(device)


def compile_mlp_for_shape(mlp_module, hidden_size, seq_len, label, use_2d=False):
    ...
    therock = os.environ.get(
        "THEROCK_DIST", "C:\\Users\\tsiddaga\\Documents\\code\\therock"
    )
    ...
    work = os.path.join(
        os.path.dirname(__file__), "..", "test", "e2e_flow", f"qwen_mlp_{label}"
    )
    ...
    print("    Step 3/4: hip-compiler → DLL")
    import time as _time

    t0 = _time.perf_counter()
    r = subprocess.run(["cmd", "/c", cmd], capture_output=True, text=True, timeout=120)
    ...
    # Load
    print("    Step 4/4: ctypes load")
    os.environ["THEROCK_DIST"] = therock
    try:
        runner = HipDllRunner(dll_path, work_dir=work)
        print(f"             Inputs:  {[m.get('shape') for m in runner.input_metas]}")
        print(f"             Outputs: {[m.get('shape') for m in runner.output_metas]}")
        return runner
    except Exception as e:
...
def main():
    ...
    if is_moe:
        print(
            f"Architecture: MoE (experts={tc.num_experts}, "
    ...
    print(f"\n{'=' * 70}")
    print("Phase 1: PyTorch Eager Baseline")
    print(f"{'=' * 70}")
    ...
    print(f"\n{'=' * 70}")
    print("Phase 2: HIP DLL Compilation")
    print(f"{'=' * 70}")
    ...
    prefill_runner = compile_mlp_for_shape(
        target_mlp, hidden_size, seq_len=prompt_len, label="prefill", use_2d=is_moe
    )
    ...
    print(
        f"  Decode DLL:    [1, 1, {hidden_size}] → "
        f"{'OK' if decode_runner else 'FAILED'}"
    )
    print(
        f"  Prefill DLL:   [1, {prompt_len}, {hidden_size}] → "
    ...
    print(f"\n{'=' * 70}")
    print("Phase 3: Hybrid Execution (MLP → GPU DLL, rest → PyTorch)")
    print(f"{'=' * 70}")
    ...
    sample = (
        model.model.layers[0].mlp.shared_expert if is_moe else model.model.layers[0].mlp
    )
    ...
    print()
    print("GPU Kernel Dispatch (per MLP DLL call):")
    print(
        f"  1. hip.matmul  gate_proj  [{hidden_size}]→[{sample._gate_w.shape[1]}]  hipBLASLt"
    )
    print(
        f"  2. hip.silu    activation [{sample._gate_w.shape[1]}]              MIOpen"
    )
    print(
        f"  3. hip.matmul  up_proj    [{hidden_size}]→[{sample._up_w.shape[1]}]  hipBLASLt"
    )
    print(
        f"  4. hip.mul     gate*up    [{sample._gate_w.shape[1]}]              MIOpen"
    )
    print(
        f"  5. hip.matmul  down_proj  [{sample._down_w.shape[1]}]→[{hidden_size}]  hipBLASLt"
    )
