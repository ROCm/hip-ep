...
def create_conv_conv_reshape_gemm_softmax_model(
    batch_size=1, in_channels=3, height=8, width=8, output_path="sample.onnx"
):
    """
    ...
    conv1_W = helper.make_tensor(
        "conv1_W", TensorProto.FLOAT, conv1_W_shape, conv1_W_data.flatten()
    )
    ...
    conv2_W = helper.make_tensor(
        "conv2_W", TensorProto.FLOAT, conv2_W_shape, conv2_W_data.flatten()
    )

    conv2_B_data = np.random.randn(conv2_out_channels).astype(np.float32) * 0.01
    conv2_B = helper.make_tensor(
        "conv2_B", TensorProto.FLOAT, [conv2_out_channels], conv2_B_data.flatten()
    )
    ...
    reshape_shape_tensor = helper.make_tensor(
        "reshape_shape", TensorProto.INT64, [2], reshape_shape.flatten()
    )
    ...
    softmax_node = helper.make_node(
        "Softmax", inputs=["gemm_out"], outputs=["Y"], name="softmax", axis=1
    )
    ...
    # Add metadata
    model.doc_string = (
        "Conv(no bias)->Conv(with bias)->Reshape->Gemm->Softmax model for ROCm testing"
    )
    ...
    print("  -> Conv1 (3x3, pad=1, NO bias)")
    print(
        f"  -> Conv1 output: [{batch_size}, {conv1_out_channels}, {conv1_out_h}, {conv1_out_w}]"
    )
    print("  -> Conv2 (3x3, pad=1, WITH bias)")
    print(
        f"  -> Conv2 output: [{batch_size}, {conv2_out_channels}, {conv2_out_h}, {conv2_out_w}]"
    )
    print("  -> Reshape")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate Conv(no bias)->Conv(with bias)->Reshape->Gemm->Softmax ONNX model"
    )
    ...
    parser.add_argument("--output", type=str, default="sample.onnx", help="Output file")
