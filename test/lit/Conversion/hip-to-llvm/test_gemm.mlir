...
// - 13-param signature: state, A, B, C, output, M, N, K,
//                        alpha, beta, transA, transB, typeCode
//
...
module {
  // Test 1: f16 Gemm with C (bias), default attributes — typical LLM FC layer
  // A=[1,5120], B=[5120,5120], C=[5120], Y=[1,5120]
  // M=1, K=5120, N=5120, typeCode=0 (f16)
  func.func @gemm_f16_with_bias(
    ...
    // CHECK: llvm.call @wrap_gemm({{.*}}) : (!llvm.ptr, !llvm.ptr, !llvm.ptr, !llvm.ptr, !llvm.ptr, i64, i64, i64, f32, f32, i64, i64, i64) -> i32

    return
  }

  // Test 2: f32 Gemm without C (no bias) — C should be null pointer
  ...
  // Test 3: f16 Gemm with transB=1
  // A=[1,256], B=[512,256] (transposed: logical [256,512]), C=[512], Y=[1,512]
  // M=1, K=256, N=512 (N from B.dim(0) when transB), typeCode=0 (f16)
  ...
  // Test 4: f32 Gemm with transA=1, custom alpha/beta
  // A=[256,128] (transposed: logical [128,256]), B=[256,512], C=[512], Y=[128,512]
  // M=128 (A.dim(1) when transA), K=256 (A.dim(0) when transA), N=512
  // alpha=2.0, beta=0.5, typeCode=1 (f32)
