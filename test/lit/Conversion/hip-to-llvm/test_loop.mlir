...
//   and the fixed 5-arg signature `(state, iter, cond, lc[], cap[]) -> i32`
...
module {
  ...
  func.func private @loop_passthrough_body(%ctx: !hip.context,
                                           ...
                                           %v_in: memref<16xf32>,
                                           %cap: memref<16xf32>,
                                           %v_out: memref<16xf32> {bufferize.result}) {
  ...
  // CHECK-DAG: llvm.func @hipdnn_ep_run_counted_loop(!llvm.ptr, !llvm.ptr, i64, i1, i32, i32, !llvm.ptr, !llvm.ptr) -> i32
  //
  ...
  // CHECK: llvm.return
  //
  // Trampoline (internal linkage, fixed 5-arg signature) is emitted at module
  ...
  // CHECK-LABEL: llvm.func internal @loop_passthrough_body_trampoline(%{{.*}}: !llvm.ptr, %{{.*}}: !llvm.ptr, %{{.*}}: !llvm.ptr, %{{.*}}: !llvm.ptr, %{{.*}}: !llvm.ptr) -> i32
...
// have an explicit `cond_out` out-param.
module {
  ...
  // Non-passthrough body: cond_out is an explicit out-param.
  func.func private @loop_dynamic_body(%ctx: !hip.context,
                                       %iter: memref<i64>,
                                       %cond_in: memref<i1>,
                                       %v_in: memref<16xf32>,
                                       %cap: memref<16xf32>,
                                       %cond_out: memref<i1> {bufferize.result},
                                       %v_out: memref<16xf32> {bufferize.result}) {
    hip.add(%ctx) ins(%v_in, %cap : memref<16xf32>, memref<16xf32>)
                  outs(%v_out : memref<16xf32>)
    return
  ...
  // CHECK-DAG: llvm.func @hipdnn_ep_run_loop(!llvm.ptr, !llvm.ptr, i64, i1, i32, i32, !llvm.ptr, !llvm.ptr) -> i32
  //
  ...
  // CHECK: llvm.return
  //
  // Trampoline still emitted, same 5-arg shape.
  // CHECK-LABEL: llvm.func internal @loop_dynamic_body_trampoline(%{{.*}}: !llvm.ptr, %{{.*}}: !llvm.ptr, %{{.*}}: !llvm.ptr, %{{.*}}: !llvm.ptr, %{{.*}}: !llvm.ptr) -> i32
