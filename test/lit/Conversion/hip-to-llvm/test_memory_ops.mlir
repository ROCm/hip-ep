...
//
// This test validates:
// - hip.get_pool(%ctx, %size) → llvm.call @hipdnn_ep_get_pool_base(state, size)
...
// wrapped as a memref<?xi8>. Slices are taken via memref.view at
...
module attributes {"hipdnn.pool_size" = 65536 : i64} {
  ...
  func.func @test_get_pool(%ctx: !hip.context) -> memref<?xi8> {
    // hip.get_pool lowers to hipdnn_ep_get_pool_base runtime call
    // CHECK: llvm.call @hipdnn_ep_get_pool_base({{.*}}) : (!llvm.ptr, i64) -> !llvm.ptr
    %size = arith.constant 65536 : index
    %pool = hip.get_pool(%ctx, %size) : memref<?xi8>
    return %pool : memref<?xi8>
  }
}
