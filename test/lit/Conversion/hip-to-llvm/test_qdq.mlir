...
//   axis, block_size, onnx_output_dtype                     (3 i64)
...
// (15 params): ONNX only defines those two on QuantizeLinear.
//
// onnx_output_dtype forwards the op attribute (a TensorProto enum) while the
// trailing three are HIPDNN_EP_DATATYPE_* derived from the memref element
// types -- two different numberings that must not be conflated.
//
...
// ONNX UINT8 = 2 reaches the runtime as onnx_output_dtype; the ui8 memref
// independently maps to HIPDNN_EP_DATATYPE_UINT8 = 7. precision = 16 pins the
// attribute that only QuantizeLinear carries.
...
func.func @quantize_linear_blocked(%ctx: !hip.context,
  ...
  hip.quantize_linear(%ctx)
      ...
      {axis = 1 : i64, block_size = 32 : i64, output_dtype = 2 : i64,
       precision = 16 : i64, saturate = 0 : i64}
  return
}
...
// unchanged. ONNX FLOAT16 = 10 is the attribute; the i16 input maps to
// HIPDNN_EP_DATATYPE_INT16 = 8.

// CHECK-LABEL: llvm.func @dequantize_linear_no_zp
// CHECK-DAG:   llvm.mlir.zero
// CHECK-DAG:   llvm.mlir.constant(10 : i64)
...
func.func @dequantize_linear_no_zp(%ctx: !hip.context,
                                   ...
                                   %y: memref<8x128xf16, 1>) {
  hip.dequantize_linear(%ctx)
      ins(%x, %scale : memref<8x128xi16, 1>, memref<128xf16, 1>)
      outs(%y : memref<8x128xf16, 1>)
      {axis = 1 : i64, block_size = 0 : i64, output_dtype = 10 : i64}
  return
}
...
func.func @dequantize_linear_bad_float(%ctx: !hip.context,
  ...
  hip.dequantize_linear(%ctx)
      ...
      {axis = 1 : i64, block_size = 0 : i64, output_dtype = 11 : i64}
  return
}
