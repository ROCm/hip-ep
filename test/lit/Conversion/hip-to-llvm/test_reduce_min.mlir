...
module {
  ...
  func.func @reduce_min_static_last_axis(
    ...
    hip.reduce_min(%ctx) ins(%input, %axes : memref<8x128x512xf16, 1>, memref<1xi64>)
                         outs(%output : memref<8x128xf16, 1>)
                         {keepdims = 0 : i64, noop_with_empty_axes = 0 : i64,
                          normalized_axes = array<i64: 2>}

    // CHECK: %[[STATUS:.*]] = llvm.call @wrap_reduce_min({{.*}}) : (!llvm.ptr, !llvm.ptr, !llvm.ptr, !llvm.ptr, i64, i64, i64, i64, i64, i64, i64) -> i32
  ...
  func.func @reduce_min_dynamic(
    ...
    %axes = memref.get_global @axis2 : memref<1xi64>
    hip.reduce_min(%ctx) ins(%input, %axes : memref<?x?x512xf16, 1>, memref<1xi64>)
