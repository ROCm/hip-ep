...
module {
  ...
  func.func @reduce_prod_static(
      %ctx: !hip.context,
      %data: memref<3x2x2xi32, 1>,
      %output: memref<3x1x2xi32, 1>) {
    // CHECK-LABEL: llvm.func @reduce_prod_static

    %axes = memref.get_global @axis1 : memref<1xi64>
    hip.reduce_prod(%ctx) ins(%data, %axes : memref<3x2x2xi32, 1>, memref<1xi64>)
                          outs(%output : memref<3x1x2xi32, 1>)
                          {keepdims = 1 : i64, noop_with_empty_axes = 0 : i64,
                           normalized_axes = array<i64: 1>}

    // CHECK: %[[STATUS:.*]] = llvm.call @wrap_reduce_prod({{.*}}) : (!llvm.ptr, !llvm.ptr, !llvm.ptr, !llvm.ptr, i64, i64, i64, i64, i64, i64, i64) -> i32
  ...
  func.func @reduce_prod_dynamic(
      %ctx: !hip.context,
      %data: memref<?x2x?xi32, 1>,
      %output: memref<?x?xi32, 1>) {
    // CHECK-LABEL: llvm.func @reduce_prod_dynamic

    %axes = memref.get_global @axis1 : memref<1xi64>
    hip.reduce_prod(%ctx) ins(%data, %axes : memref<?x2x?xi32, 1>, memref<1xi64>)
                          outs(%output : memref<?x?xi32, 1>)
