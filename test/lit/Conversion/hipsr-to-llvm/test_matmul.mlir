...
// CHECK-DAG:   %[[BATCH:.*]] = llvm.mlir.constant(1 : i64) : i64
// CHECK-DAG:   %[[B_STRIDE:.*]] = llvm.mlir.constant(0 : i64) : i64
// CHECK-DAG:   %[[ELEM_SIZE:.*]] = llvm.mlir.constant(2 : i64) : i64
// CHECK-DAG:   %[[A_PTR:.*]] = llvm.extractvalue {{.*}}[1] : !llvm.struct<(ptr<1>,
// CHECK-DAG:   %[[B_PTR:.*]] = llvm.extractvalue {{.*}}[1] : !llvm.struct<(ptr<1>,
// CHECK-DAG:   %[[OUT_PTR:.*]] = llvm.extractvalue {{.*}}[1] : !llvm.struct<(ptr<1>,
// CHECK:       llvm.call @wrap_hipblasLtMatmul(%[[CTX]], {{%.*}}, %[[A_PTR]], %[[B_PTR]], %[[OUT_PTR]], {{%.*}}, {{%.*}}, {{%.*}}, %[[BATCH]], %[[ELEM_SIZE]], %[[B_STRIDE]])
...
// CHECK:       %[[LEADING_PRODUCT:.*]] = llvm.mul
// CHECK:       %[[IS_BROADCAST:.*]] = llvm.icmp "sle" %[[LEADING_PRODUCT]],
// CHECK:       %[[KN:.*]] = llvm.mul
// CHECK:       %[[DYN_B_STRIDE:.*]] = llvm.select %[[IS_BROADCAST]], {{.*}}, %[[KN]]
// CHECK-DAG:   %[[DYN_ELEM_SIZE:.*]] = llvm.mlir.constant(2 : i64) : i64
// CHECK:       llvm.call @wrap_hipblasLtMatmul(%[[DYN_CTX]], {{%.*}}, {{.*}}, %[[DYN_BATCH]], %[[DYN_ELEM_SIZE]], %[[DYN_B_STRIDE]])
// CHECK-SAME:    (!llvm.ptr, i32, !llvm.ptr<1>, !llvm.ptr<1>, !llvm.ptr<1>, i64, i64, i64, i64, i64, i64) -> i32
