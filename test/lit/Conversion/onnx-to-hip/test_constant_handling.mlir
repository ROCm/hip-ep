...
module {
  func.func @constant_test() -> tensor<3xf32> {
    ...
    // CHECK-SAME: (%[[CTX:.*]]: !hip.context) -> tensor<3xf32>
  ...
  func.func @constant_matrix() -> tensor<2x2xf32> {
    ...
    // CHECK-SAME: (%[[CTX:.*]]: !hip.context) -> tensor<2x2xf32>
  ...
  func.func @multiple_constants() -> (tensor<4xf32>, tensor<4xf32>) {
    ...
    // CHECK-SAME: (%[[CTX:.*]]: !hip.context) -> (tensor<4xf32>, tensor<4xf32>)
    ...
    // Both arith.constant ops are emitted before both hip.get_constant ops.
