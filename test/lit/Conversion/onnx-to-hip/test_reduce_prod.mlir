...
module {
  ...
  func.func @reduce_prod_default_axes(%data: tensor<3x2x2xf32>) -> tensor<1x1x1xf32> {
    %r = "onnx.ReduceProd"(%data) {keepdims = 1 : si64} : (tensor<3x2x2xf32>) -> tensor<1x1x1xf32>
    return %r : tensor<1x1x1xf32>
  }

  // CHECK-LABEL: func.func @reduce_prod_default_axes
  // CHECK: arith.constant dense<[0, 1, 2]> : tensor<3xi64>
  // CHECK: hip.reduce_prod({{.*}}) ins({{.*}}, {{.*}} : tensor<3x2x2xf32>, tensor<3xi64>) outs({{.*}} : tensor<1x1x1xf32>)

  // ReduceProd with axes attribute (older opset).
  func.func @reduce_prod_axes_attr(%data: tensor<3x2x2xf32>) -> tensor<3x1x2xf32> {
    %r = "onnx.ReduceProd"(%data) {axes = [1 : si64], keepdims = 1 : si64} : (tensor<3x2x2xf32>) -> tensor<3x1x2xf32>
    return %r : tensor<3x1x2xf32>
  }
