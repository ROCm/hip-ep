...
module {
  ...
  // Shape/Gather/Unsqueeze/Concat is proven host-side before constant
  // externalization. Reshape consumes the scalar SSA directly and emits no
  ...
  // A constant proven shape may fold from tensor.from_elements back to
  // arith.constant before conversion. The marker remains valid and conversion
  // rematerializes its entries, including -1 inference and allowzero=0.
  // CHECK-LABEL: func.func @folded_constant_provenance
  // CHECK-NOT: hip.readback_scalar
  // CHECK: arith.cmpi eq
  // CHECK: arith.select
  // CHECK: tensor.reshape
  func.func @folded_constant_provenance(
