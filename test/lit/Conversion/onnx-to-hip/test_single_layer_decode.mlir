...
//
// This model exercises all ops in a complete decoder layer:
//   Gather → SimplifiedLayerNormalization →
//   Q/K/V MatMul → RotaryEmbedding (Q, K) →
//   GroupQueryAttention → O_proj MatMul →
//   SkipSimplifiedLayerNormalization (4 outputs, 2 are none) →
//   MLP: gate/up MatMul → Sigmoid → Mul (SiLU) → Mul → down MatMul
//   Plus: ReduceSum → Sub → Cast for attention mask
//
// Key test point: SkipSimplifiedLayerNormalization produces 4 results
// where results 1 and 2 are `none` type. The conversion must handle this.
...
// ReduceSum for attention mask
// CHECK: hip.reduce_sum

// SimplifiedLayerNormalization
// CHECK: hip.simplified_layer_norm
// CHECK-SAME: epsilon

// Sub for attention mask
// CHECK: hip.sub
