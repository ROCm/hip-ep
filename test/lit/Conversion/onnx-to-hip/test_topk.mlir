...
module {
  ...
  // CHECK: hip.top_k(%[[CTX]]) ins(%[[X]], %[[K]] : tensor<3x4xf32>, tensor<i64>) outs({{.*}}, {{.*}} : tensor<3x2xf32>, tensor<3x2xi64>) {axis = 1 : i64, largest = true, sorted = true}
  // CHECK-NOT: onnx.TopK
  ...
  // CHECK: hip.top_k(%{{.*}}) ins({{.*}} : tensor<2x5xf16>, tensor<1xi64>) outs({{.*}}, {{.*}} : tensor<2x3xf16>, tensor<2x3xi64>) {axis = 1 : i64, largest = true, sorted = true}
  ...
  // CHECK: %{{.*}} = tensor.dim %[[K3]], %[[C0]] : tensor<1xi64>
  // CHECK: tensor.empty({{.*}}, {{.*}}) : tensor<?x?xf32>
  // CHECK: tensor.empty({{.*}}, {{.*}}) : tensor<?x?xi64>
  // CHECK: hip.top_k(%[[CTX3]]) ins(%[[X3]], %[[K3]] : tensor<?x?xf32>, tensor<1xi64>) outs({{.*}}, {{.*}} : tensor<?x?xf32>, tensor<?x?xi64>) {axis = 1 : i64, largest = true, sorted = true}
}
