...
// CHECK-SAME: %[[INIT:.+]]: tensor<?x?xf16, #hipsr.mem<device>>) -> tensor<?x?xf16, #hipsr.mem<device>> {
// CHECK-NEXT: %[[RESULT:.+]] = hipsr.expand(%[[CTX]]) ins(%[[INPUT]] : tensor<?x3xf16, #hipsr.mem<device>>) outs(%[[INIT]] : tensor<?x?xf16, #hipsr.mem<device>>) {shape_attr = array<i64: 4, 3>} : tensor<?x?xf16, #hipsr.mem<device>>
// CHECK-NEXT: return %[[RESULT]] : tensor<?x?xf16, #hipsr.mem<device>>
// CHECK-NEXT: }
func.func @expand_constant_shape(
    %ctx: !hipsr.context, %input: tensor<?x3xf16, #hipsr.mem<device>>,
    %init: tensor<?x?xf16, #hipsr.mem<device>>) -> tensor<?x?xf16, #hipsr.mem<device>> {
  %shape = arith.constant dense<[4, 3]> : tensor<2xi64, #hipsr.mem<host>>
  %result = hipsr.expand(%ctx)
      ins(%input, %shape : tensor<?x3xf16, #hipsr.mem<device>>, tensor<2xi64, #hipsr.mem<host>>)
      outs(%init : tensor<?x?xf16, #hipsr.mem<device>>) : tensor<?x?xf16, #hipsr.mem<device>>
  return %result : tensor<?x?xf16, #hipsr.mem<device>>
}
...
func.func @expand_both_shapes(
    ...
    %shape: tensor<2xi64, #hipsr.mem<host>>, %init: tensor<2x3xf16, #hipsr.mem<device>>) -> tensor<2x3xf16, #hipsr.mem<device>> {
  // expected-error@+1 {{cannot have both shape operand and shape_attr attribute}}
  %result = hipsr.expand(%ctx)
      ins(%input, %shape : tensor<2x3xf16, #hipsr.mem<device>>, tensor<2xi64, #hipsr.mem<host>>)
      outs(%init : tensor<2x3xf16, #hipsr.mem<device>>)
      {shape_attr = array<i64: 2, 3>} : tensor<2x3xf16, #hipsr.mem<device>>
...
func.func @expand_missing_shape(
    %ctx: !hipsr.context, %input: tensor<2x3xf16, #hipsr.mem<device>>,
    %init: tensor<2x3xf16, #hipsr.mem<device>>) -> tensor<2x3xf16, #hipsr.mem<device>> {
  // expected-error@+1 {{must have either shape operand or shape_attr attribute}}
  %result = hipsr.expand(%ctx)
      ins(%input : tensor<2x3xf16, #hipsr.mem<device>>)
      outs(%init : tensor<2x3xf16, #hipsr.mem<device>>) : tensor<2x3xf16, #hipsr.mem<device>>
  return %result : tensor<2x3xf16, #hipsr.mem<device>>
}
...
func.func @expand_shape_rank(
    %ctx: !hipsr.context, %input: tensor<2x3xf16, #hipsr.mem<device>>,
    %shape: tensor<1x2xi64, #hipsr.mem<host>>, %init: tensor<2x3xf16, #hipsr.mem<device>>)
    -> tensor<2x3xf16, #hipsr.mem<device>> {
  // expected-error@+1 {{shape must be rank-1}}
  %result = hipsr.expand(%ctx)
      ins(%input, %shape : tensor<2x3xf16, #hipsr.mem<device>>, tensor<1x2xi64, #hipsr.mem<host>>)
      outs(%init : tensor<2x3xf16, #hipsr.mem<device>>) : tensor<2x3xf16, #hipsr.mem<device>>
  return %result : tensor<2x3xf16, #hipsr.mem<device>>
}
...
func.func @expand_shape_element_type(
    %ctx: !hipsr.context, %input: tensor<2x3xf16, #hipsr.mem<device>>,
    %shape: tensor<2xi32, #hipsr.mem<host>>, %init: tensor<2x3xf16, #hipsr.mem<device>>) -> tensor<2x3xf16, #hipsr.mem<device>> {
  // expected-error@+1 {{shape element type must be i64}}
  %result = hipsr.expand(%ctx)
      ins(%input, %shape : tensor<2x3xf16, #hipsr.mem<device>>, tensor<2xi32, #hipsr.mem<host>>)
      outs(%init : tensor<2x3xf16, #hipsr.mem<device>>) : tensor<2x3xf16, #hipsr.mem<device>>
  return %result : tensor<2x3xf16, #hipsr.mem<device>>
}
...
func.func @expand_dynamic_shape_length(
    %ctx: !hipsr.context, %input: tensor<2x3xf16, #hipsr.mem<device>>,
    %shape: tensor<?xi64, #hipsr.mem<host>>, %init: tensor<2x3xf16, #hipsr.mem<device>>) -> tensor<2x3xf16, #hipsr.mem<device>> {
  // expected-error@+1 {{shape length must be static}}
  %result = hipsr.expand(%ctx)
      ins(%input, %shape : tensor<2x3xf16, #hipsr.mem<device>>, tensor<?xi64, #hipsr.mem<host>>)
      outs(%init : tensor<2x3xf16, #hipsr.mem<device>>) : tensor<2x3xf16, #hipsr.mem<device>>
  return %result : tensor<2x3xf16, #hipsr.mem<device>>
}
...
func.func @expand_element_mismatch(
    %ctx: !hipsr.context, %input: tensor<2x3xf16, #hipsr.mem<device>>,
    %shape: tensor<2xi64, #hipsr.mem<host>>, %init: tensor<2x3xf32, #hipsr.mem<device>>) -> tensor<2x3xf32, #hipsr.mem<device>> {
  // expected-error@+1 {{input and output element types must match}}
  %result = hipsr.expand(%ctx)
      ins(%input, %shape : tensor<2x3xf16, #hipsr.mem<device>>, tensor<2xi64, #hipsr.mem<host>>)
      outs(%init : tensor<2x3xf32, #hipsr.mem<device>>) : tensor<2x3xf32, #hipsr.mem<device>>
  return %result : tensor<2x3xf32, #hipsr.mem<device>>
}
...
func.func @expand_output_rank(
    %ctx: !hipsr.context, %input: tensor<2x3xf16, #hipsr.mem<device>>,
    %shape: tensor<4xi64, #hipsr.mem<host>>, %init: tensor<2x3xf16, #hipsr.mem<device>>) -> tensor<2x3xf16, #hipsr.mem<device>> {
  // expected-error@+1 {{output rank must equal max(input rank, shape length); expected 4, got 2}}
  %result = hipsr.expand(%ctx)
      ins(%input, %shape : tensor<2x3xf16, #hipsr.mem<device>>, tensor<4xi64, #hipsr.mem<host>>)
      outs(%init : tensor<2x3xf16, #hipsr.mem<device>>) : tensor<2x3xf16, #hipsr.mem<device>>
  return %result : tensor<2x3xf16, #hipsr.mem<device>>
}
