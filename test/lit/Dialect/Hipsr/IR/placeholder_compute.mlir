...
// CHECK-SAME: %[[CTX:.+]]: !hipsr.context, %[[DATA:.+]]: tensor<2x3xf16>) -> tensor<6xf16> {
// CHECK-NEXT: %[[INIT:.+]] = hipsr.placeholder(%[[CTX]]) ins(%[[DATA]] : tensor<2x3xf16>) {placeholder_type = #hipsr.placeholder_type<normal>} : tensor<2x3xf16>
// CHECK-NEXT: %[[RESULT:.+]] = hipsr.compute(%[[CTX]]) ins(%[[DATA]] : tensor<2x3xf16>) outs(%[[INIT]] : tensor<2x3xf16>) {
// CHECK-NEXT: ^bb0(%{{.+}}: !hipsr.context, %[[IN:.+]]: tensor<2x3xf16>, %{{.+}}: tensor<2x3xf16>):
...
func.func @compute_outs_shape_differs(
    %ctx: !hipsr.context, %data: tensor<2x3xf16>) -> tensor<6xf16> {
...
// CHECK-SAME: %[[CTX:.+]]: !hipsr.context, %[[DATA:.+]]: tensor<2x3xf16>) -> (tensor<6xf16>, tensor<3x2xf16>) {
// CHECK-NEXT: %[[INITS:.+]]:2 = hipsr.placeholder(%[[CTX]]) ins(%[[DATA]] : tensor<2x3xf16>) {placeholder_type = #hipsr.placeholder_type<normal>} : tensor<2x3xf16>, tensor<2x3xf16>
// CHECK-NEXT: %[[RESULTS:.+]]:2 = hipsr.compute(%[[CTX]]) ins(%[[DATA]] : tensor<2x3xf16>) outs(%[[INITS]]#0, %[[INITS]]#1 : tensor<2x3xf16>, tensor<2x3xf16>) {
// CHECK: } : tensor<6xf16>, tensor<3x2xf16>{{$}}
...
func.func @multi_result_compute_outs(
  ...
  %out:2 = hipsr.compute(%ctx) ins(%data : tensor<2x3xf16>)
      outs(%inits#0, %inits#1 : tensor<2x3xf16>, tensor<2x3xf16>) {
  ^bb0(%body_ctx: !hipsr.context, %in: tensor<2x3xf16>,
       %dest0: tensor<2x3xf16>, %dest1: tensor<2x3xf16>):
    %flat = tensor.collapse_shape %in [[0, 1]]
        : tensor<2x3xf16> into tensor<6xf16>
...
func.func @compute_input_placeholder(
  ...
  %init = hipsr.placeholder(%ctx)
      ins(%data : tensor<2x3xf16>)
      {placeholder_type = #hipsr.placeholder_type<normal>} : tensor<2x3xf16>
  hipsr.compute(%ctx) ins(%init : tensor<2x3xf16>) outs() {
  ^bb0(%body_ctx: !hipsr.context, %in: tensor<2x3xf16>):
    hipsr.compute_yield
  }
  return
...
func.func @split_compute_and_dps_consumers(
    %ctx: !hipsr.context, %data: tensor<2x3xf16>)
    -> (tensor<6xf16>, tensor<2x3xf32>) {
  // expected-error @+1 {{requires all results to initialize the same hipsr operation}}
  %inits:2 = hipsr.placeholder(%ctx)
      ins(%data : tensor<2x3xf16>)
      {placeholder_type = #hipsr.placeholder_type<normal>}
      : tensor<2x3xf16>, tensor<2x3xf32>
  %out = hipsr.compute(%ctx) ins(%data : tensor<2x3xf16>)
                             outs(%inits#0 : tensor<2x3xf16>) {
  ^bb0(%body_ctx: !hipsr.context, %in: tensor<2x3xf16>,
       %dest: tensor<2x3xf16>):
    %flat = tensor.collapse_shape %in [[0, 1]]
        : tensor<2x3xf16> into tensor<6xf16>
  ...
  %cast = hipsr.cast(%ctx) ins(%data : tensor<2x3xf16>)
      outs(%inits#1 : tensor<2x3xf32>) : tensor<2x3xf32>
  return %out, %cast : tensor<6xf16>, tensor<2x3xf32>
}
