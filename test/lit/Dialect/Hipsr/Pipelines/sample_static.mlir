...
// CHECK-SAME:      %[[ARG2:[^:,]*]]: memref<2x4xf32, #hipsr.mem<device>> {onnx.name = "b"}) -> (memref<2x2xf32,
...
// CHECK-NEXT:      hipsr.matmul(%[[ARG0]]) ins(%[[ARG1]], %[[CONSTANT_2]] : memref<2x3xf16, #hipsr.mem<device>>, memref<3x1xf16, #hipsr.mem<device>>) outs(%[[VIEW_0]] : memref<2x1xf16, #hipsr.mem<device>>)
...
// CHECK-NEXT:      memref.store %[[CONSTANT_15]], %[[ALLOC_6]]{{\[}}%[[CONSTANT_16]]] : memref<2xi64, #hipsr.mem<host>>
...
// CHECK-NEXT:      %[[LOAD_1:.*]] = memref.load %[[ALLOC_6]]{{\[}}%[[CONSTANT_18]]] : memref<2xi64, #hipsr.mem<host>>
// CHECK-NEXT:      %[[INDEX_CAST_0:.*]] = arith.index_cast %[[LOAD_1]] : i64 to index
// CHECK-NEXT:      %[[LOAD_2:.*]] = memref.load %[[ALLOC_6]]{{\[}}%[[CONSTANT_19]]] : memref<2xi64, #hipsr.mem<host>>
// CHECK-NEXT:      %[[INDEX_CAST_1:.*]] = arith.index_cast %[[LOAD_2]] : i64 to index
...
// CHECK-NEXT:      scf.for %[[VAL_0:.*]] = %[[CONSTANT_18]] to %[[CONSTANT_20]] step %[[CONSTANT_19]] {
...
// CHECK-NEXT:      }
// CHECK-NEXT:      %[[LOAD_3:.*]] = memref.load %[[ALLOC_9]]{{\[}}%[[CONSTANT_18]]] : memref<?xindex>
// CHECK-NEXT:      %[[ALLOC_12:.*]] = memref.alloc() {alignment = 64 : i64} : memref<2xindex>
// CHECK-NEXT:      memref.store %[[LOAD_3]], %[[ALLOC_12]]{{\[}}%[[CONSTANT_18]]] : memref<2xindex>
...
// CHECK-NEXT:      %[[GET_POOL_1:.*]] = hipsr.get_pool(%[[ARG0]], %[[MULI_2]]) {domain_id = 1 : i64} : memref<?xi8, #hipsr.mem<device>>
...
// CHECK-NEXT:      hipsr.expand(%[[ARG0]]) ins(%[[VIEW_1]], %[[ALLOC_6]] : memref<2x1xf32, #hipsr.mem<device>>, memref<2xi64, #hipsr.mem<host>>) outs(%[[VIEW_2]] : memref<2x4xf32, #hipsr.mem<device>>)
// CHECK-NEXT:      hipsr.matmul(%[[ARG0]]) ins(%[[VIEW_2]], %[[CONSTANT_1]] : memref<2x4xf32, #hipsr.mem<device>>, memref<4x2xf32, #hipsr.mem<device>>) outs(%[[ALLOC_OUTPUT_0]] : memref<2x2xf32, #hipsr.mem<device>>)
