...
//   phase 3, allocation -- one tensor.empty per placeholder result, tagged
//   #hipsr.mem<device>, its dynamic extents read out of the computed shape, all
//   of them grouped after the last scf.execute_region;
...
// CHECK-NEXT:  }
func.func @matmul_domain(%ctx: !hipsr.context,
                         %a: tensor<?x256xf16, #hipsr.mem<device>>,
  ...
  ^bb0(%domain_ctx: !hipsr.context,
    ...
    %matmul = hipsr.matmul(%domain_ctx)
        ins(%domain_a, %domain_b
            : tensor<?x256xf16, #hipsr.mem<device>>,
              tensor<256x512xf16, #hipsr.mem<device>>)
        outs(%init : tensor<?x512xf16, #hipsr.mem<device>>)
        : tensor<?x512xf16, #hipsr.mem<device>>
...
// CHECK-NEXT:  }
func.func @static_shape(%ctx: !hipsr.context,
                        %a: tensor<128x256xf16, #hipsr.mem<device>>,
                        %b: tensor<256x512xf16, #hipsr.mem<device>>)
    -> tensor<128x512xf16, #hipsr.mem<device>> {
  %0 = hipsr.pool_domain(%ctx, %a, %b
      : !hipsr.context, tensor<128x256xf16, #hipsr.mem<device>>,
        tensor<256x512xf16, #hipsr.mem<device>>) {
  ^bb0(%domain_ctx: !hipsr.context,
       %domain_a: tensor<128x256xf16, #hipsr.mem<device>>,
       %domain_b: tensor<256x512xf16, #hipsr.mem<device>>):
    %init = hipsr.placeholder(%domain_ctx)
        ins(%domain_a, %domain_b
            : tensor<128x256xf16, #hipsr.mem<device>>,
              tensor<256x512xf16, #hipsr.mem<device>>)
        {placeholder_type = #hipsr.placeholder_type<normal>}
    ...
    %matmul = hipsr.matmul(%domain_ctx)
        ins(%domain_a, %domain_b
            : tensor<128x256xf16, #hipsr.mem<device>>,
              tensor<256x512xf16, #hipsr.mem<device>>)
        outs(%init : tensor<128x512xf16, #hipsr.mem<device>>)
        : tensor<128x512xf16, #hipsr.mem<device>>
...
func.func @multi_dynamic(%ctx: !hipsr.context,
                         %a: tensor<?x?x64xf16, #hipsr.mem<device>>,
                         %b: tensor<?x?x64xf16, #hipsr.mem<device>>)
    -> tensor<?x?x64xf16, #hipsr.mem<device>> {
  %0 = hipsr.pool_domain(%ctx, %a, %b
      : !hipsr.context, tensor<?x?x64xf16, #hipsr.mem<device>>,
        tensor<?x?x64xf16, #hipsr.mem<device>>) {
  ^bb0(%domain_ctx: !hipsr.context,
       %domain_a: tensor<?x?x64xf16, #hipsr.mem<device>>,
    ...
    %add = hipsr.add(%domain_ctx)
        ins(%domain_a, %domain_b
            : tensor<?x?x64xf16, #hipsr.mem<device>>,
              tensor<?x?x64xf16, #hipsr.mem<device>>)
        outs(%init : tensor<?x?x64xf16, #hipsr.mem<device>>)
        : tensor<?x?x64xf16, #hipsr.mem<device>>
...
// CHECK-NEXT:    return %[[DOMAIN]] : tensor<?x512xf16, #hipsr.mem<device>>
// CHECK-NEXT:  }
func.func @interleaved(%ctx: !hipsr.context,
                       %a: tensor<?x256xf16, #hipsr.mem<device>>,
                       %b: tensor<256x512xf16, #hipsr.mem<device>>,
                       %c: tensor<?x512xf16, #hipsr.mem<device>>)
    -> tensor<?x512xf16, #hipsr.mem<device>> {
  ...
  ^bb0(%domain_ctx: !hipsr.context,
       %domain_a: tensor<?x256xf16, #hipsr.mem<device>>,
       %domain_b: tensor<256x512xf16, #hipsr.mem<device>>,
       %domain_c: tensor<?x512xf16, #hipsr.mem<device>>):
    ...
    %matmul = hipsr.matmul(%domain_ctx)
        ins(%domain_a, %domain_b
            : tensor<?x256xf16, #hipsr.mem<device>>,
              tensor<256x512xf16, #hipsr.mem<device>>)
        outs(%matmul_init : tensor<?x512xf16, #hipsr.mem<device>>)
        : tensor<?x512xf16, #hipsr.mem<device>>
    %add_init = hipsr.placeholder(%domain_ctx)
        ins(%matmul_init, %domain_c
            : tensor<?x512xf16, #hipsr.mem<device>>,
              tensor<?x512xf16, #hipsr.mem<device>>)
        {placeholder_type = #hipsr.placeholder_type<normal>}
        : tensor<?x512xf16, #hipsr.mem<device>> shape_region {
    ...
    %add = hipsr.add(%domain_ctx)
        ins(%matmul, %domain_c
            : tensor<?x512xf16, #hipsr.mem<device>>,
              tensor<?x512xf16, #hipsr.mem<device>>)
        outs(%add_init : tensor<?x512xf16, #hipsr.mem<device>>)
        : tensor<?x512xf16, #hipsr.mem<device>>
...
// CHECK-NEXT:    return %[[DOMAIN]] : tensor<?x512xf16, #hipsr.mem<device>>
// CHECK-NEXT:  }
func.func @already_grouped(%ctx: !hipsr.context,
                           %a: tensor<?x256xf16, #hipsr.mem<device>>,
                           %b: tensor<256x512xf16, #hipsr.mem<device>>,
                           %c: tensor<?x512xf16, #hipsr.mem<device>>)
    -> tensor<?x512xf16, #hipsr.mem<device>> {
...
// the input already had, not one the pass built, so it keeps the default memory
// space: the pass only tags the allocations it materializes itself.
// CHECK-LABEL: func.func @no_placeholder(
// CHECK-SAME:      %[[IN:.+]]: tensor<3x4xf32, #hipsr.mem<device>>) -> tensor<2x?xi64> {
// CHECK-NEXT:    %[[DOMAIN:.+]] = hipsr.pool_domain(%[[IN]] : tensor<3x4xf32, #hipsr.mem<device>>) {
// CHECK-NEXT:    ^bb0(%[[DIN:.+]]: tensor<3x4xf32, #hipsr.mem<device>>):
// CHECK-NEXT:      %[[C1:.+]] = arith.constant 1 : index
// CHECK-NEXT:      %[[DIM:.+]] = tensor.dim %[[DIN]], %[[C1]] : tensor<3x4xf32, #hipsr.mem<device>>
...
// CHECK-NEXT:  }
func.func @no_placeholder(%in: tensor<3x4xf32, #hipsr.mem<device>>)
    -> tensor<2x?xi64> {
  %0 = hipsr.pool_domain(%in : tensor<3x4xf32, #hipsr.mem<device>>) {
  ^bb0(%domain_in: tensor<3x4xf32, #hipsr.mem<device>>):
    %c1 = arith.constant 1 : index
    %n = tensor.dim %domain_in, %c1 : tensor<3x4xf32, #hipsr.mem<device>>
