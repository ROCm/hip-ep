...
// CHECK-NEXT: hipsr.matmul(%[[CTX]]) ins({{.+}}) outs(%[[ALLOC]] : memref<?x2048xf32, #hipsr.mem<device>>)
func.func @dps_init(%ctx: !hipsr.context, %d0: index, %lhs: tensor<?x64xf32, #hipsr.mem<device>>,
                    %rhs: tensor<64x2048xf32, #hipsr.mem<device>>) -> tensor<?x2048xf32, #hipsr.mem<device>> {
  ...
  %out = hipsr.matmul(%ctx) ins(%lhs, %rhs : tensor<?x64xf32, #hipsr.mem<device>>, tensor<64x2048xf32, #hipsr.mem<device>>)
...
// CHECK-NEXT: hipsr.matmul(%[[CTX]]) ins({{.+}}) outs(%[[ALLOC]] : memref<?x2048xf32, #hipsr.mem<device>>)
// CHECK-NEXT: hipsr.preserve_shape %[[SHAPE]], %[[ALLOC]] : memref<?x2048xf32, #hipsr.mem<device>>
func.func @dps_result(%ctx: !hipsr.context, %d0: index, %lhs: tensor<?x64xf32, #hipsr.mem<device>>,
                      %rhs: tensor<64x2048xf32, #hipsr.mem<device>>) -> tensor<?x2048xf32, #hipsr.mem<device>> {
  %c2048 = arith.constant 2048 : index
  %shape = shape.from_extents %d0, %c2048 : index, index
  %init = tensor.empty(%d0) : tensor<?x2048xf32, #hipsr.mem<device>>
