...
// Dynamic K is runtime-checked and does not contribute an output extent.
// CHECK-LABEL: func.func @gemm_dynamic_k_both
// CHECK: return
func.func @gemm_dynamic_k_both(
    %ctx: !hip.context, %a: tensor<2x?xf16>, %b: tensor<?x8xf16>,
    %out: tensor<2x8xf16>) -> (index, index) {
  %r = hip.gemm(%ctx)
    ins(%a, %b : tensor<2x?xf16>, tensor<?x8xf16>)
    outs(%out : tensor<2x8xf16>)
    {transA = 0 : i64, transB = 0 : i64} : tensor<2x8xf16>
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
...
// CHECK-LABEL: func.func @gemm_dynamic_k_b_only
// CHECK: return
func.func @gemm_dynamic_k_b_only(
