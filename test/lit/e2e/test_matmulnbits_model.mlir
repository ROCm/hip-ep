...
// Verifies the complete morphizen-pipeline for 4-bit quantized matmul:
...
module {
  func.func @main_graph(%arg0: tensor<1x128x2880xf16> {onnx.name = "A"}) -> (tensor<1x128x5120xf16> {onnx.name = "Y"}) {
    // B: packed int4 weights [N=5120, k_blocks=90, blob_size=16]
    // k_blocks = ceil(2880/32) = 90, blob_size = 32*4/8 = 16
    %B = onnx.Constant dense<1> : tensor<5120x90x16xui8>
    // scales: [N=5120, k_blocks=90]
    %scales = onnx.Constant dense<1.000000e+00> : tensor<5120x90xf16>
