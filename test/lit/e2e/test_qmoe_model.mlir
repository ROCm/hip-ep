...
// Verifies the complete morphizen-pipeline for quantized Mixture-of-Experts:
...
module {
  func.func @main_graph(%arg0: tensor<1x128x2880xf16> {onnx.name = "input"}, %arg1: tensor<128x32xf16> {onnx.name = "router_probs"}) -> (tensor<1x128x2880xf16> {onnx.name = "output"}) {
    // fc1 weights: [num_experts=32, fusion*inter=5760, hidden/pack=1440]
    // pack_size = 8/4 = 2, so hidden/pack = 2880/2 = 1440
    // fusion_size = 2 (swiglu_fusion=1), inter_size = 2880
    %fc1_w = onnx.Constant dense<1> : tensor<32x5760x1440xui8>
    // fc1 scales: [32, 5760, k_blocks=90] where k_blocks = 2880/32
    %fc1_s = onnx.Constant dense<1.000000e+00> : tensor<32x5760x90xf16>
    // fc1 bias: [32, 5760]
    %fc1_b = onnx.Constant dense<0.000000e+00> : tensor<32x5760xf16>
    // fc2 weights: [32, 2880, 1440]
    %fc2_w = onnx.Constant dense<1> : tensor<32x2880x1440xui8>
    // fc2 scales: [32, 2880, 90]
    %fc2_s = onnx.Constant dense<1.000000e+00> : tensor<32x2880x90xf16>
    // fc2 bias: [32, 2880]
    %fc2_b = onnx.Constant dense<0.000000e+00> : tensor<32x2880xf16>
