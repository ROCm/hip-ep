...
module {
  func.func @main_graph(%arg0: tensor<128x3x256x32xf16>) -> (tensor<128x3x256x1xf16>) {
    // CHECK: llvm.func @wrap_reduce_l2
    // CHECK-NOT: onnx.ReduceL2

    %axes = "onnx.Constant"() {value = dense<[-1]> : tensor<1xi64>} : () -> tensor<1xi64>
    %0 = "onnx.ReduceL2"(%arg0, %axes) {keepdims = 1 : si64, noop_with_empty_axes = 0 : si64} : (tensor<128x3x256x32xf16>, tensor<1xi64>) -> tensor<128x3x256x1xf16>
