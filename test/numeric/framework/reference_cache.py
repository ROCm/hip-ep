...
# ---------------------------------------------------------------------------
# Disk-only reference loader (the "reference=disk" mode)
# ---------------------------------------------------------------------------


def load_outputs_from_dir(reference_dir: Path | str) -> list[np.ndarray]:
    """Load ``out_*.npy`` files from *reference_dir* in numerical order.

    Used by ``model_runner.run_sample(..., reference="disk")`` to support
    pre-baked goldens that ship outside the auto-cache (e.g. captured
    from a real model trace or generated by an external tool).
    """
    d = Path(reference_dir)
    if not d.exists():
        raise FileNotFoundError(f"reference_dir does not exist: {d}")
    files = sorted(d.glob("out_*.npy"), key=lambda p: int(p.stem.split("_")[1]))
    if not files:
        raise FileNotFoundError(f"no out_*.npy files found under reference_dir={d}")
    return [np.load(p) for p in files]
