...
#
"""Comprehensive gpt-oss-20b model test suite.
...
Architecture: 24 layers (alternating sliding/full attention, sliding_window=128
in the model graph), 64 attention heads, 8 KV heads (HPG=8), head_dim=64,
hidden=2880, vocab=201088. MoE backbone: 32 experts, 4 active per token, MXFP4
quant. attention_bias=true. RoPE uses YaRN scaling
(original_max_position_embeddings=4096, factor=32). NO `position_ids` input —
the genai_config inputs map omits it; rope is driven internally by the graph.

The HF repo splits weights across 7 external-data blobs
(`model_q4f16.onnx_data` … `model_q4f16.onnx_data_6`, ~12.5 GB total). The
shared `ensure_model` / `ensure_fixed_model` helpers in conftest.py accept a
list of data filenames so the multi-blob layout is downloaded + hardlinked
correctly into the fixed-shape sibling directory.

All MorphiZen EP tests use IOBinding with device memory.

Test order matters: DML tests run first (before EP registration), then EP
tests, then OGA tests. This avoids OOM on iGPU from concurrent EP+DML
sessions.
"""
...
# 7 external-data blobs — the HF converter split the ~12.5 GB of weights to
# stay under storage backend per-file caps. Order doesn't matter (ONNX loads
# them via the external_data refs embedded in `model_q4f16.onnx`).
_DATA_FILES = [
...
VOCAB_SIZE = 201088
BOS_TOKEN = 199998
# gpt-oss exposes two EOS IDs (200002 and 199999); the chunked-prefill test
# only needs to know the prompt is BOS-led and avoid these in filler tokens.
EOS_TOKENS = [200002, 199999]
PAD_TOKEN = 199999
MAX_SEQ_LEN = 256

# Filler IDs are arbitrary mid-range vocab tokens chosen to avoid the
# special-token cluster (>=199998). Tests compare CPU-vs-EP logits, so the
# decoded text is irrelevant — only that the sequence is reproducible and
# all IDs are in [0, vocab_size) and not EOS.
PROMPT_TOKENS = [199998, 1234, 5678, 9012, 3456, 7890, 12345]
...
def _ensure_model():
    return ensure_model(_MODEL_DIR, _ONNX_FILE, _DATA_FILES, _HF_BASE)


def _ensure_fixed_model(seq_len, kv_len):
    _ensure_model()
    return ensure_fixed_model(_MODEL_DIR, _ONNX_FILE, _DATA_FILES, seq_len, kv_len)


def _make_cfg(max_seq_len):
    # gpt-oss has no `position_ids` input — rope embeddings are produced
    # internally by the graph (YaRN-scaled). LlamaModelConfig is generic
    # enough to cover this with has_position_ids=False.
...
def _ensure_oga_files():
    ...
    # Migrate the HF-shipped genai_config to set search.chunk_size=1024 so
    # patching for MorphiZenEP later doesn't have to re-write the file.
    # Same approach as the 8B Llama suite — see CLAUDE.md "OGA chunked
    # prefill" for why 1024 is the default.
...
class TestGptOss20BOGA:
    """OGA integration tests. All use past_present_share_buffer=true unless noted.

    OGA exposes gpt-oss as `model.type = "gptoss"` (distinct from the standard
    `decoder-only` / `decoder-pipeline` types). Tests skip cleanly if the OGA
    fork pinned for this build doesn't recognize the type or our EP.
    """
