...
static void usage() {
  std::cout
      << "Usage: test_classification [options] <model.onnx> <input_file>\n"
      << "Options:\n"
...
std::vector<std::string> load_labels(const std::string &label_file) {
  ...
  if (!file.is_open()) {
    std::cerr << "Warning: Could not open label file: " << label_file
              << std::endl;
    return labels;
  }
  ...
  std::cout << "Loaded " << labels.size() << " labels from " << label_file
            << std::endl;
  return labels;
}
...
static std::vector<float> softmax(float *data, int64_t size) {
  ...
  auto f = [max](float f) -> float { return expf(f - max); };
...
static void print_topk(const std::vector<std::pair<int, float>> &topk_results,
  ...
  for (const auto &v : topk_results) {
    ...
    std::cout << std::setiosflags(std::ios::left) << std::setw(11)
              ...
              << " =  " << std::setw(12) << v.second << " text: " << label_text
...
static void preprocess(const std::string &input_file,
  ...
  CHECK(std::ifstream(input_file, std::ios::binary)
...
void run_classification(const std::filesystem::path &model_path,
                        const std::string &input_file, int top_k,
                        bool enable_ep, bool enable_profiler,
                        const std::vector<std::string> &labels) {
...
#if _WIN32
  session = std::make_unique<Ort::Session>(env, model_path.wstring().c_str(),
                                           session_options);
#else
  session = std::make_unique<Ort::Session>(env, model_path.u8string().c_str(),
                                           session_options);
#endif
  ...
  (void)status;
  std::cout << "Unregistered EP library: " << ENV_PARAM(MORPHIZEN_EP_DLL)
            << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    run_classification(model_path, input_file, top_k, enable_ep,
                       enable_profiler, labels);
