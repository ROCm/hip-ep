...
public:
  GQATester(const std::filesystem::path &model_path, GQAConfig &config)
    ...
    std::cout << "EP enabled: " << (config_.enable_ep ? "yes" : "no")
              << std::endl;
    std::cout << "EP context: " << (config_.enable_ep_context ? "yes" : "no")
              << std::endl;
    std::cout << std::endl;
    ...
    if (config_.enable_ep) {
      ...
      if (!std::filesystem::exists(library_path)) {
        std::cerr << "Warning: EP library not found: " << library_path
                  << std::endl;
        ...
        if (status != nullptr) {
          ...
          std::cout << "Registered MorphiZen EP for GQA acceleration"
                    << std::endl;
        }
      }
    }
      ...
      if (!selected_devices.empty()) {
        ...
        if (config_.enable_ep_context) {
          ...
          session_options.AddConfigEntry("ep.context_embed_mode",
                                         embed_mode.c_str());
          std::string share_ep_contexts =
              get_env("XLNX_ENABLE_EP_SHARED_CONTEXT", "");
          if (!share_ep_contexts.empty()) {
            session_options.AddConfigEntry("ep.share_ep_contexts",
                                           share_ep_contexts.c_str());
          }
          std::cout << "EP context caching enabled (embed_mode=" << embed_mode
                    << ")" << std::endl;
        }

        session_options.AppendExecutionProvider_V2(*env_, selected_devices,
                                                   provider_options);
    ...
    session_ = std::make_unique<Ort::Session>(
        *env_, model_path.wstring().c_str(), session_options);
#else
    session_ = std::make_unique<Ort::Session>(
        *env_, model_path.u8string().c_str(), session_options);
#endif
  ...
  std::vector<int64_t> ResolveShape(const std::vector<int64_t> &shape,
    ...
    for (size_t i = 0; i < resolved.size(); i++) {
      if (resolved[i] <= 0) {
        ...
        if (config_.verbose) {
          std::cout << "  Resolved " << name << " dim[" << i << "] to "
                    << resolved[i] << std::endl;
        }
      }
    }
  ...
  void Run() {
    ...
    for (size_t i = 0; i < num_inputs_; i++) {
      ...
      if (config_.verbose) {
        std::cout << "Input " << input_names_[i] << ": " << num_elements
                  << " elements" << std::endl;
      }
    ...
    for (int i = 0; i < config_.warmup; i++) {
      try {
        auto output_tensors =
            session_->Run(Ort::RunOptions{nullptr}, input_name_ptrs.data(),
                          input_tensors.data(), num_inputs_,
    ...
    // Benchmark runs
    std::cout << "Benchmark (" << config_.iterations << " iterations)..."
              << std::endl;
      ...
      try {
        ...
        if (i == 0) {
          ...
          for (size_t j = 0; j < output_tensors.size(); j++) {
            auto output_shape =
                output_tensors[j].GetTensorTypeAndShapeInfo().GetShape();
