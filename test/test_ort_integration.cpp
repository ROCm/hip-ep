...
TEST_F(OrtIntegrationTest, LoadMorphiZenProvider) {
  ...
  if (ep_available_) {
    ...
    if (!found_morphizen) {
      std::cout
          << "[Test] Note: MorphiZen EP registered but not exposing V2 devices"
          << std::endl;
      std::cout << "[Test] This is expected - ORT 2.0 device API may not be "
                   "implemented yet"
                << std::endl;
    }
...
TEST_F(OrtIntegrationTest, MorphiZenProviderInference) {
  ...
  std::vector<float> cpu_output;
  {
    ...
    std::cout << "[Test] CPU reference output[0]: " << cpu_output[0]
              << std::endl;
  }
  ...
  // Try to run with Morphizen EP using ORT 2.0 API
  {
    ...
    if (morphizen_device == nullptr) {
      ...
      std::cout << "[Test] MorphiZen EP V2 device API not yet implemented"
                << std::endl;
      std::cout << "[Test] The EP was registered successfully, but doesn't "
                   "expose V2 devices"
                << std::endl;

      GTEST_SKIP() << "Morphizen EP V2 device API not yet implemented (EP "
                      "registered OK)";
    }
    ...
    try {
      ...
      for (size_t i = 0; i < cpu_output.size(); ++i) {
        ...
        if (diff > TOLERANCE) {
          if (mismatch_count < MAX_MISMATCH_BEFORE_FATAL) {
            std::cout << "[Test] MISMATCH at index " << i
                      << ": CPU=" << cpu_output[i] << ", GPU=" << gpu_output[i]
                      << ", diff=" << diff << std::endl;
          }
          ...
          if (mismatch_count >= MAX_MISMATCH_BEFORE_FATAL) {
            std::cout << "[Test] FATAL: Too many mismatches (" << mismatch_count
                      ...
                      << std::endl;
            std::cout << "[Test] Max difference so far: " << max_diff
                      << std::endl;
            std::cout << "[Test] Stopping comparison early to avoid log flood."
                      << std::endl;
      ...
      if (mismatch_count > 0) {
        ...
        FAIL() << "CPU vs Morphizen EP output has " << mismatch_count
               << " mismatches";
      }

      std::cout << "[Test] Max difference between CPU and GPU: " << max_diff
                << std::endl;
      ...
      if (error_msg.find("No engine configurations available") !=
        ...
        GTEST_SKIP()
            << "MIOpen backend not available. This is expected if ROCm is not "
