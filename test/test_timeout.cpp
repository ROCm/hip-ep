  ...
  while (true) {
    ...
    if (err == hipSuccess) {
      ...
      if (elapsed_ms >= timeout_ms) {
        LOG(ERROR) << "[ROCm Timeout] GPU operation timed out after "
                   << elapsed_ms << "ms";
...
void test_immediate_completion() {
  ...
  if (err != hipSuccess) {
    std::cerr << "Failed to create stream: " << hipGetErrorString(err)
              << std::endl;
    return;
  }

  // Test timeout on empty stream (should succeed immediately)
  auto status = hip_ep::WaitStreamWithTimeout(stream, 1000);

  if (status == hip_ep::TimeoutStatus::SUCCESS) {
...
void test_short_operation() {
  ...
  if (err != hipSuccess) {
    std::cerr << "Failed to allocate device memory: " << hipGetErrorString(err)
              << std::endl;
    hipStreamDestroy(stream);
    return;
  }
  ...
  if (err != hipSuccess) {
    std::cerr << "Failed to launch memset: " << hipGetErrorString(err)
              << std::endl;
    hipFree(d_data);
    hipStreamDestroy(stream);
    return;
  ...
  auto start = std::chrono::steady_clock::now();
  auto status = hip_ep::WaitStreamWithTimeout(stream, 5000);
  ...
  if (status == hip_ep::TimeoutStatus::SUCCESS) {
    std::cout << "✅ PASS: Short operation completed in " << elapsed_ms << "ms"
              << std::endl;
  } else {
...
void test_timeout_detection() {
  ...
  std::cout << "Note: This test verifies timeout logic, not actual GPU hang"
            << std::endl;
  ...
  auto start = std::chrono::steady_clock::now();
  auto status = hip_ep::WaitStreamWithTimeout(stream, 1); // 1ms timeout
  auto end = std::chrono::steady_clock::now();
  auto elapsed_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
          .count();

  if (status == hip_ep::TimeoutStatus::TIMEOUT) {
    std::cout << "✅ PASS: Timeout detected after " << elapsed_ms
              << "ms (as expected with 1ms limit)" << std::endl;
  } else if (status == hip_ep::TimeoutStatus::SUCCESS) {
    std::cout
        << "⚠️  INFO: Operations completed faster than 1ms (very fast GPU!)"
        << std::endl;
