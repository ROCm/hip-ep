...
int main(int argc, char **argv) {
  ...
  // passes, this registers plugin dialects (custom ops + their bufferization and
  // HIP->LLVM-lowering interface models) into the tool's registry, mirroring
  // hip::compiler::loadAllDialects so the op set never drifts between the tool
  // and the EP. Without it convert-hip-to-llvm finds no lowering for plugin ops
  // and they survive unlowered. No-op when no plugins are selected.
