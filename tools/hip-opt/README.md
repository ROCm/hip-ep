...
- **`hip-compiler`** -- One-stop compiler that takes pre-bufferized `.hip.mlir` input and produces a `.dll` + import `.lib`
...
- `examples/gemm.hip.mlir` + `main_gemm.cpp` - 3D matmul with 2D weight broadcast
- `examples/add.hip.mlir` + `main_add.cpp` - 3D element-wise add
- `examples/mul.hip.mlir` + `main_mul.cpp` - 3D element-wise mul
- `examples/rms_norm.hip.mlir` + `main_rms_norm.cpp` - 3D RMS normalization
- `examples/softmax.hip.mlir` + `main_softmax.cpp` - 3D softmax
- `examples/attention.hip.mlir` + `attention.onnx` + `main_attention.cpp` - Full attention with ORT CPU reference comparison
...
- `scripts/run_full_pipeline_attention.bat` - Attention test (ORT CPU reference vs GPU; requires ONNX Runtime)
...
### Input Format (`.hip.mlir`)

`hip-compiler` expects **pre-bufferized memref IR**. All compute ops use
destination-passing style (DPS) with `ins(...)`/`outs(...)` in memref mode
(no tensor results, writes in-place). Input arguments use strided memrefs;
the output is the last argument with contiguous layout. Intermediate buffers
are carved from a single pool via `hip.alloc` + `memref.view`.

```mlir
module {
  func.func @example(
      %A: memref<2x4x8xf32, strided<[?, ?, ?], offset: ?>>,
      %W: memref<8x8xf32, strided<[?, ?], offset: ?>>,
      %C: memref<2x4x8xf32>) {
    %h = hip.create_handle() : !hip.handle
    %pool = hip.alloc(%h) : memref<256xi8>
    %c0 = arith.constant 0 : index
    %tmp = memref.view %pool[%c0][] : memref<256xi8> to memref<2x4x8xf32>
    hip.hipblaslt.matmul(%h)
        ins(%A, %W : memref<2x4x8xf32, strided<[?, ?, ?], offset: ?>>,
                     memref<8x8xf32, strided<[?, ?], offset: ?>>)
        outs(%tmp : memref<2x4x8xf32>)
    // ... more ops ...
    hip.free(%h, %pool) : memref<256xi8>
...
The recommended workflow uses `hip-compiler` to produce a DLL directly from
pre-bufferized `.hip.mlir` input (memref format with static shapes, pool
allocation, and `memref.view`):

```bash
hip-compiler input.hip.mlir -o model.dll
```

This performs the full pipeline internally:
1. HIP-to-LLVM lowering (`--convert-hip-to-llvm`, memref finalization, etc.)
...
Bufferization (tensor-to-memref conversion) is handled upstream before the
`.hip.mlir` is fed to `hip-compiler`.

For debugging, you can use `hip-opt` to inspect intermediate IR:

```bash
hip-opt input.hip.mlir \
    --convert-hip-to-llvm \
...
The runtime transparently handles host constants (from `memref.global`) by staging
them through heap memory before `hipMemcpy` to device, avoiding HIP PAL
compatibility issues with read-only `.rdata` sections on Windows.
...
The `hip-compiler` lowering pipeline from pre-bufferized memref HIP IR to LLVM IR:
...
Bufferization (tensor-to-memref, buffer reuse, alloc lowering) is handled
upstream before the `.hip.mlir` is produced. For reference, the upstream passes are:
`--one-shot-bufferize`, `--hip-optimize-memrefs`, `--hip-lower-allocs`.
