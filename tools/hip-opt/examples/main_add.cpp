//===- main_add.cpp - Main driver for two-add DPS test --------------------===//
//
// Two chained element-wise adds:
...
// MLIR-compiled function. Each memref<?x?xf32, 1> expands to 7 args.
...
int main() {
  ...
  printf("Two-add DPS test:  D = A + B + C  (%lld x %lld)\n", M, N);
  ...
  printf("Running two_adds on GPU...\n");
  two_adds(
      d_A, d_A, 0, M, N, N, 1,
      d_B, d_B, 0, M, N, N, 1,
      d_C, d_C, 0, M, N, N, 1,
  ...
  for (int64_t i = 0; i < M; ++i) {
    printf("  [");
    for (int64_t j = 0; j < N; ++j) printf(" %7.3f", h_D[i * N + j]);
    printf(" ]\n");
  }
  ...
  for (int64_t i = 0; i < M; ++i) {
    printf("  [");
    for (int64_t j = 0; j < N; ++j) printf(" %7.3f", h_ref[i * N + j]);
    printf(" ]\n");
  }
