//===- main_add.cpp - Main driver for two-add DPS test (3D) ---------------===//
//
...
extern "C" void two_adds(
    ...
    float *D_a, float *D_al, int64_t D_o, int64_t D_s0, int64_t D_s1, int64_t D_s2, int64_t D_st0, int64_t D_st1, int64_t D_st2);
...
int main() {
  ...
  for (int64_t i = 0; i < n; ++i) {
    h_A[i] = float(i) * 0.1f; h_B[i] = float(i) * 0.2f; h_C[i] = float(i) * 0.3f;
    h_ref[i] = h_A[i] + h_B[i] + h_C[i];
  }

  float *d_A, *d_B, *d_C, *d_D;
  HIP_CHECK(hipMalloc(&d_A, n*4)); HIP_CHECK(hipMalloc(&d_B, n*4));
  HIP_CHECK(hipMalloc(&d_C, n*4)); HIP_CHECK(hipMalloc(&d_D, n*4));
  ...
  two_adds(
      d_A, d_A, 0, B, S, D, S*D, D, 1,
      d_B, d_B, 0, B, S, D, S*D, D, 1,
      d_C, d_C, 0, B, S, D, S*D, D, 1,
      d_D, d_D, 0, B, S, D, S*D, D, 1);

  HIP_CHECK(hipMemcpy(h_D.data(), d_D, n*4, hipMemcpyDeviceToHost));

  float mx = 0;
  for (int64_t i = 0; i < n; ++i) { float d = std::fabs(h_D[i] - h_ref[i]); if (d > mx) mx = d; }
  printf("Max diff: %e  %s\n", mx, mx < 1e-4f ? "PASS" : "FAIL");

  HIP_CHECK(hipFree(d_A)); HIP_CHECK(hipFree(d_B));
  HIP_CHECK(hipFree(d_C)); HIP_CHECK(hipFree(d_D));
  return (mx < 1e-4f) ? 0 : 1;
}
