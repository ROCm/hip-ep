...
extern "C" __declspec(dllimport) void attention(
    ...
    float *Wv_a, float *Wv_al, int64_t Wv_o, int64_t Wv_s0, int64_t Wv_s1, int64_t Wv_st0, int64_t Wv_st1,
    float *sc_a, float *sc_al, int64_t sc_o, int64_t sc_s0, int64_t sc_s1, int64_t sc_s2, int64_t sc_st0, int64_t sc_st1, int64_t sc_st2,
    float *out_a, float *out_al, int64_t out_o, int64_t out_s0, int64_t out_s1, int64_t out_s2, int64_t out_st0, int64_t out_st1, int64_t out_st2);
...
static void cpu_softmax(const float *in, float *out, int64_t rows, int64_t cols) {
  for (int64_t n = 0; n < rows; ++n) {
    float mx = in[n*cols];
    for (int64_t d = 1; d < cols; ++d) mx = std::fmax(mx, in[n*cols+d]);
    float sum = 0;
    for (int64_t d = 0; d < cols; ++d) { out[n*cols+d] = std::exp(in[n*cols+d] - mx); sum += out[n*cols+d]; }
    for (int64_t d = 0; d < cols; ++d) out[n*cols+d] /= sum;
  }
}
...
int main() {
  ...
  for (int64_t i = 0; i < B*S*D; ++i) h_X[i] = float(i % 7) * 0.1f;
  for (int64_t i = 0; i < D*D; ++i) h_Wq[i] = (i / D == i % D) ? 1.0f : 0.0f;
  for (int64_t i = 0; i < D*D; ++i) h_Wk[i] = h_Wq[i];
  for (int64_t i = 0; i < D*D; ++i) h_Wv[i] = h_Wq[i];

  // CPU reference per batch
  for (int64_t b = 0; b < B; b++) {
    std::vector<float> Q(S*D), K(S*D), V(S*D), KT(D*S), sc(S*S), scaled(S*S), attn(S*S);
    ...
    for (int64_t i = 0; i < S; i++) for (int64_t j = 0; j < D; j++) KT[j*S+i] = K[i*D+j];
    cpu_matmul(Q.data(), KT.data(), sc.data(), S, D, S);
    for (int64_t i = 0; i < S*S; i++) scaled[i] = sc[i] * scale_val;
  ...
  HIP_CHECK(hipMemcpy(d_scale, h_scale.data(), B*S*S*4, hipMemcpyHostToDevice));
  ...
  attention(
      ...
      d_Wk, d_Wk, 0, D, D, D, 1,
      d_Wv, d_Wv, 0, D, D, D, 1,
      d_scale, d_scale, 0, B, S, S, S*S, S, 1,
      d_out, d_out, 0, B, S, D, S*D, D, 1);

  HIP_CHECK(hipMemcpy(h_out.data(), d_out, B*S*D*4, hipMemcpyDeviceToHost));

  printf("\nGPU output (batch 0, first 2 rows):\n");
  for (int64_t i = 0; i < 2; ++i) {
    printf("  ["); for (int64_t j = 0; j < D; ++j) printf(" %6.3f", h_out[i*D+j]); printf(" ]\n");
  }
  printf("CPU reference (batch 0, first 2 rows):\n");
  for (int64_t i = 0; i < 2; ++i) {
    printf("  ["); for (int64_t j = 0; j < D; ++j) printf(" %6.3f", h_ref[i*D+j]); printf(" ]\n");
  }

  float mx = 0;
  for (int64_t i = 0; i < B*S*D; ++i) { float d = std::fabs(h_out[i] - h_ref[i]); if (d > mx) mx = d; }
  printf("\nMax diff: %e\n%s\n", mx, mx < 1e-2f ? "PASS" : "FAIL");

  HIP_CHECK(hipFree(d_X)); HIP_CHECK(hipFree(d_Wq)); HIP_CHECK(hipFree(d_Wk));
  HIP_CHECK(hipFree(d_Wv)); HIP_CHECK(hipFree(d_scale)); HIP_CHECK(hipFree(d_out));
  return (mx < 1e-2f) ? 0 : 1;
}
