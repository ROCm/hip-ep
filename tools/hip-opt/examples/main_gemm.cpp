...
// A is 3D memref<?x?x?xf32,1> = 9 args; B0, B1 are 2D = 7 args each; C is 3D = 9 args
extern "C" __declspec(dllimport) void two_matmuls(
    ...
    float *B0_a, float *B0_al, int64_t B0_o, int64_t B0_s0, int64_t B0_s1, int64_t B0_st0, int64_t B0_st1,
    float *B1_a, float *B1_al, int64_t B1_o, int64_t B1_s0, int64_t B1_s1, int64_t B1_st0, int64_t B1_st1,
...
int main() {
  ...
  printf("3D matmul test: A[%lld,%lld,%lld] @ B0[%lld,%lld] -> tmp, tmp @ B1[%lld,%lld] -> C\n",
         B, S, K, K, N, N, P);

  std::vector<float> h_A(B*S*K), h_B0(K*N), h_B1(N*P);
  std::vector<float> h_C(B*S*P, 0), h_ref(B*S*P);
  for (int64_t i = 0; i < B*S*K; ++i) h_A[i] = float(i % 10) / 10.0f;
  ...
  HIP_CHECK(hipMemcpy(d_A, h_A.data(), B*S*K*sizeof(float), hipMemcpyHostToDevice));
  ...
  two_matmuls(
      d_A, d_A, 0, B, S, K, S*K, K, 1,
      d_B0, d_B0, 0, K, N, N, 1,
      d_B1, d_B1, 0, N, P, P, 1,
      d_C, d_C, 0, B, S, P, S*P, P, 1);

  HIP_CHECK(hipMemcpy(h_C.data(), d_C, B*S*P*sizeof(float), hipMemcpyDeviceToHost));

  float mx = 0;
  for (int64_t i = 0; i < B*S*P; ++i) { float d = std::fabs(h_C[i] - h_ref[i]); if (d > mx) mx = d; }
  printf("Max diff: %e  %s\n", mx, mx < 1e-4f ? "PASS" : "FAIL");

  HIP_CHECK(hipFree(d_A)); HIP_CHECK(hipFree(d_B0));
  HIP_CHECK(hipFree(d_B1)); HIP_CHECK(hipFree(d_C));
  return (mx < 1e-4f) ? 0 : 1;
}
