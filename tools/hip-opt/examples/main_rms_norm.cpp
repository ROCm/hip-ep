...
static void cpu_rms_norm(const float *in, const float *w, float *out,
  ...
  for (int64_t n = 0; n < rows; ++n) {
    float sq = 0;
    for (int64_t d = 0; d < D; ++d) { float v = in[n*D+d]; sq += v*v; }
    float rms = std::sqrt(sq / float(D) + eps);
    for (int64_t d = 0; d < D; ++d) out[n*D+d] = in[n*D+d] / rms * w[d];
  }
}
...
int main() {
  ...
  for (int64_t i = 0; i < n; ++i) h_A[i] = float(i+1) * 0.1f;
  for (int64_t d = 0; d < D; ++d) { h_W0[d] = 1.0f; h_W1[d] = 0.5f; }
  ...
  HIP_CHECK(hipMalloc(&d_A, n*4)); HIP_CHECK(hipMalloc(&d_W0, D*4));
  HIP_CHECK(hipMalloc(&d_W1, D*4)); HIP_CHECK(hipMalloc(&d_B, n*4));
  ...
  two_rms_norms(
      d_A, d_A, 0, Bat, S, D, S*D, D, 1,
      d_W0, d_W0, 0, D, 1,
      d_W1, d_W1, 0, D, 1,
      d_B, d_B, 0, Bat, S, D, S*D, D, 1);

  HIP_CHECK(hipMemcpy(h_B.data(), d_B, n*4, hipMemcpyDeviceToHost));

  float mx = 0;
  for (int64_t i = 0; i < n; ++i) { float d = std::fabs(h_B[i] - h_ref[i]); if (d > mx) mx = d; }
  printf("Max diff: %e  %s\n", mx, mx < 1e-3f ? "PASS" : "FAIL");

  HIP_CHECK(hipFree(d_A)); HIP_CHECK(hipFree(d_W0));
  HIP_CHECK(hipFree(d_W1)); HIP_CHECK(hipFree(d_B));
  return (mx < 1e-3f) ? 0 : 1;
}
