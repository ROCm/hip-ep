...
def load(path, layers):
    ...
    for r in rows:
        ...
        if len(name) > 52:
            name = name[:52] + "~"
        count[name] += 1
        d = _field(r, ("dur_us", "duration_us", "duration", "gpu_time_us",
                       "time_us"))
        if d is not None:
            busy[name] += float(d)
...
def main():
    ...
    if abs(sa - sb) > 0.15:
        print("  WARNING: the windows hold different amounts of work; "
    ...
    print(f"busy us per step   : {du:+.1f}  "
          f"({100*du/(sum(ta.values())/sa):+.2f}%)")
