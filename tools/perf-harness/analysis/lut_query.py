...
def cfg_str(configs, i):
    ...
    return (
        f"{c['kind']}(bm16={c['bm16']},bn16={c['bn16']},bk={c['bk']},"
...
def main():
    ...
    ap.add_argument(
        "--shape", action="append", default=[], metavar="N,K", help="repeatable"
    )
    ap.add_argument(
        "--list-shapes", action="store_true", help="every (n,k) present for the phase"
    )
    ...
    for f in fb:
        print(f"  fallback {f['bits']}: config {f['config']} {cfg_str(C, f['config'])}")
    ...
    for s in args.shape:
        ...
        for p in hits:
            print(
                f"      {p['bits']} {p['group_size']} {p['zero_point']} "
    ...
    if args.list_shapes:
        ...
        for (n, k), cnt in sorted(shapes.items()):
            cfgs = {cfg_str(C, p["config"]) for p in pts if p["n"] == n and p["k"] == k}
