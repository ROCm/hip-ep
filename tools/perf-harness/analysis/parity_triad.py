...
def main():
    ...
    names = sorted(
        {
            os.path.basename(p).split("." + args.flag)[0]
            for p in glob.glob(os.path.join(args.dir, "*.npz"))
        }
    )
    hdr = f"{'model':46} {'off-ctl':>10} {'ctl-on':>10} {'off-on':>10}  {'ctl==on':>8}"
    print(hdr)
    print("-" * len(hdr))
    for n in names:
        paths = {s: os.path.join(args.dir, f"{n}.{args.flag}.{s}.npz") for s in STATES}
        if not all(os.path.exists(p) for p in paths.values()):
            continue
        d = {s: np.load(p)["logits"] for s, p in paths.items()}
        print(
            f"{n[:46]:46} {worst(d['off'], d['control']):10.8f} "
