...
def main():
    ...
    ap.add_argument("--kernel", default="",
                    help="substring filter on the kernel name")
    ...
    agg = collections.defaultdict(
        lambda: {"n": 0, "us": 0.0, "gbps": 0.0, "occ": 0.0,
                 "wg": set(), "bound": collections.Counter()})
    for r in rows:
    ...
    hdr = (f'{"kernel":30} {"label":22} {"cfg":9} {"n":>4} '
    ...
    for k, v in sorted(agg.items(), key=lambda x: -x[1]["us"])[:args.top]:
        ...
        print(f'{k[0][:30]:30} {k[1][:22]:22} {k[3][:9]:9} {n:4d} '
