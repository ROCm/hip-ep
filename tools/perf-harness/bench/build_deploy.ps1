...
# hip-compiler was its own DLL until #838 merged it into hipgpu and #840 deleted
# the target. Deploy it only where it still exists: hardcoding it fails the
# build on current main, and dropping it unconditionally would silently leave a
# stale compiler behind on the branches that predate the merge.
if (Test-Path (Join-Path $BuildDir 'bin\hip-compiler.dll')) {
  $artifacts += 'hip-compiler.dll'
}
# The fusion patterns are read at runtime, so a stale copy rewrites the graph
# the freshly built runtime then executes -- the same silent-no-op failure the
# hip-compiler note above describes, just via a data file rather than a DLL.
if (Test-Path (Join-Path $BuildDir 'bin\HipFusionPatterns.pdl.mlir')) {
  $artifacts += 'HipFusionPatterns.pdl.mlir'
}
