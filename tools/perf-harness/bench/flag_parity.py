...
def record(args) -> None:
    ...
    try:
        ...
        tokens, prompt_len, inputs = [], 0, None
    ...
    def build_generator(use_vision):
        ...
        params.set_search_options(do_sample=False,
                                  max_length=prompt_len + args.steps + 2048)
    ...
    np.savez_compressed(args.out, logits=arr, tokens=np.array(out_tokens, dtype=np.int64),
...
def compare(a_path: str, b_path: str, min_cos: float,
            control_path: str = None) -> int:
    ...
    print(f"token sequence      : {'identical' if tok_ok else f'DIVERGES at step {first_bad}'}")
    ...
    if control_path:
        ...
        tok_ok = tok_ok and len(tc) == len(ta) and all(
            x == y for x, y in zip(tc, ta))
        print(f"first-run artefact  : {warm:.8f} (off vs control, step {warm_at})")
        print(f"control vs on       : {prim:.8f} (step {prim_at})"
        ...
        if not ok and tok_ok:
            ...
            print(f"noise budget        : {budget:.8f} divergence "
...
def main() -> int:
    ...
    ap.add_argument("--control", metavar="OFF2.npz",
    ...
    if args.compare:
        return compare(args.compare[0], args.compare[1], args.min_cos,
                       args.control)
