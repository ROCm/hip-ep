...
TEST(TarFileTest, WriteTo) {
  ...
  // directory
  {
    ...
    { write_to_stream("a.txt", *tar_file_obj); }
    { write_to_stream("b.txt", *tar_file_obj); }
    { write_to_stream("c.txt", *tar_file_obj); }
    LOG(INFO) << " ====== end write to tar file. ==== ";
    LOG(INFO) << " ====== begin to read and check ==== ";
    { read_and_check("a.txt", *tar_file_obj); }
    { read_and_check("b.txt", *tar_file_obj); }
    { read_and_check("c.txt", *tar_file_obj); }
