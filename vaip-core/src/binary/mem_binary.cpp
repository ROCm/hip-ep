...
std::vector<char> get_mem_binary_builtin(const std::string& filename) {
  auto iter = binary_map.find(filename);
  auto info = iter->second;
  if (info.compressed_size == 0) {
    auto ret = std::vector<char>(info.origin_size);
    memcpy(ret.data(), static_cast<const void*>(info.data), info.origin_size);
    return ret;
  } else {
      ...
      if (vaip_has_mem_xclbin) {
        if (vaip_has_mem_xclbin(filename.c_str())) {
          ...
          if (vaip_get_mem_xclbin) {
            vaip_get_mem_xclbin(
                filename.data(), reinterpret_cast<void*>(&mem_binary),
          ...
          MY_LOG(1) << "  -- not found mem_binary: " << filename
                    << " from backend " << ENV_PARAM(VAIP_XCLBIN_BACKEND);
        }
      } else {
...
bool has_mem_binary(const std::string& filename) {
  auto vaip_get_mem_xclbin_plugin = Plugin::get(ENV_PARAM(VAIP_XCLBIN_BACKEND));
  auto ret = binary_map.find(filename) != binary_map.end();
  if (ret) {
    MY_LOG(1) << "  -- found mem_binary: " << filename
              << " from builtin inside morphizen";
  } else {
    if (vaip_get_mem_xclbin_plugin) {
      ...
      if (vaip_has_mem_xclbin) {
        if (vaip_has_mem_xclbin(filename.c_str())) {
          MY_LOG(1) << "  -- found mem_binary: " << filename << " from backend "
                    << ENV_PARAM(VAIP_XCLBIN_BACKEND);
          ret = true;
        } else {
          MY_LOG(1) << "  -- not found mem_binary: " << filename
    ...
    if (ret == false) {
      ...
      ret = binary_map.find(filename) != binary_map.end();
      if (ret) {
        MY_LOG(1) << "  -- found mem_binary: " << filename
                  << " from builtin inside morphizen";
      } else {
