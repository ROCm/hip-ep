...
const Node& NodeBuilder::build() {
  ...
  for (auto i = 0u; i < num_of_outputs_; ++i) {
    if (!anchor_node_arg_[i].has_value()) {
      ...
      auto graph_ref = vaip_cxx::GraphConstRef(graph_);
      ...
      if (!empty_node_arg_opt.has_value()) {
        ...
        auto graph_mut = vaip_cxx::GraphRef(graph_);
        empty_node_arg =
            graph_mut
                .new_node_arg(
...
NodeBuilder& NodeBuilder::clone_op_type(const Node& node) {
  auto node_ref = vaip_cxx::NodeConstRef::from_node(graph_, node);
...
NodeBuilder& NodeBuilder::clone_data_type(const NodeArg& node_arg) {
  auto arg_ref = vaip_cxx::NodeArgConstRef::from_node_arg(graph_, node_arg);
...
void graph_replace_node_arg(const Graph& graph, const IPass& pass,
  ...
  auto from_ref = vaip_cxx::NodeArgConstRef::from_node_arg(graph, from);
  auto to_ref = vaip_cxx::NodeArgConstRef::from_node_arg(graph, to);
...
vaip_core::NodeBuilder graph_node_builder(vaip_cxx::GraphRef& graph,
                                          vaip_core::IPass& pass) {
  return vaip_core::NodeBuilder(graph, pass);
}

std::pair<std::unique_ptr<vaip_core::MetaDefProto>, vaip_core::TryFuseError>
graph_try_fuse(const vaip_cxx::GraphConstRef& graph, const std::string& name,
               const std::vector<std::string>& inputs,
               const std::vector<std::string>& outputs,
               const std::vector<std::string>& constant_initializers,
               const std::string& device) {
  return vaip_core::IPass_try_fuse(graph, name, inputs, outputs,
                                   constant_initializers, device);
}

vaip_cxx::Subgraph graph_virtual_fuse(const vaip_cxx::GraphConstRef& graph,
                                      const vaip_core::MetaDefProto& meta_def) {
  auto inputs = std::vector<vaip_cxx::NodeArgConstRef>();
  auto outputs = std::vector<vaip_cxx::NodeArgConstRef>();
  auto nodes = std::vector<vaip_cxx::NodeConstRef>();
  auto constant_initializers = std::vector<vaip_cxx::NodeArgConstRef>();
...
NodeRef graph_fuse(GraphRef& graph, const vaip_core::MetaDefProto& meta_def) {
  ...
  vaip_core::Node& fused_node = vaip_core::graph_fuse(
