...
OpInvoker::OpInvoker(PrivateTag, const Ort::Env& env,
  ...
  std::unique_ptr<vaip_cxx::Model> model =
      vaip_cxx::Model::create(model_path, {{domain, version}});
  vaip_cxx::GraphRef graph = model->main_graph();

  // inputs
  std::vector<std::optional<vaip_cxx::NodeArgConstRef>> input_args;
  ...
  // outputs
  std::vector<std::optional<vaip_cxx::NodeArgConstRef>> output_args;
  ...
  // attributes
  vaip_core::NodeAttributesPtr node_attrs =
      vaip_core::NodeAttributesPtr(VAIP_ORT_API(node_attributes_new)());
  ...
  for (size_t i = 0; i < attr_count; ++i) {
    const ONNX_NAMESPACE::AttributeProto* attr_proto =
        reinterpret_cast<const ONNX_NAMESPACE::AttributeProto*>(
            ort_attr_values[i]);
    vaip_core::AttributeProtoPtr attr_proto_ptr =
        vaip_core::attr_proto_clone(*attr_proto);
    VAIP_ORT_API(node_attributes_add)(*node_attrs, std::move(*attr_proto_ptr));
  }
  ...
  std::vector<vaip_cxx::NodeArgConstRef> inputs;
  for (auto& arg : input_args) {
    inputs.push_back(arg.value());
  }

  std::vector<vaip_cxx::NodeArgConstRef> outputs;
  ...
  // create session
  vaip_core::ModelProto* model_proto = VAIP_ORT_API(model_to_proto)(*model);
  vaip_core::DllSafe<std::string> model_string =
      VAIP_ORT_API(model_proto_serialize_as_string)(*model_proto);

  session_ = Ort::Session(env, model_string.get()->c_str(),
                          model_string.get()->size(), options);

  VAIP_ORT_API(model_proto_delete)(model_proto);
}
